...
Lemma update_indexr_hit_None : forall {A} {σ : list (option A)} {l}, l < length σ -> indexr l (update σ l None) = Some None.
  induction σ; simpl; intuition.
  destruct (Nat.eqb l (length σ)) eqn:Hls.
  apply Nat.eqb_eq in Hls. rewrite Hls. simpl. rewrite Nat.eqb_refl; auto.
  simpl. rewrite <- update_length. rewrite Hls. apply Nat.eqb_neq in Hls.
  apply IHσ. lia.
Qed.
