...
Lemma openq_bot_empty_eq : forall d, (openq ∅ d) = (openq qbot d).
Proof.
  destruct d; simpl; auto.
  destruct (NatSet.F.mem 0 t0) eqn:H.
...
Definition open_ty' {A : Type} (env : list A) (T : ty) := open_ty (length env) T.
...
(* all typing assumptions are singletons *)
Definition wf_tenv (Γ : tenv) :=
  forall x T q, indexr x Γ = Some (T, q) -> q ⊑ ∅.
...
| vtp_abs: forall Σ T1 d1 T2 d2 T3 d3 T4 d4 df1 df2 t,
  ...
  closed_ty 0 0 (length Σ) (TFun d3 d4 T3 T4) ->
  closed_ty 0 0 (length Σ) (TFun d1 d2 T1 T2) ->
...
Lemma qstp_refl : forall {d d'}, d ≡ d' -> forall {Γ Σ}, closed_qual 0 (length Γ) (length Σ) d -> qstp Γ Σ d d'.
    intros d d' Heq Γ Σ Hc. constructor. destruct d; destruct d'; auto; qdec.
...
Proof.
  intros. destruct d; destruct df; simpl; auto; intuition;
...
Lemma stp_bot_empty : forall {Σ T1 T0},
    stp [] Σ T1 ∅ T0 ∅ -> stp [] Σ T1 qbot T0 qbot.
Proof.
  intros. induction H.
  ...
  - econstructor. simpl. auto. auto. econstructor. simpl. auto.
    simpl. auto. auto. auto.
  - eapply s_trans. apply IHstp1. apply IHstp2.
Qed.
...
Lemma stp_qual_irrelevance : forall {Γ1 Γ2 Σ U du T1 d1 T2 d2}, stp (Γ2 ++ (U,du) :: Γ1) Σ T1 d1 T2 d2 -> forall {du'}, stp (Γ2 ++ (U,du') :: Γ1) Σ T1 d1 T2 d2.
  ...
  - constructor; auto. 1,2 : rewrite app_length in *; simpl in *; auto.
    ...
    specialize (IHstp2 Γ1 du U ((T3, d3) :: Γ2)).        
    intuition.
...
Lemma wf_tenv_empty : wf_tenv [].
  unfold wf_tenv. intros. simpl in H. discriminate.
Qed.
#[global] Hint Resolve wf_tenv_empty : core.

Lemma wf_tenv_cons : forall {Γ}, wf_tenv Γ -> forall {T q}, q ⊑ ∅ -> wf_tenv ((T, q) :: Γ).
  intros. unfold wf_tenv. intros. bdestruct (x =? (length Γ)).
  - subst. rewrite indexr_head in H1. inversion H1. subst. intuition.
  - rewrite indexr_skip in H1; auto. apply H in H1. intuition.
Qed.
...
Proof.
  ...
  - inversion H5. subst. unfold openq.
...
Lemma openq_subqual : forall {d1 d2 φ}, d1 ⊑ φ -> (openq ∅ d2) ⊑ φ -> (openq d1 d2) ⊑ φ.
  ...
  - destruct (NatSet.F.mem 0 t0) eqn:Hmem; simpl in H0; auto.
  - destruct (NatSet.F.mem 0 t0) eqn:Hmem; simpl in H0; intuition; fnsetdec.
  - destruct (NatSet.F.mem 0 t0) eqn:Hmem; simpl in H0; intuition; fnsetdec.
  - destruct (NatSet.F.mem 0 t0) eqn:Hmem; simpl in H0; intuition; fnsetdec.
...
Fixpoint has_type_filter {Γ φ Σ t T d} (ht : has_type Γ φ Σ t T d) : d ⊑ φ.
  destruct ht; intuition; simpl; auto.
...
Lemma weaken_gen : forall {t Γ1 Γ2 φ Σ T d},
  ...
  - (* t_var *) simpl.
    ...
    + (* |Γ2| <= x < |Γ1|+|Γ2|*)
      erewrite <- splice_qual_qplus_dist; eauto. constructor.
    ...
    + (* |Γ2| > x *)
      ...
      erewrite splice_qual_qplus_id; eauto. constructor.
  ...
  - (* t_abs *) rewrite app_length in *. destruct H eqn:Hd1; subst; simpl.
    + constructor. left. auto.
      ...
      inversion H1. subst. constructor. auto. 1,4,5: apply splice_qual_closed; auto. 1,2 : apply splice_ty_closed; auto.
      rewrite subqual_splice_lr'. auto. (*  apply weaken_saturated. auto. *)
      rewrite app_comm_cons.
      replace ((splice_ty k T1, qbot) :: splice_tenv k Γ1) with (splice_tenv k ((T1, qbot) :: Γ1)); auto.
    ...
    + rewrite splice_set_empty. constructor. right. auto.
      apply splice_closed'.
      1-4: rewrite app_length; rewrite splice_tenv_length; simpl;
      replace (length Γ1 + S (length Γ2)) with (S (length Γ1 + length Γ2)); eauto.
      inversion H1. subst. constructor. auto. 1,4,5: apply splice_qual_closed; auto. 1,2 : apply splice_ty_closed; auto.
      rewrite subqual_splice_lr'. auto. (*  apply weaken_saturated. auto. *)
      rewrite app_comm_cons.
      replace ((splice_ty k T1, ∅) :: splice_tenv k Γ1) with (splice_tenv k ((T1, ∅) :: Γ1)); auto.
      replace (splice_qual k df ⊔ just_fv (length (splice_tenv k Γ1 ++ X :: Γ2)))
        with (splice_qual k (df ⊔ just_fv (length Γ1 + length Γ2))).
      subst k. rewrite <- splice_open'. rewrite <- splice_ty_open'. rewrite <- splice_qual_open'.
      rewrite @open_tm'_len with (Γ':=(Γ1 ++ Γ2)). rewrite @open_ty'_len with (Γ':=(Γ1 ++ Γ2)).
      ...
      simpl. repeat rewrite splice_qual_glb_dist. repeat rewrite splice_qual_lub_dist.
      f_equal. rewrite splice_qual_just_fv_ge. f_equal. lia. lia.
      simpl. rewrite splice_set_empty. auto.
  - (* tapp *) simpl. rewrite splice_qual_open''.
    assert ((splice_qual k df ⋒ splice_qual k d1) = (df ⋒ d1)). {
      destruct df; simpl; auto. destruct d1; simpl; auto.
      unfold qqcap. unfold qqcap in H1.
      destruct d1; simpl; auto. inversion H1. intuition.
      rewrite <- splice_set_inter_dist.
      f_equal. apply subset_of_empty in H2. rewrite H2.
      rewrite splice_set_empty. auto. }
    apply t_app with (T1:=(splice_ty k T1)) (df:=(splice_qual k df)).
    rewrite H2.
    replace (TFun (df ⋒ d1) (splice_qual k d2) (splice_ty k T1) (splice_ty k T2)) with (splice_ty k (TFun (df ⋒ d1) d2 T1 T2)); auto.
    apply IHHT1; auto. simpl. apply qual_less_than_empty in H1.
    destruct H1; rewrite H1; simpl; auto. rewrite splice_set_empty. auto.
    apply IHHT2; auto.
    eapply splice_ty_closed'. rewrite app_length in *. rewrite splice_tenv_length. auto.
    replace (TFun ∅ (splice_qual k d2) (splice_ty k T1) (splice_ty k T2)) with (splice_ty k (TFun ∅ d2 T1 T2)); auto.
    subst k. rewrite <- @splice_qual_empty with (k := length Γ2).
    rewrite <- splice_qual_open''.
    rewrite subqual_splice_lr'; auto. simpl. rewrite splice_set_empty. auto.
    destruct df; destruct d1.
    4 : { remember (qset t t0 t3) as Q1. remember (qset t4 t5 t6) as Q2.
    assert ((splice_qual k Q1 ⋒ splice_qual k Q2) = (splice_qual k Q1 ⊓ splice_qual k Q2)). subst. simpl. auto.
    assert (Q1 ⋒ Q2 = Q1 ⊓ Q2). subst. simpl. auto.
    rewrite H3. rewrite <- splice_qual_glb_dist. rewrite H4 in H1.
    apply qual_less_than_empty in H1. destruct H1; rewrite H1; simpl; auto.
    intuition; try fnsetdec. rewrite splice_set_empty. fnsetdec. }
    all: simpl; auto.
...
Lemma subst1_preserves_separation : forall {df d1 Tx dx dx' Γ l φ},
    dx' ⋒ φ ≡ dx ->
...
Proof.
  ...
  - simpl. unfold qqcap. destruct d1. simpl. auto.
    ...
    destruct Hs as [t3 [t4 [t5]]]. rewrite H4. simpl.
    ...
    + destruct dx' as [|fx' bx' lx']; simpl.
      * destruct (mem 0 ff) eqn:Hmem1; destruct (mem 0 f1) eqn:Hmem2; simpl.
        -- assert (mem 0 (inter ff f1) = true).
           ...
           unfold qqcap. rewrite H4. simpl. f_equal.
        ...
        -- apply mem_2 in Hmem2.
           ...
           apply NatSetProperties.FM.not_mem_iff. fnsetdec.
           unfold qqcap. simpl.
           rewrite H4. f_equal. apply NatSet.eq_if_Equal.
           apply inter_unsplice_0.
        -- apply NatSetProperties.FM.not_mem_iff in Hmem1, Hmem2.
           assert (mem 0 (inter ff f1) = false).
           apply NatSetProperties.FM.not_mem_iff. fnsetdec.
           unfold qqcap. simpl. rewrite H4. f_equal.
           apply NatSet.eq_if_Equal. apply inter_unsplice_0.
      * destruct φ as [|fp bp lp].
        -- inversion H1.
        -- inversion H0. subst. apply bound_0_empty in H10, H11. subst.
           destruct dx as [|fx bx lx].
           ++ simpl in H. inversion H.
           ++ simpl in H. intuition. rewrite inter_empty_left in H4.
              rewrite inter_empty_left in H. symmetry in H4. symmetry in H.
              apply NatSet.eq_if_Equal in H4.
              apply NatSet.eq_if_Equal in H. subst.
              assert (Hempty: qset {}N {}N lx ⊑ ∅). {
               assert (Hl : indexr 0 (Γ ++ [(Tx, qset {}N {}N lx)]) = Some (Tx, qset {}N {}N lx)).
               rewrite indexr_skips; auto. apply H3 in Hl. auto.
             }
             inversion Hempty. subst. simpl in H1, H2. intuition.
              assert (Hdisf : inter lx' lf [=] {}N). fnsetdec.
              assert (Hdis1 : inter lx' l1 [=] {}N). fnsetdec.
      ...
      substitution would require more careful reasoning. *)
                ...
                repeat rewrite empty_union_right.
                apply NatSet.eq_if_Equal. apply inter_unsplice_0.
              ** (* 0 ∈ df, 0 ∉ d1 *) unfold qqcap. simpl.
                 f_equal; try fnsetdec. repeat rewrite empty_union_right.
                 apply NatSet.eq_if_Equal. apply inter_unsplice_0.
              ** (* 0 ∉ df, 0 ∈ d1, analogous to the previous case *)
                unfold qqcap. simpl. f_equal; try fnsetdec. repeat rewrite empty_union_right.
                apply NatSet.eq_if_Equal. apply inter_unsplice_0.
              ** (* 0 ∉ df, 0 ∉ d1 : trivial, since the substitution has no effect (other than unsplicing the sets) *)
                unfold qqcap. simpl. f_equal; try fnsetdec. apply NatSet.eq_if_Equal. apply inter_unsplice_0.
Qed.
...
Lemma eqqual_if_eq : forall {d1 d2}, d1 = d2 -> d1 ≡ d2.
Proof.
  intros. destruct d1; destruct d2; simpl; auto. inversion H. inversion H.
  inversion H. subst. intuition.
Qed.

Lemma eqqual_iff_eq : forall {d1 d2}, d1 = d2 <-> d1 ≡ d2.
Proof.
  split. apply eqqual_if_eq. apply eq_if_eqqual.
Qed.
...
Lemma stp_scale_qglb : forall {Γ Σ T1 d1 T2 d2},
  stp Γ Σ T1 d1 T2 d2 -> forall {d}, closed_qual 0 (length Γ) (length Σ) d -> stp Γ Σ T1 (d1 ⊓ d) T2 (d2 ⊓ d).
  intros Γ Σ T1 d1 T2 d2 HSTP. induction HSTP; intros.
...
Lemma openq_just_fv_id: forall {d1 : qual} {x}, openq d1 (just_fv x) = just_fv x.
  ...
  - assert (mem 0 {}N = false).
    apply NatSetProperties.FM.not_mem_iff. intro F. fnsetdec.
    rewrite remove_empty. unfold just_fv. auto.
  - rewrite NatSetFacts.empty_b. simpl. unfold just_fv. auto.
Qed.
...
Lemma openq_u_distribute1 : forall {d1 d2 l},
  ...
  simpl. destruct d1; simpl; auto. unfold just_loc. rewrite remove_empty.
  auto. rewrite NatSetFacts.empty_b. unfold just_loc. simpl. auto.
Qed.
...
Lemma vtp_closed:
  forall {Σ t T d}, vtp Σ t T d ->
    ...
    closed_qual 0 0 (length Σ) d .
Proof.
  intros. induction H; intuition; constructor.
  ...
  - (* vtp_abs *) inversion H. subst. econstructor; eauto.
    ...
    + assert (stp [] Σ (TFun d1 d2 T1 T2) d5 (TFun d3 d4 T3 T4) d6).
      ...
      specialize (@narrowing_stp_gen [] T1 d1 [] Σ
                                     ...
                                     (openq' ([]:tenv) d7)
                                     (open_ty' ([]:tenv) T2)
                                     (openq' ([]: tenv) d2)) as narrow.
      replace ([] ++ [(T1, d1)]) with [(T1, d1)] in narrow. intuition.
      simpl in *. apply H12 in H0_.
      eapply s_trans; eauto. auto.
  - intuition.
Qed.
...
Proof.
  ...
  - (* tabs *) eapply vtp_abs; eauto.
    + inversion H2. eapply stp_refl. subst. intuition.
      apply qstp_refl. apply eqqual_refl; intuition. auto.
    + apply qstp_refl. apply eqqual_refl; intuition. assumption.
    + apply stp_refl. destruct H6; intuition.
      1-2: eapply closed_ty_monotone; eauto; apply indexr_var_some' in H6; lia.
      1-2: intuition; apply has_type_closed in H6; intuition; inversion H8;
      eapply closed_ty_monotone; eauto; intuition; inversion H9;
      eapply closed_ty_monotone; eauto; lia.
      1-2: apply stp_closed in H7; intuition.
      * apply qstp_refl.  apply eqqual_refl.  apply has_type_closed in H6; intuition.
    + apply qual_less_than_empty in H0. auto.
  - (* tloc *) eapply vtp_loc; eauto.
    + eapply closed_qual_sub; eauto.
    + apply stp_refl. intuition. apply qstp_refl. eapply eqqual_refl. econstructor.
    + apply stp_refl. intuition. constructor. intuition. econstructor.
...
Proof.
  ...
  - assert (has_type [] df1 Σ (tabs t0) (TFun d1 d2 T1 T2) df1). {
      constructor; eauto. 1,2: apply qstp_closed in H5 as Hcl; intuition.
      apply qual_less_than_empty. auto. apply qstp_closed in H5 as Hcl.
      intuition. }
    assert (has_type [] df2 Σ (tabs t0) (TFun d1 d2 T1 T2) df1). {
      inversion H5. subst. eapply weaken_flt with  (φ' := df2) in H8; intuition. }
    eapply t_sub; eauto; intuition.
Qed.

Lemma substitution_gen1 :
  forall {t Γ φ' Tx dx dx' Σ T d}, dx' ⋒ φ' ≡ dx -> dx <> qbot ->
    forall {φ}, φ ⊑ φ' -> ∅ ⊑ φ ->
      has_type (Γ ++ [(Tx,dx)]) φ Σ t T d ->
      wf_tenv (Γ ++ [(Tx,dx)]) -> Substq dx dx' ->
      forall {tx}, vtp Σ tx Tx dx' ->
              has_type ({ 0 |-> dx' }ᴳ Γ) ({ 0 |-> dx' }ᵈ φ) Σ
                       ({ 0 |-> tx  }ᵗ t) ({ 0 |-> dx' }ᵀ T) ({ 0 |-> dx' }ᵈ d).
Proof.
  Import NatSet.F.
  intros t Γ φ' Tx dx dx' Σ T d Hsep dxlb φ Hphi Hphilb HT HwfG HSubst tx HTx.
  ...
  induction HT; intros; subst; pose (φs := {0 |-> dx' }ᵈ φ);
    replace ({0 |-> dx' }ᵈ φ) with φs in *; auto.
  ...
  - simpl. bdestruct (x =? 0).
    + (*x is 0 *) rewrite indexr_skips in H0; simpl; auto; try lia. simpl in H0. subst. simpl in H0.
      ...
      * apply eq_if_eqqual in Hsep. rewrite qqcap_commute in Hsep.
         destruct df; simpl.
        -- intuition.
        -- repeat rewrite empty_union_left. rewrite mem_singleton. simpl.
          rewrite remove_singleton_empty. rewrite unsplice_set_empty.
          ...
          assert (mem 0 t3 = true). destruct H3.
          rewrite empty_union_left in H3. apply mem_1.
          eapply singleton_subset_in. auto. rewrite H7. intuition.
      * erewrite subst1_qual_plus'; eauto. eapply weaken'; eauto.
        eapply subst_qual_subqual_monotone in H3. erewrite subst1_qual_plus' in H3; eauto.
        apply qqcap_sub_r. discriminate.
        eapply closed_qual_subst1'; eauto. apply qqcap_sub_r.
        discriminate.
  ...
  - assert (dx ⊑ ∅). {
    eapply HwfG with (x := 0). rewrite indexr_skips.
    simpl. reflexivity. simpl. lia. }
    apply qual_less_than_empty in H8. destruct H8. intuition.
    simpl. apply t_abs.
    + destruct H0; subst; simpl. left. auto.
      right. rewrite NatSetFacts.empty_b. rewrite remove_empty.
      rewrite unsplice_set_empty. auto.
    + eapply closed_tm_subst1'; eauto.
    + inversion H4. subst. constructor; try eapply closed_ty_subst1'; eauto; eapply closed_qual_subst1'; eauto.
    + eapply closed_qual_subst1'; eauto.
    + eapply closed_qual_subst1'; eauto.
    ...
    + replace (length (Γ0 ++ [(Tx, dx)])) with (S (length Γ0)) in IHHT.
      ...
      2: { subst. rewrite subst1_qlub_dist. f_equal.
      ...
      { destruct H0; subst; simpl; auto. intuition; fnsetdec. }
      { subst. apply eqqual_iff_eq. apply eqqual_iff_eq in Hsep.
        eapply qqcap_fresh_r'; eauto. lia. }
  ...
  - intuition. rename H6 into IHHT2. rename H7 into IHHT1. simpl.
    ...
    assert (Hoverlap: {0 |-> dx' }ᵈ (df ⋒ d1) = {0 |-> dx' }ᵈ df ⋒ {0 |-> dx' }ᵈ d1). {
      apply has_type_filter in HT1, HT2. symmetry. eapply subst1_preserves_separation; eauto.
      1,2: eapply subqual_trans; eauto.
    }
    apply t_app with (T1:= { 0 |-> dx' }ᵀ T1) (df:=({0 |-> dx' }ᵈ df)).
    assert (({0 |-> dx' }ᵈ df ⋒ {0 |-> dx' }ᵈ d1) = {0 |-> dx' }ᵈ (df ⋒ d1)). {
      destruct df; destruct d1; simpl; auto.
    }
    replace (TFun ({0 |-> dx' }ᵈ df ⋒ {0 |-> dx' }ᵈ d1) ({0 |-> dx' }ᵈ d2) ({0 |-> dx' }ᵀ T1) ({0 |-> dx' }ᵀ T2))
            with ({ 0 |-> dx' }ᵀ (TFun (df ⋒ d1) d2 T1 T2)).
    eapply IHHT1; eauto. simpl. rewrite H5. auto.
    eapply IHHT2; eauto.
    + eapply closed_ty_subst1'; eauto.
    + unfold openq. rewrite <- @subst1_qual_empty with (dx:=dx'). erewrite <- subst1_open_qual_comm; eauto.
      apply subst_qual_subqual_monotone. unfold openq in H4. auto.
    + rewrite <- Hoverlap. apply qual_less_than_empty in H4.
      destruct H4; rewrite H4; simpl; auto.
      assert (mem 0 {}N = false).
      apply NatSetProperties.FM.not_mem_iff. intro F. fnsetdec.
      rewrite H5. rewrite remove_empty. rewrite unsplice_set_empty.
      simpl. intuition; fnsetdec.
...
Lemma substitution_gen2 :
  forall {t Γ φ' Tx dx dx' Σ T d}, dx = qbot ->
    forall {φ}, φ ⊑ φ' -> ∅ ⊑ φ ->
      has_type (Γ ++ [(Tx,dx)]) φ Σ t T d ->
      wf_tenv (Γ ++ [(Tx,dx)]) ->
...
Proof.
  Import NatSet.F.
  intros t Γ φ' Tx dx dx' Σ T d Hdx φ Hphi Hphilb HT HwfG tx HTx Hdx'.
  specialize (vtp_closed HTx) as Hclx.
  intuition. remember (Γ ++ [(Tx, dx)]) as Γ'.
  generalize dependent Γ. generalize dependent φ'.
  induction HT; intros; pose (φs := {0 |-> dx'}ᵈ φ);
    replace ({0 |-> dx' }ᵈ φ) with φs in *; auto.
  - subst. apply t_base; auto. eapply closed_qual_subst1'; eauto.
  - subst. apply vtp_has_type in HTx. simpl. bdestruct (x =? 0).
    + (*x is 0 *) rewrite indexr_skips in H0; simpl; auto; try lia. simpl in H0. subst. simpl in H0.
      inversion H0. subst. erewrite subst1_ty_id; eauto. simpl.
      eapply weaken'; eauto. eapply closed_qual_subst1'; eauto.
    + (*x is in Γ0*) assert (Hx: 1 <= x); try lia. destruct x; try lia.
      rewrite subst1_qual_plus_dist; auto. apply t_var; auto.
      eapply indexr_subst1; eauto. rewrite <- subst1_qual_plus_dist; auto.
      repeat apply subst_qual_subqual_monotone. auto.
  ...
  - simpl. apply t_abs.
    + destruct H0; subst; simpl. left. auto.
      right. rewrite NatSetFacts.empty_b. rewrite remove_empty.
      rewrite unsplice_set_empty. auto.
    + subst. eapply closed_tm_subst1'; eauto.
    + inversion H4. subst. constructor; try eapply closed_ty_subst1'; eauto;
                             eapply closed_qual_subst1'; eauto.
    + subst. eapply closed_qual_subst1'; eauto.
    + subst. eapply closed_qual_subst1'; eauto.
    + apply subst_qual_subqual_monotone. auto.
    + subst. replace (length (Γ0 ++ [(Tx, qbot)])) with (S (length Γ0)) in IHHT.
      2: { rewrite app_length. simpl. lia. }
      rewrite subst1_tenv_length. rewrite app_comm_cons in IHHT.
      remember (df ⊔ just_fv (S (length Γ0))) as DF.
      replace ({0 |-> ⊥}ᵈ df ⊔ just_fv (length Γ0)) with ({0 |-> ⊥}ᵈ DF).
      2: { subst. rewrite subst1_qlub_dist. f_equal.
           repeat rewrite <- subst1_just_fv. auto. }
      remember (φ' ⊔ just_fv (S (length Γ0))) as φ''.
      specialize IHHT with (φ' := φ'') (Γ := (((T1, d1) :: Γ0))).
      ...
      apply wf_tenv_cons; auto.
      { destruct H0; subst; simpl; auto. intuition; fnsetdec. }
      { subst. destruct φ'. destruct φ. inversion Hphilb. simpl in Hphi. inversion Hphi.
        destruct df; simpl. intuition; try fnsetdec.
        specialize (subqual_trans H7 Hphi) as Hsub. simpl in Hsub.
        destruct Hsub. destruct H9. intuition; try fnsetdec.
      }
  - subst. intuition. simpl in *. rename H6 into IHHT2. rename H7 into IHHT1.
    replace ({ 0 |-> qbot }ᵈ (openq d1 d2)) with (openq ({ 0 |-> qbot }ᵈ d1) ({ 0 |-> qbot }ᵈ d2)).
    (*separation/overap is preserved after substitution*)
    assert (Hoverlap: {0 |-> qbot }ᵈ (df ⋒ d1) = {0 |-> qbot }ᵈ df ⋒ {0 |-> qbot }ᵈ d1). {
      apply substq_qqcap_qbot_dist.
    }
    apply t_app with (T1:= { 0 |-> qbot }ᵀ T1) (df:=({0 |-> qbot }ᵈ df)).
    assert (({0 |-> qbot }ᵈ df ⋒ {0 |-> qbot }ᵈ d1) = {0 |-> qbot }ᵈ (df ⋒ d1)). {
      destruct df; destruct d1; simpl; auto.
    }
    replace (TFun ({0 |-> qbot }ᵈ df ⋒ {0 |-> qbot }ᵈ d1) ({0 |-> qbot }ᵈ d2) ({0 |-> qbot }ᵀ T1) ({0 |-> qbot }ᵀ T2))
            with ({ 0 |-> qbot }ᵀ (TFun (df ⋒ d1) d2 T1 T2)).
    eapply IHHT1; eauto. simpl. rewrite H5. auto.
    eapply IHHT2; eauto.
    + eapply closed_ty_subst1'; eauto.
    + unfold openq. rewrite <- @subst1_qual_empty with (dx:=qbot). erewrite <- subst1_open_qual_comm; eauto.
      apply subst_qual_subqual_monotone. unfold openq in H4. auto.
    + rewrite <- Hoverlap. apply qual_less_than_empty in H4.
      destruct H4; rewrite H4; simpl; auto.
      assert (mem 0 {}N = false).
  ...
  - subst. erewrite @subst1_qual_id with (q:=(just_loc l)); eauto. simpl. erewrite subst1_ty_id; eauto.
    apply t_loc; auto. eapply closed_qual_subst1'; eauto. subst φs.
    destruct φ; simpl in *; auto; intuition;
      try (destruct (NatSet.F.mem 0 t0); intuition; fnsetdec).
    constructor; intuition. rewrite bound_empty. lia.
  - subst. erewrite @subst1_qual_id with (q:=∅); eauto. simpl in *.
    apply t_ref. eapply IHHT; eauto. erewrite subst1_ty_id; eauto.
    destruct φ; auto. unfold φs. simpl.
    destruct (mem 0 t1) eqn:Hmem; simpl; intuition; try fnsetdec.
  - simpl. apply t_deref with (d := { 0 |-> dx' }ᵈ d). eapply IHHT; eauto.
  - simpl. apply t_assign with (T := ({ 0 |-> dx' }ᵀ T)) (d := ({ 0 |-> dx' }ᵈ d)).
    eapply IHHT1; eauto. simpl in IHHT2. eapply IHHT2; eauto.
  - subst. apply t_sub with (T1:={ 0 |-> ⊥ }ᵀ T1) (d1:={ 0 |-> ⊥ }ᵈ d1).
    eapply IHHT; eauto. eapply subst_stp; eauto.
    replace (⊥) with (⊥ ⋒ ⊥) at 1. econstructor; eauto. cbn. auto.
    apply subst_qual_subqual_monotone; auto.
    Unshelve. all : exact 0.
Qed.

Lemma substitution : forall {t df Tx dx Σ T d},
    has_type [(Tx,df ⋒ dx)] (df ⊔ (just_fv 0)) Σ t T d ->
    wf_tenv [(Tx,df ⋒ dx)] -> closed_qual 0 0 (length Σ) df ->
    forall {vx}, vtp Σ vx Tx dx ->
            has_type [] ((df ⊔ dx) ⊔ ∅) Σ (* at least empty set flt *)
                     ({ 0 |-> vx }ᵗ t)
                     ({ 0 |-> dx }ᵀ T)
                     ({ 0 |-> dx }ᵈ d).
Proof.
  intros. subst.
  specialize (vtp_closed H2) as Hclx. specialize (has_type_closed H) as Hclt.
  ...
  - rewrite <- Hdfdx in H.
    ...
    rewrite <- Hdfdx in H0.
    apply qqcap_qbot' in Hdfdx as Hbot. destruct Hbot as [Hdfbot Hdxbot].
    eapply (substitution_gen2 Hdfdx Hrefl Hmt) in H; eauto.
    subst. simpl in *. assert (mem 0 (singleton 0) = true). apply mem_1. fnsetdec.
    rewrite H9 in H. rewrite remove_singleton_empty in H; rewrite unsplice_set_empty in H.
    apply H.
  - assert (Hsepf : dx ⋒ DF ≡ df ⋒ dx). {
      subst. destruct dx.
      - destruct df; simpl in *. cbv in Hdfdx. inversion Hdfdx.
        intuition; fnsetdec.
      - inversion H8. subst. simpl in *. apply bound_0_empty in H16. subst.
        destruct df; simpl; intuition; try fnsetdec.
    }
    assert (Hdx : df ⋒ dx <> qbot). { intuition. rewrite Hdfdx in H9. inversion H9. }
    assert (Hrefl : DF ⊑ DF). apply subqual_refl.
    assert (Hmt : ∅ ⊑ DF). subst. destruct df; intuition; fnsetdec.
    rewrite <- Hdfdx in H. rewrite <- Hdfdx in H0.
    eapply (substitution_gen1 Hsepf Hdx Hrefl Hmt) in H; eauto.
    destruct df eqn:Hdf; subst.
    + simpl in *. assert (mem 0 (singleton 0) = true). apply mem_1. fnsetdec.
      rewrite H9 in H.
      assert (dx <> qbot). { intro. subst. cbv in Hdx. intuition. }
      destruct dx; intuition.
      rewrite remove_singleton_empty in H; rewrite unsplice_set_empty in H.
      simpl. repeat rewrite empty_union_left in H.
      repeat rewrite empty_union_right. apply H.
    + assert (({0 |-> dx }ᵈ ((qset t4 t5 t6) ⊔ just_fv 0)) = ((qset t4 t5 t6) ⊔ dx)). {
        destruct dx; simpl.
        * assert (mem 0 (union t4 (singleton 0)) = true). apply mem_1. fnsetdec.
          rewrite H9. repeat rewrite empty_union_right.
          f_equal. inversion H1. subst. apply bound_0_empty in H17. subst.
          rewrite empty_union_left. rewrite remove_singleton_empty.
          rewrite unsplice_set_empty. auto.
        * assert (mem 0 (union t4 (singleton 0)) = true). apply mem_1. fnsetdec.
          rewrite H9. repeat rewrite empty_union_right. f_equal.
          inversion H1. inversion H8. subst. apply bound_0_empty in H17, H26.
          subst. f_equal. rewrite empty_union_left. rewrite remove_singleton_empty.
          rewrite unsplice_set_empty. auto.
      }
      rewrite H9 in H. rewrite qlub_empty_right. apply H.
      destruct dx; simpl; discriminate.
Qed.
...
Lemma substq_qbot_empty_eq : forall {q}, {0 |-> qbot }ᵈ q = {0 |-> ∅ }ᵈ q.
Proof.
  intros. destruct q; simpl; auto.
  repeat rewrite empty_union_right. auto.
Qed.

Lemma substq_lub_empty : forall {d q}, {0 |-> d }ᵈ q = {0 |-> d ⊔ ∅ }ᵈ q.
Proof.
  intros. destruct d. simpl. rewrite substq_qbot_empty_eq. auto.
  rewrite qlub_empty_right. auto. discriminate.
Qed.

Lemma open_ty_lub_empty : forall {d T}, ({0 |-> d }ᵀ T) = ({0 |-> d ⊔ ∅ }ᵀ T).
Proof.
  intros. induction T; simpl; auto.
  rewrite IHT1. rewrite IHT2. f_equal. apply substq_lub_empty. apply substq_lub_empty.
  rewrite IHT. auto.
Qed.

Lemma openq_lub_empty : forall {d1 d2}, openq d1 d2 = openq (d1 ⊔ ∅) d2.
Proof.
  intros. destruct d1; simpl; auto. destruct d2; simpl; auto.
  destruct (mem 0 t1) eqn:Hmem.
  repeat rewrite empty_union_right. auto.
  f_equal. apply NatSetProperties.FM.not_mem_iff in Hmem.
  specialize (@NatSetProperties.remove_equal t1 0) as Heq. intuition.
  apply NatSet.eq_if_Equal in H; auto.
  repeat rewrite empty_union_right. auto.
Qed.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    apply has_type_closed in H0 as HH0. intuition.
    (* t1 *) specialize (H7 (TFun (df ⋒ d1) d2 T1 T) t1). intuition.
    (* t2 *) specialize (H9 T1 t2). intuition.
    rename H into Ht1. rename H0 into Ht2.
    + (* both t1 and t2 are values, i.e. the beta case *)
      (* turn has_type to vtp *)
      assert (has_type [] (ldom Σ) Σ (tapp t1 t2) T (openq d1 d2)). { eapply t_app; eauto. }
      apply has_type_vtp in Ht1 as VHt1; intuition.
      pose (VHHt1 := VHt1). inversion VHt1. subst.
      apply vtp_canonical_form_lam in VHt1 as HHt1; intuition.
      specialize (has_type_filter Ht2) as Hfltt2.
      apply has_type_vtp in Ht2 as VTPt2; intuition.
      exists (open_tm t2 t0). exists σ. intuition.
      * (* progress *) constructor. intuition.
      * (* preservation *) apply (Preserve Σ (qbot)); auto.
        rewrite qqplus_qbot_right_neutral.
        apply qstp_closed in H31 as H31'; intuition.
        change (length []) with 0 in *. subst.
        (* apply value_filter in Ht2 as H0F; intuition. *)
        assert (df1 ⊑ df). apply qstp_inv in H31. auto.
        assert (df1 ⋒ d1 ⊑ df ⋒ d1). {
          destruct d1; destruct df; destruct df1; simpl; auto.
          simpl in H20. inversion H20. intuition; fnsetdec.
          simpl in H20. intuition; fnsetdec.
        }
        assert (df1 ⋒ d1 ⊑ d0). {
          eapply subqual_trans. apply H21.
          apply stp_qstp_inv in H29. apply qstp_inv in H29. auto.
        }
        apply qual_less_than_empty in H33. destruct H33.
        -- (* d0 = qbot, then df1 and d1 are both qbot *)
           rewrite H28 in H27. apply qqcap_qbot in H27. destruct H27.
           apply stp_qstp_inv in H29 as H29'.
           apply qstp_inv in H29'. rewrite H28 in H29'.
           apply qqcap_qbot in H29'. destruct H29'.
           (* df is also qbot *)
           assert (df1 ⋒ d1 = qbot). { subst. auto. }
           rewrite H28 in H26. rewrite <- H35 in H26.
           eapply @substitution with (vx := t2) in H26 as H26'; eauto; intuition.
           inversion H25. subst.
           unfold open_tm' in H26'. unfold open_ty' in H26'.
           unfold openq' in H26'. simpl in H26'.
           apply qstp_closed in H31 as H31'. intuition.
           unfold open_ty in H26'. unfold openq in H26'.
           erewrite <- open_subst1_tm in H26'; eauto.
           erewrite <- open_subst1_qual in H26'; eauto.
           fold (open_tm t2 t0) in H26'. fold (openq qbot d3) in H26'.
           apply @weaken_flt with (φ':= (ldom Σ)) in H26'; auto; intuition.
           apply has_type_closed in H26' as Hcl. intuition.
           assert (stp [] Σ ({0 |-> qbot }ᵀ (open_ty' (@nil (ty * qual)) T2)) (openq qbot d3) T (openq qbot d2)). {
            eapply @subst_stp with (Γ := [])(df' := qbot) in H32; eauto.
            replace (open_ty' (@nil (ty * qual)) T) with T in *. (* because T is closed *)
            inversion H24. subst.
            simpl in H32. unfold openq' in H32. unfold openq in H32. simpl in H32.
            erewrite @subst1_ty_id with (T := T) in  H32; eauto.
            erewrite <- open_subst1_qual in H32; eauto.
            erewrite <- open_subst1_qual in H32; eauto.
            unfold open_ty'. unfold open_ty. repeat erewrite closed_ty_open_id; eauto.
          }
          eapply t_sub in H26'; eauto. apply has_type_filter in H. intuition.
           assert (has_type [] (ldom Σ) Σ (tabs t0) (TFun qbot d3 T0 T2) df1). {
            constructor; intuition. inversion H31. subst.
            eapply subqual_trans in Hfltt2; eauto. rewrite H27 in H26.
            change (length []) with 0 in *.
            subst. simpl in *. auto. rewrite H35 in H26. intuition.
          }
          apply has_type_filter in H36.
          specialize (qlub_bound H36 Hfltt2). intros.
          assert ((df1 ⊔ d1) ⊔ ∅ ⊑ ldom Σ). { subst. simpl. intuition; fnsetdec. }
          apply wf_tenv_cons; auto. subst. intuition.
          subst. eapply vtp_widening; eauto.
        -- (* d0 = {}, then df1 ⋒ d1 could be either qbot or {} *)
           replace (d0) with (df1 ⋒ (d1 ⊔ ∅)) in H26.
           assert (vtp Σ t2 T0 (d1 ⊔ ∅)). {
            eapply vtp_widening; eauto.
            eapply stp_scale_qglb with (d := qbot) in H29.
            rewrite glb_qbot in H29. rewrite glb_qbot in H29.
            eapply stp_scale_qlub with (d := (d1 ⊔ ∅)) in H29.
            rewrite qlub_qbot_left in H29.
            eapply s_trans; eauto. eapply stp_refl; eauto.
            apply stp_closed in H29. intuition.
            econstructor; eauto. specialize (@qlub_is_lub d1 ∅). intuition.
            eapply closed_qual_qlub; eauto. eauto.
          }
          inversion H24. inversion H25. subst. clear H43.
          eapply @substitution with (vx := t2) in H26 as H26'; eauto; intuition.
          unfold open_tm' in H26'. unfold open_ty' in H26'.
          unfold openq' in H26'. simpl in H26'.
          apply qstp_closed in H31 as H31'. intuition.
          ...
          erewrite <- open_subst1_tm in H26'; eauto.
          erewrite <- open_subst1_qual in H26'; eauto.
          fold (open_tm t2 t0) in H26'. fold (openq (d1 ⊔ ∅) d3) in H26'.
          apply @weaken_flt with (φ':= (ldom Σ)) in H26'; auto; intuition.
          apply has_type_closed in H26' as Hcl. intuition.
          replace (openq (d1 ⊔ ∅) d3) with (openq d1 d3) in H26', H38.
          replace ({0 |-> d1 ⊔ ∅ }ᵀ ([[0 ~> just_fv 0 ]]ᵀ T2)) with
             ({0 |-> d1 }ᵀ ([[0 ~> just_fv 0 ]]ᵀ T2)) in H26'.
           assert (stp [] Σ ({0 |-> d1 }ᵀ (open_ty' (@nil (ty * qual)) T2)) (openq d1 d3) T (openq d1 d2)). {
            eapply @subst_stp with (Γ := [])(df' := d1) in H32; eauto.
            replace (open_ty' (@nil (ty * qual)) T) with T in *. (* because T is closed *)
            simpl in H32. unfold openq' in H32. unfold openq in H32. simpl in H32.
            erewrite @subst1_ty_id with (T := T) in  H32; eauto.
            erewrite <- open_subst1_qual in H32; eauto.
           ...
           eapply t_sub in H26'; eauto. apply has_type_filter in H. intuition.
           apply open_ty_lub_empty. apply openq_lub_empty.
           { apply qlub_bound. apply qlub_bound. eapply subqual_trans. apply H20.
             apply closed_qual_subqual_ldom; auto.
             apply qlub_bound. auto. simpl; intuition; fnsetdec.
             simpl; intuition; fnsetdec. }
           apply wf_tenv_cons; auto. apply qqcap_lub_empty_preserve; auto.
           { subst. destruct d1; simpl. destruct df1; simpl; auto. cbv.
             f_equal; fnsetdec. repeat rewrite empty_union_right.
             destruct df1; simpl; cbv; auto.
             simpl in H27. intuition. f_equal; fnsetdec. }
    + (* t1 value, t2 steps *)
      apply has_type_vtp in H as VH; intuition.
      apply vtp_canonical_form_lam in VH as HH; intuition. pose (HH10 := H10).
      unfold CtxOK in HH10. specialize (H7 σ). intuition.
      destruct H21 as [t2' [σ' HH21]]. exists (tapp t1 t2'). exists σ'. intuition.
      * (* progress *) constructor; intuition.
      * (* preservation *) destruct H21. apply (Preserve Σ' d'); intuition.
        inversion H23; subst.
        -- rewrite qqplus_qbot_right_neutral.
           rewrite qqplus_qbot_right_neutral in H24. eapply t_app with (df := df); eauto.
        ...
        -- specialize (has_type_closed H). specialize (has_type_closed H24). intuition.
           inversion H32. subst. rewrite <- openq_duplicate_eq.
           assert (just_loc l ⋒ df = ∅). {eapply freshness. eauto. eauto. }
           assert (df ⋒ (d1 ⋓ just_loc l) ⊑ ∅). {eapply qqcap_lub_empty_preserve'; eauto. }
          assert (df ⋒ (d1 ⋓ just_loc l) = df ⋒ d1). {
            destruct df; destruct d1; simpl; auto. cbv in H33, H36.
            cbv. repeat rewrite empty_union_right. f_equal.
            apply qset_empty_inv in H33. intuition.
            specialize (@NatSetProperties.inter_sym t4 (union t7 (singleton l))) as HI.
            apply NatSet.eq_if_Equal in HI. rewrite HI.
            specialize (@NatSetProperties.union_inter_1 t7 (singleton l) t4) as HIU.
            apply NatSet.eq_if_Equal in HIU. rewrite HIU. fnsetdec. }
           ...
           ++ apply has_type_filter in H0. apply qqplus_bound. apply openq_subqual. apply qqplus_bound.
              1, 3 : eapply subqual_trans; eauto; apply extends_ldom; auto. all : apply just_loc_ldom; auto.
    + (* t1 steps *)
      ...
      * rewrite qqplus_qbot_right_neutral. rewrite qqplus_qbot_right_neutral in H26. eapply t_app; eauto.
        ...
        eapply subqual_trans; eauto; apply extends_ldom; auto.
      * assert (just_loc l ⋒ d1 = ∅). { eapply freshness. eauto. eauto. }
        assert ((df ⋓ just_loc l) ⋒ d1 ⊑ ∅). {
          rewrite qqcap_commute. eapply qqcap_lub_empty_preserve'; eauto.
          rewrite qqcap_commute. auto. }
        assert ((df ⋓ just_loc l) ⋒ d1  = df ⋒ d1). {
          destruct df; destruct d1; simpl; auto. cbv in H29, H30.
          cbv. repeat rewrite empty_union_right. f_equal.
          apply qset_empty_inv in H29. intuition.
          specialize (@NatSetProperties.union_inter_1 t4 (singleton l) t7) as HIU.
          apply NatSet.eq_if_Equal in HIU. rewrite HIU. fnsetdec. }
        rewrite <- H31 in H26.
        eapply t_sub with (T1 := T) (d1 := (openq d1 d2)).
        -- eapply t_app; eauto. eapply weaken_flt. eapply weaken_store. eauto. auto. apply extends_ldom; auto.
           auto. simpl. eapply closed_ty_monotone; eauto. apply extends_length; auto.
           eapply subqual_trans; eauto.
        -- apply stp_refl. simpl. eapply closed_ty_monotone; eauto. apply extends_length; auto.
           constructor. apply qqplus_gt. apply closed_qual_qqplus. 1,2 : eapply closed_qual_monotone; eauto; intuition.
           apply just_loc_closed. auto.
        -- apply has_type_filter in H0. apply qqplus_bound.
           apply openq_subqual; eapply subqual_trans; eauto; apply extends_ldom; auto.
  ...
  - (* deref *)
    subst. intuition. specialize (has_type_closed H) as HH. intuition.
    specialize (H3 (TRef T0) t0). intuition.
    + right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
      ...
      simpl. apply t_sub with (T1 := T) (d1:= qbot).
      specialize (H11 l v T). intuition.
      replace Σ with (Σ ++ []); intuition.
      eapply weaken_stp_store. auto. simpl. auto.
    + right. intros. specialize (H8 σ H3).
      destruct H8 as [t' [σ' HH8]]. exists (tderef t'). exists σ'. intuition. constructor; auto.
      destruct H10. apply (Preserve Σ' qbot); intuition. simpl.
  ...
  - (* stp *)
    ...
    apply qqplus_bound. eapply subqual_trans; eauto. inversion H14; subst; auto.
    apply just_loc_ldom; auto.
Qed.
