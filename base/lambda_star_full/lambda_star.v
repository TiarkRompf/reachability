...
Definition open_qual (k : nat) (d' : qual) (d : qual) : qual :=
  match d, d' with
  | qset vs bs ls, qbot => qset vs (NatSet.F.remove k bs) ls
...
Lemma openq_bot_empty_eq : forall d, (openq ∅ ∅ d) = (openq qbot qbot d).
Proof.
  destruct d; cbv; auto.
  destruct (NatSet.F.mem 0 t0) eqn:H1.
  - repeat rewrite empty_union_right.
    destruct (NatSet.F.mem 1 (NatSet.F.remove 0 t0)) eqn:H2; auto.
    rewrite @remove_not_in with (x := 1); auto.
  - repeat rewrite empty_union_right.
    destruct (NatSet.F.mem 1 t0) eqn:H2.
    rewrite @remove_not_in with (x := 0); auto.
    rewrite @remove_not_in with (x := 0); auto.
...
| cl_qbot : forall b f l, closed_qual b f l qbot
...
| s_fun : forall Γ Σ T1 d1 T2 d2 T3 d3 T4 d4 d5 d6,
    ...
    qstp Γ Σ d5 d6 ->
    stp Γ Σ T3 d3 T1 d1 ->
    stp ((false, T3,d3) :: (true, TFun d1 d2 T1 T2, d5) :: Γ) Σ (open_ty' Γ T2) (openq' Γ d2) (open_ty' Γ T4) (openq' Γ d4) ->
...
| t_base : forall Γ Σ φ,
    ...
    has_type Γ φ Σ tunit TUnit qbot
...
| t_abs: forall Γ φ Σ T1 d1 T2 d2 df t,
    ...
    has_type ((false,T1,d1) :: ((true,TFun d1 d2 T1 T2), df) :: Γ)
             (df ⊔ (just_fv (length Γ)) ⊔ (just_fv (S (length Γ))))
             Σ (open_tm' Γ t) (open_ty' Γ T2) (openq' Γ d2) ->
...
| t_ref: forall Γ φ Σ T t,
    has_type Γ φ Σ t T qbot ->
    closed_ty 0 0 0 T ->
    ∅ ⊑ φ ->
    has_type Γ φ Σ (tref t) (TRef T) ∅

| t_deref: forall Γ φ Σ T d t,
    has_type Γ φ Σ t (TRef T) d ->
    has_type Γ φ Σ (tderef t) T qbot

| t_assign: forall Γ φ Σ T t1 d t2,
    has_type Γ φ Σ t1 (TRef T) d ->
    has_type Γ φ Σ t2 T qbot ->
    has_type Γ φ Σ (tassign t1 t2) TUnit qbot

| t_self : forall Γ φ Σ e T f df T1 d1 T2 d2,
    has_type Γ φ Σ e T (df ⊕ f) ->
    indexr f Γ = Some (true, TFun d1 d2 T1 T2, df) ->
    closed_qual 0 f (length Σ) df ->
    (just_fv f) ⊑ φ ->
    has_type Γ φ Σ e T (just_fv f)
...
Definition CtxOK (Γ : tenv) (φ : qual) (Σ : senv) (σ : store) : Prop :=
  length Σ = length σ /\
  forall l v T, indexr l Σ = Some T -> indexr l σ = Some v ->
    value v /\ has_type Γ φ Σ v T qbot.
...
Definition subst_q (q : qual) (v : nat) (q' : qual) : qual :=
  ...
  | qset fvs bvs ls =>
    ...
    | true =>  match q' with
              | qbot               => qset (unsplice_set 0 (NatSet.F.remove v fvs)) bvs ls
              | qset fvs' bvs' ls' =>
...
| disj_bot : d = qbot -> disjointq Σ Σ' d
| disj_loc : forall l,
    (length Σ) <= l ->
    l < (length Σ') ->
...
| vtp_abs: forall Σ T1 d1 T2 d2 T3 d3 T4 d4 df1 df2 t,
  ...
  has_type [(false,T1,d1) ; (true,TFun d1 d2 T1 T2, df1)]
           (df1 ⊔ (just_fv 0) ⊔ (just_fv 1)) Σ
           (open_tm' ([]: tenv) t) (open_ty' ([]: tenv) T2) (openq' ([]: tenv) d2) ->
  stp [] Σ T3 d3 T1 d1 ->
  qstp [] Σ df1 df2 ->
  stp [(false,T3, d3) ; (true,TFun d1 d2 T1 T2, df1)] Σ
      (open_ty' ([]: tenv) T2) (openq' ([] : tenv) d2)
      (open_ty' ([]: tenv) T4) (openq' ([]: tenv) d4) ->
...
Lemma extends_ge: forall {A}, forall {l1 l2 : list A}, (l1 ++ l2) ⊇ l2.
Proof. intros. unfold extends. exists l1. intuition.
Qed.
...
Proof.
  intros. induction d.
  - unfold subqual in H0. destruct d'. auto. inversion H0.
  - inversion H. subst. destruct d'. constructor.
    inversion H0. intuition. constructor; eapply subset_bound; eauto.
Qed.

Lemma closed_qual_qbot : forall {b f l}, closed_qual b f l qbot.
  intros. constructor.
Qed.
#[global] Hint Resolve closed_qual_qbot : core.
...
Proof.
  intros b f l d H. induction H; intros d' Heq; apply eq_if_eqqual in Heq.
  subst. constructor. destruct d'; intuition.
  inversion Heq. destruct H3. subst.
  constructor; eapply set_eq_bound; eauto; try fnsetdec.
Qed.
...
Lemma bot_just_fv_closed: forall {x b f l d},
    x < f -> closed_qual b f l (d ⊓ ∅ ⊕ x).
Proof.
  intros. destruct d. simpl. constructor.
  simpl. repeat rewrite inter_empty_right.
  repeat rewrite empty_union_left.
  apply just_fv_closed. auto.
Qed.
...
Proof.
  intros. destruct d; simpl; auto. inversion H. subst.
  specialize (@bound_gt_not_in _ _ _ H0 H8) as HNotIn.
  destruct x. f_equal. fnsetdec.
  assert (NatSet.F.mem n t0 = false).
  apply NatSetProperties.FM.not_mem_iff. auto. rewrite H1. auto.
Qed.
...
Proof.
  intros. destruct d; simpl. constructor. inversion H. subst.
...
Proof.
 destruct d; intros; simpl; intuition. inversion H. subst.
  ...
  - unfold bound in H9. destruct (NatSet.F.max_elt t0) eqn:Hmax.
    assert (e <= b) by lia. bdestruct (e <? b).
    + rewrite remove_not_in. unfold bound. rewrite Hmax. lia.
      apply NatSetDecide.FSetDecideAuxiliary.F.not_mem_iff.
      eapply max_gt_bound_not_in; eauto.
    + assert (e = b) by lia. subst. eapply remove_max_bound'.
      apply Hmax. apply NatSet.F.mem_1. apply NatSet.F.max_elt_1. auto. auto.
    + apply max_elt_empty' in Hmax. subst.
      rewrite remove_empty. rewrite bound_empty. lia.
  - inversion H1. subst.
...
Proof.
  intros. destruct d; intros; simpl; auto; intuition.
  destruct d'; simpl; auto; intuition.
  - unfold open_qual.
    destruct (NatSet.F.mem i t0) eqn:Hmem1.
    assert (NatSet.F.mem i (NatSet.F.remove j t0) = true).
    apply NatSet.F.mem_1. apply NatSetDecide.FSetDecideAuxiliary.F.remove_neq_iff.
    auto. apply NatSet.F.mem_2. auto. rewrite H1. f_equal.
    rewrite empty_union_right. rewrite empty_union_right.
    fnsetdec.
    assert (NatSet.F.mem i (NatSet.F.remove j t0) = false).
    rewrite NatSetDecide.FSetDecideAuxiliary.F.remove_neq_b; auto.
    rewrite H1. auto.
  - inversion H0. subst. unfold open_qual.
    destruct (just_fv x) eqn:Hfv. unfold just_fv in Hfv.
    inversion Hfv. inversion Hfv. subst.
...
Proof.
  destruct d; destruct d'; intros; simpl; auto.
  - destruct d''; simpl; auto; intuition; f_equal; try fnsetdec.
    inversion H1. subst.
    destruct (NatSet.F.mem i t0) eqn:Hmem1.
    + assert (NatSet.F.mem i (NatSet.F.remove j t0) = true).
      apply NatSet.F.mem_1. apply NatSetDecide.FSetDecideAuxiliary.F.remove_neq_iff.
      auto. apply NatSet.F.mem_2. auto. rewrite H2. simpl. f_equal.
      apply bound_0_empty in H9. subst. rewrite empty_union_right.
      rewrite empty_union_right. fnsetdec.
    + assert (NatSet.F.mem i (NatSet.F.remove j t0) = false).
      rewrite NatSetDecide.FSetDecideAuxiliary.F.remove_neq_b; auto.
      rewrite H2. auto.
  - destruct d''; simpl; auto; intuition; inversion H0. subst.
    + apply bound_0_empty in H9. subst.
      destruct (NatSet.F.mem j t0) eqn:Hmem1.
      * assert (NatSet.F.mem j (NatSet.F.remove i t0) = true).
        apply NatSet.F.mem_1. apply NatSetDecide.FSetDecideAuxiliary.F.remove_neq_iff.
        auto. apply NatSet.F.mem_2. auto. rewrite H2. simpl. f_equal.
        rewrite empty_union_right. rewrite empty_union_right. fnsetdec.
      * assert (NatSet.F.mem j (NatSet.F.remove i t0) = false).
        rewrite NatSetDecide.FSetDecideAuxiliary.F.remove_neq_b; auto.
        rewrite H2. auto.
...
Lemma open_qual_just_fv : forall i d x, open_qual i d (just_fv x) = (just_fv x).
  intros. compute. destruct d. f_equal. fnsetdec.
...
Proof.
  intros. destruct d1; destruct d2; destruct df; simpl; auto; intuition;
  unfold splice_qual; unfold openq; unfold open_qual.
  - destruct (NatSet.F.mem 0 t0) eqn: H1. f_equal. apply splice_set_union_dist. auto.
  - destruct (NatSet.F.mem 1 (NatSet.F.remove 0 t3)) eqn:Hmem; f_equal.
    apply splice_set_union_dist.
  - destruct (NatSet.F.mem 0 t3) eqn: H1.
    destruct (NatSet.F.mem 1 (NatSet.F.union (NatSet.F.remove 0 t3) t6)) eqn: H2.
    f_equal. rewrite splice_set_union_dist. rewrite splice_set_union_dist. auto.
    f_equal. rewrite splice_set_union_dist. auto.
    destruct (NatSet.F.mem 1 t3). rewrite splice_set_union_dist. auto. auto.
Qed.
...
Proof.
  intros. split; intros.
  - unfold splice_qual. inversion H0; subst. auto.
    destruct d2; auto. unfold subqual in *. subst. intros; intuition.
    eapply splice_set_preserves_superset_1; eauto.
  - unfold subqual in *. destruct d1; auto. destruct d2; auto.
...
Lemma stp_closed : forall {Γ Σ T1 d1 T2 d2},
    ...
    /\ closed_qual 0 (length Γ) (length Σ) d2.
Proof.
  intros Γ Σ T1 d1 T2 d2 HS. induction HS.
...
Proof.
  ...
  constructor. inversion H0; subst. destruct d1; auto.
...
Lemma weaken_stp_gen : forall {Γ1 Γ2 Σ T1 d1 T2 d2},
    stp (Γ1 ++ Γ2) Σ T1 d1 T2 d2 ->
...
Proof.
  ...
  - assert (stp Γ Σ (TFun d1 d2 T1 T2) d5 (TFun d3 d4 T3 T4) d6). { constructor; intuition. } intros.
    ...
    specialize (IHHstp2 ((false,T3, d3) :: (true,TFun d1 d2 T1 T2, d5) :: Γ1)). intuition.
...
Lemma narrowing_qstp_gen : forall{Γ1 b U du Γ2 Σ d1 d2},
    qstp (Γ1 ++ (b,U,du) :: Γ2) Σ d1 d2 ->
    forall {V dv}, stp Γ2 Σ V dv U du ->
              qstp (Γ1 ++ (b,V,dv) :: Γ2) Σ d1 d2.
  intros Γ1 b U du Γ2 Σ d1 d2 HQ V dv HSTP.
  inversion HQ. subst. constructor.
  auto. rewrite app_length in *. simpl in *. auto.
Qed.

Lemma narrowing_stp_gen : forall{Γ1 b U du Γ2 Σ T1 d1 T2 d2}, stp (Γ1 ++ (b,U,du) :: Γ2) Σ T1 d1 T2 d2 -> forall {V dv}, (stp Γ2 Σ V dv U du) -> stp (Γ1 ++ (b,V,dv) :: Γ2) Σ T1 d1 T2 d2.
Proof. intros Γ1 b U du Γ2 Σ T1 d1 T2 d2 HST. remember (Γ1 ++ (b,U,du) :: Γ2) as Γ.
...
Lemma narrowing_stp : forall{b U du Γ Σ T1 d1 T2 d2}, stp ((b,U,du) :: Γ) Σ T1 d1 T2 d2 -> forall {V dv}, stp Γ Σ V dv U du -> stp ((b,V,dv) :: Γ) Σ T1 d1 T2 d2.
...
Lemma closed_qual_qglb : forall {q1 q2 b f l}, closed_qual b f l q1 -> closed_qual b f l q2 -> closed_qual b f l (q1 ⊓ q2).
...
Lemma closed_qual_qqplus: forall {b f l d1 d2}, closed_qual b f l d1 -> closed_qual b f l d2 -> closed_qual b f l (d1 ⋓ d2).
  unfold qqplus. intros. destruct d1. auto.
...
Lemma qstp_qual_irrelevance : forall {Γ1 Γ2 Σ b U du d1 d2}, qstp (Γ2 ++ (b,U,du) :: Γ1) Σ d1 d2 -> forall {du'}, qstp (Γ2 ++ (b,U,du') :: Γ1) Σ d1 d2.
  intros. inversion H. subst. constructor. auto.
  rewrite app_length in *. simpl in *. auto.
Qed.

Lemma stp_qual_irrelevance : forall {Γ1 Γ2 Σ b U du T1 d1 T2 d2}, stp (Γ2 ++ (b,U,du) :: Γ1) Σ T1 d1 T2 d2 -> forall {du'}, stp (Γ2 ++ (b,U,du') :: Γ1) Σ T1 d1 T2 d2.
  intros. remember (Γ2 ++ (b,U, du) :: Γ1) as Γ. generalize dependent du'. generalize dependent Γ2.
  ...
  - constructor; auto. 1,2 : rewrite app_length in *; simpl in *; auto.
    ...
    specialize (IHstp2 Γ1 du U ((false,T3, d3) :: (true,TFun d1 d2 T1 T2, d5) :: Γ2)).
    intuition.
...
Lemma stp_qual_irrelevance' : forall {Γ1 X Σ b U du T1 d1 T2 d2}, stp (X :: (b,U,du) :: Γ1) Σ T1 d1 T2 d2 -> forall {du'}, stp (X :: (b,U,du') :: Γ1) Σ T1 d1 T2 d2.
  intros. replace (X :: (b, U, du) :: Γ1) with ([X] ++ (b, U, du) :: Γ1) in H; auto.
  replace (X :: (b, U, du') :: Γ1) with ([X] ++ (b, U, du') :: Γ1) ; auto.
...
Lemma stp_scale_qlub : forall {Γ Σ T1 d1 T2 d2}, stp Γ Σ T1 d1 T2 d2 -> forall {d}, closed_qual 0 (length Γ) (length Σ) d -> stp Γ Σ T1 (d1 ⊔ d) T2 (d2 ⊔ d).
  intros Γ Σ T1 d1 T2 d2 HSTP. induction HSTP; intros.
  - constructor. inversion H. subst. constructor.
    apply subqual_qlub. auto. apply closed_qual_qlub; auto.
  - constructor; auto. constructor. apply subqual_qlub. inversion H. subst. auto.
    inversion H. subst. apply closed_qual_qlub; auto.
  - constructor; auto. inversion H1. subst. constructor. apply subqual_qlub. auto.
    apply closed_qual_qlub; auto. eapply stp_qual_irrelevance'; eauto.
  - econstructor; auto.
Qed.

Lemma stp_scale_qglb : forall {Γ Σ T1 d1 T2 d2},
    stp Γ Σ T1 d1 T2 d2 ->
    forall {d}, closed_qual 0 (length Γ) (length Σ) d -> stp Γ Σ T1 (d1 ⊓ d) T2 (d2 ⊓ d).
Proof.
  intros Γ Σ T1 d1 T2 d2 HSTP. induction HSTP; intros.
  - constructor. inversion H. subst. constructor. apply subqual_qglb_r. auto.
    apply closed_qual_qglb; auto.
  - constructor; auto. constructor. apply subqual_qglb_r. inversion H. subst. auto. apply closed_qual_qglb; auto.
    apply qstp_closed in H. intuition.
  - constructor; auto. inversion H1. subst. constructor. apply subqual_qglb_r. auto.
    apply closed_qual_qglb; auto. eapply stp_qual_irrelevance'; eauto.
  - econstructor; auto.
Qed.

Lemma stp_shrink_var : forall {Γ Σ T1 d1 T2 d2},
    stp Γ Σ T1 d1 T2 d2 -> forall {x}, x < length Γ -> stp Γ Σ T1 (just_fv x) T2 (just_fv x).
Proof.
  intros. assert (Hstp: stp Γ Σ T1 qbot T2 qbot). {
    replace qbot with (d1 ⊓ qbot) at 1; auto. replace qbot with (d2 ⊓ qbot) at 2; auto.
    apply stp_scale_qglb; auto.
  }
  replace (just_fv x) with (qbot ⊔ (just_fv x)); auto.
  apply stp_scale_qlub; auto. apply just_fv_closed. auto.
Qed.

Lemma stp_shrink_bot_var : forall {Γ Σ T1 d1 T2 d2},
    stp Γ Σ T1 d1 T2 d2 ->
    forall {x}, x < length Γ -> stp Γ Σ T1 (d1 ⊓ ∅ ⊕ x) T2 (d2 ⊓ ∅ ⊕ x).
Proof.
  intros.
  assert (Hstp: stp Γ Σ T1 qbot T2 qbot). {
    replace qbot with (d1 ⊓ qbot) at 1; auto. replace qbot with (d2 ⊓ qbot) at 2; auto.
    apply stp_scale_qglb; auto.
  }
  specialize (stp_qstp_inv H) as Hq.
  destruct d1.
  - simpl. destruct d2. simpl. auto.
    simpl. repeat rewrite inter_empty_right. repeat rewrite empty_union_left.
    econstructor. apply Hstp. eapply stp_refl.
    apply stp_closed in H. intuition. econstructor. simpl. auto.
    apply just_fv_closed. auto.
  - destruct d2. inversion Hq. subst. simpl in H1. intuition.
    simpl. repeat rewrite inter_empty_right. repeat rewrite empty_union_left.
    eapply stp_shrink_var. apply H. auto.
Qed.
...
Lemma saturated_empty_tenv : forall {q Σ}, closed_qual 0 0 (length Σ) q -> saturated [] Σ q.
  intros. unfold saturated. intros.
  inversion H; subst; simpl in H0; intuition. apply bound_0_empty in H1. subst.
...
Lemma qmem_plus_decomp : forall {x0 q x l}, $ (x0) ∈ᵥ q ⊕ x -> closed_qual 0 x l q -> $ (x0) ∈ᵥ q \/ x0 = x.
  intros. inversion H0; subst; simpl in *; intuition.
  apply NatSet.F.union_1 in H. intuition.
  right. rewrite NatSetFacts.singleton_iff in H4. auto.
Qed.

Lemma saturated_qplus : forall {Γ Σ x b T q}, indexr x Γ = Some (b, T, q) -> closed_qual 0 x (length Σ) q -> saturated Γ Σ q -> saturated Γ Σ (q ⊕ x).
...
Proof.
  intros. destruct d1; destruct d2; simpl in *; intuition.
  - destruct (NatSet.F.mem k t0) eqn:Hmem; intuition.
    repeat rewrite empty_union_right in H0. auto.
    rewrite remove_not_in; auto.
...
Lemma saturated_qqplus : forall {Γ Σ q1 q2}, saturated Γ Σ q1 -> saturated Γ Σ q2 -> saturated Γ Σ (q1 ⋓ q2).
  intros. destruct q1; destruct q2; simpl in *; intuition. unfold saturated. intros.
...
Lemma weaken_store_saturated : forall {Γ Σ q}, saturated Γ Σ q -> forall {Σ'}, Σ' ⊇ Σ -> saturated Γ Σ' q.
  ...
  inversion H. subst. apply (sat_var b U q'); auto. eapply closed_qual_monotone; eauto.
...
Proof.
  ...
  - apply indexr_var_some' in H.
    eapply closed_ty_monotone; eauto. lia.
  - apply bot_just_fv_closed. apply indexr_var_some' in H. auto.
  - eapply closed_qual_sub; eauto.
...
Lemma open_qual_subqual : forall {d1 d2 φ}, d1 ⊑ φ -> forall {k}, ([[ k ~> ∅ ]]ᵈ d2) ⊑ φ -> ([[ k ~> d1 ]]ᵈ d2) ⊑ φ.
  ...
  - destruct (NatSet.F.mem k t0); simpl in *; intuition.
  - destruct (NatSet.F.mem k t0); simpl in *; intuition.
    rewrite empty_union_right in H1. auto.
  - destruct (NatSet.F.mem k t0); simpl in *; intuition.
    rewrite empty_union_right in H0. auto.
  - destruct (NatSet.F.mem k t0); simpl in *; intuition.
    rewrite empty_union_right in H3. auto.
...
Lemma qbot_just_fv : forall {d x},
    (d ⊓ ∅ ⊕ x = qbot /\ d = qbot) \/ d ⊓ ∅ ⊕ x = (just_fv x).
Proof.
  intros. destruct d.
  - left. simpl. auto.
  - right. simpl. repeat rewrite inter_empty_right.
    repeat rewrite empty_union_left. auto.
Qed.

Fixpoint has_type_filter {Γ φ Σ t T d} (ht : has_type Γ φ Σ t T d) : d ⊑ φ.
Proof.
  destruct ht; simpl; intuition.
  - specialize (@qbot_just_fv d x) as Hf.
    destruct Hf; intuition. rewrite H5; simpl; auto.
    rewrite H4. auto.
  - specialize (has_type_closed ht1) as Hc. intuition. eapply openq_subqual; eauto.
Qed.

Lemma closed_qual_qmem_fv : forall {b f l q}, closed_qual b f l q -> forall {x}, $x ∈ᵥ q -> x < f.
  intros. specialize (@subqual_just_fv_bound x q) as Hx. destruct q.
...
Lemma indexr_splice_tenv : forall {Γ1 i Γ2 b U du},
    indexr i (Γ1 ++ Γ2) = Some (b, U, du) ->
    forall {k}, (length Γ2) <= i -> indexr i (splice_tenv k Γ1 ++ Γ2) = Some (b, splice_ty k U, splice_qual k du).
...
Lemma splice_qual_qqcap_dist : forall {d1 d2 k}, splice_qual k (d1 ⋒ d2) = (splice_qual k d1) ⋒ (splice_qual k d2).
  intros. destruct d1; destruct d2; intuition.
  unfold qqcap. simpl. f_equal. apply splice_set_inter_dist.
Qed.

Lemma splice_qual_mem_lt : forall {x k d1}, x < k -> $x ∈ᵥ splice_qual k d1 -> $x ∈ᵥ d1.
  intros. destruct d1. simpl in *. auto.
  ...
  fnsetdec. simpl. intuition. apply splice_set_singleton_inv. auto.
Qed.
...
Lemma splice_qual_mem_ge : forall {x k d1}, x >= k -> $(S x) ∈ᵥ splice_qual k d1 -> $x ∈ᵥ d1.
  ...
  rewrite splice_set_subset_dist in H1. assert (NatSet.F.In x (NatSet.F.singleton x)).
  fnsetdec. simpl. intuition. apply splice_set_singleton_inc. auto.
Qed.

Lemma splice_qual_not_mem : forall {k d1}, $ (k) ∈ᵥ splice_qual k d1 -> False.
  intros. destruct d1. simpl in H. auto. simpl in H.
...
Lemma closed_qual_qplus : forall {b f l q x}, closed_qual b f l q -> x < f -> closed_qual b f l (q ⊕ x).
  ...
  simpl. inversion H. subst. repeat rewrite empty_union_right. constructor; auto.
...
Lemma weaken_saturated : forall {Γ1 Γ2 Σ d1},
  ...
  - specialize (H x). apply splice_qual_mem_lt in H0; auto. intuition. inversion H2.
    rewrite indexr_skips in H; try lia. apply (sat_var b1 U q'); auto.
...
Lemma splice_qual_bot_just_fv : forall {k x d},
    k <= x -> splice_qual k (d ⊓ ∅ ⊕ x) = (d ⊓ ∅ ⊕ (S x)).
Proof.
  intros. destruct d; simpl; auto.
  f_equal. repeat rewrite inter_empty_right.
  repeat rewrite empty_union_left.
  rewrite splice_set_singleton_inc; auto.
Qed.

Lemma splice_qual_bot_just_fv' : forall {k x d},
    x < k -> splice_qual k (d ⊓ ∅ ⊕ x) = (d ⊓ ∅ ⊕ x).
Proof.
  intros. destruct d; simpl; auto. f_equal.
  repeat rewrite inter_empty_right.
  repeat rewrite empty_union_left.
  rewrite splice_set_singleton_inv; auto.
Qed.
...
Lemma weaken_gen : forall {t Γ1 Γ2 φ Σ T d},
  ...
  - (* t_var *)
    ...
    + (* |Γ2| <= x < |Γ1|+|Γ2|*)
      rewrite splice_qual_bot_just_fv; auto.
      replace (d ⊓ ∅ ⊕ S x) with ((splice_qual k d) ⊓ ∅ ⊕ S x).
      apply t_var with (b:=b) (d:=(splice_qual k d)).
      rewrite <- indexr_insert_ge.
      eapply indexr_splice_tenv; eauto. lia.
      erewrite <- splice_qual_just_fv_ge; eauto.
      rewrite subqual_splice_lr'. auto.
      eapply splice_qual_closed'.
      rewrite app_length in *. rewrite splice_tenv_length. auto.
      eapply splice_ty_closed''; eauto.
      eapply splice_qual_closed''; eauto.
      { destruct d. simpl. auto.
        simpl. repeat rewrite inter_empty_right. repeat rewrite empty_union_left. auto. }
    + (* |Γ2| > x *)
  ...
  - (* t_abs *) rewrite app_length in *. simpl. constructor.
    ...
    replace ((false, splice_ty k T1, splice_qual k d1)
                :: ((true, TFun (splice_qual k d1) (splice_qual k d2) (splice_ty k T1)
                          (splice_ty k T2), splice_qual k df) :: splice_tenv k Γ1) ++ X :: Γ2)
            with (splice_tenv k ((false,T1, d1) :: (true,TFun d1 d2 T1 T2, df) :: Γ1) ++ X :: Γ2).
  ...
  - (* tapp *) simpl. rewrite splice_qual_open''. apply t_app with (T1:=(splice_ty k T1)) (df:=(splice_qual k df)).
    ...
    apply IHHT1; auto. simpl. f_equal. apply splice_qual_qqcap_dist.
    apply IHHT2; auto.
  ...
  - (* t_ref *) simpl. rewrite splice_set_empty. econstructor.
    replace qbot with (splice_qual k qbot). apply IHHT; auto. simpl. auto.
    erewrite splice_ty_id; eauto. eapply closed_ty_monotone; eauto. lia.
    simpl. destruct φ. simpl in H0. intuition. simpl. intuition; fnsetdec.
  - (* t_deref *) simpl. econstructor. simpl in IHHT. subst k. apply IHHT; auto.
  - (* t_assign *) simpl. econstructor. simpl in IHHT1. subst k. apply IHHT1; auto.
    replace qbot with (splice_qual k qbot); auto. subst k. apply IHHT2; auto.
  - (* t_self *)
    specialize (IHHT Γ2 Γ1). intuition. rename H2 into IHHT. specialize (IHHT (a0, b0, b)).
    bdestruct (f <? k).
    + rewrite splice_qual_just_fv_lt; auto.
      erewrite @splice_qual_id with (d:=(df ⊕ f)) in IHHT.
      eapply t_self; eauto. rewrite indexr_skips. rewrite indexr_skips in H. rewrite indexr_skip. eauto.
      1-3: simpl; lia.
      erewrite <- @splice_qual_id with (d:=(just_fv f)).
      eapply subqual_splice_lr'. auto. eapply just_fv_closed. auto.
      eapply closed_qual_qplus; eauto. eapply closed_qual_monotone; eauto. lia.
    + rewrite splice_qual_just_fv_ge; auto. rewrite <- splice_qual_qplus_dist in IHHT; auto.
      eapply t_self; eauto. rewrite <- indexr_insert_ge; auto.
      eapply @indexr_splice_tenv with (k:=k) in H; auto. simpl in H. subst k. eauto.
      eapply splice_qual_closed''; eauto.
      destruct φ. simpl in H1. intuition.
      simpl in H1. simpl. intuition.
      eapply @splice_set_subset_dist with (i:=k) in H3.
      rewrite splice_set_singleton_inc in H3; auto.
...
Lemma narrowing_saturated : forall {Γ1 b U du Γ2 Σ q},
    ...
    forall {V dv}, stp Γ2 Σ V dv U du -> saturated (Γ1 ++ (b,V,dv) :: Γ2) Σ q.
  intros. unfold saturated. intros.
  specialize (H x H1). inversion H. destruct (PeanoNat.Nat.lt_trichotomy x (length Γ2)) as [Hlen | [Hlen | Hlen] ].
  - apply (sat_var b0 U0 q'); auto. rewrite indexr_skips in H2; simpl; auto.
...
(* narrowing should not occur on function self refs, i.e., only on context entries with flagged with false  *)
Lemma narrowing_gen : forall {t Γ1 U du Γ2 φ Σ T d},
    has_type (Γ1 ++ (false,U,du) :: Γ2) φ Σ t T d ->
    forall {V dv}, stp Γ2 Σ V dv U du -> saturated Γ2 Σ du -> has_type (Γ1 ++ (false,V,dv) :: Γ2) φ Σ t T d.
  intros t Γ1 U du Γ2 φ Σ T d HT. remember (Γ1 ++ (false,U, du) :: Γ2) as Γ.
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    + subst. rewrite indexr_insert in H. inversion H. subst.
      apply t_sub with (T1:=V) (d1:=dv ⊓ ∅ ⊕ (length Γ2)); auto.
      apply t_var with (b:=false) (d:=dv).
      rewrite indexr_insert. auto. destruct φ. simpl. auto.
      repeat rewrite app_length in *; simpl in *; auto.
      1,2 : apply stp_closed in H4; intuition.
      rewrite app_length. simpl. auto.
      apply stp_closed in H4. intuition.
      eapply stp_shrink_bot_var; eauto. eapply weaken_stp'; eauto. eapply weaken_stp; eauto. rewrite app_length. simpl. lia.
      destruct d. simpl. auto. simpl. destruct φ. auto.
      intuition; fnsetdec.
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    rewrite @open_tm'_len with (Γ' := (Γ1 ++ (false,U, du) :: Γ2)).
    rewrite @open_ty'_len with (Γ' := (Γ1 ++ (false,U, du) :: Γ2)).
    rewrite @openq'_len with (Γ' := (Γ1 ++ (false,U, du) :: Γ2)).
  ...
  - econstructor; eauto.
    repeat rewrite app_length in *; simpl in *; auto.
  - econstructor; eauto.
  - (* deref *) econstructor; eauto.
  - (* assign *) econstructor; eauto.
  - eapply t_self; eauto. destruct (PeanoNat.Nat.lt_trichotomy f (length Γ2)) as [Hlen | [Hlen | Hlen] ].
...
Lemma narrowing : forall {Γ U du φ Σ t T d}, has_type ((false,U,du) :: Γ) φ Σ t T d -> forall {V dv}, stp Γ Σ V dv U du -> saturated Γ Σ du -> has_type ((false,V,dv) :: Γ) φ Σ t T d.
...
Lemma CtxOK_ext : forall {Γ φ Σ σ}, CtxOK Γ φ Σ σ -> forall {v T}, has_type Γ φ Σ v T qbot -> value v -> CtxOK Γ φ (T :: Σ) (v :: σ).
...
Lemma CtxOK_update : forall {Γ φ Σ σ}, CtxOK Γ φ Σ σ -> forall {l T}, l < length σ -> indexr l Σ = Some T -> forall {v}, has_type Γ φ Σ v T qbot -> value v -> CtxOK Γ φ Σ (update σ l v).
...
Lemma open_subst1_qual : forall {q b l},
  ...
  destruct d1; repeat rewrite empty_union_right.
  - destruct (NatSet.F.mem k bs) eqn: Hmem; simpl.
    ...
    assert (NatSet.F.remove 0 (NatSet.F.singleton 0) = {}N). fnsetdec.
    rewrite H4. rewrite unsplice_set_empty. auto.
    apply bound_0_empty in H0. subst.
    replace (NatSet.F.mem 0 {}N) with false.
    rewrite remove_empty. rewrite unsplice_set_empty.
    apply NatSetProperties.FM.not_mem_iff in Hmem.
    assert (NatSet.F.remove k bs = bs). fnsetdec. rewrite H0. auto.
    rewrite NatSetProperties.FM.empty_b. auto.
  - destruct (NatSet.F.mem k bs) eqn: Hmem; simpl.
    assert (NatSet.F.mem 0 (NatSet.F.union vs (NatSet.F.singleton 0)) = true).
    apply NatSet.F.mem_1. fnsetdec.
    rewrite H3. f_equal. apply bound_0_empty in H0. rewrite H0.
...
Proof.
  intros. destruct q; simpl; intuition. destruct df; inversion H; subst.
  - repeat rewrite empty_union_right.
    destruct (NatSet.F.mem 0 t) eqn:Hmem1.
    + destruct (NatSet.F.mem k t0) eqn:Hmem2.
      * simpl. rewrite Hmem2.
        assert (NatSet.F.mem 0 (NatSet.F.union t (NatSet.F.singleton (S g))) = true).
        apply NatSet.F.mem_2 in Hmem1. apply NatSet.F.mem_1. fnsetdec.
        rewrite H0. f_equal; try fnsetdec.
        rewrite remove_union_dist.
        rewrite unsplice_set_union_dist. f_equal.
        rewrite remove_singleton_inv by lia.
        apply unsplice_set_singleton.
      * simpl. rewrite Hmem2. rewrite Hmem1. auto.
    + destruct (NatSet.F.mem k t0) eqn:Hmem2.
      * simpl. rewrite Hmem2.
        assert (NatSet.F.mem 0 (NatSet.F.union t (NatSet.F.singleton (S g))) = false).
        apply NatSetProperties.FM.not_mem_iff in Hmem1.
        apply NatSetProperties.FM.not_mem_iff. intros F.
        apply NatSet.F.union_1 in F. destruct F. contradiction.
        apply NatSetProperties.Dec.F.singleton_iff in H0. inversion H0.
        rewrite H0. f_equal; try fnsetdec.
        rewrite remove_union_dist.
        rewrite unsplice_set_union_dist. f_equal.
        rewrite remove_singleton_inv by lia.
  ...
  - destruct (NatSet.F.mem 0 t) eqn: Hmem1.
    + destruct (NatSet.F.mem k t0) eqn: Hmem2.
      * simpl. rewrite NatSet.F.mem_1. rewrite NatSet.F.mem_1.
        ...
        f_equal. rewrite union_symmetry. f_equal.
        apply unsplice_set_singleton.
    ...
    + destruct (NatSet.F.mem k t0) eqn: Hmem2.
      * simpl. rewrite Hmem2. rewrite not_member_union; auto.
        ...
        rewrite unsplice_set_union_dist. f_equal.
        apply unsplice_set_singleton. rewrite mem_singleton. simpl. auto.
      * simpl. rewrite Hmem1.  rewrite Hmem2. auto.
Qed.
...
Proof.
  ...
  - simpl. destruct (NatSet.F.mem 0 vs) eqn:Hmem; constructor; auto; try lia; apply unsplice_set_dec; auto.
...
Proof.
  ...
  - simpl. auto.
  - destruct q2.
    + destruct df; simpl; auto; destruct (NatSet.F.mem 0 t) eqn: Hmem1; simpl; auto.
    + destruct df; simpl; auto.
      * destruct (NatSet.F.mem 0 t) eqn: Hmem1.
        -- rewrite NatSet.F.mem_1.
           destruct (NatSet.F.mem 0 t2) eqn:Hmem2;
             simpl; f_equal; try fnsetdec;
             rewrite <- unsplice_set_union_dist;
             f_equal; rewrite remove_union_dist; auto.
           apply NatSet.F.mem_2 in Hmem1. fnsetdec.
        -- destruct (NatSet.F.mem 0 t2) eqn: Hmem2; simpl.
           rewrite NatSet.F.mem_1. f_equal.
           rewrite <- unsplice_set_union_dist. f_equal.
           rewrite remove_union_dist. auto.
           apply NatSet.F.mem_2 in Hmem2. fnsetdec.
           assert (NatSet.F.mem 0 (NatSet.F.union t t2) = false).
           rewrite NatSetFacts.union_b. rewrite Hmem1, Hmem2. auto.
           rewrite H. f_equal.
           rewrite <- unsplice_set_union_dist. f_equal.
           rewrite remove_union_dist. auto.
      * destruct (NatSet.F.mem 0 t) eqn: Hmem1.
...
Lemma subst1_qual_plus' : forall {du du' l},
    du' ⊑ du -> du' <> qbot -> closed_qual 0 0 l du -> {0 |-> du }ᵈ (du' ⊕ 0) = du.
Proof.
  intros. destruct du'; intuition. destruct du.
  - simpl. inversion H.
  - inversion H. destruct H3. inversion H1. subst.
    simpl. rewrite NatSet.F.mem_1 by fnsetdec.
    repeat rewrite empty_union_right.
    apply bound_0_empty in H11. apply bound_0_empty in H12.
...
intros. destruct d.
  - simpl. auto.
  - destruct df; intuition; simpl.
    + destruct (NatSet.F.mem 0 t) eqn: Hmem1.
      * assert (NatSet.F.mem 0 (NatSet.F.union t (NatSet.F.singleton x)) = true).
        rewrite NatSetFacts.union_b. rewrite Hmem1. auto.
        rewrite H0. simpl. f_equal.
        rewrite remove_union_dist.
        rewrite unsplice_set_union_dist. f_equal.
        rewrite remove_singleton_inv by lia.
        symmetry. eapply unsplice_set_singleton'.
      * assert (NatSet.F.mem 0 (NatSet.F.union t (NatSet.F.singleton x)) = false).
        rewrite NatSetFacts.union_b. rewrite Hmem1. simpl.
        rewrite mem_singleton. simpl. destruct x. lia. auto.
        rewrite H0. simpl. f_equal.
        rewrite remove_union_dist.
        rewrite unsplice_set_union_dist. f_equal.
        rewrite remove_singleton_inv by lia.
...
Proof.
  intros. destruct d2; simpl; auto. destruct d1; simpl.
  - destruct q1; simpl; destruct (NatSet.F.mem 0 t) eqn:Hmem; simpl; auto.
    f_equal. inversion H. subst. apply bound_0_empty in H7.
    subst. repeat rewrite empty_union_right. auto.
  - destruct q1; simpl.
    + destruct (NatSet.F.mem k t0) eqn:Hmem1;
        destruct (NatSet.F.mem 0 t2) eqn:Hmem2;
        destruct (NatSet.F.mem 0 t) eqn:Hmem3; simpl;
          try rewrite Hmem1; try rewrite Hmem2; try rewrite Hmem3; auto.
      * assert (NatSet.F.mem 0 (NatSet.F.union t t2) = true).
        rewrite NatSetFacts.union_b. rewrite Hmem2. intuition.
        rewrite H0. f_equal. rewrite remove_union_dist.
      ...
      * assert (NatSet.F.mem 0 (NatSet.F.union t t2) = true).
        ...
        rewrite unsplice_set_union_dist. auto.
      * assert (NatSet.F.mem 0 (NatSet.F.union t t2) = false).
        rewrite NatSetFacts.union_b. rewrite Hmem2, Hmem3. intuition.
        rewrite H0. f_equal. rewrite remove_union_dist.
        rewrite unsplice_set_union_dist. auto.
      ...
      * simpl. destruct (NatSet.F.mem 0 t2) eqn:Hmem2.
        ...
        -- destruct (NatSet.F.mem 0 t) eqn:Hmem3.
           ++ rewrite NatSet.F.mem_1.
              ...
              rewrite <- unsplice_set_union_dist. rewrite <- remove_union_dist. auto.
              apply NatSet.F.union_2. apply NatSet.F.mem_2. auto.
           ++ rewrite not_member_union; auto. simpl. rewrite Hmem1. f_equal.
              rewrite <- unsplice_set_union_dist. rewrite <- remove_union_dist. auto.
...
Proof.
   intros. destruct d1; destruct d2; destruct df; simpl; inversion H; intuition.
  ...
  - destruct (NatSet.F.mem 0 t) eqn: Hmem1; destruct (NatSet.F.mem 0 t2) eqn: Hmem2;
      simpl; intuition; try fnsetdec.
    1: apply NatSetProperties.union_subset_4; apply unsplice_set_subset_monotone; auto.
    1-3: specialize (@subset_inclusion _ _ _ H0 Hmem1 Hmem2) as F; inversion F.
...
Lemma subst_stp : forall{T1 T2},
  ...
  - simpl. constructor. inversion H. subst. 2 : inversion H0. subst.
    ...
    specialize (IHHS2 Tf df ((false, T3, d3) :: (true, TFun d1 d2 T1 T2, d5) :: Γ0)). simpl in IHHS2. intuition. rename H4 into IHHS2.
...
Lemma substitution_stp : forall{T1 T2},
    forall {bx Tx bf Tf df dx Σ d1 d2},
      stp ([(bx,Tx,df ⋒ dx); (bf,Tf,df)]) Σ T1 d1 T2 d2 ->
  ...
  intros. replace [(bx, Tx, df ⋒ dx); (bf, Tf,df)] with ([(bx, Tx, df ⋒ dx)] ++ [(bf, Tf,df)]) in H; auto.
  eapply @subst_stp with (df':=df) in H; auto.
  replace ({0 |-> df }ᴳ [(bx, Tx, df ⋒ dx)]) with ([(bx, Tx, df ⋒ dx)]) in H.
  replace ([(bx, Tx, df ⋒ dx)]) with ([] ++ [(bx, Tx, df ⋒ dx)]) in H; auto.
...
Lemma subst_filter0 : forall {d φ l}, closed_qual 0 0 l d -> (just_fv 0) ⊑ φ -> d ⊑ { 0 |-> d }ᵈ φ.
  intros. destruct d; simpl in *; auto. destruct φ.
  - simpl. inversion H0.
  - intuition. simpl.
    inversion H; subst; auto. rewrite NatSet.F.mem_1. intuition.
    assert (NatSet.F.In 0 (NatSet.F.singleton 0)). fnsetdec.
    eapply NatSetProperties.in_subset; eauto.
Qed.

Lemma subst1_just_fv0 : forall {q}, q <> qbot -> {0 |-> q }ᵈ (just_fv 0) = q.
Proof.
...
Lemma subst1_just_fv0' : forall {q}, {0 |-> q }ᵈ (just_fv 0) = (q ⊔ ∅).
  intros. simpl. destruct q; intuition. repeat rewrite empty_union_left.
  rewrite NatSet.F.mem_1 by fnsetdec. rewrite remove_singleton_empty.
  rewrite unsplice_set_empty. cbn. auto.
  rewrite NatSet.F.mem_1 by fnsetdec. simpl.
  repeat rewrite empty_union_left. repeat rewrite empty_union_right.
  rewrite remove_singleton_empty. rewrite unsplice_set_empty. rewrite empty_union_left.
  auto.
Qed.
...
Lemma subst_filter1 : forall {d' q φ l}, closed_qual 0 0 l q -> q ⋒ d' ⊕ 0 ⊑ φ -> q ⊑ { 0 |-> q }ᵈ φ.
  ...
  - destruct d' eqn:Hd'.
    + subst. simpl in *.
      destruct φ eqn:Hpsi; simpl; intuition.
      inversion H. subst.
      apply bound_0_empty in H9. apply bound_0_empty in H10. subst.
      destruct (NatSet.F.mem 0 t2) eqn:Hmem; intuition.
      apply NatSetProperties.FM.not_mem_iff in Hmem.
      eapply (@NatSetProperties.in_subset _ _ 0) in H1. contradiction.
      fnsetdec.
    + destruct φ; intuition. simpl. rewrite NatSet.F.mem_1. intuition.
      simpl in *. intuition.
Qed.
...
Lemma substq_exist_qset : forall dx t0 t1 t2, exists t3 t4 t5,
    {0 |-> dx }ᵈ (qset t0 t1 t2) = qset t3 t4 t5.
Proof.
  intros. simpl.
  destruct (mem 0 t0).
  - destruct dx. exists (unsplice_set 0 (remove 0 t0)). exists t1. exists t2. auto.
    exists (union (unsplice_set 0 (remove 0 t0)) t3). exists (union t1 t4). exists (union t2 t5). auto.
  - destruct dx. exists (unsplice_set 0 (remove 0 t0)). exists t1. exists t2. auto.
    exists (unsplice_set 0 (remove 0 t0)). exists t1. exists t2. auto.
Qed.
...
Proof.
  intros. destruct df as [|ff bf lf].
  - simpl. unfold qqcap. destruct d1. simpl. auto.
    specialize (substq_exist_qset dx' t0 t1 t2) as Hs.
    destruct Hs as [t3 [t4 [t5]]]. rewrite H5. simpl.
    rewrite mem_empty. f_equal.
    rewrite remove_empty. rewrite unsplice_set_empty. auto.
  - destruct d1 as [|f1 b1 l1].
    + simpl. rewrite mem_empty.
      destruct (mem 0 ff) eqn:Hmem.
      * destruct dx' as [|fx' bx' lx']; simpl; f_equal;
          rewrite remove_empty; rewrite unsplice_set_empty; auto.
      * simpl. f_equal. rewrite remove_empty. rewrite unsplice_set_empty. auto.
    + destruct dx' as [|fx' bx' lx']; simpl.
      * destruct (mem 0 ff) eqn:Hmem1; destruct (mem 0 f1) eqn:Hmem2; simpl.
        -- assert (mem 0 (inter ff f1) = true).
           apply mem_2 in Hmem1, Hmem2. apply mem_1. fnsetdec.
           unfold qqcap. rewrite H5. simpl. f_equal.
           apply NatSet.eq_if_Equal. apply inter_unsplice_0.
        -- apply mem_2 in Hmem1.
           apply NatSetProperties.FM.not_mem_iff in Hmem2.
           assert (mem 0 (inter ff f1) = false).
           apply NatSetProperties.FM.not_mem_iff. fnsetdec.
           unfold qqcap.  simpl.
           rewrite H5. f_equal.
           apply NatSet.eq_if_Equal. apply inter_unsplice_0.
        -- apply mem_2 in Hmem2.
           apply NatSetProperties.FM.not_mem_iff in Hmem1.
           assert (mem 0 (inter ff f1) = false).
           apply NatSetProperties.FM.not_mem_iff. fnsetdec.
           unfold qqcap. simpl.
           rewrite H5. f_equal. apply NatSet.eq_if_Equal.
           apply inter_unsplice_0.
        -- apply NatSetProperties.FM.not_mem_iff in Hmem1, Hmem2.
           assert (mem 0 (inter ff f1) = false).
           apply NatSetProperties.FM.not_mem_iff. fnsetdec.
           unfold qqcap. simpl. rewrite H5. f_equal.
           apply NatSet.eq_if_Equal. apply inter_unsplice_0.
      * destruct φ as [|fp bp lp].
        -- inversion H1.
        -- inversion H0. subst. apply bound_0_empty in H11, H12. subst.
           destruct dx as [|fx bx lx].
           ++ simpl in H. inversion H.
           ++ simpl in H. intuition. rewrite inter_empty_left in H5.
              rewrite inter_empty_left in H. symmetry in H5. symmetry in H.
              apply NatSet.eq_if_Equal in H5.
              apply NatSet.eq_if_Equal in H. subst.
              destruct (mem 0 ff) eqn:Hmem0ff;
                destruct (mem 0 f1) eqn:Hmem0f1; simpl;
                rewrite NatSetFacts.inter_b;
      ...
      substitution would require more careful reasoning. *)
                cbn. f_equal; try fnsetdec. repeat rewrite empty_union_right.
                apply NatSet.eq_if_Equal. apply inter_unsplice_0.
              ** (* 0 ∈ df, 0 ∉ d1 *) cbn. f_equal; try fnsetdec.
                 repeat rewrite empty_union_right.
                 apply NatSet.eq_if_Equal. apply inter_unsplice_0.
       ...
       saturation of the sets and the boundedness of the context. *)
              ...
              ** (* 0 ∉ df, 0 ∉ d1 : trivial, since the substitution has no effect (other than unsplicing the sets) *)
                cbn. f_equal; try fnsetdec. apply NatSet.eq_if_Equal. apply inter_unsplice_0.
Qed.

Lemma subst1_mem : forall {x dx df l}, closed_qual 0 0 l dx -> $ (x) ∈ᵥ {0 |-> dx }ᵈ df -> $ (S x) ∈ᵥ df.
Proof.
  intros. inversion H; subst.
  - destruct df; simpl in *; intuition.
    ...
    + destruct x. rewrite dec_in0 in H0. fnsetdec. fnsetdec.
      change (S x) with (pred (S (S x))) in H0. rewrite <- dec_in_iff in H0. fnsetdec. lia.
  - apply bound_0_empty in H1, H2. subst. destruct df; simpl in *; intuition.
    destruct (mem 0 t0) eqn:Hmem0t0; simpl in H0;
      unfold unsplice_set in H0; rewrite filter_lt_0 in H0; rewrite filter_ge0_id in H0;
      repeat rewrite empty_union_right in H0.
    + destruct x. rewrite dec_in0 in H0. fnsetdec. fnsetdec.
...
Proof.
  ...
  - unfold saturated in *. intros. eapply subst1_mem in H1; eauto.
    apply H in H1. inversion H1. apply @indexr_subst1 with (dx:=qbot) in H2.
    simpl in H2. econstructor. eauto. apply subst_qual_subqual_monotone. auto.
    eapply closed_qual_subst1; eauto. lia.
  - apply bound_0_empty in H1.
...
Proof.
  intros. inversion H0; subst.
  - unfold qqcap in *. destruct φ'; simpl.
    unfold eqqual in H. auto.
    unfold eqqual in H. auto.
  - apply bound_0_empty in H1, H2. subst.
    destruct dx eqn:Hdx.
    + destruct φ'; simpl in *; auto; intuition; try fnsetdec.
    + destruct φ'; simpl in *; auto; intuition; try fnsetdec.
Qed.
...
Lemma qqcap_increase_fresh' : forall {dx dx' φ' l X},
  ...
  dx' ⋒ (φ' ⊔ qset X {}N {}N) ≡ dx.
Proof.
  intros. replace (φ' ⊔ qset X {}N {}N) with (φ' ⋓ qset X {}N {}N).
  eapply qqcap_increase_fresh; eauto.
  destruct φ'; intuition.
Qed.
...
Proof.
  ...
  - (* vtp_abs *) inversion H. subst. econstructor; eauto.
    ...
    + assert (stp [] Σ (TFun d1 d2 T1 T2) d5 (TFun d3 d4 T3 T4) d6).
      ...
      assert (narrow: stp [(false,T3, d3); (true,TFun d0 d7 T0 T5, df1)] Σ (open_ty' ([]: tenv) T5)(openq' ([]:tenv) d7) (open_ty' ([]:tenv) T2) (openq' ([]: tenv) d2)). {
        eapply narrowing_stp; eauto.  apply weaken_stp. auto.
      }
      (* apply trans *)
      simpl in *. eapply s_trans; eauto.
      replace ([(false,T3, d3); (true,TFun d0 d7 T0 T5, df1)]) with ([(false,T3, d3)] ++ (true,TFun d0 d7 T0 T5, df1) :: []); auto.
...
Lemma has_type_vtp: forall {Σ φ t T d},
  ...
  vtp Σ t T d.
Proof.
  intros. remember [] as Γ. induction H0; eauto; subst;
  try solve [simpl in H1; discriminate].
  7 : subst; intuition; eapply vtp_widening; eauto.
  all : inversion H.
  ...
  - (* tloc *) eapply vtp_loc; eauto.
    + eapply closed_qual_sub; eauto.
    + apply stp_refl. intuition. apply qstp_refl. eapply eqqual_refl.
      apply closed_qual_qbot.
    + apply stp_refl. intuition. constructor. intuition. apply closed_qual_qbot.
...
Proof.
  intros. induction H.
  - econstructor; eauto. constructor. constructor; intuition.
  - assert (has_type [] (just_loc l) Σ (tloc l) (TRef T) (just_loc l)). {
      econstructor; eauto. apply qstp_closed in H4. intuition. }
      eapply weaken_flt with  (φ'  := d) in H5; intuition. eapply t_sub; eauto.
      constructor; intuition. inversion H4; subst; intuition.
...
Lemma subst_qbot_just_fv : forall {d1 d2 x},
    (({0 |-> d1}ᵈ (d2 ⊓ ∅ ⊕ S x)) = qbot /\ d2 = qbot)
    \/ (({0 |-> d1}ᵈ (d2 ⊓ ∅ ⊕ S x)) = just_fv x /\ d2 <> qbot).
Proof.
  intros. destruct d2. simpl. left. auto.
  right. simpl. repeat rewrite inter_empty_right. repeat rewrite empty_union_left.
  rewrite mem_singleton. simpl. rewrite remove_singleton_inv; auto.
  rewrite unsplice_set_singleton_dec; try lia. split; auto. discriminate.
Qed.

Lemma substitution_gen1 :
  forall {t Γ φ' bx Tx dx dx' Σ T d}, dx' ⋒ φ' ≡ dx -> dx <> qbot ->
  forall {φ}, φ ⊑ φ' -> ∅ ⊑ φ ->
...
Proof.
  Import NatSet.F.
  intros t Γ φ' bx Tx dx dx' Σ T d Hsep dxlb φ Hphi Hphilb HT HSubst tx HTx.
  specialize (vtp_closed HTx) as Hclx.
  ...
  - simpl. bdestruct (x =? 0).
    + (*x is 0 *) rewrite indexr_skips in H0; simpl; auto; try lia. simpl in H0. subst. simpl in H0.
      ...
      * destruct dx'.
        ...
        -- simpl. repeat rewrite inter_empty_right. repeat rewrite empty_union_left.
           ...
           eapply closed_qual_subst1'; eauto.
      * destruct (df ⋒ dx') eqn:Hdfdx'. intuition. simpl.
        repeat rewrite inter_empty_right. repeat rewrite empty_union_left.
        rewrite mem_singleton. simpl. rewrite remove_singleton_empty. rewrite unsplice_set_empty.
        destruct dx'; repeat rewrite empty_union_left.
        -- apply eq_if_eqqual in Hsep. rewrite qqcap_commute in Hsep.
           destruct df; simpl.
           ++ intuition.
           ++ apply t_sub with (T1 := T) (d1 := qbot).
              eapply weaken'; eauto; simpl; auto.
              eapply closed_qual_subst1'; eauto.
              apply stp_refl. eapply closed_ty_monotone; eauto. lia.
              constructor; simpl; eauto. simpl in H3.
              destruct φ. inversion H3. unfold φs. simpl.
              assert (mem 0 t6 = true). destruct H3. apply mem_1.
              eapply singleton_subset_in. auto. rewrite H7. intuition.
        -- eapply weaken'; eauto.
           rewrite <- qplus_empty in H3.
           eapply subst_qual_subqual_monotone in H3. erewrite subst1_qual_plus' in H3; eauto.
           simpl. intuition. discriminate.
    ...
    + (*x is in Γ0*) assert (Hx: 1 <= x); try lia. destruct x; try lia.
      specialize (@subst_qbot_just_fv dx' d x) as Hsq.
      destruct Hsq; destruct H8.
      * rewrite H8. simpl.
        apply t_var with (b:=b) (d:=qbot). subst. change x with (pred (S x)).
        replace (⊥) with ({0 |-> dx'}ᵈ ⊥).
        ...
        simpl. econstructor.
      * rewrite H8. simpl. replace ($$ x) with ({0 |-> dx' }ᵈ d ⊓ ∅ ⊕ x).
        apply t_var with (b:=b) (d:={0 |-> dx' }ᵈ d). change x with (pred (S x)).
        eapply indexr_subst1; eauto. erewrite subst1_just_fv.
        repeat eapply subst_qual_subqual_monotone. auto.
        eapply closed_qual_subst1'; eauto. simpl. eapply closed_ty_subst1; eauto.
        simpl. eapply closed_qual_subst1; eauto.
        { rewrite <- H8. repeat rewrite qglb_empty_right; auto.
          repeat rewrite qplus_empty. simpl.
          rewrite mem_singleton. simpl. rewrite remove_singleton_inv; auto.
          rewrite unsplice_set_singleton_dec; try lia.
          auto. destruct d. intuition. simpl. destruct (mem 0 t0); destruct dx'; discriminate. }
  - simpl. intuition.
    ...
    replace (length (Γ0 ++ [(bx, Tx, dx)])) with (S (length Γ0)) in IHHT.
    ...
    specialize IHHT with (φ' := φ'') (Γ := (((false,T1, d1) :: (true,TFun d1 d2 T1 T2, df) :: Γ0))).
    ...
    { subst. destruct df; simpl; intuition; try fnsetdec. }
    { subst. repeat rewrite empty_union_right.
      destruct dx; intuition.
      destruct φ; intuition.
      eapply qqcap_increase_fresh'; eauto. destruct φ'; intuition. inversion H7.
    }
    ...
    { subst. rewrite subst1_qlub_dist. rewrite subst1_qlub_dist. f_equal.
      repeat rewrite <- subst1_just_fv. auto. }
    { rewrite app_length. simpl. lia. }
  - intuition. rename H8 into IHHT2. rename H6 into IHHT1. simpl.
  ...
  - erewrite @subst1_qual_id with (q:=(just_loc l)); eauto. simpl. erewrite subst1_ty_id; eauto.
    ...
    subst φs.
    destruct φ; destruct dx'; destruct dx; simpl in *; auto; intuition;
      try (destruct (NatSet.F.mem 0 t0); intuition; fnsetdec).
    constructor; intuition. rewrite bound_empty. lia.
  - erewrite @subst1_qual_id with (q:=∅); eauto. simpl in *.
    apply t_ref. eapply IHHT; eauto. erewrite subst1_ty_id; eauto.
    destruct φ; auto. unfold φs. simpl.
    destruct (mem 0 t1) eqn:Hmem; destruct dx'; simpl; intuition; try fnsetdec.
  - simpl. apply t_deref with (d := { 0 |-> dx' }ᵈ d). eapply IHHT; eauto.
  - simpl. apply t_assign with (T := ({ 0 |-> dx' }ᵀ T)) (d := ({ 0 |-> dx' }ᵈ d)).
    eapply IHHT1; eauto. simpl in IHHT2. eapply IHHT2; eauto.
  - intuition. specialize (H5 φ'). intuition. specialize (H5 Γ0). intuition. rename H6 into IHHT.
    pose (H0' := H0). bdestruct (f =? 0).
    * subst. rewrite indexr_skips in H0'; try solve [simpl; lia]. simpl in H0'. inversion H0'. subst.
      destruct dx' eqn:Hdx'.
      + simpl. rewrite mem_1 by fnsetdec. rewrite remove_singleton_empty.
        rewrite unsplice_set_empty.
        inversion HSubst.
        -- subst. simpl in IHHT.
           eapply t_sub with (T1 :=  ({0 |-> qbot }ᵀ T)) (d1 := qbot); eauto.
           apply has_type_closed in IHHT.
           eapply stp_refl; intuition.
        -- subst. destruct df0; intuition. simpl in IHHT.
           repeat rewrite empty_union_left in IHHT.
           rewrite mem_1 in IHHT by fnsetdec.
           rewrite remove_singleton_empty in IHHT. rewrite unsplice_set_empty in IHHT.
           auto.
      + rewrite subst1_just_fv0. 2: discriminate. inversion HSubst; subst.
        -- erewrite <- subst1_qual_plus in IHHT; eauto.
        -- erewrite subst1_qual_plus' in IHHT; eauto. apply qqcap_sub_r. discriminate.
    * rewrite indexr_skips' in H0'; try solve [simpl; lia]. destruct f; try lia. simpl in H0'.
      ...
      rewrite @subst1_just_fv with (dy := dx'). unfold φs.
...
Lemma substq_qqcap_qbot_dist : forall {d1 d2},
    {0 |-> qbot }ᵈ (d1 ⋒ d2) = {0 |-> qbot }ᵈ d1 ⋒ {0 |-> qbot }ᵈ d2.
Proof.
  intros. destruct d1; destruct d2; simpl; auto.
  - assert (mem 0 {}N = false).
    apply NatSetProperties.FM.not_mem_iff. intro F. fnsetdec.
    rewrite H. rewrite remove_empty. rewrite unsplice_set_empty.
    destruct (mem 0 t0); cbv; auto.
  - assert (mem 0 {}N = false).
    apply NatSetProperties.FM.not_mem_iff. intro F. fnsetdec.
    rewrite H. rewrite remove_empty. rewrite unsplice_set_empty.
    destruct (mem 0 t0); cbv; auto.
  - destruct (mem 0 (inter t0 t3)) eqn: Hmem1.
    + assert (mem 0 t0 = true). apply mem_1. apply mem_2 in Hmem1. fnsetdec.
      assert (mem 0 t3 = true). apply mem_1. apply mem_2 in Hmem1. fnsetdec.
      rewrite H. rewrite H0. cbn. f_equal.
      apply NatSet.eq_if_Equal. rewrite inter_unsplice_0. reflexivity.
    + destruct (mem 0 t0) eqn: Hmem2; destruct (mem 0 t3) eqn: Hmem3; cbn; f_equal;
      apply NatSet.eq_if_Equal; rewrite inter_unsplice_0; reflexivity.
Qed.

Lemma subst_preserve_lb : forall {φ}, ∅ ⊑ φ -> ∅ ⊑ {0 |-> qbot }ᵈ φ.
Proof.
  intros.
  specialize (@subst_qual_subqual_monotone ∅ φ H qbot) as Hm.
  simpl in Hm. rewrite mem_empty in Hm.
  rewrite remove_empty in Hm. rewrite unsplice_set_empty in Hm.
  auto.
Qed.

Lemma substitution_gen2 :
  forall {t Γ φ' bx Tx dx dx' Σ T d}, dx = qbot ->
  forall {φ}, φ ⊑ φ' -> ∅ ⊑ φ ->
  has_type (Γ ++ [(bx,Tx,dx)]) φ Σ t T d ->
  forall {tx}, vtp Σ tx Tx dx' -> dx' = qbot ->
          has_type ({ 0 |-> dx' }ᴳ Γ) ({ 0 |-> dx' }ᵈ φ) Σ
                   ({ 0 |-> tx  }ᵗ t)
                   ({ 0 |-> dx' }ᵀ T)
                   ({ 0 |-> dx' }ᵈ d).
Proof.
  Import NatSet.F.
  intros t Γ φ' bx Tx dx dx' Σ T d Hdx φ Hphi Hphilb HT tx HTx Hdx'.
  specialize (vtp_closed HTx) as Hclx.
  intuition. remember (Γ ++ [(bx,Tx, dx)]) as Γ'.
  generalize dependent Γ. generalize dependent φ'.
  induction HT; intros; pose (φs := {0 |-> dx' }ᵈ φ); replace ({0 |-> dx' }ᵈ φ) with φs in *; auto.
  - subst. apply t_base; auto. eapply closed_qual_subst1'; eauto.
  - subst. apply vtp_has_type in HTx. simpl. bdestruct (x =? 0).
    + (*x is 0 *) rewrite indexr_skips in H0; simpl; auto; try lia. simpl in H0. subst. simpl in H0.
      ...
      constructor; simpl; eauto. simpl in H3.
      destruct φ. inversion H3. unfold φs. simpl.
      assert (mem 0 t0 = true). destruct H3. apply mem_1.
      eapply singleton_subset_in. auto. auto.
   + (*x is in Γ0*) assert (Hx: 1 <= x); try lia. destruct x; try lia.
      specialize (@subst_qbot_just_fv ⊥ d x) as Hsq.
      destruct Hsq; destruct H8.
      * rewrite H8. simpl.
        apply t_var with (b:=b) (d:=qbot). subst. change x with (pred (S x)).
        replace (⊥) with ({0 |-> ⊥}ᵈ ⊥).
        ...
        eapply closed_qual_subst1'; eauto. simpl. eapply closed_ty_subst1; eauto.
        simpl. econstructor.
      * rewrite H8. simpl. replace ($$ x) with ({0 |-> ⊥}ᵈ d ⊓ ∅ ⊕ x).
        apply t_var with (b:=b) (d:={0 |-> ⊥}ᵈ d). change x with (pred (S x)).
        eapply indexr_subst1; eauto. erewrite subst1_just_fv.
        repeat eapply subst_qual_subqual_monotone. auto.
        eapply closed_qual_subst1'; eauto. simpl. eapply closed_ty_subst1; eauto.
        simpl. eapply closed_qual_subst1; eauto.
        { rewrite <- H8. repeat rewrite qglb_empty_right; auto.
          ...
          rewrite mem_singleton. simpl. rewrite remove_singleton_inv; auto.
          rewrite unsplice_set_singleton_dec; try lia.
          auto. destruct d. intuition. simpl. destruct (mem 0 t0); discriminate. }
  - subst. simpl. intuition. (* rename H7 into IHHT. *)
    apply t_abs; auto. eapply closed_tm_subst1'; eauto.
    inversion H3. subst. constructor; try eapply closed_ty_subst1'; eauto; eapply closed_qual_subst1'; eauto.
    eapply closed_qual_subst1'; eauto. apply subst_qual_subqual_monotone. auto.
    (* 1. instantiate the IH *)
    replace (length (Γ0 ++ [(bx, Tx, qbot)])) with (S (length Γ0)) in IHHT.
    rewrite subst1_tenv_length. rewrite app_comm_cons in IHHT. rewrite app_comm_cons in IHHT.
    remember (df ⊔ just_fv (S (length Γ0)) ⊔ just_fv (S (S (length Γ0)))) as DF.
    replace ({0 |-> ⊥}ᵈ df ⊔ just_fv (length Γ0) ⊔ just_fv (S (length Γ0))) with ({0 |-> ⊥}ᵈ DF).
    remember (φ' ⊔ just_fv (S (length Γ0)) ⊔ just_fv (S (S (length Γ0)))) as φ''.
    specialize IHHT with (φ' := φ'') (Γ := (((false,T1, d1) :: (true,TFun d1 d2 T1 T2, df) :: Γ0))).
    (* 2. reason about opening and subst, apply IH *)
    unfold open_tm' in *. unfold open_ty' in *. unfold open_ty in *.
    ...
    { subst. destruct df; simpl; intuition; try fnsetdec. }
    { subst. repeat rewrite empty_union_right.
      assert (df ⊑ φ'). eapply subqual_trans. apply H5. auto.
      apply @subqual_qlub with (d:= qset (union (singleton (S (length Γ0))) (singleton (S (S (length Γ0))))) {}N {}N) in H6. apply H6.
    }
    { subst. rewrite subst1_qlub_dist. rewrite subst1_qlub_dist. f_equal.
      repeat rewrite <- subst1_just_fv. auto. }
    { rewrite app_length. simpl. lia. }
  - intuition. rename H8 into IHHT1. rename H6 into IHHT2. simpl.
    replace ({ 0 |-> dx' }ᵈ (openq df d1 d2)) with
      (openq ({ 0 |-> dx' }ᵈ df) ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ d2)).
    (*separation/overap is preserved after substitution*)
    assert (Hoverlap: {0 |-> dx' }ᵈ (df ⋒ d1) = {0 |-> dx' }ᵈ df ⋒ {0 |-> dx' }ᵈ d1). {
      subst. apply substq_qqcap_qbot_dist.
    }
    apply t_app with (T1:= { 0 |-> dx' }ᵀ T1) (df:=({0 |-> dx' }ᵈ df)).
    replace (TFun ({0 |-> dx' }ᵈ df ⋒ {0 |-> dx' }ᵈ d1) ({0 |-> dx' }ᵈ d2) ({0 |-> dx' }ᵀ T1) ({0 |-> dx' }ᵀ T2))
            with ({ 0 |-> dx' }ᵀ (TFun (df ⋒ d1) d2 T1 T2)).
    ...
    2 : unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx');
        erewrite <- subst1_open_qual_comm; eauto; erewrite <- subst1_open_qual_comm; eauto.
    + subst. eapply closed_ty_subst1'; eauto.
    + apply subst_qual_subqual_monotone. unfold openq in H4. auto.
    + subst. eapply subst1_saturated; eauto.
    + subst. eapply subst1_saturated; eauto.
    + unfold openq. repeat erewrite <- subst1_open_qual_comm; eauto.
  - subst. erewrite @subst1_qual_id with (q:=(just_loc l)); eauto. simpl. erewrite subst1_ty_id; eauto.
    apply t_loc; auto. eapply closed_qual_subst1'; eauto.
    subst φs.
    destruct φ; simpl in *; auto; intuition;
      try (destruct (NatSet.F.mem 0 t0); intuition; fnsetdec).
    constructor; intuition. rewrite bound_empty. lia.
  - erewrite @subst1_qual_id with (q:=∅); eauto. simpl in *.
    apply t_ref. eapply IHHT; eauto. erewrite subst1_ty_id; eauto.
  ...
  - simpl. apply t_assign with (T := ({ 0 |-> dx' }ᵀ T)) (d := ({ 0 |-> dx' }ᵈ d)).
    eapply IHHT1; eauto. simpl in IHHT2. eapply IHHT2; eauto.
  - intuition. specialize (H5 φ'). intuition. specialize (H6 Γ0). intuition. rename H5 into IHHT.
    pose (H0' := H0). bdestruct (f =? 0).
    + subst. rewrite indexr_skips in H0'; try solve [simpl; lia]. simpl in H0'. inversion H0'. subst.
      simpl. rewrite mem_1 by fnsetdec. rewrite remove_singleton_empty.
      rewrite unsplice_set_empty.
      ...
      apply has_type_closed in IHHT.
      eapply stp_refl; intuition.
      unfold φs. apply subst_preserve_lb; auto.
    + subst. rewrite indexr_skips' in H0'; try solve [simpl; lia]. destruct f; try lia. simpl in H0'.
      rewrite <- subst1_just_fv. eapply t_self.
      rewrite subst1_qual_plus_dist in IHHT. simpl in IHHT. eauto. lia.
      rewrite Nat.sub_0_r in H0'. apply @indexr_subst1 with (dx:=⊥) in H0; try lia. simpl in H0.
      eauto. eapply closed_qual_subst1; eauto.
      rewrite @subst1_just_fv with (dy := ⊥). unfold φs.
      eapply subst_qual_subqual_monotone; eauto.
  - subst. apply t_sub with (T1:={ 0 |-> ⊥}ᵀ T1) (d1:={ 0 |-> ⊥}ᵈ d1).
    eapply IHHT; eauto. eapply subst_stp; eauto. apply subst_qual_subqual_monotone; auto.
    Unshelve. all : exact 0.
Qed.

Lemma substitution : forall {t bf Tf df bx Tx dx Σ T d},
    has_type [(bx,Tx,df ⋒ dx) ; (bf,Tf,df)] (df ⊔ just_fv 0 ⊔ just_fv 1) Σ t T d ->
...
Proof.
  ...
  intuition. replace ([(bx,Tx, df ⋒ dx); (bf,Tf, df)]) with ([(bx,Tx, df ⋒ dx)] ++ [(bf,Tf, df)]) in H; auto.
  remember (df ⊔ (just_fv 0) ⊔ (just_fv 1)) as DF.
  assert (Hrefl : DF ⊑ DF). { apply subqual_refl. }
  assert (Hmt : ∅ ⊑ DF). subst. destruct df; intuition; fnsetdec.
  destruct (df ⋒ dx) eqn:Hdfdx.
  - (* df and dx are both bottom *)
    rewrite <- Hdfdx in *.
    assert (Hdf : df = qbot). { apply qqcap_qbot' in Hdfdx. intuition. }
    eapply (substitution_gen2 Hdf Hrefl Hmt) in H; eauto.
    assert (Hrefl' : (df ⊔ just_fv 0) ⊑ (df ⊔ just_fv 0)). apply subqual_refl.
    assert (Hmt' : ∅ ⊑ (df ⊔ (just_fv 0))). { subst. intuition; fnsetdec. }
    replace ({0 |-> df }ᴳ [(bx, Tx, df ⋒ dx)]) with ([] ++ [(bx, Tx, df ⋒ dx)]) in H.
    replace ({0 |-> df }ᵈ DF) with (df ⊔ just_fv 0) in H.
    eapply (substitution_gen2 Hdfdx Hrefl' Hmt') in H; eauto.
    subst. simpl in *.
    assert (mem 0 (singleton 0) = true). apply mem_1. fnsetdec.
    rewrite H5 in H.
    assert (dx = qbot). { apply qqcap_qbot' in Hdfdx. intuition. }
    subst. simpl in *.
    rewrite remove_singleton_empty in H; rewrite unsplice_set_empty in H. apply H.
    (*done, prove earlier replacements *)
    { apply qqcap_qbot' in Hdfdx. intuition. }
    { subst. simpl.
      rewrite mem_1 by fnsetdec. rewrite remove_union_dist.
      rewrite remove_singleton_empty. rewrite remove_not_in.
      repeat rewrite empty_union_left. rewrite <- unsplice_set_singleton. auto.
      rewrite mem_singleton. rewrite Nat.eqb_neq. lia.
    }
    { simpl. erewrite subst1_qual_id; eauto. erewrite subst1_ty_id; eauto.
      apply closed_qual_qqcap; eauto. }
  - assert (Hdx : df ⋒ dx <> qbot). { intuition. rewrite Hdfdx in H5. inversion H5. }
    destruct df eqn:Hdf.
    + rewrite <- Hdf in *. rewrite <- Hdfdx in H.
      (* df is bottom *)
      eapply (substitution_gen2 Hdf Hrefl Hmt) in H; eauto.
      assert (Hsepf' : dx ⋒ (df ⊔ just_fv 0) ≡ df ⋒ dx). {
        rewrite (@qqcap_commute df dx). destruct dx.
        - subst. cbn. intuition.
        - simpl. inversion H8. subst. simpl in *. apply bound_0_empty in H14.
          subst. intuition.
      }
      assert (Hrefl' : (df ⊔ just_fv 0) ⊑ (df ⊔ just_fv 0)). apply subqual_refl.
      assert (Hmt' : ∅ ⊑ (df ⊔ (just_fv 0))). { subst. intuition; fnsetdec. }
      replace ({0 |-> df }ᴳ [(bx, Tx, df ⋒ dx)]) with ([] ++ [(bx, Tx, df ⋒ dx)]) in H.
      replace ({0 |-> df }ᵈ DF) with (df ⊔ just_fv 0) in H.
      rewrite Hdf in Hdx at 2.
      (* df ⋒ dx is not bottom *)
      eapply (substitution_gen1 Hsepf' Hdx Hrefl' Hmt') in H; eauto. simpl in H.
      replace ({0 |-> dx }ᵈ (df ⊔ just_fv 0)) with (df ⊔ dx) in H.
      rewrite qlub_empty_right. apply H.
      (*done, prove earlier replacements *)
      { subst. simpl. destruct dx; discriminate. }
      { destruct dx; subst; simpl.
        * cbn in Hdfdx. inversion Hdfdx.
        * rewrite mem_1 by fnsetdec. inversion H8. subst.
          apply bound_0_empty in H14. subst. f_equal; try fnsetdec.
          rewrite empty_union_right. rewrite remove_singleton_empty. rewrite unsplice_set_empty.
          auto.
      }
      { subst. simpl. rewrite mem_1 by fnsetdec. rewrite remove_union_dist.
        rewrite remove_singleton_empty. rewrite remove_not_in.
        repeat rewrite empty_union_left. rewrite <- unsplice_set_singleton. auto.
        rewrite mem_singleton. rewrite Nat.eqb_neq. lia.
      }
      { simpl. erewrite subst1_qual_id; eauto. erewrite subst1_ty_id; eauto.
        apply closed_qual_qqcap; eauto. }
    + assert (Hsepf : df ⋒ DF ≡ df). { subst. simpl. intuition. }
      assert (Hdfnbot : df <> qbot). { subst. discriminate. }
      rewrite <- Hdf in H.
      (* df is not bottom *)
      eapply (substitution_gen1 Hsepf Hdfnbot Hrefl Hmt) in H; eauto.
      assert (Hsepf' : dx ⋒ (df ⊔ just_fv 0) ≡ df ⋒ dx). {
        rewrite (@qqcap_commute df dx). destruct dx.
        - subst. cbn. intuition.
        - simpl.
      ...
      { subst. simpl. destruct dx; discriminate. }
      { destruct dx; subst; simpl.
        * assert (mem 0 (union t4 (singleton 0)) = true). apply mem_1. fnsetdec.
          rewrite H5. repeat rewrite empty_union_right.
          f_equal. inversion H7. subst. apply bound_0_empty in H15. subst.
          rewrite empty_union_left. rewrite remove_singleton_empty.
          rewrite unsplice_set_empty. auto.
        * assert (mem 0 (union t4 (singleton 0)) = true). apply mem_1. fnsetdec.
          rewrite H5. repeat rewrite empty_union_right. f_equal.
          inversion H7. inversion H8. subst. apply bound_0_empty in H15, H24.
          subst. f_equal. rewrite empty_union_left. rewrite remove_singleton_empty.
          rewrite unsplice_set_empty. auto.
      }
...
Proof.
    intros. destruct d2; destruct d1'; destruct d1; simpl; auto; intuition.
...
Proof.
    intros. destruct d2; destruct d2'; destruct d1; inversion H; simpl; intuition; try fnsetdec.
...
Lemma open_qual_qlub_dist : forall {k d1 d2 d3}, ([[ k ~> d1 ]]ᵈ (d2 ⊔ d3)) = (([[ k ~> d1 ]]ᵈ d2) ⊔ ([[ k ~> d1 ]]ᵈ d3)).
  intros. destruct d2; destruct d3; destruct d1; simpl; auto; intuition.
  - destruct (NatSet.F.mem k t1) eqn: Hmem; simpl; auto.
  - f_equal. rewrite remove_union_dist. auto.
  - destruct (NatSet.F.mem k t1) eqn: Hmem1.
...
Lemma open_qual_qqplus_dist : forall {k d1 d2 d3}, ([[ k ~> d1  ]]ᵈ (d2 ⋓ d3)) = (([[ k ~> d1  ]]ᵈ d2) ⋓ ([[ k ~> d1  ]]ᵈ d3)).
  intros. destruct d2.
  - simpl. auto.
  - unfold qqplus. rewrite open_qual_qlub_dist. simpl. destruct d1; auto.
    destruct (NatSet.F.mem k t1) eqn:Hmem; auto.
Qed.
...
Lemma openq_just_fv_id: forall {df d1 : qual} {x}, openq df d1 (just_fv x) = just_fv x.
  intros. unfold openq. simpl. destruct df; destruct d1; simpl.
  - rewrite remove_empty. rewrite remove_empty.
    unfold just_fv. f_equal.
  - repeat rewrite remove_empty.
    rewrite mem_empty. unfold just_fv. f_equal.
  - rewrite mem_empty. simpl. rewrite remove_empty.
    unfold just_fv. f_equal.
...
Lemma open_qual_just_bv_skip: forall {i j d}, j <> i -> [[ j ~> d ]]ᵈ (just_bv i) = (just_bv i).
  ...
  - rewrite remove_singleton_inv. 2: auto. auto.
  - rewrite mem_singleton. rewrite <- Nat.eqb_neq in H.
    rewrite H. auto.
Qed.

Lemma open_qual_just_bv: forall {i d}, [[ i ~> d ]]ᵈ (just_bv i) = (d ⊔ ∅).
  intros. simpl. destruct d.
  rewrite remove_singleton_empty. simpl. auto.
  rewrite mem_singleton. rewrite Nat.eqb_refl. qdec.
Qed.
...
Lemma openq_u_distribute1 : forall {df d1 d2 l},
  ...
  - repeat rewrite remove_empty. auto.
  - repeat rewrite remove_empty. rewrite mem_empty. auto.
  - rewrite mem_empty. simpl. rewrite remove_empty. auto.
...
Lemma openq_duplicate_eq : forall {df d1 d2 l},
  ...
  - destruct (mem 1 (remove 0 t1)) eqn:Hmem.
    simpl. repeat rewrite empty_union_right. f_equal. fnsetdec. auto.
...
Lemma qqcap_fresh_r : forall {d1 df f l},
    closed_qual 0 f l d1 -> closed_qual 0 f l df ->
    forall {l'}, l <= l' -> d1 ⋒ df = d1 ⋒ (df ⋓ (just_loc l')).
  intros. inversion H; subst; inversion H0; subst; simpl; auto.
  repeat rewrite empty_union_right. cbn. f_equal.
...
Lemma qqcap_fresh_r' : forall {d1 df f l},
    closed_qual 0 f l d1 -> closed_qual 0 f l d1 ->
    d1 ⋒ df = ∅ ->
    forall {f'}, f <= f' -> d1 ⋒ (df ⊔ (just_fv f')) = ∅.
Proof.
  intros. inversion H; subst; inversion H0; subst; simpl; auto.
  - destruct df; simpl. cbv in H1. inversion H1.
    cbv. reflexivity.
  - destruct df; simpl. cbv. f_equal; try fnsetdec.
    eapply bound_disjoint. apply H3. apply H2.
    cbv. repeat rewrite empty_union_right.
    cbv in H1. inversion H1.
    f_equal; try fnsetdec.
    rewrite inter_comm.
    specialize (@NatSetProperties.union_inter_1 t0 (singleton f') vs) as Hi2.
    apply NatSet.eq_if_Equal in Hi2. rewrite Hi2.
    rewrite inter_comm. rewrite H7.
    rewrite empty_union_left. rewrite H9. rewrite H8. rewrite H7.
    rewrite inter_comm. eapply bound_disjoint. apply H3. apply H2.
Qed.

Lemma freshness : forall {Σ Σ' l b f d1}, disjointq Σ Σ' (just_loc l) -> closed_qual b f (length Σ) d1 -> just_loc l ⋒ d1 = ∅.
  intros. inversion H; subst.
  - unfold just_loc in H1. inversion H1.
  - inversion H3. apply singleton_injective in H5. subst. inversion H0. subst.
    unfold just_loc. simpl. unfold qqcap. auto.
    unfold qqcap. simpl.
    repeat rewrite inter_empty_left. f_equal.
    assert (Hcontra : bound ls <= l0). lia. apply bound_le_mem_false in Hcontra.
    rewrite <- NatSetFacts.not_mem_iff in Hcontra. unfold just_loc.
    simpl. f_equal; try fnsetdec.
Qed.
...
Lemma closed_qual_subqual_ldom : forall {d} {Σ: senv}, closed_qual 0 0 (length Σ) d -> d ⊑ ldom Σ.
Proof.
  intros. unfold ldom. inversion H; simpl; intuition; auto.
  apply bound_0_empty in H0. subst. fnsetdec.
  apply bound_0_empty in H1. subst. fnsetdec.
  subst. apply bound_dom_sub. auto.
Qed.
...
Theorem type_safety: forall {Σ t T d},
  ...
  value t \/ forall {σ} , CtxOK [] (ldom Σ) Σ σ ->
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
     ...
     + (* beta *)
       ...
       * (* progress *) constructor. intuition.
       * (* preservation *)
         apply (Preserve Σ qbot); auto.  rewrite qqplus_qbot_right_neutral.
         apply qstp_closed in H33 as H33'; intuition.
         change (length []) with 0 in *. subst.
         pose (VH' := H30). eapply t_abs with (φ:=df1) in VH'; eauto. apply has_type_vtp in VH'; auto.
         assert (df1 ⊑ df). { apply qstp_inv in H33. auto. }
         assert (df1 ⋒ d1 ⊑ df ⋒ d1). {
           destruct d1; destruct df; destruct df1; simpl; intuition; auto.
           all: simpl in H22; intuition; fnsetdec.
         }
         assert (Hst : stp [] Σ T1 (df1 ⋒ d1) T1 (df ⋒ d1)). {
           apply stp_refl; auto. constructor. auto.
           inversion H33. subst. auto. apply closed_qual_qqcap; auto.
         }
         assert (HT' : has_type [(false, T1, df1 ⋒ d1) ; (true, TFun d0 d3 T0 T2, df1)] (df1 ⊔ just_fv 0 ⊔ just_fv 1) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T2) (openq' ([]:tenv) d3)). {
           assert (Hst' : stp [] Σ T1 (df1 ⋒ d1) T0 d0). eapply s_trans; eauto.
           eapply narrowing; eauto. simpl.
           eapply weaken_stp; eauto. apply saturated_cons; auto.
         ...
         eapply t_sub; eauto.
         inversion H27. subst. clear H42.
         assert (Hsub : stp [(false,T1, df1 ⋒ d1); (true,TFun d0 d3 T0 T2, df1)] Σ (open_ty' ([] : tenv) T2) (openq' ([] : tenv) d3) (open_ty' ([] : tenv) T) (openq' ([] : tenv) d2)). {
         ...
         eapply openq_mono; eauto. inversion H33. subst. apply openq_closed; auto.
         ...
         apply qlub_bound; auto. apply qlub_bound; auto.
         eapply subqual_trans. apply H22.
         apply closed_qual_subqual_ldom; auto. simpl; intuition; fnsetdec.
         inversion H27. auto. inversion H27. auto.
     + (* (tabs t) t2 -> (tabs t) t2' *)
        apply has_type_vtp in H as VH; intuition.
        apply vtp_canonical_form_lam in VH as HH; intuition.
        pose (HH11 := H11).
        unfold CtxOK in HH11. specialize (H8 σ). intuition.
        destruct H22 as [t2' [σ' HH22]]. exists (tapp t1 t2').
        exists σ'. intuition. constructor; intuition.
        ...
        * specialize (has_type_closed H). specialize (has_type_closed H25). intuition. inversion H33. subst. rewrite <- openq_duplicate_eq.
          ...
          -- eapply t_app with (T1 := T1) (df:=df); eauto.
             erewrite <- qqcap_fresh_r; eauto. eapply weaken_flt. eapply weaken_store; eauto.
     ...
     + (*  t t2 -> t' t2  *)
       ...
       * rewrite <- openq_u_distribute3. eapply t_sub with (T1 := T)(d1 := (openq (df ⋓ just_loc l) d1 d2)).
         ...
         -- apply stp_refl. simpl. eapply closed_ty_monotone; eauto. lia.
            ...
            inversion H13; subst.
            apply openq_closed. 1,2 : apply closed_qual_qqplus.
  ...
  - (* tref *)
    subst. intuition. specialize (has_type_closed H) as HH. intuition.
    specialize (H5 T t0). intuition.
    + (* t0 value *) right. intros.
      exists (tloc (length (σ))). exists (t0 :: σ). intuition.
      econstructor; eauto.
      apply (Preserve (T :: Σ) (just_loc (length σ))).
      apply extends_cons. eapply CtxOK_weaken_flt.
      apply CtxOK_ext; auto. apply H5. apply H.
      ...
      1,2: unfold CtxOK in H5; simpl; intuition.
      replace (∅ ⋓ just_loc (length σ)) with (just_loc (length σ)).
      eapply t_loc; eauto. inversion H5. rewrite <- H12.
      apply indexr_head. simpl. intuition; try fnsetdec.
      unfold dom. simpl. inversion H5. rewrite H12. fnsetdec.
      simpl. unfold just_loc. f_equal; try fnsetdec.
    + (* t0 steps *)
      right. intros. specialize (H10 σ H5). destruct H10 as [t' [σ' H10']].
      exists (tref t'). exists σ'. intuition. econstructor; auto. destruct H12.
      apply (Preserve Σ' qbot); auto. simpl. eapply t_ref; eauto.
      simpl; intuition; fnsetdec.
  - (* deref *)
    subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H3 (TRef T0) t0). intuition.
    + right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
      ...
      apply (Preserve Σ qbot); intuition.
      simpl. apply t_sub with (T1 := T)(d1 := qbot); auto.
    ...
    + right. intros. specialize (H8 σ H3).
      ...
      destruct H10. apply (Preserve Σ' qbot); intuition. simpl. eapply t_deref; eauto.
  - (* assign *)
    ...
    specialize (H4 (TRef T) t1). intuition.
    specialize (H6 T t2). intuition.
    + (* both t1 and t2 are value *)
      right. intros. pose (Ht1' := Ht1). eapply has_type_vtp in Ht1'; eauto.
      pose (Hloc := Ht1'). apply vtp_canonical_form_loc in Hloc; auto.
      inversion Ht1'. destruct Hloc. subst.
      exists tunit. exists (update σ x t2). inversion H21. subst.
      inversion H7. inversion H6. subst. intuition.
      econstructor; eauto. lia. apply (Preserve Σ qbot); auto.
      eapply CtxOK_update; eauto. lia. apply t_sub with (T1:=T) (d1:=qbot); auto.
      replace Σ with (Σ ++ []); intuition. eapply weaken_stp_store. auto.
    + (* t1 value, t2 steps *)
      right. intros. specialize (H4 σ H6). destruct H4 as [t' [σ' H4']].
      exists (tassign t1 t'). exists σ'. intuition. econstructor; eauto.
      destruct H14. apply (Preserve Σ' qbot); eauto. simpl.
      eapply t_assign; eauto. eapply weaken_flt.
      eapply weaken_store; eauto. apply extends_ldom. auto.
      auto.
    + (* t1 steps *)
      ...
      eapply weaken_store; eauto. apply extends_ldom. auto.
      auto.
  - subst. simpl in H0. discriminate.
  - (* stp *)
    ...
    simpl. auto. apply just_loc_ldom; auto.
Qed.
...
Lemma preservation : forall {Σ t T d},
    ...
    preserve [] Σ t' T d σ'.
Proof.  intros Σ t T d HT σ  HCtxOK t' σ' HStep.  specialize (type_safety HT). intuition.
...
Proof.
  ...
  specialize (preservation HT1 HOK Hstep1) as P1.
  destruct P1 as [Σ' d1 Hext1 HOK' Hdisj1 HT1'].
  assert (HT2': has_type [] (ldom Σ') Σ' t2 T2 q2). {
    eapply weaken_flt. eapply weaken_store. eauto. auto. apply extends_ldom; auto. auto.
  }
  specialize (preservation HT2' HOK' Hstep2) as P2.
  destruct P2 as [Σ'' d2 Hext2 HOK'' Hdisj2 HT2''].
  ...
  - destruct q2. simpl. destruct q1; simpl; auto.
    assert ((qset t0 t3 t4 ⋓ just_loc l) = (qset t0 t3 t4 ⊔ just_loc l)).
    simpl. auto. rewrite H.
    rewrite qglb_qlub_dist_l. rewrite (@qglb_commute q1 (just_loc l)).
    destruct q1 eqn:Hq1. simpl. auto. rewrite <- Hq1.
    assert ((just_loc l ⊓ q1) = (just_loc l ⋒ q1)). subst. simpl. auto.
    rewrite H2. erewrite freshness; eauto.
    rewrite qlub_empty_right. subst. intuition. subst. discriminate.
    apply has_type_closed in HT1. intuition. simpl in *. subst.
    eapply closed_qual_monotone; eauto.
    apply extends_length. auto.
  - destruct q1. simpl. auto.
    assert ((qset t0 t3 t4 ⋓ just_loc l) = (qset t0 t3 t4 ⊔ just_loc l)).
    simpl. auto. rewrite H1. rewrite qglb_qlub_dist_r.
    destruct q2 eqn:Hq2. simpl. auto. rewrite <- Hq2.
    assert ((just_loc l ⊓ q2) = (just_loc l ⋒ q2)). subst. simpl. auto.
    rewrite H2. erewrite freshness; eauto. rewrite qlub_empty_right. auto.
    subst. intuition. subst. discriminate.
    apply has_type_closed in HT2. intuition. subst.
    eapply closed_qual_monotone; eauto.
  - destruct q2 eqn:Hq2. simpl. destruct q1; simpl; auto.
    rewrite <- Hq2. assert ((q2 ⋓ just_loc l0) = (q2 ⊔ just_loc l0)).
    subst. auto. rewrite H1.
    rewrite qglb_qlub_dist_l.
    destruct q1 eqn:Hq1. simpl. auto. rewrite <- Hq1.
    assert ((q1 ⋓ just_loc l) = (q1 ⊔ just_loc l)). subst. auto.
    rewrite H4.
    rewrite qglb_qlub_dist_r. rewrite qglb_qlub_dist_r.
    assert ((just_loc l ⊓ q2) = (just_loc l ⋒ q2)). subst. auto.
    rewrite H5.
    erewrite freshness; eauto. rewrite (@qglb_commute (just_loc l) (just_loc l0)).
    assert ((just_loc l0 ⊓ just_loc l) = (just_loc l0 ⋒ just_loc l)). auto.
    rewrite H6. erewrite @freshness with (Σ := Σ'); eauto. repeat rewrite qlub_empty_right.
    rewrite (@qglb_commute q1 (just_loc l0)).
    assert ((just_loc l0 ⊓ q1) = (just_loc l0 ⋒ q1)). subst. auto. rewrite H7.
    erewrite freshness; eauto. rewrite qlub_empty_right.
    subst. intuition. subst. discriminate.
    2: subst; simpl; discriminate.
    2: subst; simpl; discriminate.
    + apply has_type_closed in HT1; auto. intuition. simpl in H12.
      subst q1. eapply closed_qual_monotone; eauto. lia.
    + apply just_loc_closed. lia.
    + apply has_type_closed in HT2; auto. intuition. simpl in H10.
      subst q2. eapply closed_qual_monotone; eauto.
    Unshelve. all : auto.
Qed.
