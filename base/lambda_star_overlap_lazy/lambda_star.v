...
Lemma substitution_gen :
  ...
  - intuition. rename H7 into IHHT1. rename H6 into IHHT2. simpl.
    ...
    eapply IHHT2; eauto.
    2 : unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx'); erewrite <- subst1_open_qual_comm; eauto.
    3-4 : eapply subst1_saturated; eauto.
...
Lemma substitution : forall {t df Tx dx Σ T d},
    has_type [(Tx,df ⋒ dx)] (df ⊔ (just_fv 0)) Σ t T d -> closed_qual 0 0 (length Σ) df ->
            forall {vx}, vtp Σ vx Tx dx ->
                    has_type [] (df ⊔ dx) Σ
                             ({ 0 |-> vx }ᵗ t)
                             ({ 0 |-> dx }ᵀ T)
                             ({ 0 |-> dx }ᵈ d).
  intros. subst. specialize (vtp_closed H1) as Hclx. specialize (has_type_closed H) as Hclt.
  ...
  assert (Hsepf : dx ⋒ (df ⋓ just_fv 0) ≡ df ⋒ dx). {
    ...
    inversion H7. subst. simpl in *. apply bound_0_empty in H15, H16.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    + (* beta *)
      ...
      * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
        ...
        assert (HT' : has_type [(T1, df ⋒ d1)] (df ⋓ just_fv 0) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T2) (openq' ([]:tenv) d3)). {
          eapply narrowing; eauto. eapply weaken_flt; eauto. apply subqual_qlub. inversion H33. subst. auto.
        ...
        erewrite <- open_subst1_qual in Hsub; eauto.
        fold (openq d1 d3) in Hsub. fold (openq d1 d2) in Hsub.
    ...
    + (* (tabs t) t2 -> (tabs t) t2' *)
      ...
      * specialize (has_type_closed H). specialize (has_type_closed H25). intuition. inversion H33. subst. rewrite <- openq_duplicate_eq.
        ...
        -- eapply t_app with (T1 := T1) (df:=df); eauto.  erewrite <- qqcap_fresh_r; eauto. eapply weaken_flt. eapply weaken_store; eauto.
           ...
           eapply subqual_trans; eauto; apply extends_ldom; auto. all:  eapply weaken_store_saturated; eauto.
    ...
    + (* t t2 -> t' t2 *)
      ...
      specialize (H19 σ H11). destruct H19 as [t1' [σ' HH7]]. exists (tapp t1' t2). exists σ'. intuition. apply step_c_app_l. intuition.
      ...
      * rewrite <- openq_u_distribute1. eapply t_sub with (T1 := T)(d1 := (openq d1 d2)).
        -- eapply t_app with (df:=(df ⋓ just_loc l)); eauto. erewrite <- qqcap_fresh_l; eauto.
           ...
           eapply subqual_trans; eauto. apply extends_ldom; auto. 1,2  :  eapply weaken_store_saturated; eauto.
        ...
        -- apply stp_refl. simpl. eapply closed_ty_monotone; eauto. lia.
           ...
           eapply closed_qual_open'; eauto. apply closed_qual_qqplus. inversion H13. subst.
