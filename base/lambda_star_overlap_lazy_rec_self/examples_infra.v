...
Lemma saturated_singleton : forall {x Γ Σ b T}, indexr x Γ = Some (b, T, ∅) -> saturated Γ Σ (just_fv x).
...
(* Deal with cases where applying t_var might stumble. (Seems not needed in lambda_star_lazy systems) *)
(* Ltac TVAR := 
  match goal with
  | [ |- has_type _ _ _ (tvar (varF ?x)) _ (_ ⊕ ?x) ] => apply t_var
  | [ |- has_type _ _ _ (tvar (varF ?x)) _ (qset (singleton ?x) {}N {}N) ] => replace (qset (singleton x) {}N {}N) with (∅ ⊕ x); try TVAR
  | [ |- has_type _ _ _ (tvar (varF ?x)) _ (just_fv ?x) ] => rewrite <- qplus_empty; try TVAR
  end. *)
