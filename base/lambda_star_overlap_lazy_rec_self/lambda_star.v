...
| t_abs: forall Γ φ Σ T1 d1 T2 d2 df t,
    ...
    closed_qual 0 (length Γ) (length Σ) φ ->
    df ⊑ φ ->
    has_type ((false,T1,d1) :: ((true,TFun d1 d2 T1 T2), df) :: Γ) (df ⊔ (just_fv (length Γ)) ⊔ (just_fv (S (length Γ)))) Σ (open_tm' Γ t) (open_ty' Γ T2) (openq' Γ d2) ->
...
| vtp_abs: forall Σ T1 d1 T2 d2 T3 d3 T4 d4 df1 df2 t,
  ...
  has_type [(false,T1,d1) ; (true,TFun d1 d2 T1 T2, df1)] (df1 ⊔ (just_fv 0) ⊔ (just_fv 1)) Σ (open_tm' ([]: tenv) t) (open_ty' ([]: tenv) T2) (openq' ([]: tenv) d2) ->
  stp [] Σ T3 d3 T1 d1 ->
  qstp [] Σ df1 df2 ->
  stp [(false,T3, d3) ; (true,TFun d1 d2 T1 T2, df1)] Σ (open_ty' ([]: tenv) T2) (openq' ([] : tenv) d2) (open_ty' ([]: tenv) T4) (openq' ([]: tenv) d4) ->
...
Proof.
  intros Γ1 b U du Γ2 Σ T1 d1 T2 d2 HST. remember (Γ1 ++ (b,U,du) :: Γ2) as Γ.
...
Lemma stp_shrink_var : forall {Γ Σ T1 d1 T2 d2}, stp Γ Σ T1 d1 T2 d2 -> forall {x}, x < length Γ -> stp Γ Σ T1 (just_fv x) T2 (just_fv x).
  intros. assert (Hstp: stp Γ Σ T1 ∅ T2 ∅). {
    replace (∅) with (d1 ⊓ ∅) at 1; auto. replace (∅) with (d2 ⊓ ∅) at 2; auto. apply stp_scale_qglb; auto.
  }
...
Fixpoint has_type_filter {Γ φ Σ t T d} (ht : has_type Γ φ Σ t T d) : d ⊑ φ.
  ...
  apply has_type_filter in ht. eapply subqual_trans. 2 : eapply ht. destruct df. simpl. intuition.
Qed.
...
Lemma weaken_gen : forall {t Γ1 Γ2 φ Σ T d},
  ...
  - (* t_var *) simpl.
    ...
    + (* |Γ2| <= x < |Γ1|+|Γ2|*)
      rewrite splice_set_singleton_inc; auto. apply t_var with (b:=b) (d:=(splice_qual k d)).
  ...
  - (* t_self *)
    specialize (IHHT Γ2 Γ1). intuition. rename H1 into IHHT. specialize (IHHT (a0, b0, b)). bdestruct (f <? k).
    + rewrite splice_qual_just_fv_lt; auto. erewrite @splice_qual_id with (d:=(df ⊕ f)) in IHHT.
      eapply t_self; eauto. rewrite indexr_skips. rewrite indexr_skips in H. rewrite indexr_skip. eauto.
      1-3: simpl; lia. eapply closed_qual_qplus; eauto. eapply closed_qual_monotone; eauto. lia.
...
Lemma narrowing_gen : forall {t Γ1 U du Γ2 φ Σ T d},
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    + subst. rewrite indexr_insert in H. inversion H. subst.
      apply t_sub with (T1:=V) (d1:=just_fv (length Γ2)); auto. apply t_var with (b:=false) (d:=dv).
...
Lemma substitution_gen :
  ...
  - replace ({0 |-> dx' }ᵈ ∅) with (∅) in * by auto.
    ...
    replace ({0 |-> dx' }ᵀ (TUnit)) with (TUnit) in * by auto.
    eapply t_assign; eauto.
  - intuition. specialize (H5 φ'). intuition. specialize (H5 Γ0). intuition. rename H4 into IHHT.
    pose (H0' := H0). bdestruct (f =? 0).
    + subst. rewrite indexr_skips in H0'; try solve [simpl; lia]. simpl in H0'. inversion H0'. subst.
      rewrite subst1_just_fv0. inversion HSubst; subst.
      * erewrite <- subst1_qual_plus in IHHT; eauto.
      * erewrite subst1_qual_plus' in IHHT; eauto. apply qqcap_sub_r.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    + (* beta *)
      ...
      * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
        ...
        eapply subqual_trans; eauto. inversion H27. auto. inversion H27. auto.
    + (* (tabs t) t2 -> (tabs t) t2' *)
      apply has_type_vtp in H as VH; intuition.
  ...
  - (* t_self, impossible *) subst. simpl in H0. discriminate.
