...
Ltac closed_qual_solve :=
  match goal with
  | [ |- closed_qual _ _ _ ?q ] => try solve [apply Q_lift_closed; closed_qual_solve]
  | [ H: closed_qual _ _ _ ?q |- closed_Qual _ _ _ (Q_lift ?q) ] => try solve [apply Q_lift_closed' in H; closed_qual_solve]
  | [ |- closed_Qual _ _ _ (Q_lift ?q) ] => try solve Qcrush
  end.
...
Ltac closed_ty_solve :=
  ...
  | [ H : closed_ty _ _ _ ?T |- context[ [[_ ~> _ ]]ᵀ ?T] ] => try solve [erewrite closed_ty_open_id; eauto | (closed_ty_solve)]
...
Ltac opening :=
  match goal with
  | [ H : closed_tm 0 _ _ ?t |- context[ [[0 ~> _ ]]ᵗ ?t] ] => erewrite closed_tm_open_id; eauto; try opening
  | [ H : closed_ty 0 _ _ ?t |- context[ [[0 ~> _ ]]ᵀ ?t] ] => erewrite closed_ty_open_id; eauto; try opening
...
(*   (1* | [ H : closed_qual _ _ _ (qset _ _ _) |- _ ] => destruct H; try qual_destruct *1) *)
(*   end. *)
...
Ltac lookup_solve :=
  match goal with
  | [ |- (if (Nat.eqb ?n ?n) then _ else _) = _ ] => rewrite <- Nat.eqb_neq; auto; try lookup_solve
...
Ltac saturated_solve :=
  ...
  | [ |- saturated _ _ (openq _ _)]                        => try solve [apply saturated_openq; saturated_solve]
...
Ltac lifting :=
  match goal with
  | [ |- _ = _ ] => try solve [apply Q_lift_eq; Qcrush]
  end.

Ltac cleanup := repeat (simpl; intuition).
...
Ltac ccrush := try opening; repeat (crush; try cleanup); try Qcrush.

Ltac T_VAR :=
  match goal with
  | [ |- has_type _ _ _ (tvar (varF ?x)) _ (_ ⊕ ?x) ] => apply t_var
  | [ |- has_type _ _ _ (tvar (varF ?x)) _ ((n_one ?x), n_empty, n_empty) ] => replace ((n_one x), n_empty, n_empty) with (∅ ⊕ x); try T_VAR
  (* | [ |- has_type _ _ _ (tvar (varF ?x)) _ (just_fv ?x) ] => rewrite <- qplus_empty; try T_VAR *)
  | [ |- has_type _ _ _ (tvar (varF ?x)) _ ((n_or ?s (n_one ?x)), n_empty, n_empty) ] => replace ((n_or s (n_one x)), n_empty, n_empty) with ((s, n_empty, n_empty) ⊕ x); try T_VAR
