...
Lemma closed_tm_open_succ : forall {t b f l}, closed_tm b f l t -> forall {j}, closed_tm b (S f) l (open_rec_tm j $f t).
  intros t. induction t; intros; simpl; intuition; inversion H; subst; try constructor;
    try solve [eapply IHt1; eauto | eapply IHt2; eauto | eapply IHt; eauto ].
...
Proof.
  intros. unfold openq. induction d2; auto.
  2:{ simpl. rewrite IHd2_1. rewrite IHd2_2. auto. }
  destruct v; simpl.
  - destruct (le_lt_dec k i); simpl; auto.
  - destruct i; simpl; auto. 2: destruct i; simpl; auto.
    induction df; auto. 2:{ simpl. rewrite IHdf1. rewrite IHdf2. auto. }
    destruct v; simpl.
    + destruct (le_lt_dec k i); simpl; auto.
    + bdestruct (i=?1); simpl; auto.
Qed.
...
Proof.
  intros. remember (Γ2 ++ (U, du) :: Γ1) as Γ.
  assert (length (Γ2 ++ (U, du) :: Γ1) = length (Γ2 ++ (U, du') :: Γ1)). {do 2 rewrite app_length. auto. }
  induction H; intros; subst.
  - constructor. eapply closed_qual_monotone; eauto. rewrite H0. auto.
  - constructor. rewrite <- H0. auto.
  - constructor. auto.
  - constructor. auto. rewrite <- H0. auto.
  - apply qstp_or_r_sub. auto. rewrite <- H0. auto.
  - constructor; auto.
  - eapply qstp_trans; eauto.
Qed.
...
Lemma stp_scale_plus : forall {Γ Σ T1 d1 T2 d2}, stp Γ Σ T1 d1 T2 d2 -> forall {x}, x < length Γ -> stp Γ Σ T1 (d1 ⊕ x) T2 (d2 ⊕ x).
  ...
  - constructor. apply subqual_plus. auto. auto.
  - constructor; auto. apply subqual_plus. auto. auto.
  - constructor; auto. apply subqual_plus. auto.
    destruct (length Γ) eqn:Heq. auto. lia.
    eapply stp_qual_irrelevance'; eauto.
  - econstructor; eauto.
Qed.

Lemma qstp_scale_qqplus : forall {Γ Σ d1 d2}, 
  qstp Γ Σ d1 d2 -> 
  forall {d}, closed_qual 0 (length Γ) (length Σ) d ->
  qstp Γ Σ (d1 ⋓ d) (d2 ⋓ d).
Proof.
  intros. apply qstp_sub_or.
  - apply qstp_or_l_sub; auto.
  - apply qstp_or_r_sub. apply qstp_refl; auto. apply qstp_closed in H; intuition.
Qed.
...
Proof.
  intros Γ Σ T1 d1 T2 d2 HSTP. induction HSTP; intros.
  - constructor. apply qstp_scale_qqplus; auto.
  - constructor; auto. apply qstp_scale_qqplus; auto.
  - constructor; auto. apply qstp_scale_qqplus; auto. eapply stp_qual_irrelevance'; eauto.
  - econstructor; auto.
Qed.
...
Proof.
  ...
  - constructor; auto. apply qstp_refl. constructor; auto. eapply stp_qual_irrelevance'; eauto.
  - econstructor; eauto. 
Qed.
...
Proof.
  ...
  - inversion H11. subst. unfold openq;
    erewrite open_qual_commute''; eauto.
...
Lemma open_qual_subqual : forall {Γ Σ d1 d2 φ}, 
  qstp Γ Σ d1 φ -> 
  forall {k}, qstp Γ Σ ([[ k ~> ∅ ]]ᵈ d2) φ -> 
  qstp Γ Σ ([[ k ~> d1 ]]ᵈ d2) φ.
Proof.
  ...
  destruct v; auto. simpl in H0. 
  destruct (i=?k); simpl in *; auto.
Qed.

Lemma openq_subqual : forall {Γ Σ df d1 d2 φ},
  qstp Γ Σ df φ ->
  qstp Γ Σ d1 φ -> 
  qstp Γ Σ (openq ∅ ∅ d2) φ -> 
  qstp Γ Σ (openq df d1 d2) φ.
Proof.
  intros. unfold openq in *. apply open_qual_subqual; auto. erewrite open_qual_commute''; eauto.
  erewrite open_qual_commute'' in H1; eauto. apply open_qual_subqual; auto. apply qstp_closed in H; destruct H. eauto.
  Unshelve. all : apply 0.
Qed.

Fixpoint has_type_filter {Γ φ Σ t T d} (ht : has_type Γ φ Σ t T d) : qstp Γ Σ d φ.
Proof.
  induction ht; intuition. specialize (has_type_closed ht1) as Hc. intuition. eapply openq_subqual; eauto.
  all: constructor. 1-2: apply qstp_closed in IHht; intuition. apply qstp_closed in IHht1; intuition.
Qed.
...
Lemma syntactic_to_set_closed_inverse: forall b f l q,
  lambda_star.closed_qual b f l (syntactic_to_set q) -> 
  closed_qual b f l q.
Proof.
...
Lemma syntactic_to_set_closed_inter : forall b f l q1 q2,
  closed_qual b f l q1 ->
  closed_qual b f l q2 ->
  forall {ds}, syntactic_to_set ds = qualifiers.qglb (syntactic_to_set q1) (syntactic_to_set q2) ->
  closed_qual b f l ds.
Proof.
  intros. apply syntactic_to_set_closed_inverse. rewrite H1.
  apply syntactic_to_set_closed in H. apply syntactic_to_set_closed in H0.
  apply lambda_star.closed_qual_qqcap; auto.
Qed.
...
Proof.
  intros. induction d; simpl; intuition; unfold lambda_star.openq'; unfold lambda_star.openq; simpl.
  - rewrite setfacts.mem_empty; auto. unfold lambda_star.open_qual; simpl. rewrite setfacts.mem_empty; auto.
  - destruct v; simpl.
    + rewrite setfacts.mem_empty. unfold lambda_star.open_qual; simpl. rewrite setfacts.mem_empty. auto.
    + rewrite setfacts.mem_singleton. 
      destruct i; simpl. rewrite setfacts.empty_union_left. rewrite setfacts.remove_singleton_empty. rewrite setfacts.empty_union_left. unfold qualifiers.just_fv.
      rewrite setfacts.mem_empty. 
      f_equal. unfold ctx_syntactic_to_set. rewrite map_length. auto.
      rewrite setfacts.mem_singleton. bdestruct (1 =? S i).
        * rewrite H. assert (i=0) by lia. rewrite H0. simpl. 
          unfold qualifiers.just_fv. f_equal. 
          rewrite setfacts.empty_union_left. unfold ctx_syntactic_to_set. rewrite map_length. auto.
          rewrite setfacts.empty_union_right. rewrite setfacts.remove_singleton_empty. auto.
          rewrite setfacts.empty_union_left. auto.
        * unfold openq'; unfold openq. simpl. bdestruct (i=?0). lia. simpl. unfold qualifiers.just_bv. auto.
  - rewrite setfacts.mem_empty; auto. unfold lambda_star.open_qual; simpl. rewrite setfacts.mem_empty; auto.
  - unfold openq' in *; unfold openq in *. rewrite IHd1. rewrite IHd2. unfold lambda_star.openq' in *. unfold lambda_star.openq. 
    repeat rewrite lambda_star.open_qual_qqplus_dist. auto.
Qed.
...
Proof.
  ...
  - unfold open_ty' in *; unfold open_ty in *. unfold lambda_star.open_ty' in *. unfold lambda_star.open_ty in *. simpl.
    ...
    f_equal; intuition. 
    do 2 rewrite open_syntactic_to_set_commute. f_equal. unfold ctx_syntactic_to_set; rewrite map_length; auto.
    do 2 rewrite open_syntactic_to_set_commute. f_equal. unfold ctx_syntactic_to_set; rewrite map_length; auto.
    do 2 rewrite open_ty_syntactic_to_set_commute. f_equal. unfold ctx_syntactic_to_set; rewrite map_length; auto.
    do 2 rewrite open_ty_syntactic_to_set_commute. f_equal. unfold ctx_syntactic_to_set; rewrite map_length; auto.
...
(* TODO: we just want EXISTS an inverse function, so regardless the definition of set_to_syntactic, just make sure fit this definition is OK 
    We are safe in the following proof because the definition of set_to_syntactic is never used again. We hide the definition of set_to_syntactic by always down casting from syntactic to set.
*)
...
Proof.
  ...
  - (* t_abs *) rewrite app_length in *. simpl. constructor.
    ...
    replace (TFun (splice_qual k d1) (splice_qual k d2) 
    (splice_ty k T1) (splice_ty k T2)) with (splice_ty k (TFun d1 d2 T1 T2)) by auto.
    replace ((splice_ty k (TFun d1 d2 T1 T2), splice_qual k df) :: splice_tenv k Γ1) with (splice_tenv k (((TFun d1 d2 T1 T2), df) :: Γ1)) by auto.
    replace ((splice_ty k T1, splice_qual k d1)
    :: splice_tenv k ((TFun d1 d2 T1 T2, df) :: Γ1)) with (splice_tenv k ((T1, d1) :: (TFun d1 d2 T1 T2, df) :: Γ1)) by auto.
    rewrite app_comm_cons.
    replace ((splice_ty k T1, splice_qual k d1) :: splice_tenv k ((TFun d1 d2 T1 T2, df) :: Γ1)) with (splice_tenv k ((T1, d1) :: ((TFun d1 d2 T1 T2, df) :: Γ1))); auto.
    replace (splice_qual k df ⋓ $$ (length (splice_tenv k Γ1 ++ X :: Γ2)) ⋓ $$ (S (length (splice_tenv k Γ1 ++ X :: Γ2)))) with 
      (splice_qual k (df ⊔ (just_fv (length Γ1 + length Γ2)) ⊔ just_fv (S (length Γ1 + length Γ2)))).
    ...
    f_equal. unfold just_fv. destruct (le_lt_dec k (length Γ1 + length Γ2)). assert (S (length Γ1 + length Γ2) = (length Γ1 + S (length Γ2))) by lia. rewrite H3.
    destruct (le_lt_dec k (length Γ1 + S (length Γ2))); auto. lia.
    destruct (le_lt_dec k (length Γ1 + S (length Γ2))); lia. 
...
Proof.
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    + subst. rewrite indexr_insert in H. inversion H. subst.
      ...
      eapply narrowing_qstp_gen; eauto. 
      2,3 : apply stp_closed in H4; intuition.
  ...
  - econstructor; eauto.
    ...
    eapply narrowing_qstp_gen; eauto.
    1-4: eapply narrowing_qstp_gen; eauto.
...
Proof.
  induction q; intros.
  - reflexivity.
  - destruct v; simpl. 2: reflexivity. destruct (Nat.eqb i 0) eqn:Hi; inversion H; intuition.
  - reflexivity.
...
Lemma subst1_tm_id : forall {t b l}, closed_tm b 0 l t -> forall {t1}, { 0 |-> t1 }ᵗ t = t.
  intros t. induction t; intros b loc Hc; inversion Hc; subst; intros; simpl; intuition;
                       try solve [erewrite IHt; eauto];
                       try solve [erewrite IHt1; eauto; erewrite IHt2; eauto].
Qed.
...
Lemma subst1_openq_comm : forall {l df d1 d2 q1},
    closed_qual 0 0 l q1 ->
    { 0 |-> q1 }ᵈ (openq df d1 d2) = openq ({ 0 |-> q1 }ᵈ df) ({ 0 |-> q1 }ᵈ d1) ({ 0 |-> q1 }ᵈ d2).
Proof.
  intros. unfold openq. erewrite subst1_open_qual_comm; eauto. erewrite subst1_open_qual_comm; eauto.
Qed.
...
(* Don't use that, it's wrong! use subst_qstp instaed *)
...
Lemma substitution_stp : forall{T1 T2},
    forall {Tx Tf df dx Σ d1 d2 ds},
      syntactic_to_set ds = qualifiers.qglb (syntactic_to_set df) (syntactic_to_set dx) ->
      stp ([(Tx,ds); (Tf,df)]) Σ T1 d1 T2 d2 ->
      closed_ty 0 0 (length Σ) Tx ->
      closed_qual 0 0 (length Σ) df -> 
      closed_qual 0 0 (length Σ) dx ->
      stp [] Σ ({ 0 |-> df; dx }ᵀ T1) ({ 0 |-> df ; dx }ᵈ d1) ({ 0 |-> df ; dx }ᵀ T2) ({ 0 |-> df ; dx }ᵈ d2).
Proof.
  intros. replace [(Tx, ds); (Tf,df)] with ([(Tx, ds)] ++ [(Tf,df)]) in H0; auto.
  eapply @subst_stp with (df':=df) in H0; auto.
  replace ({0 |-> df }ᴳ [(Tx, ds)]) with ([(Tx, ds)]) in H0.
  replace ([(Tx, ds)]) with ([] ++ [(Tx, ds)]) in H0; auto.
  eapply @subst_stp with (df':=dx) in H0; auto. econstructor; eauto. 
  simpl. erewrite subst1_ty_id; eauto. erewrite subst1_qual_id; eauto.
  apply (@syntactic_to_set_closed_inter _ _ _ df dx); eauto.
Qed.

Lemma substitution_stp2 : forall{T1 T2},
    forall {Tf df df' Tx dx dx' Σ d1 d2 ds'},
      syntactic_to_set ds' = qualifiers.qglb (syntactic_to_set df') (syntactic_to_set dx') ->
      stp ([(Tx, ds'); (Tf, df)]) Σ T1 d1 T2 d2 ->
      closed_ty 0 0 (length Σ) Tx ->
      qstp [] Σ df df' ->
...
Proof.
  ...
  intros. assert (H0' := H0). apply stp_closed in H0'; simpl in H0'; intuition.
  specialize (syntactic_to_set_qstp_exists H H2 H3). intros. destruct H7 as [ds Hds]. destruct Hds.
  assert (stp [(Tx, ds); (Tf, df)] Σ T1 d1 T2 d2). {
    eapply narrowing_stp; eauto. apply stp_refl; auto. eapply closed_ty_monotone; eauto. lia. 
    replace ([(Tf, df)]) with ([(Tf, df)] ++ []) by auto. eapply weaken_qstp. auto.
  }
  eapply substitution_stp; eauto.
  apply qstp_closed in H2; intuition. apply qstp_closed in H3; intuition.
Qed.
...
Proof.
  ...
  - (* tabs *) subst. simpl in *. eapply vtp_abs; eauto.
    + apply qstp_closed in H3; intuition.
    + eapply stp_refl. inversion H1. subst. intuition.
      apply qstp_refl. inversion H1; intuition.
    + apply qstp_refl. apply qstp_closed in H3; intuition.
    + apply stp_refl. inversion H1. subst.
      ...
      * apply qstp_refl. apply has_type_closed in H4; intuition.
...
Proof.
  intros. induction H.
  - econstructor; eauto. econstructor; eauto. constructor. auto. apply qstp_refl. auto.
  - assert (has_type [] (just_loc l) Σ (tloc l) (TRef T) (just_loc l)). {
    ...
    constructor; intuition. inversion H4. auto with *.
    all: apply qstp_refl; try constructor; auto. 1-2: apply qstp_closed in H6; intuition.
  - specialize (qstp_closed H5) as Hcl. intuition.
    assert (has_type [] df1 Σ (tabs t0) (TFun d1 d2 T1 T2) df1). {
    constructor; eauto.  apply qstp_refl. auto. }
    eapply weaken_flt with (φ' := df2) in H9; eauto.
    eapply t_sub; eauto. constructor; intuition. apply qstp_refl; auto.
Qed.
...
Proof. 
  ...
  - simpl. intuition. rename H7 into IHHT.
    ...
    rewrite subst1_tenv_length. do 2 rewrite app_comm_cons in IHHT.
    remember (df ⋓ $$ (S (length Γ0)) ⋓ $$ (S (S (length Γ0)))) as DF.
    replace ({0 |-> dx' }ᵈ df ⋓ $$ (length Γ0) ⋓ $$ (S (length Γ0))) with ({0 |-> dx' }ᵈ DF).
    ...
    do 2 erewrite <- open_subst1_tm_comm in IHHT; eauto.
    do 2 erewrite <- open_subst1_ty_comm in IHHT; eauto.
    do 2 erewrite <- open_subst1_qual_comm in IHHT; eauto.
    unfold "⋓" in *.
    apply IHHT; auto.
    (* done, prove some leftovers *)

    subst. do 2 rewrite syntactic_to_set_qor. simpl. rewrite setfacts.empty_union_left. erewrite lambda_star_addition.qglb_increase_fresh_equal. auto. 2: apply syntactic_to_set_closed; eauto.
    rewrite qualifiers.qlub_empty_right. auto.

    3: { rewrite app_length. simpl. lia. }
    2: { subst. simpl. auto. }
    subst. unfold qlub, just_fv. apply qstp_sub_or. 
    apply qstp_or_l_sub. 
    replace ((T1, d1) :: (TFun d1 d2 T1 T2, df) :: Γ0 ++ [(Tx, dx)]) with (((T1, d1) :: [(TFun d1 d2 T1 T2, df)]) ++ Γ0 ++ [(Tx, dx)]) by auto. apply weaken_qstp. eapply qstp_trans; eauto. constructor; constructor. simpl. rewrite app_length. simpl. lia. simpl. rewrite app_length. simpl. lia.
    apply qstp_or_r_sub. apply qstp_refl. 
    constructor; constructor. simpl. rewrite app_length. simpl. lia. simpl. rewrite app_length. simpl. lia. 
    apply qstp_closed in Hphi; destruct Hphi. eapply closed_qual_monotone; eauto. simpl. lia. 
  - intuition. rename H12 into IHHT1. rename H11 into IHHT2. simpl.
    ...
    assert (Hoverlap: syntactic_to_set ({0 |-> dx' }ᵈ ds') = qglb (syntactic_to_set ({0 |-> dx' }ᵈ df')) (syntactic_to_set ({0 |-> dx' }ᵈ d1'))). { (* transition lemmas required, syntactic_to_set ({0 |-> dx' }ᵈ df) = {0 |-> dx' }ᵈ (syntactic_to_set df)
        1. qstp -> qstp / subqual
        2. context transfer (probably just use the lambda_star one) 
        3. saturated transfer
    *)
    ...
    eapply IHHT2; eauto.
    eapply closed_ty_subst1'; eauto.
    6-7: rewrite ctx_syntactic_to_set_ext_dist in H10; rewrite ctx_syntactic_to_set_ext_dist in H9; rewrite syntactic_to_set_subst1_commute; rewrite ctx_syntactic_to_set_subst1_commute; eapply lambda_star.subst1_saturated; eauto. 
    subst φs. rewrite <- @subst1_qual_empty with (dx:=dx'). erewrite <- subst1_openq_comm; eauto. eapply subst_qstp; eauto.
...
Lemma substitution : forall {t Tf df Tx dx Σ T d ds},
    syntactic_to_set ds = qualifiers.qglb (syntactic_to_set df) (syntactic_to_set dx) ->
    has_type [(Tx,ds) ; (Tf,df)] (df ⋓ just_fv 0 ⋓ just_fv 1) Σ t T d ->
...
Proof.
  ...
  intuition. replace ([(Tx, ds); (Tf, df)]) with ([(Tx, ds)] ++ [(Tf, df)]) in H; auto.
  remember (df ⊔ (just_fv 0) ⊔ (just_fv 1)) as DF.

  assert (Hsepf : qualifiers.qglb (syntactic_to_set df) (syntactic_to_set DF) = syntactic_to_set df). {
    subst DF. simpl. rewrite qualifiers.qglb_qlub_dist_l.
    destruct (syntactic_to_set df); simpl. 
    (* disjoint_glb_lub_eq *)
    repeat rewrite setfacts.empty_union_right. f_equal.
    rewrite setfacts.inter_refl. apply setfacts.union_inter_self_id.
    1-2: rewrite setfacts.inter_refl; rewrite setfacts.inter_empty_right; rewrite setfacts.empty_union_right; auto.
  }
  assert (Hrefl : qstp ([(Tx, ds)] ++ [(Tf, df)]) Σ DF DF). {
    apply qstp_refl. subst. inversion H5; subst. constructor. 
    eapply closed_qual_monotone; eauto. 
    eapply closed_qual_monotone; eauto. }
  eapply (substitution_gen Hsepf Hrefl) in H0; eauto.
  replace ({0 |-> df }ᴳ [(Tx, ds)]) with ([] ++ [(Tx, ds)]) in H0.

  apply (weaken_flt) with (φ':=(df ⋓ just_fv 0)) in H0. 
  2: {
    subst DF. simpl. apply vtp_closed in H1; intuition. erewrite subst1_qual_id. unfold qlub.
    apply qstp_sub_or. apply qstp_or_l_sub. apply qstp_refl. 
    eapply closed_qual_monotone; eauto. lia. constructor; auto.
    apply qstp_refl. constructor. eapply closed_qual_monotone; eauto. lia. constructor; auto.
    eauto.
  }
  2: { apply vtp_closed in H1; intuition. constructor. eapply closed_qual_monotone; eauto. lia. constructor; auto. }
  ...
  assert (Hrefl' : qstp ([] ++ [(Tx, ds)]) Σ (df ⋓ just_fv 0) (df ⋓ just_fv 0)). 
  apply qstp_refl. subst. inversion H5; subst. constructor.
  apply vtp_closed in H1. intuition. eapply closed_qual_monotone; eauto. lia. constructor. auto. 
  subst DF.
  replace (qualifiers.qlub (syntactic_to_set df) (syntactic_to_set $$ 0)) with (syntactic_to_set (qor df $$0)) in Hsepf'.
  eapply (substitution_gen Hsepf' Hrefl') in H0; eauto. simpl in H0.

  replace (qor ({0 |-> dx }ᵈ df) dx) with (df ⋓ dx) in H0. apply H0.
  (*done, prove earlier replacements *)
  erewrite subst1_qual_id; eauto. apply vtp_closed in H1. intuition. eauto.
  apply syntactic_to_set_qor.
  subst DF. simpl. erewrite subst1_qual_id; eauto. erewrite subst1_ty_id; eauto. 

  apply vtp_closed in H1. apply vtp_closed in H2. intuition. 
  eapply (@syntactic_to_set_closed_inter _ _ _ df dx); eauto.
Qed.


Lemma substitution2 : forall {t Tf df df' Tx dx dx' Σ T d ds'},
    syntactic_to_set ds' = qualifiers.qglb (syntactic_to_set df') (syntactic_to_set dx') ->
    has_type [(Tx,ds') ; (Tf,df)] (df ⋓ just_fv 0 ⋓ just_fv 1) Σ t T d ->
    forall {vf}, vtp Σ vf Tf df ->
    qstp [] Σ df df' ->
    ...
    qstp [] Σ dx dx' ->
        has_type [] (df ⊔ dx) Σ
          ...
          ({ 0 |-> df ; dx }ᵈ d).
Proof.
  intros. specialize (vtp_closed H3) as Hclx. specialize (vtp_closed H1) as Hclf. specialize (has_type_closed H0) as Hclt.
  intuition. replace ([(Tx, ds'); (Tf, df')]) with ([(Tx, ds')] ++ [(Tf, df')]) in H0; auto.
  remember (df ⊔ (just_fv 0) ⊔ (just_fv 1)) as DF.
  ...
  specialize (syntactic_to_set_qstp_exists H H2 H4). intros. destruct H14 as [ds [Hds Hdsqstp]]. 
  apply narrowing with (V:=Tx) (dv:=ds) in H0. 
  3: { apply saturated_cons. apply saturated_empty_tenv. apply qstp_closed in Hdsqstp; intuition. } 
  2: { apply stp_refl; auto. eapply closed_ty_monotone; eauto. lia. replace [(Tf, df)] with ([(Tf,df)] ++ []) by auto. apply weaken_qstp. auto. }

  eapply substitution; eauto. subst DF. auto.
Qed.


(* This is strange, because if d2 is closed under 0, then the open will do nothing
  But in lambda_star.v we do that openq_qstp_monotone like this one
*)
...
Lemma open_qual_1_mono : forall {Γ Σ d1 d1' d2}, 
  qstp Γ Σ d1 d1' -> 
  closed_qual 1 (length Γ) (length Σ) d2 ->  
  qstp Γ Σ ([[ 0 ~> d1 ]]ᵈ d2) ([[ 0 ~> d1' ]]ᵈ d2).
Proof.
  intros. pose (He := H). apply qstp_closed in He. destruct He. induction d2; simpl; auto.
  - constructor. constructor.
  - destruct v. constructor. inversion H0; auto.
    bdestruct (i =? 0). auto. apply qstp_refl. inversion H0; intuition.
  - apply qstp_refl. inversion H0; constructor; auto.
  - inversion H0. intuition. 
    apply qstp_sub_or. apply qstp_or_l_sub. auto. apply qstp_closed in H11; intuition. apply qstp_or_r_sub. auto. apply qstp_closed in H10; intuition.
Qed.

(* This is trivial because d2 and d2' closed under 0, so open won't do anything *)
Lemma open_qual_mono2 : forall {Γ Σ d1 d2 d2' k}, 
  qstp Γ Σ d2 d2' -> 
  closed_qual 0 (length Γ) (length Σ) d1 ->
  qstp Γ Σ ([[ k ~> d1 ]]ᵈ d2) ([[ k ~> d1 ]]ᵈ d2').
Proof.
  intros. 
  pose (He := H). apply qstp_closed in He; destruct He.
  erewrite closed_qual_open_id; eauto. erewrite closed_qual_open_id; eauto. lia. lia.
Qed.

(* Again, the d3 d3' are closed under 0, so open won't do anything. However, in the original system it's also the case *)
Lemma openq_qstp_mono : forall {Γ Σ d1 d1' d2 d2' d3 d3'},
    qstp Γ Σ d1 d1' -> qstp Γ Σ d2 d2' -> 
    qstp Γ Σ d3 d3' -> 
    qstp Γ Σ (openq d1 d2 d3) (openq d1' d2' d3').
Proof.
  intros. 
  pose (H':=H); apply qstp_closed in H'; destruct H'.
  pose (H0':=H0); apply qstp_closed in H0'; destruct H0'.
  pose (H1':=H1); apply qstp_closed in H1'; destruct H1'.
  unfold openq. 
  repeat erewrite closed_qual_open_id; eauto.
Qed.

(* really meaningful things *)
Lemma openq_qstp_mono1 : forall {Γ Σ d1 d1' d2 d},
  qstp Γ Σ d1 d1' ->
  closed_qual 0 (length Γ) (length Σ) d2 ->
  closed_qual 2 (length Γ) (length Σ) d ->
  qstp Γ Σ (openq d1 d2 d) (openq d1' d2 d).
Proof.
  intros. unfold openq; simpl. pose (He:=H); apply qstp_closed in He; destruct He.
  erewrite open_qual_commute''; eauto. 
  replace (([[1 ~> d2 ]]ᵈ ([[0 ~> d1' ]]ᵈ d))) with ([[0 ~> d1' ]]ᵈ ([[1 ~> d2 ]]ᵈ d)). 2:{ eapply open_qual_commute''; eauto. }
  apply open_qual_1_mono; auto. 
...
Lemma open_qual_id' : forall {q k d f l},
  closed_qual 0 f l q ->
  q = [[k ~> d]]ᵈ q.
Proof.
  intros. induction q; intuition.
...
Lemma openq_qstp_or_helper1: forall {df d1 d2 l Γ Σ},
  closed_qual 0 (length Γ) (length Σ) d1 -> 
  closed_qual 0 (length Γ) (length Σ) df ->
  closed_qual 2 (length Γ) (length Σ) d2 ->
  closed_qual 0 (length Γ) (length Σ) &&l ->
  qstp Γ Σ (openq df (d1 ⋓ && l) d2) (qor (openq df d1 d2) && l).
Proof.
  intros. unfold qlub. unfold openq. induction d2; simpl; auto.
  - apply qstp_or_l_sub. constructor. auto. auto.
  - destruct v. apply qstp_or_l_sub. constructor. inversion H1; auto. auto.
    destruct i; simpl; auto.
    apply qstp_or_l_sub. erewrite <- open_qual_id'. erewrite <- open_qual_id'. apply qstp_refl; auto. 1-3: eauto.
    destruct i; simpl; auto. apply qstp_refl. constructor; auto. inversion H1; lia.
  - apply qstp_or_l_sub; auto. constructor. inversion H1; auto. 
  - inversion H1; subst; intuition. inversion H2; subst.
    apply qstp_sub_or.
    apply (@qstp_trans _ _ _ (qor ([[1 ~> d1 ]]ᵈ ([[0 ~> df ]]ᵈ d2_1)) && l)); auto.
    apply qstp_sub_or. apply qstp_or_l_sub; auto. apply qstp_or_l_sub; auto. apply qstp_refl. 
    3: apply qstp_or_r_sub; auto. 3: constructor; auto.
    4: apply (@qstp_trans _ _ _ (qor ([[1 ~> d1 ]]ᵈ ([[0 ~> df ]]ᵈ d2_2)) && l)); auto. 4: apply qstp_sub_or. 4: apply qstp_or_l_sub. 4: apply qstp_or_r_sub. 4: apply qstp_refl. 6: auto. 
    6: apply qstp_or_r_sub. 6: constructor; auto. 3,6: constructor.
    all: eapply openq_closed; eauto.
Qed.

Lemma openq_qstp_or_helper2: forall {df d1 d2 l Γ Σ},
  closed_qual 0 (length Γ) (length Σ) d1 -> 
  closed_qual 0 (length Γ) (length Σ) df ->
  closed_qual 2 (length Γ) (length Σ) d2 ->
  closed_qual 0 (length Γ) (length Σ) &&l ->
  qstp Γ Σ (openq (df ⋓ && l) d1 d2) (qor (openq df d1 d2) && l).
Proof.
  intros. unfold qlub. unfold openq. induction d2; simpl; auto.
  - apply qstp_or_l_sub. constructor. auto. auto.
  - destruct v. apply qstp_or_l_sub. constructor. inversion H1; auto. auto.
    destruct i; simpl; auto. apply qstp_refl. constructor; auto. erewrite <- open_qual_id'; eauto.
    destruct i; simpl; auto. apply qstp_or_l_sub; auto. apply qstp_refl; auto. apply qstp_or_l_sub; auto. apply qstp_refl; auto. inversion H1; lia.
  - apply qstp_or_l_sub; auto. constructor. inversion H1; auto. 
  - inversion H1; subst; intuition. inversion H2; subst.
    apply qstp_sub_or.
    apply (@qstp_trans _ _ _ (qor ([[1 ~> d1 ]]ᵈ ([[0 ~> df ]]ᵈ d2_1)) && l)); auto.
...
Proof.
  ...
  induction H; intuition; try discriminate; inversion H0; subst. all: exists t0; intuition.
Qed.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    + (* beta *)
      ...
      * apply (Preserve Σ ∅); auto.  
        apply t_sub with (T1 := T) (d1 := (openq df d1 d2)). 2: eapply stp_refl'; eauto. 2: apply qstp_or_l_sub. 2: apply qstp_refl. 3: constructor. 2: auto.
        apply qstp_closed in H38 as H38'; intuition.
        change (length []) with 0 in *. subst.

        pose (VH' := H35). eapply t_abs with (φ:=df1) in VH'; eauto. apply has_type_vtp in VH'; auto.

        specialize (syntactic_to_set_qstp_exists H H5 H4). intros. destruct H27 as [ds [Hds Hdsqstp]].

        assert (HT' : has_type [(T1, ds'); (TFun d0 d3 T0 T2, df1)] (df1 ⋓ just_fv 0 ⋓ just_fv 1) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T2) (openq' ([]:tenv) d3)). {
          eapply (@narrowing _ T0 d0); eauto. 
          eapply weaken_stp; eauto.
          eapply saturated_cons. apply saturated_empty_tenv. apply stp_closed in H37; intuition.
        }
        
        apply narrowing_stp with (V:=T1) (dv:=ds) in H39. 2: apply stp_refl; auto.

        apply (@substitution2 ((open_tm' ([] : tenv) t0)) (TFun d0 d3 T0 T2) (df1) (df') (T1) (d1) (d1') Σ ((open_ty' ([] : tenv) T2)) ((openq' ([] : tenv) d3)) (ds')) with (vf:= (tabs t0)) (vx:=t2) in HT' as HT''; auto.
        ...
        erewrite <- open_subst2_tm in HT''; eauto. erewrite <- open_subst2_qual in HT''; eauto. erewrite <- open_subst2_ty in HT''; eauto.
        ...
        replace ([(T1, ds); (TFun d0 d3 T0 T2, df1)]) with ([(T1, ds)] ++ [(TFun d0 d3 T0 T2, df1)]) in Hsub by auto.
        ...
        eapply s_trans; eauto. apply stp_refl; auto. unfold openq. 
        eapply openq_qstp_mono1; eauto. 
        inversion H32. auto. inversion H32; auto.

        unfold open_ty'. unfold open_ty.  clear H45.
        repeat erewrite closed_ty_open_id; eauto.
        apply qstp_refl; auto.
        eapply openq_subqual; eauto. apply has_type_filter in H0. auto.
        constructor; auto. apply has_type_filter in H0. eapply qstp_trans; eauto. 
        inversion H32. auto. inversion H32; auto.
        eapply qstp_trans; eauto. eapply closed_ty_monotone; eauto. lia. replace ([(TFun d0 d3 T0 T2, df1)]) with ([(TFun d0 d3 T0 T2, df1)] ++ []) by auto. eapply weaken_qstp; eauto.
        apply qstp_refl; auto.

        apply qstp_sub_or. 2: constructor; auto. eapply openq_subqual; auto. 
        apply has_type_filter in H0. auto.
    + (* (tabs t) t2 -> (tabs t) t2' *)
      apply has_type_vtp in H0 as VH; intuition.
      ...
      * assert (H30_alt: has_type [] (ldom Σ') Σ' t2' T1 d1). { apply t_sub with (T1:=T1) (d1:=(qor d1 ∅)). auto. apply stp_refl. eapply closed_ty_monotone; eauto. 2: constructor. 2: apply qstp_refl. 3: constructor. 
        ...
        apply t_sub with (T1:=T) (d1:=(openq df (d1) d2)). eapply t_app with (df := df) (ds':=ds'); eauto.
        ...
        (* 1,2 : eapply weaken_store_saturated; eauto. *)
        ...
        4: constructor. 4: apply qstp_sub_or.
        inversion H18; intuition; eapply closed_qual_monotone; eauto; apply extends_length; auto.
        4: constructor. 3: apply openq_subqual. 3-5: apply (@qstp_trans _ _ _ (ldom Σ)); auto. 4,6,8: apply extends_ldom; auto.
        1-2: eapply closed_qual_monotone; eauto; apply extends_length; auto.
        1-3: eapply weaken_qstp_store_ext; eauto. apply has_type_filter in H0; auto. apply has_type_filter in H1; auto. 
        apply closed_qual_ldom.
      * specialize (has_type_closed H0). specialize (has_type_closed H30). intuition. inversion H38; subst. 
        ...
        -- apply stp_refl. simpl. eapply closed_ty_monotone in H2;  eauto. apply extends_length. auto.
           apply openq_qstp_or_helper1.
           4: inversion H40; auto.
           all: eapply closed_qual_monotone; eauto. 1-3: apply extends_length; auto.  
        -- apply has_type_filter in H1. apply qstp_sub_or. apply openq_subqual. 4: apply just_loc_ldom; auto. 
           1-3: apply (@qstp_trans _ _ _ (ldom Σ)). 2,4,6: apply extends_ldom; auto.
    ...
    + (* t t2 -> t' t2 *)
      ...
      * assert (H32_alt: has_type [] (ldom Σ') Σ' t1' (TFun ds' d2 T1 T) df). {
        ...
        apply t_sub with (T1:=T) (d1:=(openq df (d1) d2)). eapply t_app with (df := df) (ds':=ds') (df':=df'); eauto.
        ...
        4: constructor. 4: apply qstp_sub_or.
        inversion H18; intuition; eapply closed_qual_monotone; eauto; apply extends_length; auto.
        4: constructor. 3: apply openq_subqual. 6: auto. 3-5: apply (@qstp_trans _ _ _ (ldom Σ)). 4,6,8: apply extends_ldom; auto.
        eapply closed_qual_monotone; eauto; apply extends_length; auto. eapply closed_qual_monotone; eauto; apply extends_length; auto.
        all: eapply weaken_qstp_store_ext; eauto. eapply has_type_filter in H0; auto. apply has_type_filter in H1; auto.
      * (* rewrite <- openq_u_distribute1. *)
        eapply t_sub with (T1 := T) (d1 := (openq (df ⋓ just_loc l) d1 d2)).
        ...
        -- apply stp_refl. simpl. eapply closed_ty_monotone; eauto. lia.
           apply openq_qstp_or_helper2.
           4: constructor; auto.
           all: eapply closed_qual_monotone; eauto. 1-2: apply extends_length; auto.
           apply qstp_closed in H3; intuition. 
           inversion H18; intuition. eapply closed_qual_monotone; eauto.
        -- apply has_type_filter in H1. apply has_type_filter in H0.
           apply qstp_sub_or. 
           apply openq_subqual. 1-3: apply (@qstp_trans _ _ _ (ldom Σ)). 2,4,6: apply extends_ldom; auto. 1-3: eapply weaken_qstp_store_ext; eauto. 
  ...
  - (* t_assign *) subst. intuition. rename H into Ht1. rename H0 into Ht2. intuition.
    ...
    + (* both t1 and t2 are value *) (* contraction *)
      right. intros.
    ...
    + (* t1 value, t2 steps *) (* right congruence *)
