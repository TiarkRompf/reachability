...
| t_var : forall Γ φ Σ x T d,
    ...
    (* closed_qual 0 (length Γ) (length Σ) φ -> *) (* already enclosed *)
...
| t_app : forall Γ φ Σ t1 d1 t2 d2 df T1 T2,
    ...
    (* closed_qual 0 (length Γ) (length Σ) ds  -> *) (* inferred *)
    has_type Γ φ Σ t1 (TFun ds d2 T1 T2) df ->
    has_type Γ φ Σ t2 T1 d1 ->
    closed_ty 0 (length Γ) (length Σ) T2 ->
    qstp Γ Σ (openq ∅ d2) φ ->
    (* TODO: we can probably not require d1 and df to be saturated, but the intersection ds to be saturated instead *)
...
Lemma closed_tm_open_id : forall {t b f l}, closed_tm b f l t -> forall {n}, b <= n -> forall {x}, (open_rec_tm n x t) = t.
  ...
  destruct (Nat.eqb n x) eqn:Heq; auto. apply EqNat.beq_nat_true_stt in Heq. lia.
Qed.
...
Lemma closed_tm_open : forall {T b f l}, closed_tm (S b) f l T -> forall {x}, x < f -> closed_tm b f l (open_rec_tm b (varF x) T).
  ...
  apply EqNat.beq_nat_false_stt in Heq. constructor. lia. auto. auto.
Qed.
...
Proof.
  ...
  - destruct v.
    ...
    + simpl. inversion H; subst. destruct (i =? b) eqn:Eqib.
      constructor. assumption. constructor. apply EqNat.beq_nat_false_stt in Eqib. lia.   
...
Lemma closed_tm_open_ge : forall {T b f l}, closed_tm (S b) f l T -> forall {x}, f <= x -> closed_tm b (S x) l (open_rec_tm b (varF x) T).
  induction T; intros; simpl; intuition; inversion H; subst; try constructor;
      try solve [eapply IHT1; eauto | eapply IHT2; eauto | eapply IHT; eauto ].
  destruct (Nat.eqb b x0) eqn:Heq. intuition.
  apply EqNat.beq_nat_false_stt in Heq. inversion H. subst.
...
Lemma closed_open_succ : forall {T b f l}, closed_tm b f l T -> forall {j}, closed_tm b (S f) l (open_rec_tm j (varF f) T).
  ...
  apply EqNat.beq_nat_false_stt in Heq. inversion H. subst. intuition. lia. auto.
Qed.
...
Lemma open_rec_tm_commute : forall t i j x y, i <> j -> open_rec_tm i (varF x) (open_rec_tm j (varF y) t) = open_rec_tm j (varF y) (open_rec_tm i (varF x) t).
  ...
  apply EqNat.beq_nat_true_stt in Hii0. apply EqNat.beq_nat_true_stt in Hji0. subst. contradiction.
Qed.
...
Proof.
  ...
  - destruct v. reflexivity.
    ...
    + assert ((i0 =? i) = false). apply Nat.eqb_neq. apply EqNat.beq_nat_true_stt in Hi0_j. intuition. rewrite H0. simpl. rewrite Hi0_j. reflexivity.
...
Proof.
  ...
  - apply qstp_closed in H1; destruct H1; assumption.
  - unfold openq. eapply qstp_closed in H1; destruct H1. unfold openq in H1; simpl. eapply closed_qual_open'; eauto. inversion H7; assumption.
...
Proof.
  ...
  - (* tapp *) simpl. rewrite splice_qual_open''. eapply t_app with (T1:=(splice_ty k T1)) (df:=(splice_qual k df)) (ds := (splice_qual k ds)).
    apply syntactic_to_set_splice. auto. 
    ...
    rewrite ctx_syntactic_to_set_ext_dist in H2. specialize (lambda_star.weaken_saturated H2). intros. rewrite H4. apply H5.
    rewrite ctx_syntactic_to_set_ext_dist in H3. specialize (lambda_star.weaken_saturated H3). intros. rewrite H4. apply H5. 
...
Proof.
  ...
  - eapply lambda_star.weaken_store_saturated; eauto. unfold lambda_star.extends. inversion H4. exists (store_syntactic_to_set x). rewrite H5. unfold store_syntactic_to_set. apply map_app.
  - eapply lambda_star.weaken_store_saturated; eauto. destruct H4. rewrite H4. unfold store_syntactic_to_set. rewrite map_app. unfold lambda_star.extends. exists (map (fun T : ty => ty_syntactic_to_set T) x). auto.
...
Lemma narrowing_gen : forall {t Γ1 U du Γ2 φ Σ T d},
  ...
  - econstructor; eauto.
    ...
    eapply narrowing_qstp_gen; eauto.
    rewrite ctx_syntactic_to_set_ext_dist in H2. rewrite ctx_syntactic_to_set_ext_dist. 
    ...
    rewrite ctx_syntactic_to_set_ext_dist in H3. rewrite ctx_syntactic_to_set_ext_dist. 
    replace (ctx_syntactic_to_set ((U, du) :: Γ2)) with ((ctx_transfer (U, du)) :: ctx_syntactic_to_set(Γ2)) in H2 by intuition. replace (ctx_syntactic_to_set ((V, dv) :: Γ2)) with ((ctx_transfer (V, dv)) :: ctx_syntactic_to_set(Γ2)) by intuition.
    eapply lambda_star.narrowing_saturated; eauto. apply stp_syntactic_to_set. auto. 
  - econstructor; eauto.
    repeat rewrite app_length in *; simpl in *; auto.
...
Lemma CtxOK_update : forall {Γ φ Σ σ}, CtxOK Γ φ Σ σ -> forall {l T}, l < length σ -> indexr l Σ = Some T -> forall {v}, has_type Γ φ Σ v T ∅ -> value v -> CtxOK Γ φ Σ (update σ l v).
  ...
  - apply EqNat.beq_nat_true_stt in Heq. subst.
    apply (@update_indexr_hit _ σ l0 v) in H0. rewrite H1 in H. inversion H. subst.
    rewrite H4 in H0. inversion H0. subst. intuition.
  - apply EqNat.beq_nat_false_stt in Heq. apply (@update_indexr_miss _ σ l v l0) in Heq.
...
(* The context assumption is wrong, but it's still true *)
...
Proof. 
  ...
  - simpl. intuition. rename H6 into IHHT.
    ...
    subst. rewrite syntactic_to_set_qor. simpl. eapply lambda_star.qglb_increase_fresh_equal. auto. apply syntactic_to_set_closed; eauto.
  ...
  - intuition. rename H8 into IHHT1. rename H7 into IHHT2. simpl.
    replace ({ 0 |-> dx' }ᵈ (openq d1 d2)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ d2)).
    (*separation/overap is preserved after substitution*)
    assert (Hoverlap: syntactic_to_set ({0 |-> dx' }ᵈ ds) = qglb (syntactic_to_set ({0 |-> dx' }ᵈ df)) (syntactic_to_set ({0 |-> dx' }ᵈ d1))). { (* a lot of transition lemma required, syntactic_to_set ({0 |-> dx' }ᵈ df) = {0 |-> dx' }ᵈ (syntactic_to_set df) *)
      specialize (has_type_filter HT1). specialize (has_type_filter HT2).
      symmetry.
      do 3 rewrite syntactic_to_set_subst1_commute. 
      erewrite lambda_star.subst1_preserves_separation_equal; eauto. rewrite H0. auto. 
      ...
      replace (ctx_syntactic_to_set (Γ0 ++ [(Tx, dx)])) with ((ctx_syntactic_to_set Γ0) ++ [(ty_syntactic_to_set Tx, syntactic_to_set dx)]) in H5. eauto. rewrite ctx_syntactic_to_set_ext_dist. auto.
      replace (ctx_syntactic_to_set (Γ0 ++ [(Tx, dx)])) with ((ctx_syntactic_to_set Γ0) ++ [(ty_syntactic_to_set Tx, syntactic_to_set dx)]) in H6. eauto. rewrite ctx_syntactic_to_set_ext_dist. auto.
    }
    apply t_app with (T1:= { 0 |-> dx' }ᵀ T1) (df:=({0 |-> dx' }ᵈ df)) (ds:= {0 |-> dx' }ᵈ ds).
    auto.  
    eapply IHHT1; eauto. 
    eapply IHHT2; eauto.
    3-4: rewrite ctx_syntactic_to_set_ext_dist in H6; rewrite ctx_syntactic_to_set_ext_dist in H5; rewrite syntactic_to_set_subst1_commute; rewrite ctx_syntactic_to_set_subst1_commute; eapply lambda_star.subst1_saturated; eauto. 
    eapply closed_ty_subst1'; eauto.

    subst φs. unfold openq. rewrite <- @subst1_qual_empty with (dx:=dx'). erewrite <- subst1_open_qual_comm; eauto. eapply subst_qstp; eauto.
    1-2: apply syntactic_to_set_closed; eapply closed_qual_monotone; eauto; unfold store_syntactic_to_set; rewrite map_length; auto.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    (* t1 *) specialize (H9 (TFun ds d2 T1 T) t1). intuition.
    ...
    + (* beta *)
      ...
      * apply (Preserve Σ ∅); auto.  
        ...
        apply qstp_closed in H34 as H33'; intuition.
        ...
        assert (HT' : has_type [(T1, ds)] (df ⋓ just_fv 0) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T2) (openq' ([]:tenv) d3)). {
          ...
          apply saturated_empty_tenv. apply stp_closed in H33; intuition.
        }
        eapply @substitution with ( vx:= t2) in HT' as HT''; eauto; intuition.
        unfold open_tm' in HT''. unfold open_ty' in HT''. unfold openq' in HT''. simpl in HT''. inversion H27; subst.
        ...
        eapply t_sub; eauto.
        (* inversion H26. subst. clear H44. *)
        rename H35 into Hsub.
        ...
        erewrite <- open_subst1_qual in Hsub; eauto.
        all: inversion H28; subst; simpl; eauto.
        unfold open_ty'. unfold open_ty. erewrite (@closed_ty_open_id _ 0); eauto.
        3: apply qstp_sub_or. 4: constructor; auto. 1,3: eapply openq_subqual; auto. 
        apply has_type_filter in H0. constructor; auto.
    + (* (tabs t) t2 -> (tabs t) t2' *)
      apply has_type_vtp in H0 as VH; intuition.
      apply vtp_canonical_form_lam in VH as HH; intuition.
      pose (HH11 := H12).
      unfold CtxOK in HH11. specialize (H9 σ). intuition.
      destruct H23 as [t2' [σ' HH22]]. exists (tapp t1 t2'). exists σ'. intuition. constructor; intuition.
      destruct H23.  apply (Preserve Σ' d'); intuition.
      inversion H25; subst.
      * assert (H26_alt: has_type [] (ldom Σ') Σ' t2' T1 d1). { apply t_sub with (T1:=T1) (d1:=(qor d1 ∅)). auto. apply stp_refl. eapply closed_ty_monotone; eauto. 2: constructor. 2: apply qstp_refl. 3: constructor. 
        ...
        inversion H14; intuition; eapply closed_qual_monotone; eauto; apply extends_length; auto.
        ...
        apply has_type_filter in H1. all: destruct H23; rewrite H23; eapply weaken_qstp_store; eauto.
      * specialize (has_type_closed H0). specialize (has_type_closed H26). intuition. inversion H34. subst. 
        apply t_sub with (T1 := T) (d1 := (openq (d1 ⋓ (just_loc l)) d2)).
        -- eapply t_app with (T1 := T1) (df:=df) (ds:=ds); eauto.  
           ...
           1-2: apply syntactic_to_set_closed; eauto. simpl. auto. 
    ...
    + (* t t2 -> t' t2 *)
      ...
      specialize (H20 σ H12). destruct H20 as [t1' [σ' HH7]]. exists (tapp t1' t2). exists σ'. intuition. apply step_c_app_l. intuition.
      destruct H24. apply (Preserve Σ' d'); intuition. inversion H27; subst.
      * assert (H28_alt: has_type [] (ldom Σ') Σ' t1' (TFun ds d2 T1 T) df). {
        ...
        3: constructor. 2: apply openq_subqual. 4: auto. 2,3: apply (@qstp_trans _ _ _ (ldom Σ)). 3,5: apply extends_ldom; auto.
        eapply closed_qual_monotone; eauto; apply extends_length; auto.
        apply has_type_filter in H1. all: destruct H24; rewrite H24; eapply weaken_qstp_store; eauto.
      * 
        eapply t_sub with (T1 := T)(d1 := (openq d1 d2)).
        -- eapply t_app with (df:=(df ⋓ just_loc l)); eauto. 
           rewrite syntactic_to_set_qor. replace (syntactic_to_set && l) with (qualifiers.just_loc l) by auto. erewrite <- qqcap_fresh_l; eauto; eapply syntactic_to_set_closed. simpl in H17; eauto. simpl in H25; auto.
