...
| t_abs: forall Γ φ Σ T1 T2 d2 df t,
    closed_tm 1 (length Γ) (length Σ) t ->
    closed_ty 0 (length Γ) (length Σ) (TFun ∅ d2 T1 T2) ->
    closed_qual 0 (length Γ) (length Σ) df ->
    closed_qual 0 (length Γ) (length Σ) φ ->
    df ⊑ φ ->
    has_type ((T1,∅) :: Γ) (df ⊔ (just_fv (length Γ))) Σ (open_tm' Γ t) (open_ty' Γ T2) (openq' Γ d2) ->
    has_type Γ φ Σ (tabs t) (TFun ∅ d2 T1 T2) df

| t_app : forall Γ φ Σ t1 d1 t2 d2 df T1 T2,
    has_type Γ φ Σ t1 (TFun ∅ d2 T1 T2) df ->
...
(* | SFun : forall df, Substq df df        (* we substitute a recursive function into itself, not supported in this version *) *)
...
| vtp_abs: forall Σ T1 T2 d2 T3 d3 T4 d4 df1 df2 t,
  closed_tm 1 0 (length Σ) t ->
  closed_qual 0  0 (length Σ) df2 ->
  closed_ty 0 0 (length Σ) (TFun d3 d4 T3 T4) ->  (* sub type *)
  closed_ty 0 0 (length Σ) (TFun ∅ d2 T1 T2) ->  (* super type *)
  has_type [(T1,∅)] (df1 ⊔ (just_fv 0)) Σ (open_tm' ([]: tenv) t) (open_ty' ([]: tenv) T2) (openq' ([]: tenv) d2) ->
  stp [] Σ T3 d3 T1 ∅ ->
  qstp [] Σ df1 df2 ->
...
Fixpoint has_type_filter {Γ φ Σ t T d} (ht : has_type Γ φ Σ t T d) : d ⊑ φ.
  destruct ht; intuition. apply openq_subqual. apply has_type_filter in ht2. auto.
  auto.
Qed.
...
Lemma weaken_gen : forall {t Γ1 Γ2 φ Σ T d},
  ...
  - (* t_abs *) rewrite app_length in *. simpl. rewrite splice_set_empty. constructor.
    ...
    inversion H0. subst. constructor. auto. 1,4,5: apply splice_qual_closed; auto. 1,2 : apply splice_ty_closed; auto.
  ...
  - (* tapp *) simpl. rewrite splice_qual_open''. apply t_app with (T1:=(splice_ty k T1)) (df:=(splice_qual k df)).
    ...
    apply IHHT1; auto. simpl. rewrite splice_set_empty. auto.
    apply IHHT2; auto.
    ...
    rewrite <- splice_qual_glb_dist. rewrite H1. auto.
...
Proof.
  ...
  destruct φ as [fp bp lp]. inversion H0. subst. apply bound_0_empty in H10, H11.
  subst. destruct dx as [fx bx lx]. simpl in H. intuition. rewrite inter_empty_left in H4.
  rewrite inter_empty_left in H. symmetry in H4. symmetry in H. apply NatSet.eq_if_Equal in H4.
  ...
  simpl. destruct (mem 0 ff) eqn:Hmem0ff; destruct (mem 0 f1) eqn:Hmem0f1; simpl; rewrite NatSetFacts.inter_b;
...
Lemma vtp_closed: forall {Σ t T d}, vtp Σ t T d ->
    closed_tm 0 0 (length Σ) t /\ closed_ty 0 0 (length Σ) T /\ closed_qual 0 0 (length Σ) d.
Proof.
...
Proof.
  ...
  - (* vtp_abs *) inversion H. subst. econstructor; eauto.
    ...
    + assert (stp [] Σ (TFun d1 d2 T1 T2) d5 (TFun d3 d4 T3 T4) d6).
      ...
      assert (stp [] Σ (TFun ∅ d0 T0 T5) df1 (TFun d1 d2 T1 T2) d5).
      { apply s_fun; intuition.  }
      assert (stp [] Σ (TFun ∅ d0 T0 T5) df1 (TFun d3 d4 T3 T4) d6).
      { eapply s_trans; eauto. }
      assert (stp [] Σ T3 d3 T1 d1). { eauto. }
      specialize (@narrowing_stp_gen [] T1 d1 [] Σ
        (open_ty' ([]: tenv) T5)(openq' ([]:tenv) d0) (open_ty' ([]:tenv) T2) (openq' ([]: tenv) d2)) as narrow.
...
Proof.
  ...
  - (* tabs *) eapply vtp_abs; eauto.
    ...
    + apply stp_refl. destruct H5; intuition.
      * eapply closed_ty_monotone; eauto. apply indexr_var_some' in H5. intuition.
      * intuition. apply has_type_closed in H5. intuition. inversion H7. eapply closed_ty_monotone; eauto; intuition.
        inversion H8. eapply closed_ty_monotone; eauto; lia.
      * apply stp_closed in H6; intuition.
      * apply qstp_refl.  apply eqqual_refl.  apply has_type_closed in H5; intuition.
...
Proof. intros. induction H.
  ...
  - assert (has_type [] df1 Σ (tabs t0) (TFun ∅ d2 T1 T2) df1). {
    constructor; eauto. 1,2: apply qstp_closed in H5 as Hcl; intuition. }
    assert (has_type [] df2 Σ (tabs t0) (TFun ∅ d2 T1 T2) df1). {
    inversion H5. subst. eapply weaken_flt with  (φ' := df2) in H7; intuition. }
...
Lemma substitution_gen :
  forall {t Γ φ' Tx dx dx' Σ T d}, dx' ⋒ φ' ≡ dx ->
    forall {φ}, φ ⊑ φ' ->
      has_type (Γ ++ [(Tx,dx)]) φ Σ t T d -> wf_tenv (Γ ++ [(Tx,dx)]) -> Substq dx dx' ->
        forall {tx}, vtp Σ tx Tx dx' ->
                        has_type ({ 0 |-> dx' }ᴳ Γ) ({ 0 |-> dx' }ᵈ φ) Σ
                                 ...
                                 ({ 0 |-> dx' }ᵈ d).
  Import NatSet.F.
  intros t Γ φ' Tx dx dx' Σ T d Hsep φ Hphi HT HwfG HSubst tx HTx. specialize (vtp_closed HTx) as Hclx.
  ...
  - simpl. (bdestruct (x =? 0)).
    + (*x is 0 *)
    rewrite indexr_skips in H0; simpl; auto; try lia. simpl in H0. subst. simpl in H0.
      inversion H0. subst. erewrite subst1_ty_id; eauto. inversion HSubst; subst.
      (*subst arg, dx = df ⋒ dx = dx' ⋒ φ. *)
      erewrite subst1_qual_plus'; eauto.
      apply vtp_has_type in HTx.
      eapply weaken'; eauto. eapply subst_qual_subqual_monotone in H3. erewrite subst1_qual_plus' in H3; eauto.
  ...
  - simpl. rewrite NatSetFacts.empty_b. rewrite remove_empty. rewrite unsplice_set_empty.
    ...
    1,2: eapply closed_qual_subst1'; eauto. apply subst_qual_subqual_monotone. auto.
    ...
    specialize IHHT with (φ' := φ'') (Γ := (((T1, ∅) :: Γ0))).
    ...
    rewrite NatSetFacts.empty_b in IHHT. rewrite remove_empty in IHHT. rewrite unsplice_set_empty in IHHT.
    apply IHHT; auto.
  ...
  - intuition. rename H7 into IHHT2. rename H5 into IHHT1. simpl.
    ...
    replace (TFun ∅ ({0 |-> dx' }ᵈ d2) ({0 |-> dx' }ᵀ T1) ({0 |-> dx' }ᵀ T2))
            with ({ 0 |-> dx' }ᵀ (TFun ∅ d2 T1 T2)).
    eapply IHHT1; eauto. simpl. rewrite NatSetFacts.empty_b. rewrite remove_empty. rewrite unsplice_set_empty. auto.
    eapply IHHT2; eauto.
    ...
    + rewrite <- Hoverlap. rewrite H4. auto.
...
Lemma substitution : forall {t df Tx dx Σ T d},
    has_type [(Tx,df ⋒ dx)] (df ⊔ (just_fv 0)) Σ t T d -> wf_tenv [(Tx,df ⋒ dx)] -> closed_qual 0 0 (length Σ) df ->
...
Proof.
  ...
  assert (Hsepf : dx ⋒ (df ⋓ just_fv 0) ≡ df ⋒ dx). {
    ...
    inversion H8. subst. simpl in *. apply bound_0_empty in H16, H17.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    (* t1 *) specialize (H7 (TFun ∅ d2 T1 T) t1).  intuition.
    ...
    + (* beta *)
      ...
      * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
        ...
        assert (df1 ⋒ d1 = ∅). {
          rewrite <- H3. apply eq_if_eqqual. apply subqual_antisymm.
          apply subqual_qglb_r. inversion H33. auto. rewrite H3.
          apply empty_smallest. }
        rewrite <- H22 in H30.
        eapply @substitution with ( vx:= t2) in H30 as H30'; eauto; intuition.
        inversion H27. subst.
        unfold open_tm' in H30'. unfold open_ty' in H30'. unfold openq' in H30'. simpl in H30'.
        apply qstp_closed in H33 as H33'. intuition.
        unfold open_ty in H30'. unfold openq in H30'.
        erewrite <- open_subst1_tm in H30'; eauto.
        erewrite <- open_subst1_qual in H30'; eauto.
        fold (open_tm t2 t0) in H30'. fold (openq d1 d0) in H30'.
        apply @weaken_flt with (φ':= (ldom Σ)) in H30'; auto; intuition.
        apply has_type_closed in H30' as Hcl.  intuition.
        assert (stp [] Σ ({0 |-> d1 }ᵀ (open_ty' (@nil (ty * qual)) T2)) (openq d1 d0) T (openq d1 d2)). {
          eapply @subst_stp with (Γ := [])(df' := d1) in H34; eauto.
          replace (open_ty' (@nil (ty * qual)) T) with T in *. (* because T is closed *)
          inversion H26. subst.
          simpl in H34. unfold openq' in H34. unfold openq in H34.  simpl in H34.
          erewrite @subst1_ty_id with (T := T) in  H34; eauto.
          erewrite <- open_subst1_qual in H34; eauto.
          erewrite <- open_subst1_qual in H34; eauto.
          fold (openq d1 d0) in H34. fold (openq d1 d2) in H34.
          unfold open_ty'. unfold open_ty. repeat erewrite closed_ty_open_id; eauto.
          rewrite <- H3.  auto.
        }
        eapply t_sub in H30'; eauto.
        apply has_type_filter in H9. intuition.
        apply has_type_filter in H. inversion H33. subst.
        assert (df1 ⊑ ldom Σ). { eapply subqual_trans in H; eauto. }
        specialize (qlub_bound H35 Hflt0). auto.
        rewrite H22. apply wf_tenv_cons; auto.
        eapply vtp_widening; eauto.
        eapply @stp_scale_qqplus with (Γ := [])(d := d1) in H32; eauto.
        rewrite qlub_commute in H32. rewrite qqplus_qbot_right_neutral in H32. intuition.
    + (* tabs  t t2 -> tabs  t t2' *)
      apply has_type_vtp in H as VH; intuition.
      apply vtp_canonical_form_lam in VH as HH; intuition.
      pose (HH10 := H10).
      ...
      inversion H23; subst.
      * rewrite qqplus_qbot_right_neutral. rewrite qqplus_qbot_right_neutral in H24. eapply t_app with (df := df); eauto.
      ...
      * specialize (has_type_closed H). specialize (has_type_closed H24). intuition. inversion H32. subst. rewrite <- openq_duplicate_eq.
        apply t_sub with (T1 := T) (d1 := (openq (d1 ⋓ (just_loc l)) d2)).
        -- eapply t_app with (T1 := T1) (df:=df); eauto.  eapply weaken_flt. eapply weaken_store; eauto.
           apply extends_ldom. auto. simpl. auto.
           simpl. eapply closed_ty_monotone in H1; eauto. apply extends_length. auto.
           eapply subqual_trans; eauto; apply extends_ldom; auto.
           rewrite qglb_qlub_dist_l. rewrite H3.
           rewrite qqcap_commute. erewrite freshness; eauto.
    ...
    + (*  tabs t t2 -> tabs t' -> t2  *)
      ...
      * eapply t_sub with (T1 := T)(d1 := (openq d1 d2)).
        -- eapply t_app; eauto. eapply weaken_flt. eapply weaken_store. eauto. auto. apply extends_ldom; auto.
           ...
           eapply subqual_trans; eauto. apply extends_ldom; auto.
           rewrite qglb_qlub_dist_r. rewrite H3.  intuition. erewrite freshness; eauto.
  ...
  - (* tderef*) subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H3 (TRef T0) t0). intuition.
    + (* contraction *) right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
      ...
      simpl. rewrite empty_union_left.  apply t_sub with (T1 := T)(d1:= ∅).
      specialize (H11 l v T). intuition.
      replace Σ with (Σ ++ []); intuition. eapply weaken_stp_store. auto. eapply empty_smallest.
