...
Section Warmup.
  ...
  Example idfun_nt :
    ...
    apply t_abs; ccrush.
    3 : { eapply t_sub with (d1:= $$3). T_VAR; ccrush.
  ...
  Section Church.
    ...
    (* Church  respectively pi2: λa.λb.b  *)
    ...
    Example t_pair : forall {X q},
      ...
      apply t_app with (T1:=B) (df:=q⊔$$(length Γ)); crush.
      erewrite qqcap_overlap_fresh; eauto; try lia.
      * replace (q ⊔ $$ (length Γ))
        with (openq $$(length Γ) (q ⊔ ##0)) at 1; try solve [opening; ccrush].
        apply t_app with (T1:=A) (df:=$$(S (S (length Γ)))); ccrush.
        + T_VAR; ccrush.
        + T_VAR; ccrush.
      * T_VAR; ccrush.
      * ccrush.
      * destruct q; ccrush. closed_qual_invert; ccrush. bound_simpl. lia.
    Qed.
...
Section DerivedSyntax.
  ...
  Lemma t_seq : forall Γ φ Σ t1 T1 q1 t2 T2 q2,
    ...
    + Opaque qglb qlub subqual qqcap.
      ...
      specialize (has_type_closed H0). intuition. rewrite Heqq2 in H5. inversion H5.
      Transparent qlub.
      replace q2 with (openq q2 ##0); try solve [opening; ccrush].
      apply t_app with (T1:=T2) (df:=∅); eauto; ccrush.
      * replace (∅) with (openq q1 ∅) at 2; try solve [opening; ccrush].
        eapply t_app with (T1:=T1) (df:=∅); ccrush.
        - apply t_abs; ccrush.
          2 : apply t_abs; ccrush.
          Transparent qglb qlub subqual qqcap.
          3: subst; cleanup; T_VAR; ccrush.
          all : subst; qual_destruct; ccrush.
        - ccrush.
      * ccrush.
      * subst; ccrush.
  Qed.
  ...
  Lemma t_val : forall Γ φ df Σ t1 T1 q1 t2 T2 q2,
      ...
      has_type ((T1, ∅) :: Γ) (df ⊔ $$(length Γ)) Σ (open_tm' Γ t2) (open_ty' Γ T2) (openq' Γ q2) ->
    ...
    apply t_abs; ccrush.
    erewrite closed_ty_open'_id in H7; eauto.
    unfold_open in H7. rewrite H6.
    Transparent qlub subqual qqcap qqplus. ccrush.
    apply has_type_filter in H7. unfold openq' in *.
    eapply unopen_subqual; eauto; crush.
    qual_destruct; ccrush. eapply subqual_trans; eauto; ccrush.
    rewrite H6. auto.
  Qed.
...
Section FunAbs.
  ...
  Section IntroEx.
    ...
    Example deref_ty : has_type [(TRef TUnit, q)] φ Σ deref ({TUnit | ∅} ==> {TUnit | ⊥}) (q ⊕ c).
      intros. unfold deref. destruct q eqn:Hq. inversion q_is_set. subst c.
      apply t_abs; ccrush.
      apply t_deref with (d:=q ⊕ 0). subst.
      T_VAR; ccrush.
    Qed.
    ...
    Example deref_app : has_type [(TRef TUnit, q)] φ Σ (tapp deref tunit) TUnit ⊥.
      ...
      eapply t_app with (df:=((qset t0 t1 t2) ⊕ 0)); crush.
      apply deref_T.
    Qed.
  ...
  Section FunArgs.
      ...
      *)
      ...
      Example addRef_app_bad : exists S q, has_type Γ12 φ Σ (tapp addRef $c1) S q.
        ...
        5 : { intuition. (* we end up needing to show {c1} ⊆ ∅ *) admit. }
        Abort.
      ...
      Example addRef_alt_ty : has_type Γ12 φ Σ addRef ({ (TRef TUnit) | $$c1 } ==> {TUnit | ∅}) $$c1.
        ...
        apply t_abs. (* we are stuck on the first goal here *)
        Undo.
...
Section Escaping.
  ...
  (** The type of the closure in
      ...
      at the point of its occurrence is
           ...
           (Unit => Unit)^{y}.
   *)
  Example closure_deref_internal : has_type (((TRef TUnit),∅) :: Γ) (φ ⊔ $$y) Σ (tabs !($y)) ({TUnit | ⊥} ==> {TUnit | ⊥}) $$y.
    intros.
    apply t_abs; ccrush.
    apply t_deref with ($$y); ccrush.
    subst y. T_VAR; ccrush.
  Qed.
...
Section HigherOrder.
  ...
  Section NonInterference.
    ...
    Example t_par : forall {Γ Σ φ}, closed_qual 0 (length Γ) (length Σ) φ -> ∅ ⊑ φ ->
      ...
      * replace (⊥) with (openq ⊥ ⊥) at 3; try solve [opening;crush].
    ...
    *)
    ...
    Example noninterfering_ty : has_type Γ φ Σ noninterfering TUnit ⊥.
      ...
      - apply t_ref; ccrush.
        apply t_base; ccrush.
      - replace (⊥) with (openq $$(S (length Γ)) ⊥) at 1; try solve [opening;crush].
  ...
  Section NonEscape.
    ...
    Example escaping_ty : has_type Γ $$c1 Σ escaping ({CanThrow | ∅} ==> {{TUnit | ∅} ==> {TUnit | ⊥} | ##0}) $$c1.
      ...
      * apply t_abs; ccrush.
        apply t_sub with (T1:=Nothing) (d1:=⊥); ccrush.
          replace (⊥) with (openq ⊥ ⊥) at 1; try solve [opening; auto].
          eapply t_app with (T1:=TUnit) (df:=$$1); ccrush.
          - subst CanThrow. T_VAR; ccrush.
          - apply t_base; ccrush.
    Qed.
  ...
  Section NonAccess.
    ...
    Example withoutIO_ok_ty : has_type Γ φ Σ withoutIO_ok TUnit ∅.
      ...
      * replace ($$0) with (openq $$0 ##0); try solve [opening; ccrush].
    ...
    Example withoutIO_bad_block_ty : has_type [(TRef TUnit, ∅); (CanIO, ∅)] ($$0 ⊔ $$1) Σ (tabs (seq !($1) (tapp doIO $0))) ({TUnit | ∅} ==> {TUnit | ∅}) ($$0 ⊔ $$1).
      ...
      * apply t_sub with (T1:=TUnit) (d1:=⊥); ccrush. eapply t_deref. T_VAR; ccrush.
  ...
  Section Borrowing.
    ...
    Example borrow_bad_ty : exists S q, has_type Γ φ Σ borrow_bad S q.
      ...
      replace $$c1 with (openq $$c1 ##0); try solve [opening; auto].
...
Section LightweightPoly.
  ...
  Example inc1 : has_type Γ φ Σ (tapp inclike $b) (TRef TUnit) $$b.
    intros. replace $$b with (openq $$b ##0); try solve [opening; ccrush].
    eapply t_app with (df:=∅); ccrush.
  ...
  Example inc2 : has_type Γ φ Σ (tapp inclike $c1) (TRef TUnit) ($$a ⊔ $$b ⊔ $$c1).
    intros. replace ($$a ⊔ $$b ⊔ $$c1) with (openq ($$a ⊔ $$b ⊔ $$c1) ##0);
              try solve [opening;crush].
    eapply t_app with (df:=∅); ccrush.
    * apply inclike_ty; crush.
    * apply t_sub with (T1:=TRef TUnit) (d1:=($$a ⊔ $$b) ⊕ c1); ccrush.
      T_VAR; ccrush.
  Qed.
  ...
  Example inc3 : has_type Γ φ Σ (tapp inclike $c2) (TRef TUnit) ($$a ⊔ $$b ⊔ $$c1 ⊔ $$c2).
    intros. replace ($$a ⊔ $$b ⊔ $$c1 ⊔ $$c2)
            with (openq ($$a ⊔ $$b ⊔ $$c1 ⊔ $$c2) ##0);
              try solve [opening;crush].
    eapply t_app with (df:=∅); ccrush.
    * apply inclike_ty; crush.
    * apply t_sub with (T1:=TRef TUnit) (d1:=($$a ⊔ $$b ⊔ $$c1) ⊕ c2); ccrush.
      T_VAR; ccrush.
  Qed.
