...
Section SelfRef.
  (* This version of λ* permits typing escaping closures using the t_self rule. *)
  ...
  Example esc_ty_let : has_type [] ∅ [] (tabs (tabs #3)) ({TRef TUnit | ∅} ==> { {TUnit | ∅}  ==> {TRef TUnit | ##0 } | ##1}) ∅.
    apply t_abs; ccrush.
    apply t_abs; ccrush.
    eapply t_self; ccrush; ccrush.
    apply t_sub with (T1:=(TRef TUnit)) (d1:= $$1); ccrush.
    T_VAR.
  Qed.

  (* escaping_closure :: f(Unit => Ref[Unit]^f)^{} *)
  Example esc_ty : has_type [] ∅ [] escaping_closure ({TUnit | ∅}  ==> {TRef TUnit | ##0}) ∅.
    unfold escaping_closure. replace (∅) with (openq ∅ ∅ ##1) at 3; try solve [ccrush].
    eapply t_app; ccrush.
    apply esc_ty_let.
  Qed.
...
Section Warmup.
  ...
  Context (Γ012 := [(false,TUnit,∅); (false,TUnit,∅); (false,TUnit,∅)]).
  ...
  (* ∅|∅ ⊢ λx.x : (Unit^⊥ -> Unit^⊥)^⊥ *)
  Example idfun_bot : has_type [] ⊥ [] (idfun) ({TUnit | ⊥} ==> {TUnit | ⊥}) ⊥.
    unfold idfun.
    apply t_abs; ccrush.
    T_VAR.
  Qed.
  ...
  Example idfun_nt :
    ...
    apply t_abs; ccrush.
    2 : { eapply t_sub with (d1:= $$4). eapply t_var_set; ccrush.
          3 : { simpl. intuition. admit. (* contradiction: {0,1,4} ⊈ {3,4} *)  }
  Abort.
  ...
  Section Church.
    ...
    Example t_pair : forall {X q},
      ...
      apply t_app with (T1:=B); crush.
      erewrite qqcap_overlap_fresh; eauto; try lia. (* FIXME: automate *)
      * replace (q ⊔ $$ (S (length Γ)))
        with (openq $$(S (S (S (S (S (length Γ)))))) $$(S (length Γ)) (q ⊔ ##1)) at 1; try solve [opening; ccrush].
    ...
    (** However, abstraction with function self-qualifiers makes them applicable, e.g.,
        we can assign the following alternative type: *)
    (** Γ|Σ ⊢ᵠ λa.λb.a : ((a:A^{}) -> (f(b:B^{}) -> A^{f})^{a})^{} *)
    Example church_true_ht_alt : has_type Γ φ Σ church_true ({A | ∅} ==> {{B | ∅} ==> {A | ##0 } | ##1 }) ∅.
      intros. unfold church_true.
      apply t_abs; ccrush.
      eapply t_abs; ccrush.
      eapply t_self; ccrush. (* abstract over the escaping A argument! *)
      apply t_sub with (T1:=A) (d1:=$$(S (length Γ))); ccrush.
      * T_VAR.
      * apply stp_refl; ccrush. constructor; ccrush.
    Qed.

    (* Using this type, church_true can now be applied to arguments. *)
    (** Γ|Σ ⊢ᵠ (λx.λy.x) a : (f(b:B^{}) -> A^{f})^{} *)
    Example church_true_use_good1 : has_type Γ φ Σ (tapp church_true a) ({B | ∅} ==> {A | ##0 }) ∅.
      replace (∅) with (openq ∅ ∅ ##1) at 2; try solve [opening; crush].
      eapply t_app; ccrush.
      * eapply church_true_ht_alt.
      * apply a_ht.
    Qed.

    (** Γ|Σ ⊢ᵠ (λx.λy.x) a b : A^{} *)
    Example church_true_use_good2 : has_type Γ φ Σ (tapp (tapp church_true a) b) A ∅.
      replace (∅) with (openq ∅ ∅ ##0); try solve [opening; ccrush].
      eapply t_app; ccrush.
      * eapply church_true_use_good1.
      * apply b_ht.
    Qed.
...
Section DerivedSyntax.
  ...
  Opaque qqplus qqcap qglb qlub subqual.
  ...
  Lemma t_seq : forall Γ φ Σ t1 T1 q1 t2 T2 q2,
    ...
    + replace (⊥) with (openq ⊥ ⊥ ⊥); try solve [opening; ccrush].
      apply t_app with (T1:=T2); crush.
      replace (⊥) with (openq ⊥ q1 ⊥) at 4; try solve [opening; ccrush].
      eapply t_app with (T1:=T1); ccrush.
      apply t_abs; ccrush.
      apply t_abs; ccrush.
      Transparent qglb qlub subqual qqcap.
      cleanup. T_VAR.
    + Opaque qglb qlub subqual qqcap.
      remember (qset t0 t3 t4) as q2.
      assert (Hphi: ∅ ⊑ φ).
      { apply has_type_filter in H0. subst. destruct φ. inversion H0. apply empty_smallest_set'. }
      specialize (has_type_closed H0). intuition. rewrite Heqq2 in H7. inversion H7.
      Transparent qlub.
      replace q2 with (openq ∅ q2 ##1); try solve [opening; ccrush].
      apply t_app with (T1:=T2); eauto; ccrush.
      * replace (∅) with (openq ∅ q1 ∅) at 2; try solve [opening; ccrush].
        eapply t_app with (T1:=T1); ccrush.
        - apply t_abs; ccrush.
          apply t_abs; ccrush.
          Transparent qglb qlub subqual qqcap.
  ...
  Lemma t_val : forall Γ φ df Σ t1 T1 q1 t2 T2 q2,
    ...
    Transparent qlub. Transparent subqual. ccrush.
    apply has_type_filter in H8.
    apply @subqual_qlub with (d:=$$(S (length Γ))) in H4.
    specialize (subqual_trans H8 H4) as Hf. eapply unopen_subqual; eauto.
    destruct φ. inversion H5. intuition. discriminate.
  Qed.
...
Section FunAbs.
  ...
  Section IntroEx.
    ...
    (** val c = ...                   // : Ref[Int] ^ {c...}
        def deref(x : Unit) = { ! c } // : (Unit => Unit) ^ {c...}   *)
    Example deref := (tabs (! $0)).
    Example deref_ty : has_type [(false,TRef TUnit, q)] φ Σ deref ({TUnit | ∅} ==> {TUnit | ⊥}) (q ⊕ c).
      intros. unfold deref. destruct q. inversion q_is_set. subst c.
      apply t_abs; ccrush.
    ...
    (** deref(unit) // : Unit *)
    Example deref_app : has_type [(false,TRef TUnit, q)] φ Σ (tapp deref tunit) TUnit ⊥.
      intros. specialize deref_ty as deref_T. destruct q. inversion q_is_set. subst c.
      replace (⊥) with (openq ((qset t0 t1 t2) ⊕ 0) ⊥ ⊥); try solve [opening; auto].
      eapply t_app; crush.
      apply deref_T.
    Qed.
  ...
  Section FunArgs.
      Context (phiwf: closed_qual 0 2 (length Σ) φ)
              ...
              (c1visible : $$c1 ⊑ φ)
              (c2visible : $$c2 ⊑ φ)
              (Γ12 := [(false,TRef TUnit, ∅);(false,TRef TUnit, ∅)]).
...
Section Escaping.
  ...
  Context (phiwf : closed_qual 0 (length Γ) (length Σ) φ) (phi_set : ∅ ⊑ φ).
  ...
  Example fresh_ty_external : has_type Γ φ Σ (val tunit (tabs (tref tunit))) ({TUnit | ⊥} ==> {TRef TUnit | ∅}) ∅.
    intros. replace (∅) with (openq ∅ ⊥ ∅) at 2; try solve [opening;crush].
    eapply t_val; ccrush.
    apply t_abs; ccrush.
    apply t_ref; ccrush.
    apply t_base; ccrush.
  ...
  (** The type of the closure in
      ...
      at the point of its occurrence is
           ...
           (Unit => Unit)^{y}.
   *)
  Example closure_deref_internal : has_type (((false,TRef TUnit),∅) :: Γ) (φ ⊔ $$y) Σ (tabs !($y)) ({TUnit | ⊥} ==> {TUnit | ⊥}) $$y.
    intros.
    apply t_abs; ccrush.
    apply t_deref with ($$y); ccrush.
    subst y. T_VAR.
  Qed.
  ...
  (** The type of the closure in
      ...
      at the point of its occurrence is
           ...
           (Unit => Ref[Unit]^{y})^{y}.
   *)
  Example closure_ret_internal : has_type (((false,TRef TUnit),∅) :: Γ) (φ ⊔ $$y) Σ (tabs $y) ({TUnit | ⊥} ==> {TRef TUnit | $$y}) $$y.
    apply t_abs; ccrush.
    subst y. T_VAR.
  Qed.
  ...
  (** In order to assign a type to the escaping closure
      ...
      we need abstraction by function self qualifiers (cf. SelfRef section at the beginning of this file).
  *)
...
Section HigherOrder.
  ...
  Section NonInterference.
     ...
     *)
    Example t_par : forall {Γ Σ φ}, closed_qual 0 (length Γ) (length Σ) φ -> ∅ ⊑ φ ->
      ...
      * replace (⊥) with (openq $$(S (length Γ)) ⊥ ⊥) at 8; try solve [opening;crush].
        eapply t_app; ccrush; intuition.
        - T_VAR.
        - apply t_base; ccrush.
      * replace (⊥) with (openq $$(S (S (S (length Γ)))) ⊥ ⊥) at 8; try solve [opening;crush].
        eapply t_app; ccrush; intuition.
        - T_VAR.
        - apply t_base; ccrush.
    Qed.
    ...
    *)
    ...
    Example noninterfering_ty : has_type Γ φ Σ noninterfering TUnit ⊥.
      ...
      - apply t_ref; ccrush.
        apply t_base; ccrush.
      - replace (⊥) with (openq $$(S (length Γ)) $$(S (S (S (length Γ)))) ⊥) at 3; try solve [opening;crush].
        eapply t_app with (T1:=({TUnit | ∅} ==> {TUnit | ⊥})); ccrush.
        + replace ($$(S (length Γ))) with (openq ∅ $$(S (length Γ)) ##1); try solve [opening;crush].
          ...
          ** apply t_abs; ccrush. eapply t_deref; T_VAR.
        + apply t_abs; ccrush. eapply t_deref; T_VAR.
    Qed.

    (**
        Bad use of par:
        val c1 = alloc; val c2 = alloc; par { !c1; !c2 } { !c1; !c2 }
    *)
    ...
    Example aux1 : has_type [(false,TRef TUnit, ∅) ; (false,TRef TUnit, ∅)] ($$0 ⊔ $$1) [] (tabs (seq !($0) !($1))) ({TUnit | ∅} ==> {TUnit | ⊥}) ($$0 ⊔ $$1).
      apply t_abs; ccrush. eapply t_seq with (q1:=⊥); ccrush.
      * eapply t_deref. T_VAR.
      * eapply t_deref. T_VAR.
    Qed.

    (** now, show that interfering can't be typed *)
    Example interfering_bad : exists S q, has_type [(false,TRef TUnit, ∅) ; (false,TRef TUnit, ∅)] ($$0 ⊔ $$1) []
  ...
  Section NonEscape.
    ...
    Example try_ty : forall A Γ φ Σ,
        closed_qual 0 (length Γ) (length Σ) φ -> ∅ ⊑ φ ->
        closed_ty 0 (length Γ) (length Σ) A ->
    ...
    Example nonescaping_ok : has_type Γ $$c1 Σ (tapp try_ nonescaping) TUnit ∅.
      ...
      * subst Γ. apply t_abs; ccrush.
        ...
        + apply t_sub with (T1:=Nothing) (d1:=⊥); ccrush.
          replace (⊥) with (openq $$2 ⊥ ⊥); try solve [opening; auto].
          eapply t_app with (T1:=TUnit); ccrush.
          - subst CanThrow. T_VAR.
          - apply t_base; ccrush.
    Qed.
    ...
    Example escaping_ty : has_type Γ $$c1 Σ escaping ({CanThrow | ∅} ==> {{TUnit | ∅} ==> {TUnit | ⊥} | ##1}) $$c1.
      ...
      * apply t_abs; ccrush.
        apply t_sub with (T1:=Nothing) (d1:=⊥); ccrush.
          replace (⊥) with (openq $$2 ⊥ ⊥) at 3; try solve [opening; auto].
          eapply t_app with (T1:=TUnit); ccrush.
          - subst CanThrow. T_VAR.
          - apply t_base; ccrush.
    Qed.
  ...
  Section NonAccess.
    ...
    Context (canIO := 0)
            (c1 := 1)
            (Γ := [(false,TRef TUnit, ∅); (false,CanIO, ∅)])
            (φ := $$canIO ⊔ $$c1)
            (Σ : senv).
    ...
    Example withoutIO_ok_ty : has_type Γ φ Σ withoutIO_ok TUnit ∅.
      ...
      * replace ($$0) with (openq ∅ $$0 ##1); try solve [opening; ccrush].
        apply t_app with (T1:=CanIO); ccrush.
        + apply t_withoutIO; ccrush.
        + T_VAR.
      * apply t_abs; ccrush.
        apply t_sub with (T1:=TUnit) (d1:=⊥); ccrush.
        eapply t_deref. T_VAR.
    Qed.
    ...
    Example withoutIO_bad_block_ty : has_type [(false,TRef TUnit, ∅); (false,CanIO, ∅)] ($$0 ⊔ $$1) Σ (tabs (seq !($1) (tapp doIO $0))) ({TUnit | ∅} ==> {TUnit | ∅}) ($$0 ⊔ $$1).
      apply t_abs; ccrush. apply t_seq with (T1:=TUnit) (q1:=∅); ccrush.
      * apply t_sub with (T1:=TUnit) (d1:=⊥); ccrush. eapply t_deref. T_VAR.
    ...
    Example withoutIO_bad_ty : exists S q, has_type Γ φ Σ withoutIO_bad S q.
      ...
      2: T_VAR. 6: apply withoutIO_bad_block_ty.
...
Section LightweightPoly.
  ...
  Context (a := 0) (b := 1) (c1 := 2) (c2 := 3)
          (Γ := [(false,TRef TUnit, ($$a ⊔ $$b ⊔ $$c1)) ;
                 (false,TRef TUnit, ($$a ⊔ $$b)) ;
                 (false,TRef TUnit, ∅) ;
                 (false,TRef TUnit, ∅)] )
          (φ := ($$a ⊔ $$b ⊔ $$c1 ⊔ $$c2)).
  Variable (Σ : senv).
  ...
  Example inc1 : has_type Γ φ Σ (tapp inclike $b) (TRef TUnit) $$b.
    intros. replace $$b with (openq ∅ $$b ##1); try solve [opening; ccrush].
    eapply t_app; ccrush.
