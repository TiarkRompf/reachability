...
Ltac bound_simpl := try set_simpl;
  match goal with
  | [ H : bound _ <= 0 |- _ ]           => apply bound_0_empty in H; rewrite H in *; try bound_simpl
...
Ltac closed_qual_solve :=
  ...
  | [ |- closed_qual _ _ _ _ ]                                            => try solve [closed_qual_invert; simpl; constructor; bound_simpl; lia]
  end.
...
Lemma open_qual_empty_id : forall k q, [[ k ~> q]]ᵈ ∅ = ∅.
  intros. destruct q; compute. rewrite remove_empty. auto.
  rewrite NatSetFacts.empty_b. auto.
Qed.

Lemma open_qual_qbot_id : forall k q, [[ k ~> q]]ᵈ qbot = qbot.
  intros. destruct q; compute; auto.
Qed.
...
Lemma unopen_subqual : forall {x q2 df l}, closed_qual 2 x l q2 -> closed_qual 0 x l df -> df <> qbot ->
  openq (just_fv x) (just_fv (S x)) q2 ⊑ df ⊔ (just_fv (S x)) -> openq ∅ ∅ q2 ⊑ df.
  intros. unfold openq in *. destruct q2; destruct df; simpl in *; auto. contradiction.
  inversion H. subst. inversion H0. subst.
  assert (H9' : bound t0 <= S x). lia.
  assert (H12' : bound t3 <= S x). lia. clear H9. clear H12. rename H9' into H9. rename H12' into H12.
  apply bound_le_mem_false in H9, H12, H13.
  destruct (NatSet.F.mem 0 t1) eqn:Hmem; simpl in *; intuition.
  destruct (mem 1 (union (remove 0 t1) {}N)) eqn:Hmem'; rewrite <- NatSetFacts.not_mem_iff in H9, H12, H13.
  * rewrite <- NatSetFacts.mem_iff in Hmem'. repeat rewrite empty_union_right in *.
    repeat rewrite empty_union_right in H2. simpl in *; intuition.
    intros z Hz. assert (Hz' : In z (union (union t0 (singleton x)) (singleton (S x)))). fnsetdec.
    apply H3 in Hz'. apply union_1 in Hz'. intuition. apply NatSetFacts.singleton_iff in H4. subst.
    contradiction.
  * simpl in H2. intuition. repeat rewrite empty_union_right in *.
    rewrite empty_union_right in H2. simpl. intuition; fnsetdec.
  * destruct (mem 1 t1) eqn:Hmem1; simpl in *.
    ** repeat rewrite empty_union_right in *. intuition; try fnsetdec.
       ...
       apply H3 in Hz'. apply union_1 in Hz'. intuition. apply NatSetFacts.singleton_iff in H4. subst.
       rewrite <- NatSetFacts.not_mem_iff in H9. contradiction.
    ** intuition; try fnsetdec. rewrite <- NatSetFacts.not_mem_iff in H12, H9.
...
Lemma saturated_qbot : forall {Γ Σ}, saturated Γ Σ qbot.
  intros. unfold saturated. intros y Hmem. simpl in Hmem. contradiction.
Qed.

Lemma saturated_singleton : forall {x Γ Σ b T}, indexr x Γ = Some (b, T, ∅) -> saturated Γ Σ (qset (singleton x) {}N {}N).
  intros. fold (just_fv x). unfold saturated. intros y Hmem. rewrite just_fv_mem_iff in Hmem. subst.
  econstructor; eauto. apply empty_smallest_set. compute. discriminate.
Qed.

Lemma saturated_singleton' : forall {x Γ Σ b T}, indexr x Γ = Some (b, T, qbot) -> saturated Γ Σ (qset (singleton x) {}N {}N).
  intros. fold (just_fv x). unfold saturated. intros y Hmem. rewrite just_fv_mem_iff in Hmem. subst.
  econstructor; eauto.
Qed.

Lemma qqcap_overlap_fresh : forall f l q x y, closed_qual 0 f l q -> f <= x -> f <= y -> x <> y -> (q ⊔ (just_fv x)) ⋒ (just_fv y) = ∅.
  intros. destruct q. qdec. compute. f_equal; try fnsetdec.
  apply NatSet.eq_if_Equal. rewrite NatSetProperties.union_inter_1.
  assert (inter (singleton x) (singleton y) = {}N). fnsetdec.
  rewrite H3. rewrite empty_union_right. inversion H. subst.
  erewrite bound_disjoint; eauto. fnsetdec.
Qed.
...
Lemma open_qual_qglb_dist : forall {k d1 d2 d3}, d2 ⊓ d1 ⊑ d2 ⊓ d3  -> d3 ⊓ d1 ⊑ d2 ⊓ d3 -> ([[ k ~> d1 ]]ᵈ (d2 ⊓ d3)) = (([[ k ~> d1 ]]ᵈ d2) ⊓ ([[ k ~> d1 ]]ᵈ d3)).
  intros. destruct d2; destruct d3; destruct d1; simpl; auto; intuition; try qdec.
  destruct (mem k t1) eqn:Hmem; destruct (mem k t4) eqn:Hmem'.
  assert (HI : mem k (inter t1 t4) = true).
  3-5 : assert (HI : mem k (inter t1 t4) = false).
  1,3,5,7 : rewrite NatSetFacts.inter_b; rewrite Hmem; rewrite Hmem'; auto.
  all : rewrite HI; simpl; try qdec.
  all : repeat match goal with
  | [ H : mem _ _ = true |- _ ] => apply mem_2 in H
  | [ H : mem _ _ = false |- _ ] => rewrite <- NatSetFacts.not_mem_iff in H
  end.
  all : simpl in *; intuition. all : f_equal; try fnsetdec.
Qed.

Lemma open_qual_qqcap_dist : forall {k d1 d2 d3}, d2 ⊓ d1 ⊑ d2 ⊓ d3 -> d3 ⊓ d1 ⊑ d2 ⊓ d3 -> ([[ k ~> d1  ]]ᵈ (d2 ⋒ d3)) = (([[ k ~> d1  ]]ᵈ d2) ⋒ ([[ k ~> d1  ]]ᵈ d3)).
  intros. destruct d2; destruct d3. 1-3: repeat rewrite open_qual_qbot_id; auto; destruct d1; compute; try qdec.
  destruct d1; compute; try qdec. destruct (mem k t1) eqn:Hmem; destruct (mem k t4) eqn:Hmem'.
  assert (HI : mem k (inter t1 t4) = true).
  3-5 : assert (HI : mem k (inter t1 t4) = false).
  1,3,5,7 : rewrite NatSetFacts.inter_b; rewrite Hmem; rewrite Hmem'; auto.
  all : rewrite HI; simpl; try qdec.
...
Ltac opening :=
  ...
  | [ |- context[open_qual _ _ (_ ⋒ _)] ]                         => rewrite open_qual_qqcap_dist; try opening
  | [ |- context[open_qual _ _ (_ ⊓ _)] ]                         => rewrite open_qual_qglb_dist; try opening
  ...
  | [ |- context[open_qual ?j ?d qbot] ]                          => try rewrite (@open_qual_qbot_id j d); try lia; try opening
...
Ltac subqual :=
  ...
  | [ H : has_type _ ?φ _ _ _ ?q |- ?q ⊑ ?φ ] => try solve [eapply has_type_filter; eauto]
...
Ltac saturated_solve :=
  match goal with
  | [ |- saturated _ _ (just_loc _) ]                      => apply saturated_just_loc
  | [ |- saturated _ _ ∅ ]                                 => apply saturated_empty
  | [ |- saturated _ _ ⊥ ]                                 => apply saturated_qbot
  | [ |- saturated [] _ _ ]                                => try solve [apply saturated_empty_tenv; closed_qual_solve]
  ...
  | [ |- saturated _ _ (qset (singleton _) {}N {}N) ]      => try solve [eapply saturated_singleton; lookup | eapply saturated_singleton'; lookup]
  | [ |- saturated _ _ (just_fv _) ]                       => try solve [unfold just_fv; saturated_solve]
  ...
  | [ |- saturated _ _ (openq _ _ _)]                      => try solve [apply saturated_openq; try closed_qual_solve; saturated_solve]
  | [ |- saturated _ _ ([[_ ~> _ ]]ᵈ ([[_ ~> _ ]]ᵈ ∅))]    => try solve [opening; apply saturated_empty]
  | [ |- saturated _ _ ([[_ ~> _ ]]ᵈ _)]                   => try solve [apply saturated_open_qual; saturated_solve]
  end.
...
Ltac notbot :=
  match goal with
  | [ |- ?q <> qbot ] => try solve [simpl; intuition; discriminate]
  | [ |- qbot <> ?q ] => try solve [simpl; intuition; discriminate]
  end.

Ltac cleanup := repeat (try unfold qqcap; try set_simpl; try opening; try set_bool_simpl; simpl).
...
(* Specialize t_var to the two cases *)
Lemma t_var_set : forall Γ φ Σ x b T d, indexr x Γ = Some (b,T,d) -> d <> ⊥ -> (just_fv x) ⊑ φ ->
  ...
  intros. replace $$x with ((d ⊓ ∅) ⊕ x).
  eapply t_var; eauto. destruct d. contradiction. ccrush.
Qed.

Lemma t_var_bot : forall Γ φ Σ x b T, indexr x Γ = Some (b,T,⊥) -> (just_fv x) ⊑ φ ->
  closed_qual 0 (length Γ) (length Σ) φ ->
  closed_ty 0 x (length Σ) T ->
  has_type Γ φ Σ (tvar (varF x)) T ⊥.
  intros. replace (⊥) with ((⊥ ⊓ ∅) ⊕ x); auto.
  eapply t_var; eauto.
Qed.

Ltac T_VAR := try solve [eapply t_var_set; ccrush | eapply t_var_bot; ccrush].