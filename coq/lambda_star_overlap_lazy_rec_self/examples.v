...
(* Check (TFun ∅ ##1 (TRef TUnit) (TFun ∅ ##0 TUnit (TFun ∅ ##1 (TRef TUnit) (TFun ∅ ##0 TUnit (TRef TUnit))))). *)

Section WhatsNew.
  (* This version of λ* finally permits typing escaping closures using the new t_self rule. *)

  (* Assume an allocation primitive (not needed once we add the ⊥ qualifier to the system). *)
  Variable (alloc : tm).
  Context (t_alloc : forall Γ φ Σ, closed_qual 0 (length Γ) (length Σ) φ -> has_type Γ φ Σ alloc (TRef TUnit) ∅).

  (* { val y = alloc; { x => y } } *)
  Example escaping_closure := (tapp (tabs (tabs #3)) alloc).
...
Section Warmup.
  ...
  Example idfun_nt :
    ...
    apply t_abs; ccrush.
    2 : { eapply t_sub with (d1:= $$4). eapply t_var. ccrush.
  ...
  Section Church.
    ...
    Example church_true_use_good1 : has_type Γ φ Σ (tapp church_true a) ({B | ∅} ==> {A | ##0 }) ∅.
      replace (∅) with (openq ∅ ∅ ##1) at 2; try solve [opening; auto].
      eapply t_app; ccrush.
...
Lemma t_val : forall Γ φ df Σ t1 T1 q1 t2 T2 q2,
  ...
  apply has_type_filter in H7. eapply unopen_subqual; eauto. unfold_open in H7.
...
Section Escaping.
  ...
  Example fresh_ty_external : has_type Γ φ Σ (val tunit (tabs (tref tunit))) ({TUnit | ∅} ==> {TRef TUnit | ∅}) ∅.
    ...
    eapply t_val; ccrush.
    apply t_abs; ccrush.
    eapply t_ref. apply t_base. crush.
  Qed.
  ...
  (** The type of the closure in
      ...
      at the point of its occurrence is
           ...
           (Unit => Unit)^{y}.
   *)
  Example closure_deref_internal : has_type (((false,TRef TUnit),∅) :: Γ) (φ ⊔ (just_fv y)) Σ (tabs !($y)) ({TUnit | ∅} ==> {TUnit | ∅}) $$y.
    apply t_abs; ccrush.
    apply t_deref with ($$y); ccrush.
    subst y. eapply t_var; ccrush.
  Qed.
  ...
  (** In order to assign a type to the escaping closure
      ...
      we need abstraction by function self qualifiers (cf. WhatsNew section at the beginning of this file).
  *)
    ...
    *)
    ...
    Example noninterfering_ty : has_type Γ φ Σ noninterfering TUnit ∅.
      ...
      * replace (∅) with (openq $$(S (length Γ)) $$(S (S (S (length Γ)))) ∅) at 8; try solve [opening;crush].
        ...
        + apply t_abs; ccrush. eapply t_deref; eapply t_var; ccrush.
