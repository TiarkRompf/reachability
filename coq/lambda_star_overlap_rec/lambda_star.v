...
Lemma narrowing_gen : forall {t Γ1 U du Γ2 φ Σ T d},
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    + subst. rewrite indexr_insert in H. inversion H. subst.
      apply t_sub with (T1:=V) (d1:=(dv ⊕ length Γ2)); auto. constructor; auto.
      ...
      eapply weaken_stp'; eauto. eapply weaken_stp; eauto. rewrite app_length. simpl. lia.
...
Proof.
  ...
  - specialize (qstp_closed H5) as Hcl. intuition.
    ...
    constructor; eauto. apply saturated_empty_tenv. inversion H2. auto. }
...
Lemma substitution_gen :
  forall {t Γ φ' Tx dx dx' Σ T d}, dx' ⋒ φ' ≡ dx ->
    forall {φ}, φ ⊑ φ' ->
      has_type (Γ ++ [(Tx,dx)]) φ Σ t T d -> wf_tenv (Γ ++ [(Tx,dx)]) Σ -> Substq dx dx' ->
  ...
  - simpl. (bdestruct (x =? 0)).
    + (*x is 0 *) rewrite indexr_skips in H0; simpl; auto; try lia. simpl in H0. subst. simpl in H0.
      ...
      * (*subst fun, dx = dx' *)
         erewrite <- subst1_qual_plus; eauto. apply vtp_has_type in HTx.
         eapply weaken'; eauto. eapply subst_filter0; eauto.
         eapply closed_qual_subst1'; eauto.
      * (*subst arg, dx = df ⋒ dx = dx' ⋒ φ, requires different reasoning steps. *)
  ...
  - simpl. assert (HwfG' : wf_tenv ((T1, d1) :: (TFun d1 d2 T1 T2, df) :: Γ0 ++ [(Tx, dx)]) Σ). {
       repeat apply wf_tenv_cons; auto. apply saturated_cons; auto. }
  ...
  - intuition. rename H7 into IHHT2. rename H5 into IHHT1. simpl.
    ...
    eapply IHHT2; eauto.
    2,3 : unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx');
...
Lemma substitution : forall {t Tf df Tx dx Σ T d},
  ...
  simpl. apply wf_tenv_cons; auto. apply saturated_empty_tenv. apply closed_qual_qqcap; auto.
  ...
  apply closed_qual_qqcap; eauto.
  simpl. repeat apply wf_tenv_cons; auto. apply saturated_cons. apply saturated_empty_tenv. apply closed_qual_qqcap; auto.
Qed.
...
Proof.
  ...
  - (* tapp *) right. subst. intuition.
    ...
    + (* beta *)
      ...
      * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
        ...
        pose (VH' := H29). eapply t_abs with (φ:=df1) in VH'; eauto. apply has_type_vtp in VH'.
        assert (Hst : stp [] Σ T1 (df1 ⋒ d1) T1 (df ⋒ d1)). { apply stp_refl; auto. constructor. apply subqual_qglb_r.
          inversion H32. subst. auto. apply closed_qual_qqcap; auto. }
        assert (HT' : has_type [(T1, df1 ⋒ d1) ; (TFun d0 d3 T0 T2, df1)] (df1 ⊔ just_fv 0 ⊔ just_fv 1) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T2) (openq' ([]:tenv) d3)). {
          ...
          apply saturated_empty_tenv. apply stp_closed in H31. intuition.
        }
        ...
        eapply openq_mono; eauto. inversion H32. auto. apply openq_closed; auto.
        ...
        apply qlub_bound; auto. apply has_type_filter in H. inversion H32. subst.
        eapply subqual_trans; eauto. auto. apply saturated_empty_tenv.
        apply stp_closed in H31. intuition.
    +  (* (tabs t) t2 -> (tabs t) t2' *)
      apply has_type_vtp in H as VH; intuition.
      ...
      * specialize (has_type_closed H). specialize (has_type_closed H24). intuition. inversion H32. subst. rewrite <- openq_duplicate_eq.
        ...
        -- apply saturated_qqplus; auto. apply saturated_openq.
           apply saturated_empty_tenv. 3 : apply saturated_qqplus; auto.
           3,5 : apply saturated_empty_tenv. 5 : apply closed_qual_qqplus.
           1-4,5: eapply closed_qual_monotone; eauto; try lia.
           apply openq_closed; auto. eapply closed_qual_monotone; eauto.
    ...
    + (* t t2 -> t' t2 *)
      ...
      * rewrite <- openq_u_distribute3. eapply t_sub with (T1 := T)(d1 := (openq (df ⋓ just_loc l) d1 d2)).
        ...
        -- apply stp_refl. simpl. eapply closed_ty_monotone; eauto. lia.
           ...
           inversion H12; subst. apply openq_closed. 1,2 : apply closed_qual_qqplus.
           2,4 : apply just_loc_closed; auto. all : eapply closed_qual_monotone; eauto; lia.
        ...
        -- rewrite openq_u_distribute1. apply saturated_qqplus; auto. apply saturated_openq. apply saturated_empty_tenv.
           1,2 : apply closed_qual_qqplus. 1,3,6 : eapply closed_qual_monotone; eauto; lia.
           1,2 : apply just_loc_closed; auto. all :  eapply weaken_store_saturated; eauto.
  ...
  - (* tderef *) subst. intuition.  specialize (has_type_closed H) as HH. intuition. specialize (H3 (TRef T0) t0). intuition.
    + (* congruence *) right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
    ...
    + (* contraction *) right. intros. specialize (H8 σ H3).
  ...
  - (* subsumption *) subst. intuition. specialize (stp_closed H0) as H00. intuition.
    specialize (H6 T1 t0). intuition. right.
    intros. specialize (H11 σ H6). destruct H11 as [t' [σ' HH9]]. exists t'. exists σ'. intuition.
    destruct H13. apply (Preserve Σ' d'); intuition. eapply t_sub; eauto. apply stp_scale_qqplus.
    eapply weaken_stp_store_ext; eauto. inversion H15; subst; intuition; constructor; intuition; try rewrite bound_singleton; try rewrite bound_empty; try lia; auto.
    apply qqplus_bound. eapply subqual_trans; eauto. apply extends_ldom; auto. inversion H15; subst; auto.
    apply just_loc_ldom; auto. apply saturated_qqplus. eapply weaken_store_saturated; eauto.
