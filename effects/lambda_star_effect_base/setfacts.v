...
Lemma remove_0_empty: forall s,
  mem 0 s = true ->
  remove 0 s = empty ->
  max_elt s = Some 0.
Proof.
  intros. assert (In 0 s). apply mem_2; auto.   
  apply max_elt_iff; split; auto. intros.
  destruct m'; auto. 
  assert (In (S m') (remove 0 s)). apply remove_2; auto. rewrite H0 in H3. apply mem_1 in H3. rewrite mem_empty in H3. discriminate.
Qed.
