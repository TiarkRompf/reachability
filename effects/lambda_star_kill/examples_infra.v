...
Lemma has_type_open_tm_id : forall {Γ φ Σ t T d k t' e1 e2}, has_type Γ φ Σ t T d e1 e2 -> open_rec_tm k t' t = t.
  intros. apply has_type_closed in H. intuition. eapply closed_tm_open_id; eauto. lia.
Qed.

Lemma has_type_open_ty_id : forall {Γ φ Σ t T d k q e1 e2}, has_type Γ φ Σ t T d e1 e2 -> open_rec_ty k q T = T.
  intros. apply has_type_closed in H. intuition. eapply closed_ty_open_id; eauto. lia.
Qed.

Lemma has_type_open_qual_id1 : forall {Γ φ Σ t T d k q e1 e2}, has_type Γ φ Σ t T d e1 e2 -> open_qual k q φ = φ.
  intros. apply has_type_closed in H. intuition. eapply closed_qual_open_id; eauto. lia.
Qed.

Lemma has_type_open_qual_id2 : forall {Γ φ Σ t T d k q e1 e2}, has_type Γ φ Σ t T d e1 e2 -> open_qual k q d = d.
  intros. apply has_type_closed in H. intuition. eapply closed_qual_open_id; eauto. lia.
Qed.

Lemma stp_open_ty_id1 : forall {Γ Σ T d e1 e2 S q e3 e4 k q'}, stp Γ Σ S q e3 e4 T d e1 e2 -> open_rec_ty k q' S = S.
  intros. apply stp_closed in H. intuition. eapply closed_ty_open_id; eauto. lia.
Qed.

Lemma stp_open_ty_id2 : forall {Γ Σ T d e1 e2 S q e3 e4 k q'}, stp Γ Σ S q e3 e4 T d e1 e2 -> open_rec_ty k q' T = T.
  intros. apply stp_closed in H. intuition. eapply closed_ty_open_id; eauto. lia.
Qed.

Lemma stp_open_qual_id1 : forall {Γ Σ T d e1 e2 S q e3 e4 k q'}, stp Γ Σ S q e3 e4 T d e1 e2 -> open_qual k q' q = q.
  intros. apply stp_closed in H. intuition. eapply closed_qual_open_id; eauto. lia.
Qed.

Lemma stp_open_qual_id2 : forall {Γ Σ T d e1 e2 S q e3 e4 k q'}, stp Γ Σ S q e3 e4 T d e1 e2 -> open_qual k q' d  = d.
  intros. apply stp_closed in H. intuition. eapply closed_qual_open_id; eauto. lia.
Qed.
...
Lemma saturated_singleton : forall {x Γ Σ T}, indexr x Γ = Some (T, ∅) -> saturated Γ Σ (just_fv x).
  intros. unfold saturated. intros y Hmem. rewrite just_fv_mem_iff in Hmem. subst.
  econstructor; eauto.
Qed.
...
Lemma saturated_union : forall {Γ Σ s1 s2}, saturated Γ Σ (qset s1 {}N {}N) -> saturated Γ Σ (qset s2 {}N {}N) -> saturated Γ Σ (qset (union s1 s2) {}N {}N).
  intros. replace (qset (union s1 s2) {}N {}N) with ((qset s1 {}N {}N) ⊔ (qset s2 {}N {}N)); try qdec.
...
Ltac saturated_solve :=
  ...
  | [ |- saturated _ _ (just_fv _) ]                       => try solve [eapply saturated_singleton; lookup]
  | [ |- saturated _ _ (qset (singleton _) {}N {}N)]       => try solve [eapply saturated_singleton; lookup]
  ...
  | [ |- saturated _ _ (qset (union _ _) {}N {}N)]         => try solve [apply saturated_union; saturated_solve]
  | [ |- saturated _ _ (openq _ ∅)]                      => try solve [opening; apply saturated_empty]
  | [ |- saturated _ _ (openq ∅ ?q)]                     => try solve [opening;
    (match goal with
    | [ |- saturated _ _ (openq ∅ q)] => fail
    | [ |- saturated _ _ _ ] => saturated_solve
    end)]
  | [ |- saturated _ _ (openq _ _)]                        => try solve [apply saturated_openq; try closed_qual_solve; saturated_solve]
  | [ |- saturated _ _ ([[_ ~> _ ]]ᵈ ∅)]                 => try solve [opening; apply saturated_empty]
  | [ |- saturated _ _ ([[_ ~> ∅ ]]ᵈ ?q)]                => try solve [opening;
   (match goal with
    | [ |- saturated _ _ ([[_ ~> ∅ ]]ᵈ q)] => fail
    | [ |- saturated _ _ _ ] => saturated_solve
    end)]
