...
Definition eff_seq_ok (u1 k1 u2 k2 : eff): Prop := k1 ⊓ u2 = ∅.
#[global] Hint Unfold eff_seq_ok : core.

Inductive ty : Type :=
| TUnit : ty                             (* Unit base type*)
| TFun  : qual -> qual -> eff -> eff -> ty -> ty -> ty (* Dependent function type TFun q1 q2 qε T1 T2 ~> ((x: T1^q1) =>ε T2^q2),
...
| tfree   : tm  -> tm       (* Free construct (tfree t) ~> free t. *)
.
...
Fixpoint open_rec_tm (k : nat) (u : tm) (t : tm) {struct t} : tm :=
  ...
  | tfree   t       => tfree   (open_rec_tm k u t)
  end
.
...
Fixpoint open_rec_ty (k : nat) (d' : qual) (T : ty) : ty :=
  match T with
  | TUnit            => TUnit
  | TFun d1 d2 eu ek T1 T2 => TFun (open_qual k d' d1) (open_qual (S k) d' d2) (open_qual (S k) d' eu) (open_qual (S k) d' ek) (open_rec_ty k d' T1) (open_rec_ty (S k) d' T2)
...
Fixpoint ty_size (T : ty) : nat :=
  match T with
  | TUnit           => 0
  | TFun  _ _ _ _ T1 T2 => S (ty_size T1 + ty_size T2)
  | TRef  T         => S (ty_size T)
  end.
...
Fixpoint splice_ty (n : nat) (T : ty) {struct T} : ty :=
  match T with
  | TUnit            => TUnit
  | TFun d1 d2 eu ek T1 T2 => TFun (splice_qual n d1) (splice_qual n d2) (splice_qual n eu) (splice_qual n ek) (splice_ty n T1) (splice_ty n T2)
...
| cl_tfree:  forall b f l tm,
    closed_tm b f l tm ->
    closed_tm b f l (tfree tm)
.
...
| cl_TFun : forall b f l d1 d2 eu ek T1 T2,
    closed_qual b f l d1 ->
    closed_qual (S b) f l d2 ->
    closed_qual (S b) f l eu ->
    closed_qual (S b) f l ek ->
    closed_ty b f l T1 ->
    closed_ty (S b) f l T2 ->
    closed_ty b f l (TFun d1 d2 eu ek T1 T2)
.
...
Inductive stp : tenv -> senv -> ty -> qual -> eff -> eff -> ty -> qual -> eff -> eff -> Prop :=
| s_base : forall Γ Σ d1 d2 eu1 eu2 ek1 ek2,
    qstp Γ Σ d1 d2 ->
    qstp Γ Σ eu1 eu2 ->
    qstp Γ Σ ek1 ek2 ->
    stp Γ Σ TUnit d1 eu1 ek1 TUnit d2 eu2 ek2
| s_ref : forall Γ Σ T1 T2 d1 d2 eu1 eu2 ek1 ek2,
    qstp Γ Σ d1 d2 ->
    qstp Γ Σ eu1 eu2 ->
    qstp Γ Σ ek1 ek2 ->
    stp [] [] T1 ∅ ∅ ∅ T2 ∅ ∅ ∅ -> (* we don't have bottom, so use ∅ here *)
    stp [] [] T2 ∅ ∅ ∅ T1 ∅ ∅ ∅ ->
    stp Γ Σ (TRef T1) d1 eu1 ek1 (TRef T2) d2 eu2 ek2
| s_fun : forall Γ Σ T1 d1 T2 d2 T3 d3 T4 d4 d5 d6 eu1 eu2 ek1 ek2 eu3 ek3 eu4 ek4,
    closed_ty 0 (length Γ) (length Σ) (TFun d1 d2 eu3 ek3 T1 T2) ->
    closed_ty 0 (length Γ) (length Σ) (TFun d3 d4 eu4 ek4 T3 T4) ->
    qstp Γ Σ d5 d6 ->
    qstp Γ Σ eu1 eu2 ->
    qstp Γ Σ ek1 ek2 ->
    stp Γ Σ T3 d3 ∅ ∅ T1 d1 ∅ ∅ ->
    stp ((T3,d3) :: Γ) Σ (open_ty' Γ T2) (openq' Γ d2) (openq' Γ eu3) (openq' Γ ek3) (open_ty' Γ T4) (openq' Γ d4) (openq' Γ eu4) (openq' Γ ek4) ->
    stp Γ Σ (TFun d1 d2 eu3 ek3 T1 T2) d5 eu1 ek1 (TFun d3 d4 eu4 ek4 T3 T4) d6 eu2 ek2
| s_trans : forall Γ Σ T1 d1 T2 d2 T3 d3 eu1 ek1 eu2 ek2 eu3 ek3,
    stp Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 -> stp Γ Σ T2 d2 eu2 ek2 T3 d3 eu3 ek3 -> stp Γ Σ T1 d1 eu1 ek1 T3 d3 eu3 ek3
.
...
Inductive has_type : tenv -> qual -> senv -> tm -> ty -> qual -> eff -> eff -> Prop :=
| t_base : forall Γ Σ φ,
...
| t_abs: forall Γ φ Σ T1 d1 T2 d2 df t eu ek,
    closed_tm 1 (length Γ) (length Σ) t ->
    closed_ty 0 (length Γ) (length Σ) (TFun d1 d2 eu ek T1 T2) ->
    closed_qual 0 (length Γ) (length Σ) φ ->
    closed_qual 1 (length Γ) (length Σ) eu ->
    closed_qual 1 (length Γ) (length Σ) ek ->
    df ⊑ φ ->
    saturated Γ Σ d1 ->
    saturated Γ Σ df ->
    has_type ((T1,d1) :: Γ) (df ⊔ (just_fv (length Γ))) Σ (open_tm' Γ t) (open_ty' Γ T2) (openq' Γ d2) (openq' Γ eu) (openq' Γ ek) ->
    has_type Γ φ Σ (tabs t) (TFun d1 d2 eu ek T1 T2) df ∅ ∅

| t_app : forall Γ φ Σ t1 d1 t2 d2 df T1 T2 eu1 ek1 eu2 ek2 eu3 ek3,
    has_type Γ φ Σ t1 (TFun (df ⊓ d1) d2 eu3 ek3 T1 T2) df eu1 ek1 ->
    has_type Γ φ Σ t2 T1 d1 eu2 ek2 ->
    ...
    saturated Γ Σ (openq ∅ eu3) ->
    saturated Γ Σ (openq ∅ ek3) ->
    eff_seq_ok eu1 ek1 eu2 ek2 ->
    eff_seq_ok (eu1 ⊔ eu2) (ek1 ⊔ ek2) (openq d1 eu3) (openq d1 ek3) ->
    has_type Γ φ Σ (tapp t1 t2) T2 (openq d1 d2) (eu1 ⊔ eu2 ⊔ (openq d1 eu3)) (ek1 ⊔ ek2 ⊔ (openq d1 ek3))

| t_let : forall Γ φ Σ df t1 T1 d1 t2 T2 d2 eu1 ek1 eu2 ek2,
    closed_tm 1 (length Γ) (length Σ) t2 ->
    closed_ty 0 (length Γ) (length Σ) T2 ->
    closed_qual 1 (length Γ) (length Σ) d2 ->
    closed_qual 1 (length Γ) (length Σ) eu2 ->
    closed_qual 1 (length Γ) (length Σ) ek2 ->
    (openq ∅ d2) ⊑ φ ->
    (openq ∅ eu2) ⊑ φ ->
    (openq ∅ ek2) ⊑ φ ->
    d2 ⊓ ek2 ⊑ ∅ -> (* legal return *)
    saturated Γ Σ df ->
    saturated Γ Σ (openq ∅ d2) ->
    saturated Γ Σ (openq ∅ eu2) ->
    saturated Γ Σ (openq ∅ ek2) ->
    df ⊑ φ ->
    eff_seq_ok eu1 ek1 (openq d1 eu2) (openq d1 ek2)  ->
    has_type Γ φ Σ t1 T1 d1 eu1 ek1 ->
    has_type ((T1,d1 ⊓ df) :: Γ) (df ⊔ (just_fv (length Γ))) Σ (open_tm' Γ t2) T2 (openq' Γ d2) (openq' Γ eu2) (openq' Γ ek2) ->
    has_type Γ φ Σ (tlet t1 t2) T2 (openq d1 d2) (eu1 ⊔ (openq d1 eu2)) (ek1 ⊔ (openq d1 ek2))
...
| t_ref: forall Γ φ Σ t d eu ek,
    has_type Γ φ Σ t TUnit d eu ek ->
    has_type Γ φ Σ (tref t) (TRef TUnit) ∅ eu ek

| t_deref: forall Γ φ Σ T d t eu ek,
    eff_seq_ok eu ek d ∅ ->
    has_type Γ φ Σ t (TRef T) d eu ek ->
    has_type Γ φ Σ (tderef t) T ∅ (eu ⊔ d) ek

| t_assign: forall Γ φ Σ t1 t2 d1 d2 eu1 ek1 eu2 ek2,
    has_type Γ φ Σ t1 (TRef TUnit) d1 eu1 ek1 ->
    has_type Γ φ Σ t2 TUnit d2 eu2 ek2  ->
    eff_seq_ok eu1 ek1 eu2 ek2 ->
    eff_seq_ok (eu1 ⊔ eu2) (ek1 ⊔ ek2) d1 ∅ ->
    has_type Γ φ Σ (tassign t1 t2) TUnit ∅ (eu1 ⊔ eu2 ⊔ d1) (ek1 ⊔ ek2)

| t_free: forall Γ φ Σ t d eu ek,
    has_type Γ φ Σ t (TRef TUnit) d eu ek ->
    (* this is implied since u2 is empty *)
    (* eff_seq_ok ε (∅, d) -> *)
    (* killing d from ε *)
    has_type Γ φ Σ (tfree t) (TUnit) ∅ eu (ek ⊔ d)

| t_sub: forall Γ φ Σ e T1 d1 T2 d2 eu1 ek1 eu2 ek2,
    has_type Γ φ Σ e T1 d1 eu1 ek1->
    stp Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 ->
    d2 ⊑ φ ->
    eu2 ⊑ φ ->
    ek2 ⊑ φ ->
    saturated Γ Σ d2 ->
    saturated Γ Σ eu2->
    saturated Γ Σ ek2 ->
    has_type Γ φ Σ e T2 d2 eu2 ek2
.
...
| step_ref : forall v σ,
    value v ->
    step (tref v) σ (tloc (length σ)) ((Some v) :: σ)
| step_deref : forall σ l v,
    (* ensures there is a value. E.g. we don't dereference a killed store location. *)
    indexr l σ = Some (Some v) ->
    step (tderef (tloc l)) σ v σ
| step_assign : forall σ l v,
    l < (length σ) ->
    value v ->
    step (tassign (tloc l) v) σ tunit (update σ l (Some v))
| step_free : forall σ l,
    (* allow double free operationally *)
    (* indexr l σ = Some (Some v) -> *)
    step (tfree (tloc l)) σ tunit (update σ l (None))
(*congruence rules*)
...
| step_c_free : forall t t' σ σ',
    step t σ t' σ' ->
    step (tfree t) σ (tfree t') σ'
...
Definition CtxOK (Γ : tenv) (φ : qual) (Σ : senv) (σ : store) (kill : qual) : Prop :=
				  length Σ = length σ /\
          closed_qual 0 0 (length Σ) kill /\
				  forall l T, indexr l Σ = Some T ->
				  ((l ∈ₗ kill -> indexr l σ = Some None) /\
          (~(l ∈ₗ kill) -> exists v, indexr l σ = Some (Some v) /\ value v /\ has_type Γ φ Σ v T ∅ ∅ ∅)).
...
Fixpoint subst_ty (T : ty) (v : nat) (q : qual) : ty :=
  match T with
  | TUnit            => TUnit
  | TFun q1 q2 eu ek T1 T2 => TFun (subst_q q1 v q) (subst_q q2 v q) (subst_q eu v q) (subst_q ek v q) (subst_ty T1 v q) (subst_ty T2 v q)
...
Definition removeq (q k : qual) : qual :=
  match q,k with
  | qset bq fq lq, qset bk fk lk =>
      qset (NatSet.F.diff bq bk) (NatSet.F.diff fq fk) (NatSet.F.diff lq lk)
  end.
...
| vtp_abs: forall Σ T1 d1 T2 d2 T3 d3 T4 d4 df1 df2 t eu1 ek1 eu2 ek2,
    closed_tm 1 0 (length Σ) t ->
    closed_qual 0 0 (length Σ) df2 ->
    closed_ty 0 0 (length Σ) (TFun d1 d2 eu1 ek1 T1 T2) ->
    closed_ty 0 0 (length Σ) (TFun d3 d4 eu2 ek2 T3 T4) ->
    has_type [(T1,d1)] (df1 ⊔ (just_fv 0)) Σ (open_tm' ([]: tenv) t) (open_ty' ([]: tenv) T2) (openq' ([]: tenv) d2) (openq' ([]: tenv) eu1) (openq' ([]: tenv) ek1) ->
    qstp [] Σ df1 df2 ->
    stp [] Σ T3 d3 ∅ ∅ T1 d1 ∅ ∅ ->
    stp [(T3,d3)] Σ (open_ty' ([]: tenv) T2) (openq' ([]: tenv) d2) (openq' ([]: tenv) eu1) (openq' ([]: tenv) ek1) (open_ty' ([]: tenv) T4) (openq' ([]: tenv) d4) (openq' ([]: tenv) eu2) (openq' ([]: tenv) ek2) ->
  vtp Σ (tabs t) (TFun d3 d4 eu2 ek2 T3 T4) df2
.
...
(* kk is the newly killed location during stepping *)
(* t', the term we step into, must not have kk in its used effect *)
(* ε' is ε with kk removed from its used set, but still large enough to type the term after stepping *)
Inductive preserve (Γ : tenv) (Σ : senv) (t' : tm) (T : ty) (d : qual) (eu ek : eff) (σ' : store) (k : qual): Prop :=
  | Preserve : forall Σ' d' eu'' ek'' k' eu' ek',
      eu' ⊑ eu ->
      ek' ⊑ ek ->
      Σ' ⊇ Σ ->
      (* ε'' in paper = (εu'', εk'') *)
      ...
      disjointq Σ Σ' eu'' ->
      disjointq Σ Σ' ek'' ->
      (* the kill set must be specified in the previous effect *)
      k' ⊑ ek ⊔ && (length Σ) ->
      CtxOK Γ (ldom Σ') Σ' σ' (k ⊔ k') ->
      (* does not use killed location in the future *)
      (* (fst ε') ⊓ k' ⊑ ∅ -> *)
      (* an alternative expression, it gives us (k ⊔ k') ⊓ (fst ε' ⊔ εk'') = ∅
        if we kill fresh, then it shouldn't appear in the future effect, so it's fine
        if we not kill fresh, then fresh doesn't appear in k'
      *)
      eff_seq_ok ∅ (k ⊔ k') (eu' ⊔ eu'') (ek' ⊔ ek'') ->
      (* cannot kill and grow the qualifier in the same step *)
      d' ⊓ k' ⊑ ∅ ->
      d' ⊓ ek'' ⊑ ∅ ->
      has_type Γ (ldom Σ') Σ' t' T (d ⊔ d') (eu' ⊔ eu'') (ek' ⊔ ek'') ->
      preserve Γ Σ t' T d eu ek σ' k.
Arguments Preserve {Γ Σ t' T d eu ek σ' k}.
...
| Separate : forall Σ' Σ'' q1' q2' eu1' eu2' ek1' ek2',
    Σ' ⊇ Σ ->
    Σ'' ⊇ Σ' ->
    has_type [] (ldom Σ') Σ' t1' T1 q1' eu1' ek1' ->
    has_type [] (ldom Σ'') Σ'' t2' T2 q2' eu2' ek2' ->
    q1' ⊓ q2' ⊑ ∅ ->
    eu1' ⊓ eu2' ⊑ ∅ ->
    ek1' ⊓ ek2' ⊑ ∅ ->
    ek1' ⊓ eu2' ⊑ ∅ ->
...
Lemma just_bv_closed : forall {x b f l}, x < b <-> closed_qual b f l (just_bv x).
Proof.
  split; intros.
  - unfold just_bv. constructor; unfold bound.
    rewrite max_elt_empty. lia.
    rewrite max_elt_singleton. lia.
    rewrite max_elt_empty. lia.
  - inversion H. subst.
    unfold bound in H7. rewrite max_elt_singleton in H7. lia.
Qed.
...
Lemma qstp_empty : forall {Γ Σ}, qstp Γ Σ ∅ ∅.
  intros. constructor; auto.
Qed.
#[global] Hint Resolve qstp_empty : core.
...
Lemma closed_ty_monotone : forall {T b l f}, closed_ty b f l T -> forall {b' f' l'}, b <= b' -> f <= f' -> l <= l' -> closed_ty b' f' l' T.
  ...
  1-3: eapply closed_qual_monotone; eauto; lia. eapply IHclosed_ty2; eauto. lia.
Qed.
...
Lemma closed_ty_open_id : forall {T b f l}, closed_ty b f l T -> forall {n}, b <= n -> forall {x}, (open_rec_ty n x T) = T.
  ...
  erewrite closed_qual_open_id; eauto. erewrite closed_qual_open_id; eauto.  erewrite closed_qual_open_id; eauto.  erewrite closed_qual_open_id; eauto.
  all : lia.
Qed.
...
Lemma closed_ty_open : forall {T b f l}, closed_ty (S b) f l T -> forall {x}, x < f -> closed_ty b f l (open_rec_ty b (just_fv x) T).
  induction T; intros; simpl; intuition; inversion H; subst; try constructor;
    try solve [apply IHT1; auto | apply IHT2; auto | apply IHT; auto ].
  1-4 : eapply closed_qual_open; eauto. erewrite closed_ty_open_id; eauto. lia.
Qed.
...
Lemma closed_ty_open_ge : forall {T b f l}, closed_ty (S b) f l T -> forall {x}, f <= x -> closed_ty b (S x) l (open_rec_ty b (just_fv x) T).
  induction T; intros; simpl; intuition; inversion H; subst; try constructor;
    try solve [eapply IHT1; eauto | eapply IHT2; eauto | eapply IHT; eauto ].
  1-4:eapply closed_qual_open_ge; eauto. erewrite closed_ty_open_id; eauto. lia.
Qed.
...
Lemma closed_ty_open_succ : forall {T b f l}, closed_ty b f l T -> forall {j}, closed_ty b (S f) l (open_rec_ty j (just_fv f) T).
  induction T; intros; simpl; intuition; inversion H; subst; try constructor;
    try solve [eapply IHT1; eauto | eapply IHT2; eauto | eapply IHT; eauto ].
  1-4: eapply closed_qual_open_succ; eauto. erewrite closed_ty_open_id; eauto. lia.
Qed.
...
Lemma open_rec_ty_commute : forall T i j x y,
  ...
  f_equal; try  erewrite open_qual_commute; eauto.
Qed.
...
Lemma splice_ty_id : forall {T b f l}, closed_ty b f l T -> (splice_ty f T) = T.
  induction T; intros; inversion H; subst; simpl; auto;
    try solve [erewrite IHT1; eauto; erewrite IHT2; eauto | erewrite IHT; eauto].
  f_equal; try erewrite splice_qual_id; eauto.
...
Lemma splice_ty_open : forall {T j n m}, splice_ty n (open_rec_ty j (just_fv (m + n)) T) = open_rec_ty j (just_fv (S (m + n))) (splice_ty n T).
  induction T; intros; simpl; auto;
    try solve [erewrite IHT1; eauto; erewrite IHT2; eauto | erewrite IHT; eauto].
  f_equal; try rewrite splice_qual_open; eauto.
Qed.
...
Lemma splice_ty_closed : forall {T b n m l}, closed_ty b (n + m) l T -> forall {i}, i <= m -> closed_ty b (S (n + m)) l (splice_ty i T).
  induction T; simpl; intros; inversion H; subst; intuition.
  constructor. 1-4 : apply splice_qual_closed; auto. all: intuition.
...
Lemma splice_ty_closed'' : forall {T x b l k}, closed_ty b x l T -> k <= x -> closed_ty b (S x) l (splice_ty k T).
  ...
  1-4 : eapply splice_qual_closed''; eauto. erewrite splice_ty_id; eauto.
...
Lemma splice_ty_open_succ : forall {T b n l j}, closed_ty b n l T -> splice_ty n (open_rec_ty j (just_fv n) T) = open_rec_ty j (just_fv (S n)) T.
  induction T; simpl; intros; inversion H; subst; auto;
    try solve [erewrite IHT1; eauto; erewrite IHT2; eauto | erewrite IHT; eauto].
  f_equal; try  erewrite splice_qual_open_succ; eauto.
...
Lemma stp_closed : forall {Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2},
    stp Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 ->
    closed_ty 0 (length Γ) (length Σ) T1
    /\ closed_qual 0 (length Γ) (length Σ) d1
    /\ closed_qual 0 (length Γ) (length Σ) eu1
    /\ closed_qual 0 (length Γ) (length Σ) ek1
    /\ closed_ty 0 (length Γ) (length Σ) T2
    /\ closed_qual 0 (length Γ) (length Σ) d2
    /\ closed_qual 0 (length Γ) (length Σ) eu2
    /\ closed_qual 0 (length Γ) (length Σ) ek2.
Proof.  intros Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 HS. induction HS.
  - apply qstp_closed in H, H0, H1. intuition.
  - apply qstp_closed in H, H0, H1. intuition.
  - apply qstp_closed in H1, H2, H3. intuition.
  - intuition.
Qed.

Lemma stp_refl' : forall {n T}, ty_size T < n -> forall {Γ Σ}, closed_ty 0 (length Γ) (length Σ) T -> forall {d d'}, qstp Γ Σ d d' -> forall {eu eu'}, qstp Γ Σ eu eu' -> forall {ek ek'}, qstp Γ Σ ek ek'-> stp Γ Σ T d eu ek T d' eu' ek'.
  induction n; try lia; destruct T; simpl; intros Hsize Γ Σ Hc d d' Hstp eu eu' Hestpu ek ek' Hestpk; inversion Hc; subst.
  - (*TUnit*) constructor; auto.
  - (*TFun*) constructor; auto. apply IHn. lia. auto. 1-3: apply qstp_refl; try apply eqqual_refl; auto.
    ...
    all: apply qstp_refl; try apply eqqual_refl; unfold openq'; unfold openq;
    simpl; eapply closed_qual_open; try lia; eapply closed_qual_monotone; eauto.
  - (*TRef*) constructor; auto.
    all : apply IHn; try lia; auto. all: try apply qstp_refl; try apply eqqual_refl; auto.
Qed.

Lemma stp_refl : forall {T Γ Σ}, closed_ty 0 (length Γ) (length Σ) T -> forall {d d'}, qstp Γ Σ d d' -> forall {eu eu'}, qstp Γ Σ eu eu' -> forall {ek ek'}, qstp Γ Σ ek ek'-> stp Γ Σ T d eu ek T d' eu' ek'.
  intros. eapply stp_refl'; eauto.
Qed.

Lemma stp_qstp_inv : forall {Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> qstp Γ Σ d1 d2.
  intros Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 HS. induction HS; intuition.  eapply qstp_trans; eauto.
Qed.

Lemma stp_qstp_invu : forall {Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> qstp Γ Σ e1 e2.
  intros Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 HS. induction HS; intuition.  eapply qstp_trans; eauto.
Qed.

Lemma stp_qstp_invk : forall {Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> qstp Γ Σ ek1 ek2.
  intros Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 HS. induction HS; intuition.  eapply qstp_trans; eauto.
Qed.
...
Lemma weaken_stp_gen : forall {Γ1 Γ2 Σ T1 d1 e1 ek1 T2 d2 e2 ek2},  stp (Γ1 ++ Γ2) Σ T1 d1 e1 ek1 T2 d2 e2 ek2 ->
    forall T', stp ((splice_tenv (length Γ2) Γ1) ++ T' :: Γ2) Σ (splice_ty (length Γ2) T1) (splice_qual (length Γ2) d1) (splice_qual (length Γ2) e1) (splice_qual (length Γ2) ek1) (splice_ty (length Γ2) T2) (splice_qual (length Γ2) d2) (splice_qual (length Γ2) e2) (splice_qual (length Γ2) ek2).
Proof. intros Γ1 Γ2 Σ T1 d1 e1 ek1 T2 d2 e2 ek2 Hstp T'. remember (Γ1 ++ Γ2)  as Γ. generalize dependent Γ1.  induction Hstp. intros Γ1.
  - constructor. all :  eapply weaken_qstp_gen; subst; auto.
  - intros. assert (stp Γ Σ (TRef T1) d1 eu1 ek1 (TRef T2) d2 eu2 ek2) as Hstp. { constructor; intuition. } subst.
    apply stp_closed in Hstp as Hcl. intuition.
    inversion H2. inversion H6. subst.
    constructor.
    1-3: apply weaken_qstp_gen; subst; auto.
    1,2: fold splice_ty.
  ...
  - assert (stp Γ Σ (TFun d1 d2 eu3 ek3 T1 T2) d5 eu1 ek1 (TFun d3 d4 eu4 ek4 T3 T4) d6 eu2 ek2). { constructor; intuition. } intros.
    subst. intuition. inversion H0; inversion H; subst. apply qstp_closed in H1 as Hcl. intuition.
    constructor; try fold splice_ty. 3 : eapply weaken_qstp_gen; eauto.
    3,4: eapply weaken_qstp_gen; eauto.
    1-2: constructor.
    1-4,7-10: apply splice_qual_closed'. 9-12 : apply splice_ty_closed'.
    1-12: rewrite app_length in *; rewrite splice_tenv_length in *; auto.
    specialize (IHHstp1 Γ1). intuition. repeat rewrite splice_qual_empty in H7. auto.
    specialize (IHHstp2 ((T3, d3) :: Γ1)). intuition.
    rewrite <- splice_ty_open'. rewrite <- splice_ty_open'. rewrite <- splice_qual_open'. repeat rewrite <- splice_qual_open'.
    unfold open_ty' in *. unfold open_ty in *. unfold openq' in *. unfold openq in *. rewrite app_length in *. rewrite splice_tenv_length. simpl. auto.
  - intros. specialize (IHHstp1 Γ1). specialize (IHHstp2 Γ1). intuition.
    eapply s_trans; eauto.
Qed.

Lemma weaken_stp : forall {Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall T', stp (T' :: Γ) Σ T1 d1 e1 ek1 T2 d2 e2 ek2.
Proof. intros Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 HST. specialize (@weaken_stp_gen [] Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2) as Hsp. simpl in *.  specialize (Hsp HST).
       ...
       replace (splice_qual (length Γ) d1) with d1 in Hsp. replace (splice_qual (length Γ) d2) with d2 in Hsp.
       replace (splice_qual (length Γ) e1) with e1 in Hsp. replace (splice_qual (length Γ) e2) with e2 in Hsp.
       replace (splice_qual (length Γ) ek1) with ek1 in Hsp. replace (splice_qual (length Γ) ek2) with ek2 in Hsp.
       intuition.
       1-6 : erewrite splice_qual_id; eauto. 1,2 : erewrite splice_ty_id; eauto.
Qed.

Lemma weaken_stp' : forall {Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall Γ', stp (Γ' ++ Γ) Σ T1 d1 e1 ek1 T2 d2 e2 ek2.
  intros. induction Γ'.
  - simpl. auto.
...
Lemma weaken_stp_store : forall {Σ Γ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall Σ', stp Γ (Σ' ++ Σ) T1 d1 e1 ek1 T2 d2 e2 ek2.
Proof. intros Σ Γ T1 d1 e1 ek1 T2 d2 e2 ek2 HSTP. induction HSTP; intros.
...
Lemma weaken_stp_store_ext : forall {Σ Γ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall {Σ'}, Σ' ⊇ Σ ->  stp Γ Σ' T1 d1 e1 ek1 T2 d2 e2 ek2.
...
Lemma narrowing_qstp_gen : forall{Γ1 U du Γ2 Σ d1 d2},
    qstp (Γ1 ++ (U,du) :: Γ2) Σ d1 d2 -> forall {V dv e1 ek1 e2 ek2}, stp Γ2 Σ V dv e1 ek1 U du e2 ek2 -> qstp (Γ1 ++ (V,dv) :: Γ2) Σ d1 d2.
  intros Γ1 U du Γ2 Σ d1 d2 HQ V dv e1 e2 HSTP.
  inversion HQ. subst. constructor.
  auto. rewrite app_length in *. simpl in *. auto.
Qed.

Lemma narrowing_stp_gen : forall{Γ1 U du Γ2 Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp (Γ1 ++ (U,du) :: Γ2) Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall {V dv e3 ek3 e4 ek4}, (stp Γ2 Σ V dv e3 ek3 U du e4 ek4) -> stp (Γ1 ++ (V,dv) :: Γ2) Σ T1 d1 e1 ek1 T2 d2 e2 ek2.
Proof. intros Γ1 U du Γ2 Σ T1 d1 e1 ek1 T2 d2 e2 ek2 HST. remember (Γ1 ++ (U,du) :: Γ2) as Γ.
  generalize dependent Γ1; induction HST; intros; intuition.
  - subst. constructor. all: eapply narrowing_qstp_gen; eauto.
  - subst. constructor; auto. all: eapply narrowing_qstp_gen; eauto.
  - rewrite HeqΓ in *. constructor.
    subst. rewrite app_length in *. simpl in *. auto.
    subst. rewrite app_length in *. simpl in *. auto.
    1-3 : eapply narrowing_qstp_gen; subst; eauto.
    eapply IHHST1; eauto.
...
Lemma narrowing_stp : forall{U du Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp ((U,du) :: Γ) Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall {V dv e3 ek3 e4 ek4}, stp Γ Σ V dv e3 ek3 U du e4 ek4  -> stp ((V,dv) :: Γ) Σ T1 d1 e1 ek1 T2 d2 e2 ek2.
  intros. specialize (@narrowing_stp_gen [] U du Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2) as narrow. simpl in *. eapply narrow; eauto.
Qed.

Lemma stp_scale_plus : forall {Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2}, stp Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 -> forall {x}, x < length Γ -> stp Γ Σ T1 (d1 ⊕ x) e1 ek1 T2 (d2 ⊕ x) e2 ek2.
  intros Γ Σ T1 d1 e1 ek1 T2 d2 e2 ek2 HSTP. induction HSTP; intros.
  - constructor. inversion H. subst. constructor. apply subqual_plus. auto.
    destruct (length Γ) eqn:Heq. lia. eapply qplus_closed_qual; eauto. auto. auto.
...
Lemma saturated_openq : forall {Γ Σ d1 d2}, saturated Γ Σ d1 -> saturated Γ Σ (openq ∅ d2) -> saturated Γ Σ (openq d1 d2).
  intros. destruct d1. destruct d2. simpl in *.
  destruct (NatSet.F.mem 0 t3) eqn:Hmem; intuition.
  repeat rewrite empty_union_right in H0.
  unfold saturated in *. intros. specialize (H x). specialize (H0 x).
  assert (Hx : $ (x) ∈ᵥ qset t t0 t1 \/ $ (x) ∈ᵥ qset t2 (NatSet.F.remove 0 t3) t4). {
    simpl. simpl in H1. fnsetdec.
  }
  intuition.
  - inversion H3. subst. econstructor; eauto. eapply subqual_trans; eauto. simpl. intuition.
...
Proof.
  ...
  - pose proof @qmem_reflect (inl $ x) d1. rewrite Hin1 in H2. inversion H2. specialize (H x H3). destruct H. econstructor. eassumption. destruct q', d1, d2. simpl in *. unfold NatSet.F.Subset in *. intuition. assumption.
  - assert ($ x ∈ᵥ d2). { destruct d1, d2. simpl in *. apply NatSet.F.union_1 in H1. destruct H1. apply NatSet.F.mem_1 in H1. rewrite H1 in Hin1. discriminate Hin1. assumption. }
    pose proof @qmem_reflect (inl $ x) d2. specialize (H0 x H2). destruct H0. econstructor. eassumption. destruct q', d1, d2. simpl in *. unfold NatSet.F.Subset in *. intuition. assumption.
Qed.
...
Lemma closed_qual_qlub_inv_left : forall {b f l d1 d2}, closed_qual b f l (d1 ⊔ d2) -> closed_qual b f l d1.
  intros. destruct d1,d2. inversion H; subst. apply bound_union in H6,H7,H8; intuition.
Qed.

Lemma closed_qual_qlub_inv_right : forall {b f l d1 d2}, closed_qual b f l (d1 ⊔ d2) -> closed_qual b f l d2.
  intros. destruct d1,d2. inversion H; subst. apply bound_union in H6,H7,H8; intuition.
Qed.

Fixpoint has_type_closed  {Γ φ Σ t T d e ek} (ht : has_type Γ φ Σ t T d e ek) :
  closed_qual 0 (length Γ) (length Σ) φ /\
  closed_tm 0 (length Γ) (length Σ) t /\
  closed_ty 0 (length Γ) (length Σ) T /\
  closed_qual 0 (length Γ) (length Σ) d /\
  closed_qual 0 (length Γ) (length Σ) e /\
  closed_qual 0 (length Γ) (length Σ) ek.
Proof.
  destruct ht.
  - intuition.
  - apply indexr_var_some' in H. intuition. eapply closed_ty_monotone; eauto. lia.
    eapply closed_qual_sub; eauto.
  - intuition. eapply closed_qual_sub; eauto.
  - apply has_type_closed in ht1. apply has_type_closed in ht2. intuition.
    2-3 : repeat apply closed_qual_qlub; auto. all : inversion H12; eapply closed_qual_open'; eauto.
  - apply has_type_closed in ht1, ht2. intuition.
    2-3 : repeat apply closed_qual_qlub; auto. all: eapply closed_qual_open'; eauto.
  - apply indexr_var_some' in H0. intuition. apply just_loc_closed. auto.
  - apply has_type_closed in ht. intuition.
  - apply has_type_closed in ht. intuition. inversion H1. subst. eapply closed_ty_monotone; eauto; lia.
    apply closed_qual_qlub; auto.
  - apply has_type_closed in ht1. apply has_type_closed in ht2. intuition.
    all : repeat apply closed_qual_qlub; auto.
  - apply has_type_closed in ht. intuition. apply closed_qual_qlub; auto.
  - apply has_type_closed in ht. apply stp_closed in H. intuition.
Qed.
...
Lemma openq_subqual : forall {d1 d2 φ}, d1 ⊑ φ -> (openq ∅ d2) ⊑ φ -> (openq d1 d2) ⊑ φ.
  ...
  destruct (NatSet.F.mem 0 t3) eqn:Hmem; simpl in *; intuition; fnsetdec.
Qed.

Lemma openq_subqual' : forall {d1 d2 d'}, openq (d1 ⋓ d') d2 ⊑ openq d1 d2 ⋓ d'.
  intros. destruct d1,d',d2. simpl. destruct (NatSet.F.mem 0 t6); simpl; intuition; fnsetdec.
Qed.
#[global] Hint Resolve openq_subqual' : core.
...
Lemma open_qual_qlub_dist : forall {k d1 d2 d3}, ([[ k ~> d1  ]]ᵈ (d2 ⊔ d3)) = (([[ k ~> d1  ]]ᵈ d2) ⊔ ([[ k ~> d1  ]]ᵈ d3)).
  ...
  - rewrite NatSet.F.mem_1. 2: apply NatSet.F.union_2; apply NatSet.F.mem_2; auto.
    ...
    rewrite remove_union_dist; try fnsetdec. rewrite (@remove_not_in k t3); auto. fnsetdec. fnsetdec.
  - destruct (NatSet.F.mem k t3) eqn: Hmem2.
...
Fixpoint has_type_filter' {Γ φ Σ t T d e ek} (ht : has_type Γ φ Σ t T d e ek) : d ⊑ φ /\ e ⊑ φ /\ ek ⊑ φ.
  specialize (has_type_closed ht) as Hcl. destruct ht;
  try apply has_type_filter' in ht2; try apply has_type_filter' in ht1; try apply has_type_filter' in ht;
  intuition; repeat apply qlub_bound; auto; apply openq_subqual; auto.
Qed.

Lemma has_type_filter : forall {Γ φ Σ t T d e ek}, has_type Γ φ Σ t T d e ek -> d ⊑ φ.
  intros. apply has_type_filter' in H. intuition.
Qed.

Lemma has_type_filter_use : forall {Γ φ Σ t T d e ek}, has_type Γ φ Σ t T d e ek -> e ⊑ φ.
  intros. apply has_type_filter' in H. intuition.
Qed.

Lemma has_type_filter_kill : forall {Γ φ Σ t T d e ek}, has_type Γ φ Σ t T d e ek -> ek ⊑ φ.
  intros. apply has_type_filter' in H. intuition.
Qed.

Fixpoint has_type_saturated' {Γ φ Σ t T d e ek} (wfG : wf_tenv Γ Σ) (ht : has_type Γ φ Σ t T d e ek) : saturated Γ Σ d /\ saturated Γ Σ e /\ saturated Γ Σ ek.
  destruct ht; try solve [intuition].
  - intuition. eapply saturated_qplus; eauto.
  - apply has_type_saturated' in ht1; auto. apply has_type_saturated' in ht2; auto. intuition; repeat apply saturated_qqplus; auto; apply saturated_open_qual; auto.
  - apply has_type_saturated' in ht1; auto. intuition; repeat apply saturated_qqplus; auto; apply saturated_open_qual; auto.
  - apply has_type_saturated' in ht; auto. intuition.
  - apply has_type_saturated' in ht; auto. intuition. repeat apply saturated_qqplus; auto.
  - apply has_type_saturated' in ht1; auto. apply has_type_saturated' in ht2; auto. intuition; repeat apply saturated_qqplus; auto.
  - apply has_type_saturated' in ht; auto. intuition. repeat apply saturated_qqplus; auto.
Qed.

Lemma has_type_saturated: forall {Γ φ Σ t T d e ek}, wf_tenv Γ Σ -> has_type Γ φ Σ t T d e ek -> saturated Γ Σ d.
  intros. apply has_type_saturated' in H0; intuition.
Qed.

Lemma has_type_saturated_use: forall {Γ φ Σ t T d e ek}, wf_tenv Γ Σ -> has_type Γ φ Σ t T d e ek -> saturated Γ Σ e.
  intros. apply has_type_saturated' in H0; intuition.
Qed.

Lemma has_type_saturated_kill: forall {Γ φ Σ t T d e ek}, wf_tenv Γ Σ -> has_type Γ φ Σ t T d e ek -> saturated Γ Σ ek.
  intros. apply has_type_saturated' in H0; intuition.
Qed.
...
Lemma bound_vars_untypable : forall {Γ φ Σ T d e ek i}, has_type Γ φ Σ #i T d e ek -> False.
  intros Γ φ Σ T d e ek i HT. remember (tvar #i) as t. induction HT; try discriminate.
  intuition.
Qed.
...
Lemma weaken_gen : forall {t Γ1 Γ2 φ Σ T d eu ek},
    has_type (Γ1 ++ Γ2) φ Σ t T d eu ek ->
    let k := (length Γ2) in
    forall X, has_type ((splice_tenv k Γ1) ++ X :: Γ2) (splice_qual k φ) Σ (splice k t) (splice_ty k T) (splice_qual k d) (splice_qual k eu) (splice_qual k ek).
  intros t Γ1 Γ2 φ Σ T d eu ek HT. remember (Γ1 ++ Γ2) as Γ. generalize dependent Γ1. generalize dependent Γ2.
  ...
  - (* t_var *) simpl.
    ...
    + (* |Γ2| <= x < |Γ1|+|Γ2|*)
      erewrite <- splice_qual_qplus_dist; eauto. rewrite splice_set_empty. constructor.
    ...
    + (* |Γ2| > x *)
      ...
      erewrite splice_qual_qplus_id; eauto. rewrite splice_set_empty. constructor.
  ...
  - (* t_abs *) rewrite app_length in *. simpl. rewrite splice_set_empty. constructor.
    apply splice_closed'.
    1-5: rewrite app_length; rewrite splice_tenv_length; simpl;
      replace (length Γ1 + S (length Γ2)) with (S (length Γ1 + length Γ2)); eauto.
    inversion H0. subst. constructor. 1,2,3,4,7,8,9: apply splice_qual_closed; auto. 1,2 : apply splice_ty_closed; auto.
    ...
    subst k. rewrite <- splice_open'. rewrite <- splice_ty_open'. repeat rewrite <- splice_qual_open'.
    rewrite @open_tm'_len with (Γ':=(Γ1 ++ Γ2)). rewrite @open_ty'_len with (Γ':=(Γ1 ++ Γ2)).
    rewrite @openq'_len with (Γ':=(Γ1 ++ Γ2)). rewrite @openq'_len with (Γ':=(Γ1 ++ Γ2)) (q:=eu). rewrite @openq'_len with (Γ':=(Γ1 ++ Γ2)) (q:= ek).
    apply IHHT; intuition. all : repeat rewrite app_length; rewrite splice_tenv_length; auto.
  ...
  - (* tapp *) simpl. repeat rewrite splice_qual_lub_dist. repeat rewrite splice_qual_open''. apply t_app with (T1:=(splice_ty k T1)) (df:=(splice_qual k df)).
    replace (TFun (splice_qual k df ⊓ splice_qual k d1) (splice_qual k d2) (splice_qual k eu3) (splice_qual k ek3) (splice_ty k T1) (splice_ty k T2))
    with (splice_ty k (TFun (df ⊓ d1) d2 eu3 ek3 T1 T2)); auto.
    apply IHHT1; auto. simpl. f_equal. rewrite <- splice_qual_glb_dist. auto.
    apply IHHT2; auto.
    eapply splice_ty_closed'. rewrite app_length in *. rewrite splice_tenv_length. auto. 4: rewrite <- splice_qual_glb_dist.
    1-4 : subst k; rewrite <- @splice_qual_empty with (k := length Γ2); try rewrite <- splice_qual_open''.
    1-4: rewrite subqual_splice_lr'; auto.
    1-3: subst k; apply saturated_openq; auto; rewrite <- @splice_qual_empty with (k := length Γ2); rewrite <- splice_qual_open''; apply weaken_saturated; auto. all : red. rewrite <- splice_qual_glb_dist. red in H7. rewrite H7. auto.
    red in H8. rewrite <- splice_qual_open''. rewrite <- splice_qual_lub_dist. rewrite <- splice_qual_glb_dist. rewrite H8. auto.
  - (* t_let *) repeat rewrite splice_qual_lub_dist. repeat rewrite splice_qual_open''.
    apply t_let with (T1:=splice_ty k T1) (df:=splice_qual k df); auto. fold splice. apply splice_closed'.
    2: apply splice_ty_closed'. 3-5: apply splice_qual_closed'.
    1-5: rewrite app_length in *; rewrite splice_tenv_length; simpl;
        replace (length Γ1 + S (length Γ2)) with (S (length Γ1 + length Γ2)); eauto.
    1-3: rewrite <- @splice_qual_empty with (k:=k); rewrite <- splice_qual_open''; rewrite subqual_splice_lr'; auto.
    rewrite <- splice_qual_glb_dist. rewrite <- @splice_qual_empty with (k:=k). rewrite subqual_splice_lr'; auto.
    2-4 : rewrite <- @splice_qual_empty with (k:=k); rewrite <- splice_qual_open''. 1-4 : apply weaken_saturated; auto.
    rewrite subqual_splice_lr'. auto. red. red in H13. rewrite <- splice_qual_open''. rewrite <- splice_qual_glb_dist.
    rewrite H13. auto. all : fold splice. apply IHHT1; auto.
    rewrite app_comm_cons.
    replace ((splice_ty k T1, splice_qual k d1 ⊓ splice_qual k df) :: splice_tenv k Γ1) with (splice_tenv k ((T1, d1 ⊓ df) :: Γ1)); auto.
    replace (splice_qual k df ⊔ just_fv (length (splice_tenv k Γ1 ++ X :: Γ2)))
            with (splice_qual k (df ⊔ just_fv (length Γ1 + length Γ2))).
    subst k. rewrite <- splice_open'. repeat rewrite <- splice_qual_open'.
    rewrite @open_tm'_len with (Γ':=(Γ1 ++ Γ2)).
    rewrite @openq'_len with (Γ':=(Γ1 ++ Γ2)). rewrite @openq'_len with (Γ':=(Γ1 ++ Γ2)) (q:=eu2). rewrite @openq'_len with (Γ':=(Γ1 ++ Γ2)) (q:= ek2).
    rewrite app_length in *. apply IHHT2; intuition. all : repeat rewrite app_length; try rewrite splice_tenv_length; auto.
    simpl. repeat rewrite splice_qual_lub_dist. f_equal. rewrite splice_qual_just_fv_ge. f_equal. lia. lia.
    simpl. rewrite splice_qual_glb_dist. auto.
  ...
  - (* t_deref *) simpl. rewrite splice_set_empty. rewrite splice_qual_lub_dist. eapply t_deref.
    red in H. red. rewrite <- splice_qual_glb_dist. rewrite H. auto.
    simpl in IHHT. subst k. apply IHHT; auto.
  - (* t_assign *) replace (splice_qual k ∅) with (∅) by auto. replace (splice_ty k TUnit) with (TUnit) by auto. simpl.
    repeat rewrite splice_qual_lub_dist. apply t_assign with (d2:=(splice_qual k d2)); eauto.
    (* replace (TRef TUnit) with (splice_ty k (TRef TUnit)) by auto. apply IHHT1; auto. *)
    all : red; red in H, H0. 2: rewrite <- splice_qual_lub_dist. all : rewrite <- splice_qual_glb_dist. rewrite H. auto. rewrite H0. auto.
  - (* t_free *) simpl. rewrite splice_set_empty. rewrite splice_qual_lub_dist. constructor. apply IHHT. auto.
  - (* t_sub *) eapply t_sub. eapply IHHT; auto.
    apply @weaken_stp_gen; eauto; lia. 1-3: apply subqual_splice_lr'; auto.
    all : apply weaken_saturated; auto.
Qed.

Lemma weaken_flt : forall {Γ φ Σ t T d e ek},
    has_type Γ φ Σ t T d e ek->
    forall {φ'}, φ ⊑ φ' ->
                 closed_qual 0 (length Γ) (length Σ) φ' ->
    has_type Γ φ' Σ t T d e ek.
  intros Γ φ Σ t T d e ek HT. induction HT; intros; intuition.
  1-3:econstructor; eauto; eapply subqual_trans; eauto.
  apply t_let with (df:=df) (T1:=T1); auto. 1-4: eapply subqual_trans; eauto.
(* eapply subqual_qlub; eauto. apply closed_qual_qlub. simpl. eapply closed_qual_monotone; eauto. apply just_fv_closed; auto. *)
  all :  econstructor; eauto; eapply subqual_trans; eauto.
Qed.

Lemma weaken : forall {φ Σ t T d e ek},
    has_type [] φ Σ t T d e ek -> forall {Γ}, has_type Γ φ Σ t T d e ek.
  intros φ Σ t T d e ek HT. induction Γ; auto.
  specialize (@weaken_gen t [] Γ φ Σ T d e ek) as Hsp. simpl in *.
  ...
  replace (splice_qual (length Γ) e) with e in Hsp.
  replace (splice_qual (length Γ) ek) with ek in Hsp. auto. all : symmetry.
  1-4: eapply splice_qual_id; eauto; eapply closed_qual_monotone; eauto; lia.
  eapply splice_ty_id; eauto.   eapply closed_ty_monotone; eauto; lia.
  eapply splice_id; eauto.      eapply closed_tm_monotone; eauto; lia.
Qed.

Lemma weaken' : forall {φ Σ t T d e ek},
    has_type [] φ Σ t T d e ek -> forall {φ'}, φ ⊑ φ' -> forall {Γ}, closed_qual 0 (length Γ) (length Σ) φ' -> has_type Γ φ' Σ t T d e ek.
  intros. eapply weaken_flt; eauto. apply weaken. auto.
Qed.

Lemma weaken_store : forall {Γ φ Σ t T d e ek}, has_type Γ φ Σ t T d e ek -> forall {Σ'}, Σ' ⊇ Σ -> has_type Γ φ Σ' t T d e ek.
  intros Γ φ Σ t T d e ek HT.
  induction HT; intros; intuition; match goal with
  | [ |- has_type _ _ _ (tlet _ _) _ _ _ _ ] => apply t_let with (T1:=T1) (d1:=d1) (df:=df); eauto
  | [ |- has_type _ _ _ _ _ _ _ _ ] => econstructor; eauto
  end; try solve [eapply closed_qual_monotone; eauto; apply extends_length; auto];
...
Lemma narrowing_saturated : forall {Γ1 U du eu1 ek1 Γ2 Σ q},
    saturated (Γ1 ++ (U,du) :: Γ2) Σ q ->
    forall {V dv eu2 ek2}, stp Γ2 Σ V dv eu1 ek1 U du eu2 ek2 ->
...
Lemma stp_eff : forall {Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2}, stp Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 -> forall {eu3 ek3 eu4 ek4}, qstp Γ Σ eu3 eu4 -> qstp Γ Σ ek3 ek4 -> stp Γ Σ T1 d1 eu3 ek3 T2 d2 eu4 ek4.
  intros Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 HS.
  induction HS; intuition. eapply s_trans. eapply (IHHS1 _ _ _ _ H H0). apply IHHS2; apply qstp_refl; auto.
  apply qstp_closed in H; intuition. apply qstp_closed in H0; intuition.
Qed.

Lemma narrowing_gen : forall {t Γ1 U du Γ2 φ Σ T d eu ek},
    has_type (Γ1 ++ (U,du) :: Γ2) φ Σ t T d eu ek ->
    forall {V dv eu1 ek1 eu2 ek2}, stp Γ2 Σ V dv eu1 ek1 U du eu2 ek2 -> saturated Γ2 Σ du -> saturated Γ2 Σ eu2 -> saturated Γ2 Σ ek2 -> has_type (Γ1 ++ (V,dv) :: Γ2) φ Σ t T d eu ek.
  intros t Γ1 U du Γ2 φ Σ T d eu ek HT. remember (Γ1 ++ (U, du) :: Γ2) as Γ.
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    + subst. rewrite indexr_insert in H. inversion H. subst.
      apply t_sub with (T1:=V) (d1:=(dv ⊕ length Γ2)) (eu1:=∅) (ek1:=∅); auto. constructor; auto.
    ...
    + constructor; auto. destruct x. lia. rewrite <- indexr_insert_ge; try lia.
      ...
      repeat rewrite app_length in *; simpl in *; auto.
  - repeat rewrite app_length in *; simpl in *; auto.
    constructor; auto. 1-5 : rewrite app_length in *; simpl in *; auto.
    1,2 : eapply narrowing_saturated; eauto.
    rewrite @open_tm'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)).
    rewrite @open_ty'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)).
    rewrite @openq'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)). rewrite @openq'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)) (q:=eu). rewrite @openq'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)) (q:=ek).
    2-6 : repeat rewrite app_length; simpl; auto.
  ...
  - apply t_let with (T1:=T1) (df:=df); eauto. 1-5 : rewrite app_length in *; simpl in *; auto. 1-4 : eapply narrowing_saturated; eauto.
    rewrite @open_tm'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)).
    rewrite @openq'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)). rewrite @openq'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)) (q:=eu2). rewrite @openq'_len with (Γ' := (Γ1 ++ (U, du) :: Γ2)) (q:=ek2).
    2-5 : repeat rewrite app_length; simpl; auto.
    rewrite app_length in *. simpl in *.
    rewrite app_comm_cons.
    eapply IHHT2; eauto. simpl. auto.
  - econstructor; eauto.
    repeat rewrite app_length in *; simpl in *; auto.
  - eapply t_ref; eauto.
  - econstructor; eauto.
...
Lemma narrowing : forall {Γ U du φ Σ t T d eu ek}, has_type ((U,du) :: Γ) φ Σ t T d eu ek -> forall {V dv eu1 ek1 eu2 ek2}, stp Γ Σ V dv eu1 ek1 U du eu2 ek2 -> saturated Γ Σ du -> saturated Γ Σ eu2 -> saturated Γ Σ ek2 -> has_type ((V,dv) :: Γ) φ Σ t T d eu ek.
  intros. specialize (@narrowing_gen t [] U du Γ φ Σ T d eu ek) as narrow. simpl in *. eapply narrow; eauto.
Qed.
...
Lemma CtxOK_ext : forall {Γ φ Σ σ k}, CtxOK Γ φ Σ σ k -> forall {v T}, has_type Γ φ Σ v T ∅ ∅ ∅ -> value v -> CtxOK Γ φ (T :: Σ) ((Some v) :: σ) k.
  intros. unfold CtxOK in *. destruct H as [Hl [Hcl H']].
  split. simpl. lia.
  split. simpl. eapply closed_qual_monotone; eauto. intros.
  destruct (Nat.eqb l (length Σ)) eqn:Heql.
  - simpl in *. rewrite Heql in H. inversion H. subst. rewrite <- Hl. rewrite Heql. assert (~ l ∈ₗ k). {
    unfold not, "∈ₗ". intros. inversion Hcl. subst. apply bound_le_not_in in H5. apply Nat.eqb_eq in Heql. rewrite Heql in H2. contradiction.
  } split. intros. contradiction. intros. exists v. intros. intuition. eapply weaken_store; eauto. apply extends_cons.
  - simpl in *. rewrite <- Hl in *. rewrite Heql in *.
    specialize (H' _ _ H) as H'. destruct H' as [H' H'']; intuition. destruct H3 as [v0 H3']; exists v0; intuition.
    eapply weaken_store; eauto. apply extends_cons.
Qed.

Lemma CtxOK_update : forall {Γ φ Σ σ k}, CtxOK Γ φ Σ σ k ->
    forall {l T}, l < length σ ->
    ~ l ∈ₗ k ->
    indexr l Σ = Some T ->
    forall {v}, has_type Γ φ Σ v T ∅ ∅ ∅ ->
    value v ->
    CtxOK Γ φ Σ (update σ l (Some v)) k.
    intros. unfold CtxOK in *. destruct H as [Hlen [Hcl Hprev]].
    split. rewrite <- update_length. auto.
    split. auto.
    intros. destruct (Nat.eqb l l0) eqn:Heq.
    - apply Nat.eqb_eq in Heq. subst. split. intros. contradiction. intros. specialize (Hprev l0 T0). intuition. destruct H6. intuition. exists v. apply (@update_indexr_hit _ σ l0 (Some v)) in H0. rewrite H2 in H. inversion H. subst. intuition; auto.
    - apply Nat.eqb_neq in Heq. split. intros. specialize (Hprev l0 T0). intuition. apply (@update_indexr_miss _ σ l (Some v) l0) in Heq. rewrite Heq. auto. intros. specialize (Hprev l0 T0). intuition. destruct H6. exists x. intuition; auto. apply (@update_indexr_miss _ σ l (Some v) l0) in Heq. rewrite Heq. auto.
Qed.

Lemma CtxOK_empty : forall {Γ φ k},
  closed_qual 0 0 0 k ->
  CtxOK Γ φ [] [] k.
  intros. remember []. remember []. constructor; intuition; subst; simpl; eauto; inversion H0.
Qed.
#[global] Hint Resolve CtxOK_empty : core.

Lemma CtxOK_weaken_flt : forall {Γ φ Σ σ k}, CtxOK Γ φ Σ σ k -> forall {φ'}, closed_qual 0 (length Γ) (length Σ) φ' -> φ ⊑ φ' -> CtxOK Γ φ' Σ σ k.
  intros. inversion H. subst. constructor; intuition.
  all : specialize (H5 _ _ H3); intuition. destruct H5 as [v0 H5]. exists v0. intuition.
...
Lemma subst1_ty_id : forall {T b l}, closed_ty b 0 l T -> forall {d1}, { 0 |-> d1 }ᵀ T = T.
  ...
  repeat erewrite subst1_qual_id; eauto.
Qed.
...
Lemma open_subst1_ty : forall {T b l},
  ...
  induction T; intros; inversion H; subst; simpl; intuition.
  erewrite IHT1; eauto. erewrite IHT2; eauto.
  repeat erewrite <- open_subst1_qual; eauto.
  erewrite IHT; eauto.
Qed.
...
Fixpoint open_subst1_ty_comm {T : ty} :
  forall {k g df ff lf}, closed_qual 0 ff lf df ->
    ...
    repeat erewrite open_subst1_qual_comm; eauto.
...
Lemma subst_stp : forall{T1 T2},
    forall {Γ Tf df df' Σ d1 eu1 ek1 d2 eu2 ek2},
      stp (Γ ++ [(Tf,df)]) Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 ->
      closed_qual 0 0 (length Σ) df' ->
      Substq df df' ->
      stp ({ 0 |-> df' }ᴳ Γ) Σ
          ({ 0 |-> df' }ᵀ T1) ({ 0 |-> df' }ᵈ d1) ({ 0 |-> df' }ᵈ eu1) ({ 0 |-> df' }ᵈ ek1)
          ({ 0 |-> df' }ᵀ T2) ({ 0 |-> df' }ᵈ d2) ({ 0 |-> df' }ᵈ eu2) ({ 0 |-> df' }ᵈ ek2).
  intros T1 T2 Γ Tf df df' Σ d1 eu1 ek1 d2 eu2 ek2 HS.
  remember (Γ ++ [(Tf, df)]) as Γ'.
  generalize dependent Γ. generalize dependent df.  generalize dependent Tf. induction HS; intros; subst.
  - simpl. constructor. all: eapply subst_qstp; eauto.
  - specialize (stp_closed HS1). intuition. specialize (stp_closed HS2). intuition.
    simpl. constructor. 1-3: eapply subst_qstp; eauto.
  ...
  - simpl. constructor. inversion H. subst. 2 : inversion H0. subst.
    ...
    1-3: eapply subst_qstp; eauto. rewrite subst1_qual_empty in IHHS1. eapply IHHS1; eauto.
    ...
    specialize (IHHS2 Tf df ((T3, d3) :: Γ0)). simpl in IHHS2. intuition. rename H6 into IHHS2.
...
Lemma substitution_stp : forall{T1 T2},
    forall {Tf df dx Σ d1 eu1 ek1 d2 eu2 ek2},
      stp ([(Tf,df ⊓ dx)]) Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 -> closed_qual 0 0 (length Σ) dx ->
      stp [] Σ ({ 0 |-> dx }ᵀ T1) ({ 0 |-> dx }ᵈ d1)({ 0 |-> dx }ᵈ eu1) ({ 0 |-> dx }ᵈ ek1) ({ 0 |-> dx }ᵀ T2) ({ 0 |-> dx }ᵈ d2) ({ 0 |-> dx }ᵈ eu2) ({ 0 |-> dx }ᵈ ek2).
...
Lemma subst_filter0 : forall {d φ l}, closed_qual 0 0 l d -> d ⊕ 0 ⊑ φ -> d ⊑ { 0 |-> d }ᵈ φ.
  ...
  assert (NatSet.F.In 0 (NatSet.F.union t (NatSet.F.singleton 0))). fnsetdec.
...
Lemma saturated_openq_iff : forall {Γ Σ d1}, saturated Γ Σ (openq ∅ d1) <-> saturated Γ Σ d1.
  intros. destruct d1. unfold openq. simpl. destruct (mem 0 t1) eqn:Hmem0t1.
  repeat rewrite empty_union_right. unfold saturated. intuition.
  simpl in *. apply H in H0. inversion H0. subst. inversion H3. subst. apply bound_0_empty in H5. subst.
  econstructor; eauto. qdec.
  simpl in *. apply H in H0. inversion H0. subst. inversion H3. subst. apply bound_0_empty in H5. subst.
  econstructor; eauto. qdec. intuition.
Qed.

Lemma openq_subqual_bv : forall {d φ},
  openq ∅ d ⊑ φ ->
  d ⊑ φ ⋓ ## 0.
Proof.
  intros. destruct d,φ. simpl in *. destruct (mem 0 t1); simpl in *; intuition; fnsetdec.
Qed.

Lemma subst1_preserves_separation' : forall {df d1 Tx dx dx' Γ Σ φ},
    dx' ⊓ φ ≡ dx ->
    closed_qual 0 0 (length Σ) dx' ->
    openq ∅ df ⊑ φ -> openq ∅ d1 ⊑ φ ->
    saturated (Γ ++ [(Tx, dx)]) Σ (openq ∅ d1) ->
    saturated (Γ ++ [(Tx, dx)]) Σ (openq ∅ df) ->
    {0 |-> dx' }ᵈ df ⋒ {0 |-> dx' }ᵈ d1 = {0 |-> dx' }ᵈ (df ⋒ d1).
  intros. specialize (@subst1_preserves_separation df d1 Tx dx dx' Γ Σ (φ ⊔ ##0)) as Hsep.
  apply Hsep; auto.
  rewrite qglb_qlub_dist_l. assert (Hemp: dx' ⋒ ## 0 = ∅). destruct dx'. inversion H0. subst. simpl. f_equal; try fnsetdec. rewrite @bound_0_empty with (s:=t1); eauto. fnsetdec. rewrite Hemp. rewrite qlub_empty_right. auto.
  1,2: apply openq_subqual_bv; auto.
  all : rewrite <- saturated_openq_iff; auto.
Qed.
...
Lemma vtp_widening: forall {Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 t},
  vtp Σ t T1 d1 -> stp ([]: tenv) Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 -> vtp Σ t T2 d2.
Proof.
  intros. remember t0 as tt. remember [] as Γ. induction H0; subst.
  - (* vtp_base *)
  ...
  - (* vtp_abs *) inversion H. subst. eapply vtp_abs with (T2:=T5) (d2:=d7); eauto.
    ...
    + eapply qstp_trans; eauto.
    + eapply s_trans; eauto.
    + assert (stp [] Σ (TFun d1 d2 eu3 ek3 T1 T2) d5 ∅ ∅ (TFun d3 d4 eu4 ek4 T3 T4) d6 ∅ ∅).
      { apply s_fun; intuition.  }
      assert (stp [] Σ (TFun d0 d7 eu0 ek0 T0 T5) df1 ∅ ∅ (TFun d1 d2 eu3 ek3 T1 T2) d5 ∅ ∅).
      { apply s_fun; intuition.  }
      assert (stp [] Σ (TFun d0 d7 eu0 ek0 T0 T5) df1 ∅ ∅ (TFun d3 d4 eu4 ek4 T3 T4) d6 ∅ ∅).
      { eapply s_trans; eauto. }
      ...
      simpl in *. apply H9 in H8.
      eapply s_trans; eauto.
  - intuition.
Qed.

Lemma has_type_vtp: forall {Σ φ t T d eu ek},
  value t ->
  has_type [] φ Σ t T d eu ek ->
  vtp Σ t T d.
Proof.
  intros. remember [] as Γ.  induction H0; eauto; subst; try inversion H.
  - subst. (* tabs *) eapply vtp_abs; eauto.
    + eapply closed_qual_sub; eauto.
    + eapply qstp_refl; eauto. simpl. eapply closed_qual_sub; eauto.
    + eapply stp_refl; auto. inversion H1. subst. intuition.
    ...
    + apply stp_refl. inversion H1. subst.
      ...
      * apply qstp_refl.  apply eqqual_refl.  apply has_type_closed in H8; intuition.
      * apply qstp_refl.  apply eqqual_refl.  apply has_type_closed in H8; intuition.
      * apply qstp_refl.  apply eqqual_refl.  apply has_type_closed in H8; intuition.
...
Proof.
  ...
  - specialize (qstp_closed H4) as Hcl. intuition.
    assert (has_type [] df1 Σ (tabs t0) (TFun d1 d2 eu1 ek1 T1 T2) df1 ∅ ∅). {
        inversion H1. subst. constructor; eauto. }
    eapply weaken_flt with (φ' := df2) in H9; eauto.
    eapply t_sub; eauto. constructor; intuition. inversion H4. auto.
Qed.

Lemma substitution_gen :
  forall {t Γ φ' Tx dx dx' Σ T d eu ek}, dx' ⊓ φ' ≡ dx ->
    forall {φ}, φ ⊑ φ' ->
      has_type (Γ ++ [(Tx,dx)]) φ Σ t T d eu ek -> wf_tenv (Γ ++ [(Tx,dx)]) Σ -> Substq dx dx' ->
        forall {tx}, vtp Σ tx Tx dx' ->
                        has_type ({ 0 |-> dx' }ᴳ Γ) ({ 0 |-> dx' }ᵈ φ) Σ
                                 ({ 0 |-> tx  }ᵗ t)
                                 ({ 0 |-> dx' }ᵀ T)
                                 ({ 0 |-> dx' }ᵈ d) ({ 0 |-> dx' }ᵈ eu) ({ 0 |-> dx' }ᵈ ek).
  Import NatSet.F.
  intros t Γ φ' Tx dx dx' Σ T d eu ek Hsep φ Hphi HT HwfG HSubst tx HTx. specialize (vtp_closed HTx) as Hclx.
  ...
  - repeat rewrite subst1_qual_empty. simpl. (bdestruct (x =? 0)).
  ...
  - repeat rewrite subst1_qual_empty. simpl. assert (HwfG' : wf_tenv ((T1, d1) :: Γ0 ++ [(Tx, dx)]) Σ). apply wf_tenv_cons; auto.
    intuition. rename H10 into IHHT.
    ...
    1-3: eapply closed_qual_subst1'; eauto. apply subst_qual_subqual_monotone. auto.
    ...
    erewrite <- open_subst1_qual_comm in IHHT; eauto. erewrite <- open_subst1_qual_comm in IHHT; eauto. erewrite <- open_subst1_qual_comm in IHHT; eauto.
    apply IHHT; auto.
  ...
  - intuition. rename H14 into IHHT2. rename H12 into IHHT1. simpl. repeat rewrite subst1_qlub_dist.
    replace ({ 0 |-> dx' }ᵈ (openq d1 d2)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ d2)).
    replace ({0 |-> dx' }ᵈ (openq d1 eu3)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ eu3)).
    replace ({0 |-> dx' }ᵈ (openq d1 ek3)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ ek3)).
    ...
    assert (Hoverlap2: {0 |-> dx' }ᵈ (d2 ⊓ ek3) = {0 |-> dx' }ᵈ d2 ⊓ {0 |-> dx' }ᵈ ek3). {
        symmetry. eapply subst1_preserves_separation'; eauto. 1,2: eapply subqual_trans; eauto.
    }
    assert (Hoverlap3: {0 |-> dx' }ᵈ (ek1 ⊓ eu2) = {0 |-> dx' }ᵈ ek1 ⊓ {0 |-> dx' }ᵈ eu2). {
        specialize (has_type_filter_kill HT1). specialize (has_type_filter_use HT2).
        specialize (has_type_saturated_kill HwfG HT1). specialize (has_type_saturated_use HwfG HT2). intros.
        symmetry. eapply subst1_preserves_separation; eauto. 1,2: eapply subqual_trans; eauto.
    }
    assert (Hoverlap4: {0 |-> dx' }ᵈ ((ek1 ⊔ ek2) ⊓ openq d1 eu3) = {0 |-> dx' }ᵈ (ek1 ⊔ ek2) ⊓ {0 |-> dx' }ᵈ (openq d1 eu3)). {
        specialize (has_type_filter_kill HT1). specialize (has_type_filter' HT2). intuition.
        specialize (has_type_saturated_kill HwfG HT1). specialize (has_type_saturated_kill HwfG HT2). intros.
        symmetry. eapply subst1_preserves_separation; eauto. apply qlub_bound. 3: apply openq_subqual. 1-4: eapply subqual_trans; eauto.
        apply saturated_openq; eauto. eapply has_type_saturated; eauto. apply saturated_qqplus; auto.
      }
    apply t_app with (T1:= { 0 |-> dx' }ᵀ T1) (df:=({0 |-> dx' }ᵈ df)).
    replace (TFun ({0 |-> dx' }ᵈ df ⊓ {0 |-> dx' }ᵈ d1) ({0 |-> dx' }ᵈ d2) ({0 |-> dx' }ᵈ eu3) ({0 |-> dx' }ᵈ ek3) ({0 |-> dx' }ᵀ T1) ({0 |-> dx' }ᵀ T2))
            with ({ 0 |-> dx' }ᵀ (TFun (df ⊓ d1) d2 eu3 ek3 T1 T2)).
    eapply IHHT1; eauto. simpl. f_equal. apply Hoverlap.
    eapply IHHT2; eauto.
    2-4,6-8 : unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx'); erewrite <- subst1_open_qual_comm; eauto.
    8 : rewrite <- Hoverlap2; replace (∅) with ({0 |-> dx' }ᵈ ∅); auto.
    eapply closed_ty_subst1'; eauto. 1-3,7: apply subst_qual_subqual_monotone; eauto.
    1-3: eapply subst1_saturated; eauto.
    1,2 : red; red in H10, H11. rewrite <- Hoverlap3. rewrite H10. auto. rewrite <- subst1_qlub_dist.
    replace (openq ({0 |-> dx' }ᵈ d1) ({0 |-> dx' }ᵈ eu3)) with ({0 |-> dx' }ᵈ (openq d1 eu3)). rewrite <- Hoverlap4.
    rewrite H11. auto. all: unfold openq; repeat erewrite <- subst1_open_qual_comm; eauto.
  - intuition. rename H17 into IHHT1. simpl. repeat rewrite subst1_qlub_dist.
    replace ({ 0 |-> dx' }ᵈ (openq d1 d2)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ d2)).
    replace ({0 |-> dx' }ᵈ (openq d1 eu2)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ eu2)).
    replace ({0 |-> dx' }ᵈ (openq d1 ek2)) with (openq ({ 0 |-> dx' }ᵈ d1) ({ 0 |-> dx' }ᵈ ek2)).
    assert (Hoverlap: {0 |-> dx' }ᵈ (d1 ⊓ df) = {0 |-> dx' }ᵈ d1 ⊓ {0 |-> dx' }ᵈ df). { erewrite subst1_preserves_separation; eauto. apply has_type_filter in HT1. eapply subqual_trans; eauto. eapply subqual_trans; eauto. eapply has_type_saturated; eauto. }
    assert (Hoverlap2: {0 |-> dx' }ᵈ (d2 ⊓ ek2) = {0 |-> dx' }ᵈ d2 ⊓ {0 |-> dx' }ᵈ ek2). { erewrite subst1_preserves_separation'; eauto. eapply subqual_trans; eauto. eapply subqual_trans; eauto. }
    assert (Hoverlap3: {0 |-> dx' }ᵈ (ek1 ⊓ openq d1 eu2) = {0 |-> dx' }ᵈ ek1 ⊓ {0 |-> dx' }ᵈ (openq d1 eu2)). { pose proof has_type_filter HT1. pose proof has_type_filter_kill HT1. erewrite subst1_preserves_separation; eauto. eapply subqual_trans; eauto. eapply openq_subqual; eapply subqual_trans; eauto. eapply saturated_openq; eauto. apply has_type_saturated in HT1; eauto. apply has_type_saturated_kill in HT1; eauto. }
    apply t_let with (T1:={ 0 |-> dx' }ᵀ T1) (df:={ 0 |-> dx' }ᵈ df); eauto.
    eapply closed_tm_subst1'; eauto. eapply closed_ty_subst1'; eauto. 1-3: eapply closed_qual_subst1'; eauto.
    1-3,6-8: unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx'); erewrite <- subst1_open_qual_comm; eauto.
    1-3: apply subst_qual_subqual_monotone; auto. 1-3,5: eapply subst1_saturated; eauto. rewrite <- Hoverlap2. replace (∅) with ({0 |-> dx' }ᵈ ∅); auto.
    1,2: apply subst_qual_subqual_monotone; auto. red. red in H16.
    replace (openq ({0 |-> dx' }ᵈ d1) ({0 |-> dx' }ᵈ eu2)) with ({0 |-> dx' }ᵈ (openq d1 eu2)). rewrite <- Hoverlap3. replace (∅) with ({0 |-> dx' }ᵈ ∅); auto.
    f_equal; auto. 1,3-5: unfold openq; erewrite <- subst1_open_qual_comm; eauto.
    (* 1. instantiate the IH *)
    replace (length (Γ0 ++ [(Tx, dx)])) with (S (length Γ0)) in IHHT2.
    rewrite subst1_tenv_length. rewrite app_comm_cons in IHHT2.
    remember (df ⊔ just_fv (S (length Γ0))) as DF.
    replace ({0 |-> dx' }ᵈ df ⊔ just_fv (length Γ0)) with ({0 |-> dx' }ᵈ DF).
    specialize IHHT2 with (φ' := φ' ⊔ $$ (S (length Γ0))) (Γ := (((T1, d1 ⊓ df) :: Γ0))).
    (* 2. reason about opening and subst, apply IH *)
    unfold open_tm' in *. unfold open_ty' in *. unfold open_ty in *.
    unfold openq' in *. unfold openq in *.
  move IHHT2 at bottom.
  ...
  erewrite <- open_subst1_qual_comm in IHHT2; eauto. erewrite <- open_subst1_qual_comm in IHHT2; eauto. erewrite <- open_subst1_qual_comm in IHHT2; eauto.
  (* subst φs. rewrite HeqDF in IHHT2. rewrite subst1_qlub_dist in IHHT2. rewrite <- @subst1_just_fv with (x:=(length Γ0)) (dy:=dx') in IHHT2. *)
    rewrite <- Hoverlap. apply IHHT2; auto.
    apply wf_tenv_cons; auto. apply saturated_qqcap; auto. eapply has_type_saturated; eauto.
    rewrite qglb_qlub_dist_l. replace (dx' ⊓ φ' ⊔ dx' ⊓ $$ (S (length Γ0))) with (dx' ⊓ φ'); auto.
  assert (Hint: dx' ⋒ $$ (S (length Γ0)) = ∅). destruct dx'. simpl. f_equal; try fnsetdec. inversion H2. subst. rewrite @bound_0_empty with (s:=t0); auto. fnsetdec. rewrite Hint. auto.
    subst DF. apply subqual_qlub; auto. eapply subqual_trans; eauto.
    subst DF. rewrite subst1_qlub_dist. f_equal. erewrite <- subst1_just_fv. auto. rewrite app_length. simpl. lia.
  - erewrite @subst1_qual_id with (q:=(just_loc l)); eauto. repeat rewrite subst1_qual_empty. simpl. erewrite subst1_ty_id; eauto.
  ...
  - replace ({0 |-> dx' }ᵈ ∅) with (∅) in * by auto.
    ...
    replace ({0 |-> dx' }ᵀ (TUnit)) with (TUnit) in * by auto. simpl.
    eapply t_ref. eapply IHHT; eauto.
  - simpl. rewrite NatSetFacts.empty_b. rewrite remove_empty.
    rewrite unsplice_set_empty. rewrite subst1_qlub_dist.
    apply t_deref with (d := { 0 |-> dx' }ᵈ d). red. red in H0.
    specialize (has_type_filter' HT) as Hf. specialize (has_type_saturated' HwfG HT) as Hsat. intuition.
    erewrite subst1_preserves_separation; eauto. rewrite H0. auto. 1,2 : eapply subqual_trans; eauto.
    eapply IHHT; eauto.
  - replace ({0 |-> dx' }ᵈ ∅) with (∅) in * by auto.
    replace ({0 |-> dx' }ᵀ (TRef TUnit)) with (TRef TUnit) in * by auto.
    replace ({0 |-> dx' }ᵀ (TUnit)) with (TUnit) in * by auto. repeat rewrite subst1_qlub_dist. simpl.
    specialize (has_type_filter' HT1) as Hf1. specialize (has_type_saturated' HwfG HT1) as Hsat1.
    specialize (has_type_filter' HT2) as Hf2. specialize (has_type_saturated' HwfG HT2) as Hsat2. intuition.
    eapply t_assign; eauto. all : red; red in H0, H3.
    erewrite subst1_preserves_separation; eauto. rewrite H0. auto. 1,2: eapply subqual_trans; eauto.
    erewrite <- subst1_qlub_dist. erewrite subst1_preserves_separation; eauto. rewrite H3. auto. apply qlub_bound. 1-3: eapply subqual_trans; eauto.
    apply saturated_qqplus; auto.
  - rewrite subst1_qual_empty. rewrite subst1_qlub_dist. simpl. apply t_free. eauto.
  - apply t_sub with (T1:={ 0 |-> dx' }ᵀ T1) (d1:={ 0 |-> dx' }ᵈ d1) (eu1:={ 0 |-> dx' }ᵈ eu1) (ek1:={ 0 |-> dx' }ᵈ ek1); eauto.
    eapply subst_stp; eauto. 1-3: apply subst_qual_subqual_monotone; auto.
    all : eapply subst1_saturated; eauto.
    Unshelve. all : auto.
Qed.

Lemma substitution : forall {t df Tx dx Σ T d eu ek},
    has_type [(Tx,df ⊓ dx)] (df ⊔ (just_fv 0)) Σ t T d eu ek -> wf_tenv [(Tx,df ⊓ dx)] Σ -> closed_qual 0 0 (length Σ) df ->
  ...
  assert (Hsepf : dx ⊓ (df ⊔ just_fv 0) ≡ df ⊓ dx). {
    ...
    inversion H8. subst. simpl in *. apply bound_0_empty in H18, H19.
...
Lemma stp_scale_qqplus : forall {Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2}, stp Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 -> forall {d}, closed_qual 0 (length Γ) (length Σ) d -> stp Γ Σ T1 (d1 ⊔ d) eu1 ek1 T2 (d2 ⊔ d) eu2 ek2.
  intros Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 HSTP. induction HSTP; intros.
  - constructor; auto. inversion H. subst. constructor.
...
Lemma freshness' : forall {l b f d1}, closed_qual b f l d1 -> just_loc l ⊓ d1 = ∅.
  intros. destruct d1. unfold just_loc. simpl. inversion H. subst. f_equal; try fnsetdec. rewrite inter_comm. eapply bound_disjoint; eauto.
Qed.
...
Lemma stp_subqual: forall {Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2},
    stp Γ Σ T1 d1 eu1 ek1 T2 d2 eu2 ek2 -> d1 ⊑ d2.
Proof.
...
Lemma vtp_canonical_form_lam : forall {Σ t T1 T2 d1 d2 eu ek df},
    vtp Σ t (TFun d1 d2 eu ek T1 T2) df -> value t -> exists (t' : tm), t = tabs t'.
Proof.
  intros Σ t T1 T2 d1 d2 eu ek df H. remember (TFun d1 d2 eu ek T1 T2) as T.
  generalize dependent d1. generalize dependent d2. generalize dependent eu. generalize dependent ek. generalize dependent T1. generalize dependent T2.
  induction H; intuition; try discriminate; inversion H0; subst. exists t0. intuition.
Qed.

Lemma disjointq_qlub: forall {Σ Σ' d1 d2},
  disjointq Σ Σ' d1 ->
  disjointq Σ Σ' d2 ->
  disjointq Σ Σ' (d1 ⊔ d2).
Proof.
  intros. inversion H; inversion H0; subst.
  - rewrite qlub_empty_right. auto.
  - rewrite qlub_empty_left. auto.
  - rewrite qlub_empty_right. auto.
  - rewrite qlub_idem. auto.
Qed.

Lemma disjointq_qglb_empty: forall {Σ Σ' d'},
  disjointq Σ Σ' d' ->
  forall d b f,
  closed_qual b f (length Σ) d ->
  d ⊓ d' = ∅.
Proof.
  intros. inversion H. rewrite H1. auto. rewrite <- H1 in H3. rewrite H3. unfold just_loc, qglb. inversion H0. f_equal; try fnsetdec. eapply bound_disjoint; eauto.
Qed.

Lemma disjointq_closed: forall {Σ Σ' d'},
  disjointq Σ Σ' d' ->
  closed_qual 0 0 (length Σ') d'.
Proof.
  intros. inversion H. rewrite H0. auto. rewrite H2. apply just_loc_closed. lia.
Qed.

Lemma removeq_disjoint_id: forall {d q},
  d ⋒ q = ∅ -> removeq d q = d.
intros. destruct d,q. simpl in *. inversion H. rewrite H1 in H2. rewrite H2 in H3.
f_equal; apply NatSet.eq_if_Equal; unfold "[=]"; intros; split; try apply diff_1.
all: intros; apply diff_3; auto; unfold not; intros; specialize (inter_3 H0 H4); intros.
  - rewrite H1 in H5. fnsetdec.
  - rewrite H2 in H5. fnsetdec.
  - rewrite H3 in H5. fnsetdec.
Qed.

Lemma removeq_disjoint_empty: forall {d q}, removeq d q ⋒ q = ∅.
intros. destruct d,q. simpl in *. f_equal; apply NatSet.eq_if_Equal; unfold "[=]"; intros; split; intros; try fnsetdec.
Qed.

Lemma closed_qual_removeq:
  forall {b f l d}, closed_qual b f l d -> forall {d'}, closed_qual b f l (removeq d d').
intros. destruct d,d'. simpl in *. inversion H. subst. constructor; eapply Nat.le_trans. 1,3,5: eapply subset_bound'; apply NatSetProperties.diff_subset. all: auto.
Qed.

Lemma removeq_subqual: forall {d q},
  removeq d q ⊑ d.
intros. destruct d,q. simpl in *. intuition; fnsetdec.
Qed.
#[global] Hint Resolve removeq_subqual : core.

Lemma removeq_subqual': forall {d1 d2 q},
  d1 ⊑ d2 -> removeq d1 q ⊑ removeq d2 q.
intros. destruct d1,d2,q. simpl in *. intuition; fnsetdec.
Qed.

(* Lemma openq_subqual_qlub_fv0 : forall {d1 d2}, *)
(*   openq ∅ d1 ⊑ d2 <-> *)
(*   d1 ⊑ d2 ⋓ ## 0. *)
(*   intros. split. *)
(*   - destruct d1,d2; simpl; destruct (mem 0 t1) eqn:Eqm; simpl; intuition; try fnsetdec. rewrite empty_union_right. *)

(* HERE *)

(* Lemma closed_qual_open': forall {d}, *)
(*   d *)
(* Search (openq _ _). *)
...
Ltac qdisj :=
  try match goal with
      | _ : _ |- context[∅ ⊔ _] => rewrite qlub_empty_left; try qdisj
      | _ : _ |- context[_ ⊔ ∅] => rewrite qlub_empty_right; try qdisj
      | _ : _ |- context[∅ ⊓ _] => rewrite qglb_empty_left; try qdisj
      | _ : _ |- context[_ ⊓ ∅] => rewrite qglb_empty_right; try qdisj
      | _ : _ |- context[_ ⊓ (_ ⊔ _)] => rewrite qglb_qlub_dist_l; try qdisj
      | _ : _ |- context[(_ ⊔ _) ⊓ _] => rewrite qglb_qlub_dist_r; try qdisj
      | H : context[_ ⊓ (_ ⊔ _)] |- _ => rewrite qglb_qlub_dist_l in H; try qdisj
      | H : context[(_ ⊔ _) ⊓ _] |- _ => rewrite qglb_qlub_dist_r in H; try qdisj
      | H : _ ⊑ ∅ |- _ => apply empty_superqual in H; try qdisj
      | H : (_ ⊔ _) = ∅ |- _ => pose proof (qlub_empty_left' H); pose proof (qlub_empty_right' H); clear H; try qdisj
      | H : ?a ⊓ ?b = ∅ |- context[?a ⊓ ?b] => rewrite H; try qdisj
      | H : ?b ⊓ ?a = ∅ |- context[?a ⊓ ?b] => rewrite qglb_commute in H; rewrite H; try qdisj
      | _ => auto
      end.
...
Ltac closed_qual_solve :=
  ...
  | [ |- closed_qual _ _ _ (_ ⊓ _) ] => try solve [apply closed_qual_qqcap; closed_qual_solve]
  | [ |- closed_qual _ _ _ (removeq ?d _) ] => try solve [apply closed_qual_removeq; closed_qual_solve]
  | [ |- closed_qual _ _ _ (just_fv _) ] => try solve [apply just_fv_closed; simpl; lia | closed_qual_solve]
  | [ H : closed_qual _ _ _ ?q |- closed_qual _ _ _ ?q  ] => try solve [simpl in *; eapply closed_qual_monotone in H; eauto; try lia | (closed_qual_solve)]
  | [ H : closed_qual _ _ _ (?q ⊔ _) |- closed_qual _ _ _ ?q  ] => try solve [apply closed_qual_qlub_inv_left in H; try closed_qual_solve]
  | [ H : closed_qual _ _ _ (_ ⊔ ?q) |- closed_qual _ _ _ ?q  ] => try solve [apply closed_qual_qlub_inv_right in H; try closed_qual_solve]
  | [ H : closed_ty _ _ _ (TFun ?q _ _ _ _ _) |- closed_qual _ _ _ ?q ] => try solve [inversion H; clear H; try closed_qual_solve]
  | [ H : closed_ty _ _ _ (TFun _ ?q _ _ _ _) |- closed_qual _ _ _ ?q ] => try solve [inversion H; clear H; try closed_qual_solve]
  | [ H : closed_ty _ _ _ (TFun _ _ ?q _ _ _) |- closed_qual _ _ _ ?q ] => try solve [inversion H; clear H; try closed_qual_solve]
  | [ H : closed_ty _ _ _ (TFun _ _ _ ?q _ _) |- closed_qual _ _ _ ?q ] => try solve [inversion H; clear H; try closed_qual_solve]
  | [ H : has_type _ ?q _ _ _ _ _ _ |- closed_qual _ _ _ ?q  ] => try solve [apply has_type_closed in H; intuition; simpl in *; eapply closed_qual_monotone; eauto | (closed_qual_solve)]
  | [ H : has_type _ _ _ _ _ ?q _ _ |- closed_qual _ _ _ ?q  ] => try solve [apply has_type_closed in H; intuition; simpl in *; eapply closed_qual_monotone; eauto | (closed_qual_solve)]
  | [ H : has_type _ _ _ _ _ _ ?q _ |- closed_qual _ _ _ ?q  ] => try solve [apply has_type_closed in H; intuition; simpl in *; eapply closed_qual_monotone; eauto | (closed_qual_solve)]
  | [ H : has_type _ _ _ _ _ _ _ ?q |- closed_qual _ _ _ ?q  ] => try solve [apply has_type_closed in H; intuition; simpl in *; eapply closed_qual_monotone; eauto | (closed_qual_solve)]
  | [ H : stp _ _ _ ?q _ _ _ _ |- closed_qual _ _ _ ?q  ] => try solve [apply stp_closed in H; intuition; simpl in *; eapply closed_qual_monotone; eauto | (closed_qual_solve)]
  | [ H : stp _ _ _ _ _ _ ?q _ |- closed_qual _ _ _ ?q  ] => try solve [apply stp_closed in H; intuition; simpl in *; eapply closed_qual_monotone; eauto | (closed_qual_solve)]
  ...
  | [ H : disjointq _ ?Σ ?q  |- closed_qual _ _ (length ?Σ) ?q  ] => try solve [apply disjointq_closed in H; try closed_qual_solve]
  | _ => idtac
  end.

Ltac closed_ty_solve :=
  match goal with
  | [ H : closed_ty _ _ _ ?T |- closed_ty _ _ _ ?T  ] => try solve [simpl in *; eapply closed_ty_monotone in H; eauto; try lia | (closed_ty_solve)]
  | [ H : has_type _ _ _ _ ?T _ _ _ _ |- closed_ty _ _ _ ?T  ] => try solve [apply has_type_closed in H; intuition; simpl in *; eapply closed_ty_monotone; eauto | (closed_ty_solve)]
  ...
  | [ |- closed_ty _ _ _ (TFun _ _ _ _ _ _) ] => try solve [simpl; constructor; try closed_qual_solve; try closed_qual_solve; closed_ty_solve]
...
Ltac closed_solve := try closed_ty_solve || closed_qual_solve.

Ltac opening := try unfold openq',openq,open_tm',open_ty',open_ty; try rewrite open_qual_qlub_dist.

(* Main results: type soundness & preservation of separation *)

Theorem type_safety: forall {Σ t T d eu ek},
  has_type [] (ldom Σ) Σ t T d eu ek ->
    value t \/
    forall {σ k} , CtxOK [] (ldom Σ) Σ σ k ->
    eff_seq_ok ∅ k eu ek ->
    exists t' σ', step t σ t' σ' /\ preserve [] Σ t' T d eu ek σ' k.
Proof.
  intros Σ t T d eu ek H.
  ...
  - (* tapp *)
    ...
    (* t1 *) specialize (H16 (TFun (df ⊓ d1) d2 eu3 ek3 T1 T) t1).  intuition.
    (* t2 *) specialize (H18 T1 t2). intuition.
    + (* beta *)
      ...
      * eapply (Preserve Σ ∅ ∅ ∅ ∅ (eu1 ⊔ eu2 ⊔ openq d1 eu3) (ek1 ⊔ ek2 ⊔ openq d1 ek3)); auto.
        simpl.
        repeat rewrite qlub_empty_right. auto.
        repeat rewrite qlub_empty_right. auto.
        rewrite qglb_empty_right. auto.
        rewrite qglb_empty_right. auto.
        assert (HT' : has_type [(T1, df ⊓ d1)] (df ⊔ just_fv 0) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T2) (openq' ([]:tenv) d3) (openq' ([]:tenv) eu0) (openq' ([]:tenv) ek0)). {
          eapply narrowing; eauto. eapply weaken_flt; eauto. apply subqual_qlub. inversion H47. subst. auto.
          apply closed_qual_qlub. eapply closed_qual_monotone; eauto. lia. apply just_fv_closed. auto.
          apply saturated_empty_tenv. apply stp_closed in H48. intuition.
        }
        eapply @substitution with ( vx:= t2) in HT' as HT''; eauto; intuition.
        inversion H43. inversion H45. subst.
        unfold open_tm' in HT''. unfold open_ty' in HT''. unfold openq' in HT''. unfold openq' in HT''. simpl in HT''. inversion H40; subst.
        unfold open_ty in HT''. unfold openq in HT''.
        erewrite <- open_subst1_tm in HT''; eauto. erewrite <- open_subst1_qual in HT''; eauto. erewrite <- open_subst1_qual in HT''; eauto. erewrite <- open_subst1_qual in HT''; eauto. erewrite <- open_subst1_ty in HT''; eauto.
        fold (open_tm t2 t0) in HT''. fold (openq d1 d3) in HT''. fold (openq d1 eu0) in HT''. fold (openq d1 ek0) in HT''.
        apply @weaken_flt with (φ':= (ldom Σ)) in HT''; auto; intuition.
        repeat rewrite qlub_empty_right.
        eapply t_sub; eauto.
        rename H49 into Hsub.
        ...
        unfold open_ty', open_ty in Hsub.
        ...
        erewrite <- open_subst1_qual in Hsub; eauto.
        erewrite <- open_subst1_qual in Hsub; eauto.
        unfold openq. pose proof (stp_qstp_invu Hsub). pose proof (stp_qstp_invk Hsub).
        eapply stp_eff; eauto.
        1,2: eapply qstp_trans; eauto; constructor; repeat apply subqual_qqplus_l'; auto.
        unfold open_ty', open_ty. simpl. erewrite closed_ty_open_id. auto. eapply H1. lia.
        apply openq_subqual; auto.
        1-2: apply closed_qual_subqual_ldom; auto.
        apply closed_qual_subqual_ldom; apply closed_qual_qlub; auto.
        eapply wf_tenv_cons. auto.
        apply has_type_saturated in H; eauto. eapply vtp_has_type in VH0; eapply has_type_saturated in VH0; eauto. eapply saturated_qqcap; eauto.
    + (* (tabs t) t2 -> (tabs t) t2' *)
      apply has_type_vtp in H as VH; intuition.
      apply vtp_canonical_form_lam in VH as HH; intuition.
      pose (HH19 := H19).
      unfold CtxOK in HH19. specialize (H16 σ k).
      assert (eff_seq_ok ∅ k eu2 ek2). inversion H20. unfold eff_seq_ok. rewrite qglb_commute. apply @qglb_empty_sub with (d2:=(eu1 ⊔ eu2 ⊔ openq d1 eu3)). apply subqual_qqplus_l'. apply subqual_qqplus_r'. auto. rewrite qglb_commute. auto.
      intuition.
      destruct H16 as [t2' [σ' HH16]]. exists (tapp t1 t2'). exists σ'. intuition. constructor; intuition.
      destruct H35.
      pose proof has_type_closed H48.
      pose proof has_type_closed H.
      pose proof has_type_closed H0. intuition.
      simpl in *. apply extends_length in H39 as Hl.
      apply has_type_vtp in H; auto. apply vtp_has_type in H.
      destruct d2 as [d2v d2b d2l]. destruct (mem 0 d2b) eqn:Eqrt.
      -- (* (λ x.x) t', returns the argument *)
         destruct ek3 as [ek3v ek3b ek3l]. assert (Eqk: mem 0 ek3b = false). { simpl in H5. intuition. clear - H5 Eqrt. apply NatSetFacts.not_mem_iff. apply NatSetFacts.mem_iff in Eqrt. apply subset_of_empty in H5. unfold not. intros. pose proof (inter_3 Eqrt H). rewrite H5 in H0. fnsetdec. }
         remember (qset ek3v ek3b ek3l) as ek3.
         remember (qset d2v d2b d2l) as d2.
         apply (Preserve Σ' d' (d' ⊔ eu'') ek'' k' (∅ ⊔ eu' ⊔ openq d1 eu3) (∅ ⊔ ek' ⊔ openq d1 ek3)); auto.
         1-2: rewrite qlub_empty_left; apply subqual_qqplus_l'; apply subqual_qlub; auto.
         apply disjointq_qlub; auto.
         eapply subqual_trans; eauto. apply subqual_qlub. apply subqual_qqplus_l'. apply subqual_qqplus_r'. auto.
         unfold eff_seq_ok in *.
         assert (k' ⊓ openq d1 eu3 = ∅). inversion H22. eapply qglb_empty_sub; eauto. qdisj. eapply freshness'. eapply closed_qual_open'; eauto.
         assert (k ⊓ d' = ∅). eapply disjointq_qglb_empty; eauto.
         qdisj.
         assert (Heq: (openq d1 d2 ⊔ d') = (openq (d1 ⊔ d') (d2 ⊔ d'))). {
           inversion H40; subst. repeat rewrite qlub_empty_right. auto. erewrite <- openq_duplicate_eq. destruct d1. simpl. repeat rewrite empty_union_right. destruct (mem 0 d2b); simpl; repeat rewrite empty_union_right; f_equal; fnsetdec.
         }
         assert (Hemp: (df ⊓ d') = (∅)). erewrite disjointq_qglb_empty; eauto.
         rewrite Heq.
         apply t_sub with (T1:=T) (d1:=(openq (d1 ⊔ d') (d2 ⊔ d'))) (eu1:=(∅ ⊔ (eu' ⊔ eu'') ⊔ openq (d1 ⊔ d') eu3)) (ek1:=(∅ ⊔ (ek' ⊔ ek'') ⊔ openq (d1 ⊔ d') ek3)); auto. apply t_app with (df:=df) (T1:=T1); auto.
         apply t_sub with (T1:=(TFun (df ⊓ d1) d2 eu3 ek3 T1 T)) (d1:=df) (eu1:=∅) (ek1:=∅); auto.
         apply extends_ldom in H39 as Hldom. eapply weaken_flt; eauto. apply closed_qual_subqual_ldom in H24. eapply @weaken_flt with (φ:=df); eauto. eapply weaken_store; eauto. eapply closed_qual_monotone; eauto.
         rewrite qglb_qlub_dist_l. constructor; auto.
         constructor; closed_solve.
         constructor; closed_solve.
         eapply qstp_refl; eauto; closed_qual_solve.
         eapply stp_refl; eauto. rewrite Hemp. rewrite qlub_empty_right. apply qstp_refl; eauto. closed_qual_solve.
         rewrite Hemp. rewrite qlub_empty_right. eapply stp_refl; eauto.
         simpl. eapply closed_ty_open'; simpl; eauto; closed_solve.
         constructor; opening. apply subqual_qqplus_r'. auto.
         apply closed_qual_qlub; apply closed_qual_open; eauto; closed_qual_solve.
         1,2: eapply qstp_refl; eauto; opening; eapply closed_qual_open; eauto; closed_solve.
         apply extends_ldom in H39. eapply @subqual_trans with (d2:=(ldom Σ)); eauto. eapply closed_qual_subqual_ldom; eauto.
         eapply saturated_empty_tenv; closed_solve.
         closed_ty_solve.
         1-3: opening. apply qlub_bound. 1-4: eapply closed_qual_subqual_ldom; closed_solve.
         1-4: eapply closed_qual_open'; eauto; closed_solve.
         eapply @disjointq_qglb_empty with (d:=ek3) in H40; closed_solve. qdisj.
         1-3: eapply saturated_empty_tenv; opening.
         apply closed_qual_qlub; closed_solve.
         1-4: eapply closed_qual_open'; eauto; simpl; closed_solve.
         unfold eff_seq_ok in *. qdisj. assert (ek' ⋒ openq (d1 ⋓ d') eu3 = ∅). apply @qglb_empty_sub with (d2:=(ek2)); auto. apply @qglb_empty_sub' with (d3:=(openq d1 eu3 ⋓ d')); auto. qdisj. eapply disjointq_qglb_empty; eauto. assert (ek'' ⋒ openq (d1 ⋓ d') eu3 = ∅). apply @qglb_empty_sub' with (d3:=(openq d1 eu3 ⋓ d')); auto. qdisj. rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. unfold openq. eapply closed_qual_open'; closed_solve; eauto. qdisj.
         repeat rewrite qlub_empty_left.
         eapply stp_eff. eapply stp_refl; eauto. closed_solve.
         eapply qstp_refl; eauto. opening. apply closed_qual_qlub; eapply closed_qual_open'; closed_solve; lia.
         constructor. apply @subqual_trans with (d2:=(eu' ⋓ eu'') ⋓ openq d1 eu3 ⋓ d'). apply subqual_qlub_l. auto. rewrite <- qlub_assoc.
         rewrite @qlub_commute with (d1:=eu''). repeat rewrite <- qlub_assoc. auto.
         repeat apply closed_qual_qlub; closed_solve.
         unfold openq. eapply closed_qual_open'; eauto; closed_solve.
         constructor. subst. simpl. rewrite Eqk. destruct d1,d',ek',ek''. simpl. intuition; fnsetdec. repeat apply closed_qual_qlub; closed_solve. unfold openq. eapply closed_qual_open'; eauto; closed_solve.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         1,4: rewrite <- Heq; apply closed_qual_qlub; closed_solve.
         1-4: rewrite qlub_empty_left; repeat apply closed_qual_qlub; closed_solve; unfold openq; eapply closed_qual_open'; eauto; closed_solve.
      -- (* (λ x.kill x; 0) t', kills the argument *)
         apply (Preserve Σ' ∅ (d' ⊔ eu'') (d' ⊔ ek'') (k') (∅ ⊔ eu' ⊔ openq d1 eu3) (∅ ⊔ ek' ⊔ openq d1 ek3)); auto. 1-8: try rewrite qlub_empty_left; auto.
         1,2: rewrite qlub_assoc; apply subqual_qlub; apply subqual_qqplus_l'; auto.
         1,2: apply disjointq_qlub; auto.
         rewrite @qlub_commute with (d1:=ek2). rewrite qlub_assoc. rewrite <- @qlub_assoc with (q2:=ek2). apply subqual_qqplus_l'. auto.
         unfold eff_seq_ok in *.
         assert ((k' ⋒ openq d1 eu3) = ∅).
         apply @qglb_empty_sub with (d2:=ek2 ⋓ && (length Σ)); auto. qdisj. eapply freshness'. unfold openq. eapply closed_qual_open'; eauto; closed_solve.
         assert ((k ⋒ d') = ∅). eapply disjointq_qglb_empty; eauto. qdisj.
         1,2: rewrite qglb_empty_left; auto.
         remember (qset d2v d2b d2l) as d2.
         replace (openq d1 d2) with (openq (d1 ⊔ d') d2).
         apply t_sub with (T1:=T) (d1:=(openq (d1 ⊔ d') d2)) (eu1:=(((∅ ⊔ (eu' ⊔ eu'') ⊔ openq (d1 ⊔ d') eu3)))) (ek1:=(((∅ ⊔ (ek' ⊔ ek'') ⊔ openq (d1 ⊔ d') ek3)))) ; auto. apply t_app with (df:=df) (T1:=T1); auto.
         apply extends_ldom in H39 as Hldom. eapply weaken_flt; eauto. apply closed_qual_subqual_ldom in H59. eapply @weaken_flt with (φ:=df); eauto. eapply weaken_store; eauto. rewrite qglb_qlub_dist_l. erewrite @disjointq_qglb_empty with (d':=d'); eauto. rewrite qlub_empty_right. auto. eapply closed_qual_monotone; eauto.
         closed_ty_solve.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         1-6: inversion H21; eapply closed_qual_open'; subst; simpl; eauto; closed_solve.
         unfold eff_seq_ok in *. qdisj.
         assert ((ek' ⋒ openq (d1 ⋓ d') eu3) = ∅). apply @qglb_empty_sub with (d2:=(ek2)); auto. apply @qglb_empty_sub' with (d3:=openq d1 eu3 ⊔ d'); auto. qdisj. eapply disjointq_qglb_empty; eauto.
         assert ((ek'' ⋒ openq (d1 ⋓ d') eu3) = ∅). apply @qglb_empty_sub' with (d3:=openq d1 eu3 ⊔ d'); auto. qdisj. rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. eapply closed_qual_open'; eauto; closed_solve. qdisj.
         repeat rewrite qlub_empty_left. repeat rewrite qlub_empty_right.
         eapply stp_eff. eapply stp_refl; eauto. closed_solve.
         1-3: constructor; eauto; opening; repeat apply closed_qual_qlub; try eapply closed_qual_open'; eauto; closed_solve.
         rewrite @qlub_assoc with (q2:=d'). rewrite @qlub_commute with (d2:=eu''). rewrite @qlub_commute with (d2:=eu''). repeat rewrite <- qlub_assoc. apply subqual_qlub_l. apply subqual_qlub_l. auto.
         rewrite @qlub_assoc with (q2:=d'). rewrite @qlub_commute with (d2:=ek''). rewrite @qlub_commute with (d2:=ek''). repeat rewrite <- qlub_assoc. apply subqual_qlub_l. apply subqual_qlub_l. auto.
         1-6: try rewrite qlub_empty_left; try rewrite qlub_empty_right.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         1,4: opening; eapply closed_qual_open'; eauto; subst d2; closed_solve.
         1-4: repeat apply closed_qual_qlub; closed_solve; eapply closed_qual_open'; eauto; closed_solve.
         subst d2. unfold openq,open_qual. rewrite Eqrt. destruct d1,d'. simpl. auto.
    + (* left congruence *)
      assert (Heffok: eff_seq_ok ∅ k eu1 ek1). unfold eff_seq_ok in *. qdisj.
      specialize (H32 σ k). intuition.
      destruct H32 as [t' H32].
      destruct H32 as [σ' H32].
      intuition. inversion H34. exists (tapp t' t2). exists σ'. intuition. constructor. auto.
      apply (Preserve Σ' d' eu'' ek'' k' ((eu' ⊔ eu2 ⊔ openq d1 eu3)) ((ek' ⊔ ek2 ⊔ openq d1 ek3))); auto.
      1,2: apply subqual_qlub; auto.
      rewrite <- qlub_assoc. rewrite @qlub_commute with (d2:=just_loc (length Σ)). rewrite qlub_assoc. apply subqual_qqplus_r'. auto.
      unfold eff_seq_ok in *. assert (k' ⋒ (eu2 ⋓ openq d1 eu3) = ∅). apply @qglb_empty_sub with (d2:=ek1 ⋓ && (length Σ)); auto. qdisj. repeat erewrite freshness'; eauto. unfold openq. eapply closed_qual_open'; auto; closed_solve. qdisj.
         replace ((eu' ⋓ eu2 ⋓ openq d1 eu3) ⋓ eu'') with ((eu' ⋓ eu'') ⋓ eu2 ⋓ openq d1 eu3).
         replace ((ek' ⋓ ek2 ⋓ openq d1 ek3) ⋓ ek'') with ((ek' ⋓ ek'') ⋓ ek2 ⋓ openq d1 ek3).

         erewrite <- @closed_qual_open_id with (d:=d') (n:=0) (x:=d1) (l:=(length Σ')) (f:=0); try closed_solve; try lia. rewrite <- openq_u_distribute.
         apply t_app with (df:=df ⊔ d') (T1:=T1); auto.
         rewrite qglb_qlub_dist_r. rewrite @qglb_commute with (d1:=d'). erewrite @disjointq_qglb_empty with (d':=d'); eauto. rewrite qlub_empty_right.
         apply t_sub with (T1:=(TFun (df ⊓ d1) (d2) eu3 ek3 T1 T)) (d1:=(df ⊔ d')) (eu1:=(eu' ⊔ eu'')) (ek1:=(ek' ⊔ ek'')); auto.
         apply extends_length in H36. apply has_type_closed in H45. intuition. inversion H47. subst.
         constructor. 1,2: constructor; eauto. apply closed_qual_qlub; auto. eapply disjointq_closed in H37. eapply closed_qual_monotone; eauto.
         4,5: apply stp_refl; auto.
         1-4,7-8: apply qstp_refl; auto.
         1-4: opening.
         1-2: apply closed_qual_open; auto; closed_solve.
         clear H65. erewrite closed_ty_open_id; eauto; closed_solve.
         constructor. apply subqual_qqplus_r'; auto. apply closed_qual_qlub; eapply closed_qual_open; eauto; closed_solve.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         1-6: apply has_type_closed in H45 as Hc; intuition; closed_solve.
         apply extends_ldom in H36 as Hldom. apply closed_qual_subqual_ldom in H26. eapply @weaken_flt with (φ:=(ldom Σ)); eauto. eapply weaken_store; eauto. apply extends_length in H36. closed_ty_solve.
         4: qdisj; rewrite qglb_commute; erewrite disjointq_qglb_empty; eauto; closed_solve.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         1-6: apply extends_length in H36; apply disjointq_closed in H37; inversion H22; subst; opening; try apply closed_qual_qlub; eauto; eapply closed_qual_open'; eauto; closed_solve.
         1,2: unfold eff_seq_ok in *.
         assert ((ek' ⊔ ek'') ⊓ eu2 = ∅). qdisj. repeat rewrite @qglb_empty_sub with (d1:=ek') (d2:=ek1); auto. repeat rewrite @qglb_commute with (d1:=ek''). repeat erewrite disjointq_qglb_empty; eauto. qdisj.
         assert ((ek' ⊔ ek'') ⊓ openq d1 eu3 = ∅). qdisj. repeat rewrite @qglb_empty_sub with (d1:=ek') (d2:=ek1); auto. repeat rewrite @qglb_commute with (d1:=ek''). repeat erewrite disjointq_qglb_empty; eauto. qdisj. opening. eapply closed_qual_open'; eauto; closed_solve. qdisj.
         rewrite <- qlub_assoc. rewrite <- qlub_assoc. rewrite @qlub_commute with (d2:=(ek'')). repeat rewrite qlub_assoc. auto.
         rewrite <- qlub_assoc. rewrite <- qlub_assoc. rewrite @qlub_commute with (d2:=(eu'')). repeat rewrite qlub_assoc. auto.
  - (* tlet *)
    right. subst. intuition.
    apply has_type_closed in H14 as HH. intuition.
    (* t1 *) specialize (H23 T1 t1). intuition.
    + (* value t1 *)
      exists (open_tm t1 t2). exists σ. intuition.
      * constructor. intuition.
      * eapply (Preserve Σ ∅ ∅ ∅ ∅ (eu1 ⊔ openq d1 eu2) (ek1 ⊔ openq d1 ek2)); repeat rewrite qlub_empty_right; auto.
        1,2: rewrite qglb_empty_right; auto.
        rename H14 into HT. rename H15 into HT'.
        assert (closed_qual 0 0 (length Σ) df). eapply @closed_qual_sub with (d:=ldom Σ); eauto.
        erewrite <- @closed_ty_open'_id with (G:=([]:tenv)) (T:=T) in HT'; eauto. rewrite qglb_commute in HT'. eapply substitution in HT' as HT''; eauto.
        unfold open_tm', open_ty', openq', open_ty, openq in HT''. simpl in HT''.
        erewrite <- open_subst1_tm in HT''; eauto. erewrite <- open_subst1_qual in HT''; eauto. erewrite <- open_subst1_qual in HT''; eauto. erewrite <- open_subst1_qual in HT''; eauto. erewrite <- open_subst1_ty in HT''; eauto. erewrite closed_ty_open_id in HT''; eauto.
        fold (open_tm t1 t2) in HT''. fold (openq d1 d2) in HT''. fold (openq d1 eu2) in HT''. fold (openq d1 ek2) in HT''.
        apply @weaken_flt with (φ':= (ldom Σ)) in HT''; auto; intuition.
        eapply t_sub; eauto.
        eapply stp_refl; eauto.
        apply qstp_refl; eauto.
        1,2: constructor; try apply subqual_qqplus_l'; auto.
        1-4: apply closed_qual_subqual_ldom; closed_solve.
        eapply wf_tenv_cons. auto.
        eapply saturated_qqcap; eauto.
        apply has_type_vtp in HT as VH; auto.
    + (* (tlet t1 t2) -> (tlet t1' t2) *)
      pose (HH24 := H24).
      unfold CtxOK in HH24. specialize (H31 σ k).
      assert (eff_seq_ok ∅ k eu1 ek1). unfold eff_seq_ok in *. qdisj. intuition.
      destruct H31 as [t1' [σ' HH31]]. exists (tlet t1' t2). exists σ'. intuition. constructor; intuition.
      destruct H34.
      inversion H18. subst.
      simpl in *. apply extends_ldom in H38 as Hldom. apply extends_length in H38 as Hl.
      destruct d2 as [d2v d2b d2l]. destruct (mem 0 d2b) eqn:Eqrt.
      -- (* let body returns the argument *)
         destruct ek2 as [ek2v ek2b ek2l]. assert (Eqk: mem 0 ek2b = false). { simpl in H7. intuition. clear - H7 Eqrt. apply NatSetFacts.not_mem_iff. apply NatSetFacts.mem_iff in Eqrt. apply subset_of_empty in H7. unfold not. intros. pose proof (inter_3 Eqrt H). rewrite H7 in H0. fnsetdec. }
         remember (qset ek2v ek2b ek2l) as ek2.
         remember (qset d2v d2b d2l) as d2.
         apply (Preserve Σ' d' (d' ⊔ eu'') ek'' k' (∅ ⊔ eu' ⊔ openq d1 eu2) (∅ ⊔ ek' ⊔ openq d1 ek2)); auto.
         1-2: rewrite qlub_empty_left; apply subqual_qlub; auto.
         apply disjointq_qlub; auto.
         eapply subqual_trans; eauto. apply subqual_qlub. apply subqual_qqplus_r'. auto.
         unfold eff_seq_ok in *. assert (k' ⊓ openq d1 eu2 = ∅). inversion H22. eapply qglb_empty_sub; eauto. qdisj. eapply freshness'. eapply closed_qual_open'; closed_solve; lia. assert (k ⊓ d' = ∅). eapply disjointq_qglb_empty; eauto. qdisj.
         assert (Heq: (openq d1 d2 ⊔ d') = (openq (d1 ⊔ d') (d2 ⊔ d'))). {
           inversion H39; subst. repeat rewrite qlub_empty_right. auto. erewrite <- openq_duplicate_eq. destruct d1. simpl. repeat rewrite empty_union_right. destruct (mem 0 d2b); simpl; repeat rewrite empty_union_right; f_equal; fnsetdec.
         }
         rewrite Heq.
         repeat rewrite qlub_empty_left.
         assert (closed_qual 0 0 (length Σ) df). closed_solve.
         apply has_type_closed in H47 as Hcl. intuition.
         apply t_sub with (T1:=T) (d1:=(openq (d1 ⊔ d') d2)) (eu1:=((eu' ⊔ eu'') ⊔ openq (d1 ⊔ d') eu2)) (ek1:=((ek' ⊔ ek'') ⊔ openq (d1 ⊔ d') ek2)); auto.
         assert (wf_tenv [(T1, d1 ⋒ df)] Σ). apply wf_tenv_cons; auto. apply saturated_qqcap. 1,2: apply saturated_empty_tenv; eauto.
         apply t_let with (df:=df) (T1:=T1); closed_solve; eauto.
         eapply closed_tm_monotone; eauto.
         1-3: apply closed_qual_subqual_ldom.
         5-7: apply saturated_empty_tenv.
         1-3,5-7: unfold openq; eapply closed_qual_open'; eauto; closed_solve.
         assert (d' ⋒ ek2 = ∅). rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. qdisj.
         apply saturated_empty_tenv. closed_solve.
         apply closed_qual_subqual_ldom; closed_solve.
         unfold eff_seq_ok in *. qdisj. assert (ek' ⋒ openq (d1 ⋓ d') eu2 = ∅). apply @qglb_empty_sub with (d2:=(ek1)); auto. apply @qglb_empty_sub' with (d3:=(openq d1 eu2 ⋓ d')); auto. qdisj. eapply disjointq_qglb_empty; eauto. assert (ek'' ⋒ openq (d1 ⋓ d') eu2 = ∅). apply @qglb_empty_sub' with (d3:=(openq d1 eu2 ⋓ d')); auto. qdisj. rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. unfold openq. eapply closed_qual_open'; closed_solve; eauto. qdisj.
         rewrite qglb_qlub_dist_r. rewrite @qglb_commute with (d1:=d'). erewrite @disjointq_qglb_empty with (d':=d'); eauto. rewrite qlub_empty_right.
         simpl. apply t_sub with (T1:=T) (d1:=(openq' ([]:tenv) (d2))) (eu1:=(openq' ([]:tenv) eu2)) (ek1:=(openq' ([]:tenv) ek2)); auto. eapply @weaken_flt with (φ:=(df ⋓ $$ 0)); eauto.
         eapply weaken_store; eauto.
         repeat apply closed_qual_qlub; closed_solve.
         eapply stp_eff. eapply stp_refl; eauto. closed_solve.
         constructor; opening. auto.
         simpl. closed_solve.
         1,2: eapply qstp_refl; eauto; opening; eapply closed_qual_open'; eauto; closed_solve.
         1-4: apply has_type_filter' in H15 as Hf; intuition; simpl in H15; auto.
         1-3: apply has_type_saturated' in H15; intuition; eapply weaken_store_saturated; eauto.
         assert (Heqop: (openq (d1 ⋓ d') d2) = (openq (d1 ⋓ d') (d2 ⋓ d'))). { apply disjointq_closed in H39. inversion H39. subst. destruct d1. simpl. assert (Eqrt2: mem 0 (union d2b bs) = true). apply mem_1. apply mem_2 in Eqrt. fnsetdec. rewrite Eqrt,Eqrt2. f_equal; fnsetdec. }
         assert (Heqop': (openq (d1 ⋓ d') ek2) = (openq d1 ek2)). { subst. simpl. rewrite Eqk. destruct d1,d'. simpl. auto. }
         rewrite Heqop, Heqop'. eapply stp_refl; closed_solve.
         eapply qstp_refl; auto. opening. apply closed_qual_qlub; eapply closed_qual_open'; eauto; closed_solve.
         constructor. rewrite @qlub_commute with (d1:=(eu' ⋓ eu'')). rewrite <- @qlub_assoc with (q1:=(eu')). rewrite @qlub_commute with (d2:=openq d1 eu2 ⋓ d' ⋓ eu''). repeat rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=(eu'')). repeat rewrite qlub_assoc. apply subqual_qlub. apply subqual_qlub. auto. repeat apply closed_qual_qlub; closed_solve.
         constructor. rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=ek''). rewrite qlub_assoc. auto. repeat apply closed_qual_qlub; closed_solve.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         all: repeat apply closed_qual_qlub; closed_solve; opening; apply closed_qual_qlub; eapply closed_qual_open'; auto; closed_solve.
      -- (* (λ x.kill x; 0) t', kills the argument *)
         apply (Preserve Σ' ∅ (d' ⊔ eu'') (d' ⊔ ek'') (k') (∅ ⊔ eu' ⊔ openq d1 eu2) (∅ ⊔ ek' ⊔ openq d1 ek2)); auto. 1-8: try rewrite qlub_empty_left; auto.
         1,2: apply subqual_qlub; auto.
         1,2: apply disjointq_qlub; auto.
         rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=openq d1 ek2). rewrite qlub_assoc. apply subqual_qqplus_r'. auto.
         unfold eff_seq_ok in *.
         assert ((k' ⋒ openq d1 eu2) = ∅).
         apply @qglb_empty_sub with (d2:=ek1 ⋓ && (length Σ)); auto. qdisj. eapply freshness'. unfold openq. eapply closed_qual_open'; eauto; closed_solve.
         assert ((k ⋒ d') = ∅). eapply disjointq_qglb_empty; eauto. qdisj.
         1,2: rewrite qglb_empty_left; auto.
         remember (qset d2v d2b d2l) as d2.
         replace (openq d1 d2) with (openq (d1 ⊔ d') d2).
         assert (closed_qual 0 0 (length Σ) df). closed_solve.
         repeat rewrite qlub_empty_left, qlub_empty_right.
         apply has_type_closed in H47 as Hcl. intuition.
         apply t_sub with (T1:=T) (d1:=(openq (d1 ⊔ d') d2)) (eu1:=((((eu' ⊔ eu'') ⊔ openq (d1 ⊔ d') eu2)))) (ek1:=((((ek' ⊔ ek'') ⊔ openq (d1 ⊔ d') ek2)))); auto. apply t_let with (df:=df) (T1:=T1); auto; closed_solve. eapply closed_tm_monotone; eauto.
         1-3: eapply subqual_trans; eauto.
         apply saturated_empty_tenv; closed_solve.
         1-3: eapply weaken_store_saturated; eauto.
         eapply subqual_trans; eauto.
         unfold eff_seq_ok in *. qdisj.
         assert ((ek' ⋒ openq (d1 ⋓ d') eu2) = ∅). apply @qglb_empty_sub with (d2:=(ek1)); auto. apply @qglb_empty_sub' with (d3:=openq d1 eu2 ⊔ d'); auto. qdisj. eapply disjointq_qglb_empty; eauto.
         assert ((ek'' ⋒ openq (d1 ⋓ d') eu2) = ∅). apply @qglb_empty_sub' with (d3:=openq d1 eu2 ⊔ d'); auto. qdisj. rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. eapply closed_qual_open'; eauto; closed_solve. qdisj.
         repeat rewrite qlub_empty_left. repeat rewrite qlub_empty_right.
         rewrite qglb_qlub_dist_r. rewrite @qglb_commute with (d1:=d'). erewrite @disjointq_qglb_empty with (d':=d'); eauto. rewrite qlub_empty_right.
         eapply @weaken_store; eauto.
         eapply stp_eff. eapply stp_refl; eauto. closed_solve.
         eapply qstp_refl; eauto. opening. eapply closed_qual_open'; eauto; closed_solve.
         constructor. rewrite @qlub_commute with (d1:=(eu' ⋓ eu'')). rewrite <- @qlub_assoc with (q1:=(eu')). rewrite @qlub_commute with (d2:=openq d1 eu2 ⋓ d' ⋓ eu''). repeat rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=(eu'')). repeat rewrite qlub_assoc. apply subqual_qlub. apply subqual_qlub. auto. repeat apply closed_qual_qlub; closed_solve.
         rewrite qlub_empty_left. constructor. rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=ek''). repeat rewrite qlub_assoc. apply subqual_qlub. rewrite <- qlub_assoc. apply subqual_qlub_l. auto.
         opening; repeat apply closed_qual_qlub; try eapply closed_qual_open'; eauto; closed_solve.
         1-6: try rewrite qlub_empty_left; try rewrite qlub_empty_right.
         1-3: apply closed_qual_subqual_ldom.
         4-6: apply saturated_empty_tenv.
         1,4: opening; eapply closed_qual_open'; eauto; subst d2; closed_solve.
         ...
         subst d2. unfold openq,open_qual. rewrite Eqrt. destruct d1,d'. simpl. auto.
  - (* tref *)
    subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H5 TUnit t0). intuition.
    + (* contraction *) right. intros.
      exists (tloc (length (σ))). exists ((Some t0) :: σ). intuition.
      econstructor; eauto. apply (Preserve (TUnit :: Σ) (just_loc (length σ)) ∅ ∅ ∅ eu ek); auto. apply extends_cons. inversion H5. rewrite <- H15. apply disj_loc with (l:=(length Σ)); simpl; auto.
      rewrite qlub_empty_right. eapply @CtxOK_weaken_flt with (φ:=(ldom Σ)); eauto. apply CtxOK_ext; auto.
      pose (HV := H). apply has_type_vtp in HV; intuition. inversion HV; subst. constructor. auto.
      simpl. intuition. unfold dom. simpl. fnsetdec.
      repeat rewrite qlub_empty_right. auto.
      1,2: rewrite qglb_empty_right; auto.
      inversion H5. subst.
      apply t_sub with (T1:=(TRef TUnit)) (d1:=just_loc (length σ)) (eu1:=∅) (ek1:=∅); auto.
      eapply t_loc; eauto. rewrite <- H15.
      apply indexr_head. simpl. intuition; try fnsetdec.
      unfold dom. simpl. rewrite H15. fnsetdec.
      constructor; intuition.
      rewrite qlub_empty_left. apply qstp_refl; auto. apply just_loc_closed. simpl. lia.
      1,2: rewrite qlub_empty_right; constructor; auto; simpl; inversion H12; eapply closed_qual_monotone; eauto.
      rewrite <- H15. rewrite qlub_empty_left. eapply just_loc_ldom. simpl. lia.
      1-2: rewrite qlub_empty_right; simpl; apply has_type_filter' in H; intuition; eapply subqual_trans; eauto; apply extends_ldom; apply extends_cons.
      rewrite qlub_empty_left. apply saturated_just_loc.
      1-2: rewrite qlub_empty_right; simpl; inversion H11;
      apply saturated_empty_tenv; eapply closed_qual_monotone; eauto; simpl; lia.
    + (* congruence *) right. intros. specialize (H12 σ k H5 H14). destruct H12 as [t' [σ' HH12]]. exists (tref t'). exists σ'. intuition. econstructor; eauto.
      destruct H15. apply (Preserve Σ' d' eu'' ek'' k' eu' ek'); intuition.
      pose proof has_type_closed H26. intuition.
      rewrite qlub_empty_left.
      eapply t_sub with (T1:=(TRef TUnit)) (d1:=∅) (eu1:=(eu' ⊔ eu'')) (ek1:=(ek' ⊔ ek'')); eauto.
      apply stp_refl; eauto. constructor; auto; closed_solve.
      1,2: apply qstp_refl; auto.
      apply closed_qual_subqual_ldom; closed_solve.
      1,2: apply closed_qual_subqual_ldom; closed_solve.
      apply saturated_empty_tenv; auto; closed_solve.
  - (* tderef *)
    subst. intuition. specialize (has_type_closed H0) as HH. intuition. specialize (H6 (TRef T0) t0). intuition.
    + (* contraction *) right. intros. pose (HV := H0). apply has_type_vtp in HV; intuition.
      specialize (vtp_canonical_form_loc HV) as Hcan. intuition. destruct H16 as [l HH16]. subst.
      pose (HHV := HV). inversion HHV. subst.  pose (HH6 := H6). destruct HH6. destruct H17.
      pose (HH20 := H20). apply indexr_var_some' in HH20. rewrite H16 in HH20. apply indexr_var_some in HH20.
      destruct HH20 as [v HHH20].
      destruct (l ∈?ₗ k) eqn:Eqlin; pose (@qmem_reflect (inr l) k) as Hr; rewrite Eqlin in Hr; inversion Hr; specialize (H22 _ _ H20); intuition.
      * inversion H24. subst. unfold eff_seq_ok in H15. clear - H25 H26 H15. exfalso. unfold just_loc in H26. destruct k,d,eu. rewrite qglb_qlub_dist_l in H15. apply qlub_empty_right' in H15. simpl in *. inversion H26. intuition. apply singleton_subset_in in H5. pose proof (inter_3 H25 H5). inversion H15. pose empty_1. rewrite H7 in H8. rewrite H8 in H9. rewrite <- H9 in e. apply in_non_empty in H2. contradiction.
      * destruct H22 as [v0 H22]. exists v0. exists σ. intuition. apply step_deref; intuition. apply (Preserve Σ ∅ ∅ ∅ ∅ (eu ⊔ d) (ek)); intuition. all: repeat rewrite qlub_empty_right; auto.
        1,2: rewrite qglb_empty_right; auto.
        apply t_sub with (T1:=T) (d1:=∅) (eu1:=∅) (ek1:=∅); eauto.
        apply @stp_eff with (eu1:=∅) (ek1:=∅) (eu2:=∅) (ek2:=∅). replace Σ with (Σ ++ []). eapply weaken_stp_store. eauto. rewrite app_nil_r; auto.
        1,2: constructor; auto.
        1,2: apply closed_qual_subqual_ldom.
        apply closed_qual_qlub.
        all: closed_solve.
    + (* congruence *)
      right. intros. specialize (H13 σ k H6).
      destruct H13 as [t' [σ' HH13]].
      unfold eff_seq_ok in *. qdisj.
      exists (tderef t'). exists σ'. destruct HH13. inversion H16. intuition. constructor; auto.
      eapply (Preserve Σ' ∅ (d' ⊔ eu'') ek'' k' ((removeq eu k') ⊔ d) ek); intuition.
      apply @subqual_trans with (d2:=removeq eu k' ⋓ d). apply subqual_qlub_l. auto. apply subqual_qlub. auto.
      apply disjointq_qlub; auto.
      unfold eff_seq_ok in *. assert (k' ⋒ d = ∅). rewrite @qglb_empty_sub with (d2:=ek ⋓ && (length Σ)); auto. qdisj. eapply freshness'; eauto. assert (k' ⋒ removeq eu k' = ∅). rewrite qglb_commute. rewrite removeq_disjoint_empty. auto. assert (k ⋒ d' = ∅). inversion H6. intuition. eapply disjointq_qglb_empty; eauto. assert (k ⋒ removeq eu k' = ∅). rewrite @qglb_empty_sub' with (d3:=eu); auto. qdisj. qdisj.
      1,2: rewrite qglb_empty_left; auto.
      apply has_type_closed in H28 as Ht'. intuition.
      rewrite qlub_empty_left. replace ((removeq eu k' ⋓ d) ⋓ d' ⋓ eu'') with ((removeq eu k' ⋓ eu'') ⋓ d ⋓ d').
      apply t_deref.
      unfold eff_seq_ok in *. assert (ek'' ⋒ d = ∅). rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. assert (ek ⋒ d' = ∅). eapply disjointq_qglb_empty; eauto. qdisj.
      apply extends_length in H19.
      eapply t_sub with (T1:=(TRef T0)) (d1:=(d ⊔ d')) (eu1:=(eu' ⊔ eu'')) (ek1:=(ek' ⊔ ek'')); eauto. eapply stp_refl; eauto. apply qstp_refl; eauto.
      constructor. unfold eff_seq_ok in *. apply subqual_qlub. eapply @subqual_trans with (d2:=removeq eu' k'); eauto. rewrite removeq_disjoint_id; auto. qdisj. apply removeq_subqual'. auto. apply closed_qual_qlub; closed_solve. constructor. apply subqual_qlub. auto. apply closed_qual_qlub; closed_solve.
      1-3: apply closed_qual_subqual_ldom.
      4-5: eapply saturated_empty_tenv.
      1-5: apply closed_qual_qlub; closed_solve.
      rewrite <- @qlub_assoc with (q2:=eu'').
      rewrite @qlub_commute with (d1:=(eu'')).
      repeat rewrite qlub_assoc. auto.
  - (* tassign *)
    ...
    specialize (H8 (TRef TUnit) t1). intuition.
    specialize (H10 TUnit t2). intuition.
    + (* contraction *)
      right. intros.
      ...
      inversion Ht2'. subst.
      exists tunit. exists (update σ l (Some tunit)).
      inversion H11. inversion H10. inversion H30. subst. intuition.
      eapply step_assign; eauto. lia. apply (Preserve Σ ∅ ∅ ∅ ∅ (eu1 ⊔ eu2 ⊔ d1) (ek1 ⊔ ek2)); auto.
      eapply CtxOK_update; try rewrite qlub_empty_right; eauto. lia.
      unfold eff_seq_ok in H22. inversion H31. subst. clear - H22 H33 H31. qdisj. intuition.
      destruct k,d1. simpl in *. inversion H2. intuition. apply singleton_subset_in in H9. rewrite H4 in H5. rewrite H5 in H6. specialize (inter_3 H0 H9). intuition. rewrite H6 in H7. apply in_non_empty in H7. unfold not in H7. intuition.
      apply t_sub with (T1:=TUnit) (d1:=∅) (eu1:=∅) (ek1:=∅); auto.
      replace Σ with (Σ ++ []); intuition. eapply weaken_stp_store. auto.
      unfold eff_seq_ok in *. qdisj.
      1-2: rewrite qglb_empty_right; auto.
      repeat rewrite qlub_empty_right.
      apply t_sub with (T1:=TUnit) (d1:=∅) (eu1:=∅) (ek1:=∅); auto.
      eapply stp_eff; eauto. eapply s_base; eauto.
      1,2: constructor; auto.
      1,2: apply closed_qual_subqual_ldom; closed_solve.
    + (* right congruence *)
      right. intros. specialize (H8 σ k H10).
      assert (eff_seq_ok ∅ k eu2 ek2). unfold eff_seq_ok in *. qdisj.
      intuition. destruct H24 as [t' [σ' H24']]. exists (tassign t1 t'). exists σ'. intuition. econstructor; eauto.
      pose (HV := Ht1). apply has_type_vtp in HV; intuition. inversion HV; subst.
      destruct H24.
      apply (Preserve Σ' d' (d' ⊔ eu'') ek'' k' ((removeq eu1 k') ⊔ eu' ⊔ d1) (ek1 ⊔ ek')); eauto.
      repeat rewrite qlub_assoc. eapply subqual_qlub. apply @subqual_trans with (d2:=eu1 ⋓ eu'). eapply subqual_qlub. auto. eapply subqual_qlub_l. auto.
      eapply subqual_qlub_l. auto.
      apply disjointq_qlub; auto. eapply subqual_trans; eauto. apply subqual_qlub. apply subqual_qqplus_l'. auto.
      unfold eff_seq_ok in *. assert (k ⋒ removeq eu1 k' = ∅). eapply qglb_empty_sub'; eauto; qdisj. assert (k' ⋒ removeq eu1 k' = ∅). rewrite qglb_commute. eapply removeq_disjoint_empty. assert (k' ⋒ d1 = ∅). eapply qglb_empty_sub; eauto; qdisj. eapply freshness'; eauto. assert (k ⋒ d' = ∅). inversion H10. intuition. eapply disjointq_qglb_empty; eauto. qdisj.
      assert (has_type [] (ldom Σ) Σ & (l) (TRef T) (just_loc l) ∅ ∅). { constructor; auto. apply qstp_closed in H33. intuition. apply closed_qual_subqual_ldom in H42. auto. }
      assert (ldom Σ ⊑ ldom Σ'). apply extends_ldom. auto.
      assert (length Σ <= length Σ'). apply extends_length. auto.
      apply t_sub with (T1:=TUnit) (d1:=∅) (eu1:=∅ ⊔ (eu' ⊔ eu'') ⊔ d1) (ek1:=∅ ⊔ (ek' ⊔ ek'')); intuition.
      apply t_assign with (d2:=(d2 ⊔ d')); auto.
      apply vtp_has_type in HV. apply @weaken_flt with (φ:=d1); auto. apply @weaken_store with (Σ:=Σ); auto. apply closed_qual_subqual_ldom. closed_solve.
      unfold eff_seq_ok in *. assert (ek' ⋒ d1 = ∅). eapply qglb_empty_sub; eauto. qdisj. assert (ek'' ⋒ d1 = ∅). rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. qdisj.
      repeat rewrite qlub_empty_left. constructor.
      constructor; auto; closed_solve.
      constructor.
      rewrite @qlub_commute with (d2:=d' ⋓ eu''). repeat rewrite @qlub_assoc with (q3:=d1). apply subqual_qlub. rewrite @qlub_commute with (d1:=removeq eu1 k'). repeat rewrite @qlub_assoc with (q3:=removeq eu1 k'). apply subqual_qqplus_r'. repeat rewrite <- qlub_assoc. apply subqual_qqplus_l'. rewrite qlub_commute. auto.
      repeat apply closed_qual_qlub; closed_solve. apply @closed_qual_sub with (d:=eu2); auto; closed_solve.
      constructor. rewrite <- qlub_assoc. apply subqual_qqplus_l'. auto.
      repeat apply closed_qual_qlub; closed_solve. apply @closed_qual_sub with (d:=ek2); auto; closed_solve.
      1-3: apply closed_qual_subqual_ldom.
      4-6: apply saturated_empty_tenv.
      all: repeat apply closed_qual_qlub; closed_solve.
      1,3: apply @closed_qual_sub with (d:=eu2); auto; closed_solve.
      1,2: apply @closed_qual_sub with (d:=ek2); auto; closed_solve.
    + (* left congruence *)
      right. intros.
      specialize (H20 σ k H8).
      assert (eff_seq_ok ∅ k eu1 ek1). unfold eff_seq_ok in *. qdisj.
      intuition. destruct H24 as [t' [σ' H24']].
      exists (tassign t' t2). exists σ'. intuition. econstructor; eauto. destruct H24.
      apply (Preserve Σ' d' (d' ⊔ eu'') ek'' k' (eu' ⊔ eu2 ⊔ d1) (ek' ⊔ ek2)); eauto.
      1,2: apply subqual_qlub; auto.
      apply disjointq_qlub; eauto.
      eapply subqual_trans; eauto. eapply subqual_qlub. apply subqual_qqplus_r'. auto.
      unfold eff_seq_ok in *. assert (k ⋒ d' = ∅). inversion H8. intuition. eapply disjointq_qglb_empty; eauto. assert (k' ⋒ (d1 ⋓ eu2) = ∅). eapply qglb_empty_sub; eauto. qdisj. repeat erewrite freshness'; eauto. qdisj.
      pose proof has_type_closed H35. intuition.
      pose proof has_type_filter H35.
      pose proof extends_length H26.
      assert (ldom Σ ⊑ ldom Σ'). apply extends_ldom. auto.
      apply t_sub with (T1:=TUnit) (d1:=∅) (eu1:=(eu' ⊔ eu'') ⊔ eu2 ⊔ (d1 ⊔ d')) (ek1:=((ek' ⊔ ek'') ⊔ ek2)). eapply t_assign; eauto.
      eapply weaken_store in Ht2; eauto. eapply weaken_flt in Ht2; eauto.
      unfold eff_seq_ok in *. assert (ek' ⋒ eu2 = ∅). eapply qglb_empty_sub; eauto. assert (ek'' ⋒ eu2 = ∅). rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. qdisj.
      unfold eff_seq_ok in *. qdisj. assert (ek' ⋒ d1 = ∅). eapply qglb_empty_sub; eauto. assert (ek'' ⋒ d1 = ∅). rewrite qglb_commute. eapply disjointq_qglb_empty; eauto. assert (ek' ⋒ d' = ∅). eapply qglb_empty_sub; eauto. eapply disjointq_qglb_empty; eauto. assert (ek2 ⋒ d' = ∅). eapply disjointq_qglb_empty; eauto. qdisj.
      constructor. constructor; auto; closed_solve.
      constructor.
      rewrite @qlub_commute with (d1:=d') (d2:=eu''). repeat rewrite qlub_assoc. apply subqual_qlub. repeat rewrite <- qlub_assoc. apply subqual_qlub_l. rewrite @qlub_commute with (d1:=eu''). repeat rewrite <- qlub_assoc. auto.
      repeat apply closed_qual_qlub; closed_solve.
      constructor. repeat rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=ek2). auto.
      repeat apply closed_qual_qlub; closed_solve.
      1-3: apply closed_qual_subqual_ldom.
      4-6: apply saturated_empty_tenv.
      all: repeat apply closed_qual_qlub; closed_solve.
  - (* free *)
    subst. intuition. rename H into Ht1.
    apply has_type_closed in Ht1 as Ht1C. intuition.
    specialize (H5 (TRef TUnit) t0). intuition.
    + (* contraction *)
      right. intros.
      pose (Ht1' := Ht1). eapply has_type_vtp in Ht1'; eauto.
      pose (Hloc := Ht1'). apply vtp_canonical_form_loc in Hloc; auto.
      inversion Ht1'. destruct Hloc. subst.
      exists tunit. exists (update σ x None). inversion H21. subst.
      split. constructor. apply qstp_inv in H22.
      apply (Preserve Σ ∅ ∅ ∅ (just_loc x) (removeq eu (just_loc x)) (ek ⊔ d)); intuition; auto.
      apply subqual_qqplus_r'. apply subqual_qqplus_l'. auto.
      {
        inversion H5. subst. intuition. constructor. rewrite <- update_length; eauto. split. apply closed_qual_qlub; auto. inversion H8. apply just_loc_closed; auto. intros. split; intros.
         ++ specialize (H24 l T0). intuition. destruct (Nat.eqb l x) eqn:Eqlx. apply Nat.eqb_eq in Eqlx. rewrite Eqlx. rewrite update_indexr_hit. auto. apply indexr_var_some' in H20. rewrite H14 in H20. lia. apply Nat.eqb_neq in Eqlx. rewrite update_indexr_miss; auto. apply H24. destruct k. unfold just_loc in H25. simpl in *. apply union_1 in H25. destruct H25. auto. apply singleton_1 in H25. inversion H25. rewrite H26 in Eqlx. contradiction.
         ++ assert (~ l ∈ₗ k). { clear - H25. unfold just_loc in H25. destruct k. simpl in *. unfold not in *. intros. eapply union_2 in H. apply H25 in H. auto. } specialize (H24 l T0). intuition. destruct H27. exists x0. intuition. destruct (Nat.eqb l x) eqn:Eqlx. apply Nat.eqb_eq in Eqlx. rewrite Eqlx in H25. clear - H25. exfalso. apply H25. destruct k. unfold just_loc. simpl. apply union_3. fnsetdec. apply Nat.eqb_neq in Eqlx. rewrite update_indexr_miss; auto.
      }
      unfold eff_seq_ok in *. assert (&& x ⋒ removeq eu && x = ∅). rewrite qglb_commute. apply removeq_disjoint_empty. assert (k ⋒ removeq eu && x = ∅). eapply qglb_empty_sub'; eauto. qdisj.
      1,2: rewrite qglb_empty_left; auto.
      eapply t_sub; eauto. constructor. apply qstp_refl; auto. rewrite qlub_empty_right; auto.
      1,2: constructor; auto; closed_solve.
      1-3: apply closed_qual_subqual_ldom.
      4-6: apply saturated_empty_tenv.
      all: repeat apply closed_qual_qlub; closed_solve.
    + (* congruence *)
      right. intros.
      specialize (H11 σ k).
      assert (eff_seq_ok ∅ k eu ek). unfold eff_seq_ok in *. qdisj. intuition.
      destruct H11 as [x H11]; auto. destruct H11 as [σ' H11]. intuition.
      exists (tfree x). exists σ'. intuition. constructor. auto.
      destruct H16.
      apply (Preserve Σ' ∅ eu'' (ek'' ⊔ d') k' eu' (ek ⊔ d)); intuition; eauto.
      apply disjointq_qlub; eauto.
      rewrite <- qlub_assoc. rewrite @qlub_commute with (d1:=d). rewrite qlub_assoc. apply subqual_qqplus_r'. auto.
      1,2: rewrite qglb_empty_left; auto.
      rewrite qlub_empty_right. replace ((ek ⋓ d) ⋓ ek'' ⋓ d') with ((ek ⋓ ek'') ⋓ d ⋓ d'). eapply t_free.
      pose proof has_type_closed H26. intuition. apply extends_length in H17.
      apply t_sub with (T1:=(TRef TUnit)) (d1:=d ⊔ d') (eu1:=(eu' ⊔ eu'')) (ek1:=(ek' ⊔ ek'')).
      assumption.
      constructor.
      constructor; auto; closed_solve.
      constructor; auto; closed_solve.
      constructor; auto. apply subqual_qlub. auto. apply closed_qual_qlub; closed_solve.
      1,2: constructor; auto.
      1-3: apply closed_qual_subqual_ldom.
      4-6: apply saturated_empty_tenv.
      all: repeat apply closed_qual_qlub; closed_solve.
      repeat rewrite <- qlub_assoc.
      repeat rewrite @qlub_assoc with (q3:=d').
      rewrite @qlub_commute with (d2:=d).
      auto.
  - (* subsumption *)
    subst. intuition. specialize (stp_closed H0) as H00. intuition.
    specialize (H12 T1 t0). intuition. right.
    intros. specialize (H21 σ k).
    apply stp_qstp_invu in H0 as Heffu.
    apply stp_qstp_invk in H0 as Heffk.
    assert (eff_seq_ok ∅ k eu1 ek1).
    unfold eff_seq_ok in *. inversion Heffu. eapply qglb_empty_sub'; eauto.
    intuition.
    destruct H21 as [t' [σ' HH21]]. exists t'. exists σ'. intuition.
    destruct H25. apply (Preserve Σ' d' eu'' ek'' k' eu' ek'); intuition.
    1,2: eapply subqual_trans; eauto; eapply qstp_inv; eauto.
    eapply subqual_trans; eauto. apply subqual_qlub. eapply qstp_inv; eauto.
    apply extends_length in H27 as Hl. apply has_type_closed in H36 as Hcl. intuition.
    eapply t_sub; eauto. apply stp_scale_qqplus.
    eapply stp_eff. eapply weaken_stp_store_ext; eauto.
    1,2: constructor; auto; closed_solve.
    eapply disjointq_closed; eauto.
    1-3: apply closed_qual_subqual_ldom.
    4: apply saturated_empty_tenv.
    all: apply closed_qual_qlub; closed_solve.
Qed.
...
Lemma progress : forall {Σ t T d eu ek k},
    has_type [] (ldom Σ) Σ t T d eu ek ->
    eff_seq_ok ∅ k eu ek ->
    value t \/ forall {σ}, CtxOK [] (ldom Σ) Σ σ k -> exists t' σ', step t σ t' σ'.
Proof.
  intros Σ t T d eu ek k HT Heff.
  specialize (type_safety HT). intuition. right. intros σ HCtxOK.
  specialize (H0 σ k HCtxOK Heff). destruct H0 as [t' [σ' [Hstep  HPreserve]]].
  exists t'. exists σ'. intuition.
Qed.

Lemma preservation : forall {Σ t T d eu ek k},
    has_type [] (ldom Σ) Σ t T d eu ek ->
    eff_seq_ok ∅ k eu ek ->
    forall{σ}, CtxOK [] (ldom Σ) Σ σ k ->
    forall {t' σ'}, step t σ t' σ' ->
    preserve [] Σ t' T d eu ek σ' k.
Proof.
  intros Σ t T d eu ek k HT Heff σ HCtxOK t' σ' HStep. specialize (type_safety HT). intuition.
  - inversion HStep; subst; inversion H0.
  - specialize (H0 σ k HCtxOK Heff). destruct H0 as [t'' [σ'' [HStep2 HPreserve]]].
    assert (t'' = t' /\ σ' = σ''). { intuition. 1,2: eapply step_deterministic; eauto.  }
    intuition. subst. intuition.
Qed.

(* needs to figure out the statement *)
Corollary preservation_of_separation : forall {Σ t1 T1 q1 eu1 ek1 t2 T2 q2 eu2 ek2 k},
  has_type [] (ldom Σ) Σ t1 T1 q1 eu1 ek1 ->
  has_type [] (ldom Σ) Σ t2 T2 q2 eu2 ek2 ->
  q1 ⊓ q2 ⊑ ∅ ->
  eu1 ⊓ eu2 ⊑ ∅ ->
  ek1 ⊓ ek2 ⊑ ∅ ->
  eff_seq_ok ∅ k eu1 ek1 ->
  eff_seq_ok ∅ k eu2 ek2 ->
  eff_seq_ok eu1 ek1 eu2 ek2 ->
  eff_seq_ok eu2 ek2 eu1 ek1 ->
    forall{σ}, CtxOK [] (ldom Σ) Σ σ k->
      forall {t1' σ'}, step t1 σ t1' σ' ->
      forall {t2' σ''}, step t2 σ' t2' σ'' -> separate Σ t1' T1 t2' T2.
  (* intros. *)
  intros Σ t1 T1 q1 eu1 ek1 t2 T2 q2 eu2 ek2 k HT1 HT2 Hsep Hsepu Hsepk Heff1 Heff2 Heffk Heffk' σ HOK t1' σ' Hstep1 t2' σ'' Hstep2.
  (* execute preservation in sequence *)
  specialize (preservation HT1 Heff1 HOK Hstep1) as P1. destruct P1 as [Σ' d1 eu1'' ek1'' k' eu1' ek1' Hsubu1 Hsubk1 Hext1 Hdisjd1 Hdisjeu1 Hdisjek1 Hsubk' HOK' HeffOk1' Hdisjk' Hdisjek1'' HT1'].
  assert (HT2': has_type [] (ldom Σ') Σ' t2 T2 q2 eu2 ek2). {
    eapply weaken_flt. eapply weaken_store. eauto. auto. apply extends_ldom; auto. auto.
  }
  apply has_type_closed in HT2 as HTcl2. intuition.
  assert (Heff2': eff_seq_ok ∅ (k ⋓ k') eu2 ek2). unfold eff_seq_ok in *. qdisj. eapply qglb_empty_sub; eauto. qdisj. eapply freshness'; eauto.
  specialize (preservation HT2' Heff2' HOK' Hstep2) as P2. destruct P2 as [Σ'' d2 eu2'' ek2'' k'' eu2' ek2' Hsubu2 Hsubk2 Hext2 Hdisjd2 Hdisjeu2 Hdisjek2 Hsubk'' HOK'' HeffOk2' Hdisjk'' Hdisjek2'' HT2''].
  apply (Separate Σ' Σ'' (q1 ⊔ d1) (q2 ⊔ d2) (eu1' ⋓ eu1'') (eu2' ⋓ eu2'') (ek1' ⋓ ek1'') (ek2' ⋓ ek2'') Hext1 Hext2 HT1' HT2'').

(* assert ((q1 ⋓ d1) ⋒ (q2 ⋓ d2) = ∅). *)
unfold eff_seq_ok in *.
assert (q2 ⋒ d1 = ∅). eapply disjointq_qglb_empty; eauto.
assert (q1 ⋒ d2 = ∅). eapply disjointq_qglb_empty; eauto. apply extends_length in Hext1. apply has_type_closed in HT1; intuition. eapply closed_qual_monotone; eauto.
assert (d1 ⋒ d2 = ∅). apply disjointq_closed in Hdisjd1. eapply disjointq_qglb_empty; eauto. qdisj.

(* assert ((eu1' ⋓ eu1'') ⋒ (eu2' ⋓ eu2'') = ∅). *)
unfold eff_seq_ok in *.
assert (eu1' ⋒ eu2' = ∅). eapply qglb_empty_sub; eauto. eapply qglb_empty_sub'; eauto. qdisj.
assert (eu1' ⋒ eu2'' = ∅). eapply has_type_closed in HT1. intuition. eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=eu1); eauto. eapply closed_qual_monotone; eauto. eapply extends_length; eauto.
assert (eu2' ⋒ eu1'' = ∅). eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=eu2); eauto.
qdisj.
assert (eu1'' ⋒ eu2'' = ∅). apply disjointq_closed in Hdisjeu1. eapply disjointq_qglb_empty; eauto.
qdisj.

(* assert ((ek1' ⋓ ek1'') ⋒ (ek2' ⋓ ek2'') = ∅). *)
unfold eff_seq_ok in *.
assert (ek1' ⋒ ek2' = ∅). eapply qglb_empty_sub; eauto. eapply qglb_empty_sub'; eauto. qdisj.
assert (ek1' ⋒ ek2'' = ∅). eapply has_type_closed in HT1. intuition. eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=ek1); eauto. eapply closed_qual_monotone; eauto. eapply extends_length; eauto.
assert (ek2' ⋒ ek1'' = ∅). eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=ek2); eauto.
qdisj.
assert (ek1'' ⋒ ek2'' = ∅). apply disjointq_closed in Hdisjek1. eapply disjointq_qglb_empty; eauto.
qdisj.

(* assert ((ek1' ⋓ ek1'') ⋒ (eu2' ⋓ eu2'') = ∅). *)
unfold eff_seq_ok in *.
assert (ek1' ⋒ eu2' = ∅). eapply qglb_empty_sub; eauto. eapply qglb_empty_sub'; eauto.
assert (ek1' ⋒ eu2'' = ∅). eapply has_type_closed in HT1. intuition. eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=ek1); eauto. eapply closed_qual_monotone; eauto. eapply extends_length; eauto.
assert (eu2' ⋒ ek1'' = ∅). eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=eu2); eauto.
qdisj.
assert (ek1'' ⋒ eu2'' = ∅). apply disjointq_closed in Hdisjek1. eapply disjointq_qglb_empty; eauto.
qdisj.

(* assert ((ek2' ⋓ ek2'') ⋒ (eu1' ⋓ eu1'') = ∅). *)
unfold eff_seq_ok in *.
assert (ek2' ⋒ eu1' = ∅). eapply qglb_empty_sub; eauto. eapply qglb_empty_sub'; eauto.
assert (ek2' ⋒ eu1'' = ∅). eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=ek2); eauto.
assert (eu1' ⋒ ek2'' = ∅). eapply has_type_closed in HT1. intuition. eapply disjointq_qglb_empty; eauto. eapply @closed_qual_sub with (d:=eu1); eauto. eapply closed_qual_monotone; eauto. eapply extends_length; eauto.
assert (eu1'' ⋒ ek2'' = ∅). apply disjointq_closed in Hdisjeu1. eapply disjointq_qglb_empty; eauto.
qdisj.
Qed.
