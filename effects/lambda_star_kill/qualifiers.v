...
Lemma qlub_empty_left' : forall {d1 d2}, (d1 ⊔ d2) = ∅ -> d1 = ∅.
Proof. 
  intros. destruct d1, d2. simpl in *. inversion H. rewrite H1 in H2. rewrite H2 in H3. rewrite H3. symmetry in H1,H2,H3. apply union_empty_inv in H1,H2,H3. f_equal; intuition. 
Qed.

Lemma qlub_empty_right' : forall {d1 d2}, (d1 ⊔ d2) = ∅ -> d2 = ∅.
Proof. 
  intros. destruct d1, d2. simpl in *. inversion H. rewrite H1 in H2. rewrite H2 in H3. rewrite H3. symmetry in H1,H2,H3. apply union_empty_inv in H1,H2,H3. f_equal; intuition. 
Qed.
...
Lemma subqual_qqplus_r' : forall {d1 d2 d}, d1 ⊑ d2 -> d1 ⊑ d2 ⋓ d.
  destruct d1. destruct d2. destruct d. qdec.
Qed.
...
Lemma qqcap_sub_r : forall {q1 q2}, q2 ⋒ q1 ⊑ q1.
  intros. destruct q1. destruct q2. simpl. intuition.
Qed.
Lemma qglb_empty_sub: forall {d1 d2 d3},
  d1 ⊑ d2 ->
  d2 ⋒ d3 = ∅ ->
  d1 ⋒ d3 = ∅.
Proof.
  intros. destruct d1,d2,d3. simpl in *. inversion H0. intuition. rewrite H2 in H3. rewrite H3 in H4. repeat rewrite H4. f_equal; erewrite <- subset_of_empty; eauto.
  apply NatSetFacts.Subset_trans with (s':=(inter t3 t6 )). eapply NatSetFacts.inter_s_m; fnsetdec. rewrite H2. fnsetdec.
  apply NatSetFacts.Subset_trans with (s':=(inter t4 t7 )). eapply NatSetFacts.inter_s_m; fnsetdec. rewrite H3. fnsetdec.
  apply NatSetFacts.Subset_trans with (s':=(inter t5 t8 )). eapply NatSetFacts.inter_s_m; fnsetdec. rewrite H4. fnsetdec.
Qed.
...
Proof.
  intros. destruct d1,d2,d3. simpl in *. inversion H0. intuition. rewrite H2 in H3. rewrite H3 in H4. repeat rewrite H4. f_equal; erewrite <- subset_of_empty; eauto.
  apply NatSetFacts.Subset_trans with (s':=(inter t0 t6 )). eapply NatSetFacts.inter_s_m; fnsetdec. rewrite H2. fnsetdec.
  apply NatSetFacts.Subset_trans with (s':=(inter t1 t7 )). eapply NatSetFacts.inter_s_m; fnsetdec. rewrite H3. fnsetdec.
  apply NatSetFacts.Subset_trans with (s':=(inter t2 t8 )). eapply NatSetFacts.inter_s_m; fnsetdec. rewrite H4. fnsetdec.
Qed.
