...
THIS FILE:
     ...
     - beta rule (arguements has the emtpy fitler and no use effects, and returns boolean values )
...
Fixpoint val_type (M:stty) (H1 H2:venv) v1 v2 T1 T2 : Prop :=
  ...
  | vabs G1 py1 ty1, vabs G2 py2 ty2, TFun T1 (* qempty *) T2 q2 e2, TFun T1' (* qempty *) T2' q2' e2' =>
        ...
        (forall S1' S2' M' vx1 vx2,
            ...
            ->
            length2 M <= length2 M'   (* needed for applying IH in st_subst *)
            ->    
            store_type S1' S2' M'
            ->
            val_type
...
Lemma has_type_closed: forall M H1 H2 G p t T q e,
  env_type M H1 H2 G (plift p) -> 
...
Proof. 
  ...
  split. intros ? ? ? ?. eauto.   
   ...
   rewrite val_locs_abs in H7. rewrite plift_and in H7.
   assert (env_type
        M'
        (vx1 :: H1) (vx2 :: H2) 
        (T1 :: G)
   ...
   destruct VL. {
     ...
     rewrite plift_one in H10. unfoldq. intuition.
   }
   eauto.
   ...
   destruct VL. {
     ...
     right. destruct H9. replace x with (length H2) in H9.
     rewrite indexr_head in H9. intuition. congruence.
     rewrite plift_one in H10. unfoldq. intuition.
   }
   eauto.
  ...
  (* from func *)
    ...
    exists x0. intuition. 
    destruct H14 as [? [? ?]]. 
    rewrite indexr_skip in H14. exists x1. split; eauto. lia.
    exists x0. split.
    destruct H11; try lia. eapply H11.
...
Proof.
  ...
  intros ? [? ?]. unfoldq. intuition.
  intros ? [? ?]. unfoldq. intuition. 
  eapply stchain_tighten'. eapply stchain_empty'. rewrite plift_empty. rewrite vars_locs_empty. unfoldq; intuition.
  rewrite plift_empty. rewrite vars_locs_empty. unfoldq; intuition.
  unfold st_empty, length2. simpl. lia.
  eapply storet_empty.
Qed.
...
Theorem store_invariance2 : forall t G T p p' q e
  ...
  forall S1', store_effect S1 S1' p' ->
                 length S1 <= length S1' ->
  exists S1'' S2' M' v1 v2,
  exp_type S1' S2 (length S1', length S2, strel M) H1 H2 t t S1'' S2' M' v1 v2 T (plift p) (plift q) (plift e).
Proof.
  intros. 
  eapply fundamental_property; eauto.
  eapply envt_store_change. eauto.
  ...
  unfold st_locs2, pnat in H8.
  destruct H0. lia.
  assert (length2 M = length S2). destruct H0; intuition.
  rewrite H6. 
  unfold length2. simpl. lia.
...
Proof.
  ...
  2: { eapply envt_store_change; eauto.
       subst Q. assert (length2 M = length S2). destruct H0; intuition.
       rewrite H7. unfold length2. simpl. lia.    
     }
...
Proof.
  ...
  2: { eapply envt_store_change; eauto. 
       subst Q. simpl in *. assert (length2 M = length S2). destruct H0; intuition.
       rewrite H7. unfold length2. simpl. lia.    }
  2: { subst Q. destruct H0 as (? & ? & ? & ? & ?).
       split. unfold length1. simpl. eauto.
       split. unfold length2. simpl. eauto.
