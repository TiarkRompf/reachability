...
THIS FILE:
     ...
     - make st_chain mechanism localized (from ../pub/...mut_nested.v)

     - prove localization lemmas for values and environments
       (valt_store_change, envt_store_change)

     - localization for expression evaluation (store_invariance)

     - prove reordering: equivalence of tseq t1 t2 = tseq t2 t1 (require empty p for both exprs)
...
| t_seq: forall env t1 t2 p q1 q2 e1 e2,
    (* for now, require empty p for both exprs *)
    has_type env t1 TBool qempty q1 e1 ->
    has_type env t2 TBool qempty q2 e2 ->
...
Definition st_chain (M:stty) (M1:stty) :=
  st_chain_partial M M1 (st_locs1 M) (st_locs2 M) /\
    st_chain_partial M1 M (st_locs1 M) (st_locs2 M).
...
Proof.
  ...
  destruct (length2 M). lia. eapply H4. lia. 
Qed.
...
Proof.
  intros. unfold st_extend, st_chain, st_chain_partial, strel. 
  simpl. intuition.
  intros ? ?. eauto.
  intros ? ?. unfold st_locs1, length1, pnat in *. simpl. eauto.
  intros ? ?. eauto.
  intros ? ?. unfold st_locs2, length2, pnat in *. simpl. eauto.
  intros ? ?. unfold st_locs1, length1, pnat in *. simpl. eauto.
  intros ? ?. eauto.
  intros ? ?. unfold st_locs2, length2, pnat in *. simpl. eauto.
  intros ? ?. eauto.
  unfold st_locs1, st_locs2, pnat in *. lia.
  unfold st_locs1, st_locs2, pnat in *. lia. 
Qed.
...
Proof.  
  ...
  + eapply H0. rewrite val_locs_ref. unfoldq. eauto. 
  + eapply H0. rewrite val_locs_ref. unfoldq. eauto. eauto.
  + eapply H0. 
  + eapply H0. 
...
Proof.
  ...
  eapply stchain_tighten. eapply H0.
...
Proof. 
  ...
  *)
  ...
  - (* Abs shrink *)
    ...
    rewrite vars_locs_extend in H35; eauto.
...
Proof.
  ...
  assert (l1 < length1 M). { eapply indexr_var_some' in H14. destruct ST as (L & ?). lia. }
  intros ?. eapply H11. destruct H16. {
  ...
  assert (l1 < length2 M). { eapply indexr_var_some' in H14. destruct ST as (L & ?). lia. }
  intros ?. eapply H11. destruct H16. {
...
Proof. 
  ...
  split. intros ? ? ? ?. eauto.   
   ...
   destruct VL. {
     ...
     right. destruct H7. replace x with (length H1) in H7.
     rewrite indexr_head in H7. intuition. congruence.
     rewrite plift_one in H8. unfoldq. intuition.
   }
   eauto.
   ...
   destruct VL. {
     ...
     right. destruct H7. replace x with (length H2) in H7.
     rewrite indexr_head in H7. intuition. congruence.
     rewrite plift_one in H8. unfoldq. intuition.
   }
   eauto.
  ...
  rewrite val_locs_abs in *. rename H3 into HVX;
...
Lemma bi_seq: forall S1 S2 M H1 H2 t1 t2 S1' S2' M' M'' S1'' S2'' v1 v2 v3 v4 p q1 q2 e1 e2 t3 t4
 (E1: exp_type S1 S2 M H1 H2 t1 t2 S1' S2' M' v1 v2 TBool pempty q1 e1)
 (E2: exp_type S1' S2' M' H1 H2 t3 t4 S1'' S2'' M'' v3 v4 TBool pempty q2 e2),
 exists S1''' S2''' M'' v5 v6,
...
Proof.
  ...
  split. eapply se_sub. eapply se_trans; eauto. intros ? (? & [] & ?).
  split. eapply se_sub. eapply se_trans; eauto. intros ? (? & [] & ?).
...
Proof.
  ...
  - (* Case tseq *)
    ...
    assert (env_type M H1 H2 env pempty) as WFE0. eapply envt_tighten; eauto. unfoldq. intuition.     
    specialize (IHW1 M H1 H2 WFE0 S1 S2 ST) as [S1' [S2' [M' [v1 [v2 IE]]]]].
    assert (st_chain M M'). eapply IE. 
    assert (env_type M' H1 H2 env pempty) as WFE1. eapply envt_store_extend; eauto. 
    assert (store_type S1' S2' M') as ST'. eapply IE.
    specialize (IHW2 M' H1 H2 WFE1 S1' S2' ST') as [S1'' [S2'' [M'' [v3 [v4 IE2]]]]].
    eapply bi_seq; eauto. 
  - (* Case qsub  *)
...
Proof.
  ...
  - (* t_seq *)
    intros ? ? ? WFE ? ? ST. subst t0 t3.

    assert (exists S1' S1x' M' v1 v1x, exp_type S1 S1 (st_empty S1 S1) H2 H3 t1 t1 S1' S1x' M' v1 v1x TBool (plift qempty) (plift q1) (plift e1)) as A. {    
      eapply store_invariance in W1. 2: eapply envt_tighten; eauto. 2: unfoldq; intuition. eauto. }

    destruct A as (S1' & S1x' & M' & v1 & v1x & E1 & E1x & ? & ? & SE1 & SE1x & ? & ? & ?). 
    
    assert (exists S1'' S2' M'' v2 v2x, exp_type S1' S2 (st_empty S1' S2) H2 H3 t2 t2 S1'' S2' M'' v2 v2x TBool (plift qempty) (plift q2) (plift e2)) as B. {
      eapply store_invariance in W2. 2: eapply envt_tighten; eauto. 2: unfoldq; intuition. eauto. }

    destruct B as (S1'' & S2' & M'' & v2 & v2x & E2' & E2 & ? & ? & SE2' & SE2 & ? & ? & ?).

    assert (exists S1y' S2'' M''' v1y v1z, exp_type S1 S2' (st_empty S1 S2') H2 H3 t1 t1 S1y' S2'' M''' v1y v1z TBool (plift qempty) (plift q1) (plift e1)) as C. {    
      eapply store_invariance in W1. 2: eapply envt_tighten; eauto. 2: unfoldq; intuition. eauto. }
    ...
    destruct v1; destruct v1x; destruct v1z; simpl in H1; simpl in H13; intuition. subst b0 b1.
    destruct v2; destruct v2x; simpl in H8; intuition. subst b0.
    ...
    rewrite H13 in *. 
(*
   H6 : st_chain (st_empty S1' S2) M''
   H11 : st_chain (st_empty S1 S2') M'''
...
*)    
    ...
    assert (length S1 <= length S1'). eapply st_mono1 in H11. rewrite R1 in H11. lia.
    assert (length S2 <= length S2'). eapply st_mono2 in H6. rewrite R2 in H6. lia.
    assert (length S1' <= length S1''). eapply st_mono1 in H6. rewrite R1 in H6. lia.
    assert (length S2' <= length S2''). eapply st_mono2 in H11. rewrite R2 in H11. lia.
    ...
    assert (st_chain M MM). {
      subst MM. unfold st_chain. unfold st_chain_partial. intuition. 
      + unfoldq; intuition.
      + unfoldq; intuition. unfold st_locs1, pnat, length1. simpl.
        unfold st_locs1, pnat in H30. lia. 
      + unfoldq; intuition.
      + unfoldq; intuition. unfold st_locs2, pnat, length2. simpl.
        unfold st_locs2, pnat in H30. lia.
      + simpl. eauto. 
        (* second half *)
      + simpl. eauto. 
      + unfoldq. intuition. unfold st_locs1, pnat, length1. simpl.
      ...
      + unfoldq. intuition.
      + simpl in *.
        destruct H34 as [A | [B | C]].
        eauto.
        eapply H11 in B. simpl in *. contradiction. left. unfold st_locs1, st_empty, length1 in *. simpl. unfoldq. lia.
        eapply H6 in C. simpl in *. contradiction. left. unfold st_locs1, st_empty, length1 in *. simpl. unfoldq. lia.
      + simpl in *.
        destruct H34 as [A | [B | C]].
        eauto.
        eapply H11 in B. simpl in *. contradiction. right. unfold st_locs2, st_empty, length2 in *. simpl. unfoldq. lia.
        eapply H6 in C. simpl in *. contradiction. right. unfold st_locs2, st_empty, length2 in *. simpl. unfoldq. lia.
    }
    ...
    assert (forall l1 l2,
               ...
               strel M''' l1 l2 ->
               strel (st_empty S1 S2') l1 l2) as CES'''. {
      intros. eapply H11. unfold st_locs1, st_locs2, length1, length2, st_empty. simpl. unfoldq. lia. eauto. 
    }
    assert (forall l1 l2,
               l1 < length S1' \/ l2 < length S2 ->
               strel M'' l1 l2 ->
               strel (st_empty S1' S2) l1 l2) as CES''. {
      intros. eapply H6. unfold st_locs1, st_locs2, length1, length2, st_empty. simpl. unfoldq. lia. eauto. 
    }
    ...
    assert (store_type S1'' S2'' MM). {
      ...
      - simpl in H30. destruct H30 as [A | [B | C]].
        + destruct (H27 l1 l2) as (b0 & b2 & ? & ? & ?). eauto.
          exists b0, b2. split. 2: split.
          eapply se_trans. eapply SE1. eauto.
          intros (? & ?). unfoldq. intuition.
          eauto.
          eapply se_trans. eapply SE2. eauto. 
          intros (? & ?). unfoldq. intuition.
          eauto.
          eauto.
        + destruct (H20 l1 l2) as (b0 & b2 & ? & ? & ?). eauto.
          exists b0, b2. split. 2: split. 
          eapply SE2'. 
          intros (? & ?). unfoldq. intuition.
          eauto.
          eauto.
          eauto. 
        + destruct (H18 l1 l2) as (b0 & b2 & ? & ? & ?). eauto.
          exists b0, b2. split. 2: split.
          eapply H30. 
          eapply SE2''.
          intros (? & ?). unfoldq. intuition.          
          ...
          eauto. 
      - simpl in H30, H34.
        destruct H30 as [A | [B | C]]; destruct H34 as [U | [V | W]].
        + eauto.
        + eapply CES''' in V. simpl in V. contradiction.
          left. destruct (H27 l1 l2) as (b0 & b2 & IX1 & IX2 & ?); eauto.
          eapply indexr_var_some' in IX1. eauto.
        + eapply CES'' in W. simpl in W. contradiction.
          left. destruct (H27 l1 l2) as (b0 & b2 & IX1 & IX2 & ?); eauto.
          eapply indexr_var_some' in IX1. lia.
          
        + eapply CES''' in B. simpl in B. contradiction.
          left. destruct (H27 l1 l2') as (b0 & b2 & IX1 & IX2 & ?); eauto.
          eapply indexr_var_some' in IX1. eauto.
        + eauto.
        + eapply CES'' in W. simpl in W. contradiction.
          left. destruct (H20 l1 l2) as (b0 & b2 & IX1 & IX2 & ?); eauto.
          eapply indexr_var_some' in IX1. eauto.
          
        + eapply CES'' in C. simpl in C. contradiction.
          left. destruct (H27 l1 l2') as (b0 & b2 & IX1 & IX2 & ?); eauto.
          eapply indexr_var_some' in IX1. lia.
        + eapply CES'' in C. simpl in C. contradiction.
          left. destruct (H20 l1 l2') as (b0 & b2 & IX1 & IX2 & ?); eauto.
      ...
      - simpl in H30, H34.
        ...
        + eauto.
        + eapply CES''' in B. simpl in B. contradiction.
          right. destruct (H18 l1' l2) as (b0 & b2 & IX1 & IX2 & ?); eauto.
    ...
    + eauto. 
    + eauto.
    + eapply se_sub. eapply se_trans. eauto. eauto. intros ? (? & ?). unfoldq. intuition.
    + eapply se_sub. eapply se_trans. eauto. eauto. intros ? (? & ?). unfoldq. intuition. 
    + simpl. eauto.
    + eapply valq_bool.
    + eapply valq_bool. 
...
Lemma bi_tseq2: forall G t1 t2 t3 t4 p q1 q2 e1 e2,
  sem_type G t1 t2 TBool (plift qempty) q1 e1 ->
  sem_type G t3 t4 TBool (plift qempty) q2 e2 ->
  sem_type G (tseq t1 t3) (tseq t2 t4) TBool p pempty (por e1 (por e2 q1)).
Proof. 
  intros.
  intros M H1 H2 WFE S1 S2 ST.
  unfold sem_type in H. unfold sem_type in H0.
  assert (env_type M H1 H2 G (plift qempty)) as WFE0. eapply envt_tighten; eauto. unfoldq. intuition. 
  destruct (H M H1 H2 WFE0 S1 S2 ST) as [S1' [S2' [M' [v1 [v2 IE]]]]].
  assert (st_chain M M'). eapply IE. 
  assert (env_type M' H1 H2 G (plift qempty)) as WFE1. eapply envt_store_extend; eauto.
  assert (store_type S1' S2' M') as ST1. eapply IE.
  destruct (H0 M' H1 H2 WFE1 S1' S2' ST1) as [S1'' [S2'' [M'' [v3 [v4 IE2]]]]].
  eapply bi_seq; rewrite <-plift_empty; eauto. 
Qed.
...
| cx_seq1: forall G t1 p q1 e1 e2 q2,
    has_type G t1 TBool qempty q1 e1 ->
    ctx_type (fun x => tseq t1 x) G TBool (plift qempty) q2 e2 G TBool (plift p) pempty (por (plift e1) (por e2 (plift q1)))
| cx_seq2: forall G t2 p q1 e1 e2 q2,
    has_type G t2 TBool qempty q2 e2 ->
    ctx_type (fun x => tseq x t2) G TBool (plift qempty) q1 e1 G TBool (plift p) pempty (por e1 (por (plift e2) q1))
