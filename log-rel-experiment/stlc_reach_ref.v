...
| t_app: forall env f t T1 T2 p qf q1 q2,
    ...
    psub (plift q2) (plift p) ->     (* XXX get rid of this XXX *)
...
Fixpoint val_type (M:stor) (H:venv) v T : Prop :=
  ...
  | vref l, TRef T => 
     ...
     (exists vx,
      indexr l M = Some vx /\
        val_type M H vx T)
  | vabs G py ty, TFun T1 (* qempty *) T2 q2 =>
...
Proof. 
  intros T. induction T; intros; destruct v; simpl in *; intuition.
  + econstructor.
  + destruct H2 as [? [? ?]]. econstructor. eapply IHT. eapply H2.
  + econstructor; eauto. 
Qed.
...
Proof.
  ...
  - (* Abs shrink *)
    inversion H0. subst.
    rename q into q2. 
    destruct (H6 M' vx) as [M'' [vy [HEY HVY]]].
      eapply IHT1; eauto.
    eauto.
    exists M'', vy. intuition.
    eapply IHT2; eauto.
    rewrite vars_locs_extend in H9.
    eauto. eauto.
  - eapply closedty_extend; eauto.
  - eapply closedty_extend; eauto.
  - unfoldq. rewrite app_length. intuition. eapply H3 in H5. lia.
  - (* Abs grow *)
    inversion H0. subst.
    rename q into q2. 
    destruct (H6 M' vx) as [M'' [vy [HEY [HVY HQY]]]]. 
      eapply IHT1; eauto.
      eauto.
    exists M'', vy. intuition.
    eapply IHT2; eauto.
...
Lemma sem_app1: forall M M' M'' H Hf G T1 T2 ey vx pv p q1 q2 qf
    (WFE: env_type M H G p)
    (HVF: val_type (M'++M) H (vabs Hf pv ey) (TFun T1 T2 q2))
    (HQF: val_qual M H (vabs Hf pv ey) p qf)
    (HVX: val_type (M''++M'++M) H vx T1)
    (HQX: val_qual (M'++M) H vx p q1),
    psub (pand qf q1) pempty ->
    psub (plift q2) p  -> (* XXX get rid of this XXX *)
