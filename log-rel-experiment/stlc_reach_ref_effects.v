...
| t_app2: forall env f t T1 T2 p qf q1 q2 ef e1 e2,
    has_type env f (TFun T1 (* qempty  *) T2 q2 e2) p qf ef ->  
    has_type env t T1 p q1 e1 ->
    psub (pand (plift qf) (plift q1)) pempty ->          (* no overlapping *)
    has_type env (tapp f t) T2 p (qor qf q1) (qor ef (qor e1 e2))
...
Definition exp_type M H t M' v T p q (e: pl) :=
  tevaln M H t (M'++M) v /\
    val_type (M'++M) H v T /\
    val_qual M H v p q.
...
Lemma sem_app1: forall M M' M'' H Hf G T1 T2 ey vx pv p q1 q2 qf e2
    (WFE: env_type M H G p)
...
(* 
   ...
   On the surface it may seem yes. Consider this example:

      In an environment, mapping var -> locations:
      
        a -> l   (not observable)
        b -> l

      We could have a function that returns location l  with type:

        f: ( Bool -> T2^a )^b

      So, the function would reach l through b, and the return 
      value would reach l through a, all the while {a} /\ {b} = {}.

      It seems like the current val_type/val_qual rules don't
      prevent this -- a is not observable, so no separation.
      
   However, such as function cannot exist -- it cannot have
   been created. If it truly returns l, then it must do so
   through a variable originally captured via b.

   How can we strengthen our invariants to reflect this?

   Or are we overlooking something and we can already prove it?

----------------------

   Counterexample - argument above does not consider that
   by passing through the env, the functions qualifier
   (drastically) changes.

   it now seems we can typecheck the result
   of the application with only qf, but we can't take
   the intersection of q2 and qf:

   let x = new Ref(1)
   let y = new Ref(2)
   def f() = x  // : ( () -> T^x )^x
   def g(f1: ( () -> T^x )^{} ) =    // : ( (..) -> ... )^{}
     f1() // : T^x <-- not allowed b/c x not in filter {}
          // or
          // : T^f1
          // and we can't do the intersection {x} /\ {f1}
          // because that would be empty
...
*)
  ...
  assert (val_qual M H vy p (pand p (por (plift q2) q1))) as A. {
...
Lemma sem_app2: forall M M' M'' H Hf G T1 T2 ey vx pv p q1 q2 qf e2
    (WFE: env_type M H G p)
    (HVF: val_type (M'++M) H (vabs Hf pv ey) (TFun T1 T2 q2 e2))
    (HQF: val_qual M H (vabs Hf pv ey) p qf)
    (HVX: val_type (M''++M'++M) H vx T1)
...
Lemma sem_abs1: forall M M1 H G T1 T2 t vx p q2 qf e2
...
Theorem full_total_safety : forall t G T p q e,
  has_type G t T p q e -> forall M E, env_type M E G (plift p) ->
  exists M' v, exp_type M E t M' v T (plift p) (plift q) (plift e).
Proof.
  intros ? ? ? ? ? ? W. 
...
Corollary safety : forall t T q e,
  ...
  exists M v, exp_type [] [] t M v T (plift qempty) (plift q) (plift e).
Proof. 
