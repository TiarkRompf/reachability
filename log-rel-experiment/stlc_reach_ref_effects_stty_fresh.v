...
Inductive ty : Type :=
  | TBool  : ty
  | TRef   : ty -> ty
  | TFun   : ty -> (* qempty -> *) ty -> ql -> ql -> bool -> ty
.
...
| c_ref: forall m T,
    closed_ty m T ->
    closed_ty m (TRef T) 
| c_fun: forall m T1 T2 q2 e2 fe2,
    closed_ty m T1 ->
    closed_ty m T2 ->   (* not dependent *)
    closed_ql m q2 ->
    closed_ql m e2 ->   
    closed_ty m (TFun T1 (* qempty *) T2 q2 e2 fe2)
.


(* assume no overlap *)
Inductive has_type : tenv -> tm -> ty -> ql -> ql -> ql -> bool -> Prop :=
| t_true: forall env p,
    has_type env ttrue TBool p qempty qempty false   (* nothing reachable from a primitive *)
| t_false: forall env p,
    has_type env tfalse TBool p qempty qempty false
| t_var: forall x env T  p,
    indexr x env = Some T ->
    (plift p) x ->                         (* x in phi *)
    has_type env (tvar x) T p (qone x) qempty false (* x can reach only itself (no overlap) *)
| t_ref: forall t env p q e fe,
    has_type env t TBool p q e fe ->
    has_type env (tref t) (TRef TBool) p q e true
| t_get: forall t env p q e fe,
    has_type env t (TRef TBool) p q e fe ->
    has_type env (tget t) TBool p qempty (qor e q) fe
| t_put: forall t1 t2 env p q1 e1 fe1 q2 e2 fe2,
    has_type env t1 (TRef TBool) p q1 e1 fe1 ->
    has_type env t2 TBool p q2 e2 fe2 ->
    has_type env (tput t1 t2) TBool p qempty (qor e1 (qor e2 q1)) (fe1 || fe2) 
| t_app: forall env f t T1 T2 p qf q1 fe1 q2 ef e1 e2 fe2 fef,
    has_type env f (TFun T1 (* qempty  *) T2 q2 e2 fe2) p qf ef fef ->  
    has_type env t T1 p q1 e1 fe1 ->
    psub (pand (plift qf) (plift q1)) pempty ->          (* no overlapping *)
    psub (plift q2) (plift p) ->
    psub (plift e2) (plift p) ->
    has_type env (tapp f t) T2 p (qor q2 q1) (qor ef (qor e1 (qor q1 e2))) (fef || fe1 || fe2)
| t_abs: forall env t T1 T2 p q2 qf e2 fe2, 
    has_type (T1::env) t T2 (qor (qand p qf) (qone (length env))) (qor q2 (qone (length env))) (qor e2 (qone (length env))) fe2 ->
    closed_ty (length env) T1 ->
    closed_ty (length env) T2 ->
    closed_ql (length env) q2 ->
    closed_ql (length env) qf ->
    closed_ql (length env) e2 ->
    has_type env (tabs (qand p qf) t) (TFun T1 T2 q2 e2 fe2) p qf qempty false
| t_sub: forall env y T p q1 q2 e1 fe1 e2,
    has_type env y T p q1 e1 fe1 ->
    psub (plift q1) (plift q2) ->
    psub (plift q2) (pdom env)  ->
    psub (plift e1) (plift e2) ->
    psub (plift e2) (pdom env)  ->
    has_type env y T p q2 e2 fe1
.
...
Fixpoint val_type (M:stty) (H:venv) v T : Prop :=
  ...
  | vabs G py ty, TFun T1 (* qempty *) T2 q2 e2 fe2 =>
        ...
        (forall S' M' vx,
            ...
            ->
              exists S'' M'' vy,
                ...
                /\
               (store_type S'' (if fe2 then (M''++M'++M) else (M'++M)))
                /\
                  val_type
                    (if fe2 then (M''++M'++M) else (M'++M))
                    H
                    vy
                    T2
                /\
...
Definition exp_type S M H t S' M' v T p q (e: pl) (fe:bool):=
  tevaln S H t S' v /\
  store_effects S S' (vars_locs H e) /\
  (store_type S' (if fe then (M'++M) else M)) /\
  val_type (if fe then (M'++M) else M) H v T /\
  val_qual M H v p q.
...
Lemma sem_app1: forall S S' S'' M M' M'' H Hf G T1 T2 ey vx pv p q1 q2 qf e2 (fe1 fef fe2:bool)
    (WFE: env_type M H G p)
    (ST: store_type S M)
    (ST1: store_type S' (if fef then (M'++M) else M))
    (HVF: val_type (if fef then (M'++M) else M) H (vabs Hf pv ey) (TFun T1 T2 q2 e2 fe2))
    (HQF: val_qual M H (vabs Hf pv ey) p qf)
    (HVX: val_type (if fe1 then M'' ++  (if fef then M' ++ M  else M) else if fef then M' ++ M  else M) H vx T1)
    (HQX: val_qual (if fef then (M'++M)  else M) H vx p q1),
    psub (pand qf q1) pempty ->
    psub (plift q2) p  -> 
    psub (plift e2) p ->
    (* exists vy, exp_type H ey vy T2 p q2. *) (* not exactly this!! *)
    store_type S'' (if fe1 then M'' ++  (if fef then M' ++ M  else M)  else  if fef then M' ++ M else M) ->
    exists S''' M''' vy,
      tevaln S'' (vx::Hf) ey S''' vy /\
        store_effects S'' S''' (por (val_locs vx ) (vars_locs H (plift e2))) /\ 
        store_type S''' (if fe2 then (M''' ++ (if fe1 then M'' ++  (if fef then M' ++ M  else M)  else  if fef then M' ++ M else M)) else (if fe1 then M'' ++  (if fef then M' ++ M  else M)  else  if fef then M' ++ M else M))/\ 
        val_type (if fe2 then (M''' ++ (if fe1 then M'' ++  (if fef then M' ++ M  else M)  else  if fef then M' ++ M else M)) else (if fe1 then M'' ++  (if fef then M' ++ M  else M)  else  if fef then M' ++ M else M)) H vy T2 /\
        val_qual M H vy p (por (plift q2) q1).
Proof. 
  intros. apply valt_wf in HVF as WFQF. apply valt_wf in HVX as WFQX.
  destruct HVF; intuition.  
  rename H10 into HVF.
  destruct (HVF S'' (if fe1 then M'' else []) vx) as [S''' [M''' [vy [IW3 [HEFF [HST [HVY HQY]]]]]]].
  (* store typing*)
  destruct fe1; destruct fef; intuition.
  (* val_type *)
  destruct fe1; destruct fef; intuition.
  ...
  *)
  ...
  (* TODO 2: SEPARATION *)
  destruct fef eqn: Heqn.
  subst fef. eapply separation; eauto.
  subst fef. eapply separation with (M' := []); eauto.
  ...
  *)
  ...
  (* store typing *)
  split. 
  destruct fe1; destruct fe2; destruct fef; intuition.
  ...
  *)
  ...
  destruct (HQY x). 
  destruct fe1; destruct fef; intuition.
  1-3: repeat rewrite app_length; intuition.
  + (* part of function *)
    destruct (HQF x). intuition.
...
Lemma sem_abs1: forall S M M1 H G T1 T2 t vx p q2 qf e2 fe2
    ...
    (IHW : (* induction *)
        ...
        exists (S'' : stor) (M'': stty) (vy : vl),
          exp_type S (M1 ++ M) (vx :: H) t S'' M'' vy T2 (plift (qor (qand p qf) (qone (length H))))
            (plift (qor q2 (qone (length H)))) (plift (qor e2 (qone (length H)))) fe2) 
  ...
  exists (S'': stor) (M'' : stty) (vy : vl),
    tevaln S (vx :: H) t S'' vy /\
      store_effects S S'' (por (val_locs vx) (vars_locs H (plift e2)))  /\
      (store_type S'' (if fe2 then (M'' ++ M1 ++ M) else (M1 ++ M))) /\
      val_type (if fe2 then (M'' ++ M1 ++ M) else (M1 ++ M)) H vy T2 /\
...
Theorem full_total_safety : forall t G T p q e fe,
    has_type G t T p q e fe ->
    forall M E, env_type M E G (plift p) ->
    forall S, store_type S M ->
    exists S' M' v, exp_type S M E t S' M' v T (plift p) (plift q) (plift e) fe.
Proof.
  intros ? ? ? ? ? ? ? W. 
  ...
  - (* Case "Ref". *)
    ...
    remember (fun v => val_type M E v TBool) as vt_no_fresh.
    destruct fe eqn:Heqn. 
    ++ subst fe. 
    (* has fresh effects *)
      ...
      + eapply valq_fresh.
    ++ (* no fresh effects*)
       exists (vx::S1). exists [vt_no_fresh]. exists (vref (length M)). split. 2: split. 3: split.  4: split.
       + (* expression evaluates *)
        eapply storet_length in ST1.
        destruct IW1 as [n1 IW1].
        rename S into Sx.
        ...
        simpl. rewrite IW1. rewrite ST1. eauto. lia.
      + (* effects *)
        eapply fresh_no_harm; eauto. 
        unfoldq; intuition. destruct ST as [L1 ?]. destruct ST1 as [L2 ?].
        lia.
      + (* store typing *)
        simpl.
        subst vt_no_fresh. eapply storet_extend; eauto.

      + (* result well typed *)
        constructor. constructor.
        exists vt_no_fresh; intuition.
        simpl. bdestruct (length M =? length M); intuition.
        subst vt_no_fresh. eauto. subst vt_no_fresh. eauto. 
      + eapply valq_fresh1.
  ...
  - (* Case "Put" *)
    ...
    assert (env_type (if fe1 then (M1++M) else M) E env (plift p)) as WFE1. 
    destruct fe1; intuition.
    eapply envt_store_extend. eauto.
       
    destruct (IHW2 (if fe1 then (M1++M) else M) E WFE1 S1) as [S2 [M2 [vx [IW2 [EFF2 [ST2 [HVX HQX]]]]]]]; eauto.
    ...
    destruct fe2; destruct fe1; intuition.
    rewrite indexr_extend with (H' := M2) in MI. eapply MI.
    rewrite indexr_extend with (H' := M2) in MI. eapply MI.
    exists (update S2 i vx).
    exists (if fe1 || fe2 then (if fe2 then M2 ++ (if fe1 then M1 else []) else if fe1 then M1 else M) ++ [] else []).
    ...
    + (* effects *) 
      ...
      destruct fe1; intuition.
      destruct fe1; destruct fe2; intuition.
      destruct fe1; destruct fe2; intuition.
      eapply valt_store_extend; eauto.
      eapply valt_store_extend; eauto.
   
    + assert (val_type (if fe1 || fe2 then (if fe1 || fe2 then if fe2 then M2 ++ (if fe1 then M1 else [])  else if fe1 then M1 else M else []) ++ M else M) E vy TBool) as HVY.
      ...
      repeat rewrite app_nil_r. 
      destruct fe1; destruct fe2; simpl; intuition.
      rewrite <- app_assoc. destruct H4 as [vt' ?].
      exists vt'; intuition. rewrite indexr_extend in H6. eapply H6. 
      destruct H4 as [vt' ?]; intuition. 
      exists vt'; intuition. rewrite app_nil_r. rewrite indexr_extend in H6. eapply H6. 
  ...
  - (* Case "App". *)
    ...
    assert (env_type (if fef then (M1++M) else M) E env (plift p)) as WFE1. 
    destruct fef; intuition.
    eapply envt_store_extend. eauto.
    destruct (IHW2 (if fef then (M1++M) else M) E WFE1 S1) as [S2 [M2 [vx [IW2 [EFF2 [ST2 [HVX HQX]]]]]]]; eauto.
    ...
    assert (exists S3 M3 vy,
               tevaln S2 (vx::l) t0 S3 vy /\
                 ...
                 store_type S3 (if fe2 then (M3 ++ (if fe1 then M2 ++  (if fef then M1 ++ M  else M)  
else  if fef then M1 ++ M else M)) else (if fe1 then M2 ++  (if fef then M1 ++ M  else M)  else  if fef then M1 ++ M else M)) /\
                 val_type (if fe2 then (M3 ++ (if fe1 then M2 ++  (if fef then M1 ++ M  else M)  
else  if fef then M1 ++ M else M)) else (if fe1 then M2 ++  (if fef then M1 ++ M  else M)  else  if fef then M1 ++ M else M)) E vy T2 /\
    ...
    (* result *)
    exists S3. exists (if fe2 then M3 ++ (if fe1 then M2 ++ (if fef then M1  else [])  else if fef then M1 else []) else if fe1 then  M2 ++ (if fef then M1  else []) else if fef then M1  else []). 
    exists vy. split. 2: split.
    + (* expression evaluates *)
      (* - initial fuel value *)
      destruct IW1 as [n1 IW1].
    ...
    + (* effects *) 
      repeat rewrite plift_or.
      eapply effects_app. eapply ST.
      eapply ST1. destruct fef. rewrite app_length. lia. auto.
      eapply ST2.
      destruct fe2; destruct fe1; destruct fef; intuition.
      eapply ST3.
      destruct fe1; destruct fef; destruct fe2; intuition.
      auto. auto. eapply HQX. eapply HQF.  eauto. eauto. 
      
    + (* result well typed *)
      (* eapply valt_qual_widen1. *)    
      repeat rewrite <-app_assoc. split. 2: split.
      destruct fef; destruct fe1; destruct fe2; intuition.
      simpl. repeat rewrite <- app_assoc. auto.
      simpl. rewrite <- app_assoc. auto.
      simpl. rewrite <- app_assoc. auto.
      simpl. rewrite app_nil_r. rewrite <- app_assoc. auto.
  ...
  -  destruct (IHW M E WFE S) as [S1 [M1 [vx [IW1 [SW1 [HVX HQX]]]]]]. { eauto. }
     ...
     exists S1. exists (if fe1 then M1 else []). exists vx.
...
Corollary safety : forall t T q e fe,
  has_type [] t T qempty q e fe -> 
  exists S M v, exp_type [] [] [] t S M v T (plift qempty) (plift q) (plift e) fe.
Proof. 
