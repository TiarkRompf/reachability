                    ...
                    (por (pand (vars_locs H (plift q2)) (val_locs v)) (* allow v \/ vx? *)
                       (por (pif fn2 (val_locs v))
                          (por (pif ar2 (val_locs vx))
                             (pif fr2 (pdiff (pdom (M''++M'++M)) (pdom (M'++M))))))))
  | _,_ =>
      False
  end.


Definition val_qual (M M1: stor) H v p fr (q: pl) :=
  psub (val_locs v)
    (por (vars_locs H (pand p q))
       (pif fr (pdiff (pdom M1) (pdom M)))).


Definition exp_type M H t T p fr q :=
  exists M' v,
    tevaln M H t (M'++M) v /\
    val_type (M'++M) H v T /\
    val_qual M (M'++M) H v p fr q.
...
Definition sem_type G e T p fr q := 
  forall M E, env_type M E G p ->  
  exp_type M E e T p fr q. 
...
Ltac unfoldq := unfold val_qual, psub, pdom, pdiff, pnot, pif, pand, por, pempty, pone, var_locs, vars_locs in *.
...
Lemma valq_fresh: forall M M' H v p q,
    val_qual M (v::M'++M) H (vref (length (M'++M))) p true q.
Proof.
  intros. right. unfoldq. simpl.
  (* valq: index out of range*)
  rewrite app_length in *.
  rewrite val_locs_ref in *.
  intuition. unfoldq. lia. unfoldq. lia.
Qed.
...
Lemma envt_extend_full: forall M M1 H G vx T1 p fr1 q1 qf,
    ...
    env_type (M1 ++ M) (vx :: H) ((T1, fr1, q1) :: G) (por qf (pone (length H))).
Proof.
  intros. apply wf_env_type in H0 as H0'. destruct H0' as (L' & PD (*& SG*)). 

  assert (env_type (M1 ++ M) H G p) as WFE. {
  ...
  - (* 2nd invariant *)
    ...
    + (* qf, x *)
      rename H4 into SEP.
      unfold pone in H10. subst x1. destruct (SEP x).
      ...
      destruct H13. rewrite indexr_extend in H13. eapply H13. 
    ...
    + (* x, qf *)
      rename H4 into SEP.
      unfold pone in H9. subst x0. destruct (SEP x).
...
Lemma overlapping: forall M M' M'' H G p vf vx frf qf frx qx q1
    ...
    (HQF: val_qual M (M'++M)H vf p frf qf)
    (HQX: val_qual (M'++M) (M''++M'++M) H vx p frx qx),
...
Proof. 
  ...
  - destruct (HQF x) as [Hf_q | Hf_fr]. eauto.
    ...
    (* vx fresh *)
    destruct frx. repeat rewrite app_length in *. lia. contradiction.
    (* vf fresh *)
...
Proof.
  intros. intros M H WFE.
  exists []. exists (vbool true). split.
...
Proof.
  intros. intros M H WFE.
  exists []. exists (vbool false). split.
  exists 0. intros. destruct n. lia. intuition. simpl. intuition.
  eapply valq_bool.
Qed.
...
Proof.
  intros. intros ? ? WFE.
  destruct WFE as [? [? [WFE ?]]].
  destruct (WFE x T fr q H) as [vx [HQ [TL [HI ?]]]]. 
  exists []. eexists. 
...
Proof.
  intros. intros ? ? WFE. 
  destruct (H M E WFE) as [M1 [vx [IW1 [HVX HQX]]]]. 
...
Proof.
  intros. intros ? ? WFE.
  destruct (H M E WFE) as [M1 [vx [IW1 [HVX HQX]]]]. auto. auto.
...
Proof.
  ...
  + (* result well typed *)
    ...
    (* INDUCTION *)
    destruct (H (M'++M) (vx::E)) as [M2 [vy IHW1]].
    ...
    intros. subst fr1. intros ? ?. eapply H7 in H8.
    ...
    destruct fn1; eapply H5; unfoldq. eapply H8. contradiction.
    
    intros ? ?. eapply H8. eapply H9.
    ...
    (* vy < vf \/ vx *)
    apply valt_wf in H6(*HVX*). apply valt_wf in H9.
    rename H10 into HVY.
    ...
    - (* q *) destruct HY_q as (? & (? & ?) & ?).
      ...
      * (* from arg *)
        subst x0. eapply var_locs_head in H13.
        right. right. left.
        destruct ar2. eauto.
        destruct H12. { eapply H4 in H12. lia. }
        destruct H12. simpl in H12. contradiction.
        destruct fn2. simpl in H12. rewrite plift_and in H12. destruct H12. eapply H5 in H14. lia.
        simpl in H12. contradiction.
      * (* from func *)
        assert (x0 < length (vx::E)). destruct H13. rewrite indexr_extend1 in H13. eapply H13. simpl in H15.
        eapply var_locs_shrink in H13; try lia.
        destruct H11. 2: { contradiction. }
        destruct H12. 2: { 
          destruct ar2; destruct fn2; destruct H12; try contradiction. 
...
Proof.
  ...
  assert (env_type (M1++M) E env p) as WFE1. { eapply envt_store_extend. eauto. }
  ...
  rename H8 into HVF.
  destruct (HVF M2 vx) as [M3 [vy [IW3 HVY]]]. eauto.
  ...
  (* EVALUATION *)
  exists (M3++M2++M1), vy. split. 2: split.
  + (* expression evaluates *)
    (* - initial fuel value *)
    destruct IW1 as [n1 IW1].
  ...
  (* VAL_TYPE *)
  + repeat rewrite <-app_assoc.  eapply HVY.
  ...
  + destruct HVY as [HVY HQY].
    ...
    * (* part of function *)
      ...
      -- (* fr *)
         ...
         right. simpl. repeat rewrite app_length in *. intuition.
    * (* part of arg *)
...
Proof.
  intros. rename x into z. intros ? ? WFE.
  destruct (H M E WFE) as [M1 [vx [IW1 [HVX HQX]]]].
  exists M1. exists vx. intuition.
...
Corollary safety : forall e T fr q,
  ...
  exp_type [] [] e T (plift qempty) fr (plift q).
Proof. 
