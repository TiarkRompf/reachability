...
Proof.
  ...
  - (* Abs shrink *)
    inversion H0. subst.
    rename q into q2.
    destruct (H7 S' M' vx fex) as [S'' [M'' [vy [IEY [EFF [STC [HST [HEY HVY]]]]]]]].
  ...
  - (* Abs grow *)
    inversion H0. subst.
    rename q into q2. 
    destruct (H7 S' M' vx fex) as [S'' [M'' [vy [HEY [HEFF [HSTC [HSK [HVY HQY]]]]]]]].
    eauto. auto.
    eapply IHT1; eauto.
    eauto.
    exists S'', M'', vy. intuition.
...
Lemma sem_app1: forall S S' S'' M M' M'' H Hf G T1 T2 ey vx pv p q1 q2 qf e2 (fe1 fef fe2:bool)
    ...
    (ST: store_type S M)
    (ST1: store_type S' M')
    (HVF: val_type M' H (vabs Hf pv ey) (TFun T1 T2 q2 e2 fe2))
    (HQF: val_qual M H (vabs Hf pv ey) p qf)
...
Proof. 
  ...
  rename H10 into HVF.
  destruct (HVF S'' M'' vx fe1) as [S''' [M''' [vy [IW3 [HEFF [HSTC [HST [HVY HQY]]]]]]]]; auto.
  ...
  *)
  ...
  assert (length M <= length M''). 
  eapply st_mono in STC.
  eapply st_mono in STC1;  lia. lia. 
  + (* part of function *)
    destruct (HQF x). intuition.
...
Lemma sem_abs1: forall S M H G T1 T2 t p q2 qf e2 fe2 
      ...
      forall M E,
        ...
        exists (S'' : stor) (M'': stty) (vy : vl),
          exp_type S M E t S'' M'' vy T2 (plift (qor (qand p qf) (qone (length G))))
            (plift (qor q2 (qone (length G)))) (plift (qor e2 (qone (length G)))) fe2) 
    (HCLT1: closed_ty (length G) T1)        
    (HCLT2: closed_ty (length G) T2)
    (HCLQ:  closed_ql (length G) (qand p qf))
    (HCLQ2: closed_ql (length G) q2)
    (HCLE: closed_ql (length G) e2)
    (SUBP: psub(plift p) (pdom H)),
  exists (S'': stor) (M'' : stty) (vf : vl),
    exp_type S M H (tabs (qand p qf) t) S'' M'' vf (TFun T1 T2 q2 e2 fe2) (plift p) (plift qf) (plift qempty) false.
Proof. 
  intros.
  ...
  + (* val type *)
    repeat split.
    1-2: rewrite L; auto.
    unfoldq; intuition. eapply HCLQ2 in H0. lia.
    unfoldq; intuition. eapply HCLE in H0. lia.
    rewrite val_locs_abs.
    ...
    destruct (IHW M' (vx::H) WFE' S') as [S2 [M2 [vy IHW1]]].
    auto. 
    
    destruct IHW1 as [HE [HEFF [HCT [HST [HVY HQY]]]]].
    ...
    unfoldq; intuition. eapply HCLE in H4. lia.
...
Proof.
  ...
  - (* Case "Var". *)
    ...
    intuition. eapply no_effects. apply st_no_extend.
  ...
  - (* Case "Ref". *)
    destruct (IHW M E WFE S) as [S1 [M1 [vx [IW1 [EFF1 [SC1 [ST1 [HVX HQX]]]]]]]]. eauto. 
  ...
  - (* Case "Put" *)
    ...
    exists M2.
    exists vy. split. 2: split. 3: split. 
    + (* expression evaluates *)
      (* - initial fuel value *)
      destruct IW1 as [n1 IW1].
    ...
    + assert (val_type M2 E vy TBool) as HVY.
      ...
      erewrite indexr_st_extend in MI; eauto.
      apply indexr_var_some' in MI. auto.
      destruct H4 as [vt' [? ?]]. rewrite MI in H4.
      inversion H4. subst vt'.
      assert (st_chain M1 M1 false). eapply st_no_extend; eauto.
      specialize (H7 M1 false H8 vx0). intuition.
  ...
  - (* Case "App". *)
    ...
    destruct (IHW1 M E WFE S) as [S1 [M1 [vf [IW1 [EFF1 [STC1 [ST1 [HVF HQF]]]]]]]]. eauto.
    assert (env_type M1 E env (plift p)) as WFE1. 
    eapply envt_store_extend; eauto.
    destruct (IHW2 M1 E WFE1 S1) as [S2 [M2 [vx [IW2 [EFF2 [STC2 [ST2 [HVX HQX]]]]]]]]; eauto.
    ...
    destruct HVY as [S3 [M3 [vy [IW3 [EFF3 [STC3 [ST3 [HVY HQY]]]]]]]]; eauto.

    (* result *)
    exists S3. exists M3. 
    exists vy. split. 2: split. 3: split. 4: split.  5: split.
    + (* expression evaluates *)
      (* - initial fuel value *)
      destruct IW1 as [n1 IW1].
      destruct IW2 as [n2 IW2].
