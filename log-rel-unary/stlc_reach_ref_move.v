...
| t_ref: forall t env p q e k m,
    has_type env t TBool p q e k m ->
    has_type env (tref t) (TRef TBool) p q e k m
| t_get: forall t env p q e k m,
    has_type env t (TRef TBool) p q e k m ->
      ...
      closed_ty (length H) T1 /\
        ...
        (forall S' M' K' vx,
            ...
            ->
              psub
                (pand K'
                   (por (vars_locs H (plift m2))
                      (if m2x then (val_locs vx) else pempty)))
                pempty
            ->
              exists S'' M'' vy,
                tevaln
                    ...
                    (por K'
                       (por (vars_locs H (plift k2))
                          (por (if k2x then (val_locs vx) else pempty)  
                             (por (pdiff (vars_locs H (plift m2)) (val_locs vy))
                                (por (if m2x then (pdiff (val_locs vx) (val_locs vy)) else pempty)  (* - vy?*)
                  ...
                  psub
                    ...
                    (por (pand (vars_locs H (plift q2)) (val_locs v))
                       (por (if q2x then (val_locs vx) else pempty)
                          (por (pand (vars_locs H (plift m2)) (val_locs v))
                             (if m2x then (val_locs vx) else pempty)))))
  | _,_ =>
      False
  end.
...
Definition val_qual (M: stty) H v (p q m: pl) :=
  psub (pand (pdom M) (val_locs v)) (vars_locs H (pand p (por q m))).
  ...
  tevaln S H t S' v /\
    store_type S' (M'++M) (por K
                             (por (vars_locs H (pand p k))
                                (por (pdiff (vars_locs H (pand p m)) (val_locs v)) 
                                   (pdiff (pdiff (pdom (M'++M)) (pdom M))
                                      (val_locs v))))) /\ 
    val_type (M'++M) H v T /\
    val_qual M H v p q m.
...
Proof.  
  ...
  + destruct (H8 S' (M'0 ++ M') K' vx) as [S'' [M'' [vy  [SEY [IEY [HVY HSEP]]]]]].
...
Lemma valq_bool: forall M H b p q m,
    val_qual M H (vbool b) p q m.
Proof.
...
Lemma valq_abs: forall M H t p q,
    val_qual M H (vabs H (qand p q) t) (plift p) (plift q) pempty.
Proof.
  intros. unfoldq.
  rewrite val_locs_abs.
  rewrite plift_and.
  intuition.
  destruct H2. exists x0. unfoldq. intuition.
Qed.


Lemma valq_sub: forall M H v p q q' m m',
    val_qual M H v p q m ->
    psub q q' ->
    psub m m' ->
    psub q' (pdom H) ->
...
Lemma valq_alt: forall M1 M H v p q m,
  val_qual M H v p q m ->
  psub (val_locs v) (pdom (M1++M)) ->
  psub (val_locs v)
    (por (vars_locs H (pand p q))
       (por (vars_locs H (pand p m))
          (pdiff (pdom (M1++M)) (pdom M)))).
Proof.
...
Lemma valq_alt1: forall M1 M H T1 v p q m,
  val_qual M H v p q m ->
  val_type (M1++M) H v T1 ->
  psub (val_locs v)
    (por (vars_locs H (pand p q))
       (por (vars_locs H (pand p m))
          (pdiff (pdom (M1++M)) (pdom M)))).
Proof.
  intros. eapply valq_alt. eauto. eapply valt_wf; eauto. 
Qed.

Lemma valq_alt2: forall M1 M H vt i p q m,
  val_qual M H (vref i) p q m ->
  indexr i (M1++M) = Some vt -> 
  (por (vars_locs H (pand p q))
     (por (vars_locs H (pand p m))
        (pdiff (pdom (M1++M)) (pdom M))) i).
Proof.
  intros. eapply valq_alt. eauto.
  rewrite indexr_extend with (H':=[]) in H1.
  rewrite val_locs_ref. unfoldq. lia.
  rewrite val_locs_ref. unfoldq. lia.
Qed.
...
Lemma separation: forall M M' H G p vf vx qf q1 mf m1
    (WFE: env_type M H G p)
    (HQF: val_qual M H vf p qf mf)
    (HQX: val_qual (M'++M) H vx p q1 m1),
...
Lemma storet_restrict2: forall S M K G H p q v,
    ...
    store_type S M K ->
    psub q (pdom H) ->
    store_type S M (por K (por (vars_locs H q) (por (pdiff (vars_locs H q) (val_locs v)) (pdiff (pdiff (pdom M) (pdom M)) (val_locs v))))).
Proof.
  intros.
...
Lemma sem_app1: forall S'' M M' M'' K'' H Hf G T1 T2 ey vx pv p q1 q2 qf m1 mf e2 k2 m2 q2x e2x k2x m2x
    ...
    (HQF: val_qual M H (vabs Hf pv ey) p qf mf)
    ...
    psub (pand K''
            (por (vars_locs H (plift m2))
               (if m2x then (val_locs vx) else pempty))) pempty ->
...
Proof.
  ...
  *)
  ...
  unfoldq. intros. 
  destruct (HQY x) as [H_q2 | [H_q2x | [H_m2 | H_m2x]]]. repeat rewrite app_length. intuition.
  + (* q2 part of function *)
    destruct (HQF x). intuition.
    destruct H_q2 as [[? ?] ?]. 
    exists x1. intuition.
    
  + (* q2x part of argument *)
    ...
    tevaln S (vx :: H) t S'' vy /\
      ...
      psub (pand (pdom (M1 ++ M)) (val_locs vy))
        (por (pand (vars_locs H (plift q2)) (val_locs (vabs H (qand p qf) t)))
           (por (if q2x then (val_locs vx) else pempty)
              (por (pand (vars_locs H (plift m2)) (val_locs (vabs H (qand p qf) t)))
                 (if m2x then (val_locs vx) else pempty)))).
Proof.
  intros.
  ...
  assert (x < length (M)). eapply envt_var_store_wf. eauto. eapply H6. 
  rewrite app_length, app_length. lia.
  eapply valt_wf. eapply valt_store_extend. eauto. destruct m2x. eapply H2. contradiction.
  eapply H2. 
  ...
  rewrite plift_or, plift_and, plift_one, plift_or, plift_or in H2.
  unfoldq.
  destruct H2. destruct H2. 
  destruct H7 as [H_q2 | H_m2].
  
  {
  destruct q2x. rewrite plift_one in H_q2.
  bdestruct (x0 =? length H).
  - (* from arg *)
  ...
  (* now the same for m2 ... *)
  {
  destruct m2x. rewrite plift_one in H_m2.
  bdestruct (x0 =? length H).
  - (* from arg *)
    right. right. right. destruct H6 as [? [? ?]]. subst x0. rewrite indexr_head in H6. inversion H6. eauto.
  - (* from func *)
  ...
  (* m2x is false, hence can't be arg *)
  - destruct H_m2. 

    2: { rewrite plift_empty in H7. unfoldq. contradiction. }
    assert (x0 < length H). { eapply HCLM in H7. eauto. }
    right. right. left. split.
    exists x0. intuition. destruct H6 as [? [? ?]]. rewrite indexr_skip in H6. exists x1. split; eauto. lia.
    exists x0. rewrite plift_and. split.
    destruct H2; try lia. eapply H2.
...
Theorem full_total_safety : forall t G T p q e k m,
    ...
    psub (pand K (vars_locs E (pand (plift p) (plift m)))) pempty ->
...
Proof.
  ...
  - (* Case "Ref". *)
    ...
    + (* result well typed *)
      ...
      (* m *)
      right. right. left.
      assert (x < length M). { destruct HX_m. 
      eapply env_type_store_wf in H2. 2: eapply WFE. unfoldq. eauto. }
      intuition. rewrite app_length in H5. lia. 
      (* fresh *)
      right. right. right.
      rewrite app_length. intuition.
      simpl. lia. rewrite app_length in H4. lia.
      
      rewrite val_locs_ref. rewrite app_length. unfoldq. intros. 
      destruct H2 as [HX_K | [HX_k | [HX_m | HX_fresh]]].
      (* K *) eapply ST1. intuition.
      (* k *) eapply ST1. intuition.
      (* m *) 
      assert (x < length M). { destruct HX_m. 
      eapply env_type_store_wf in H2. 2: eapply WFE. unfoldq. eauto. }
      ...
      (* fresh *)
      simpl in HX_fresh. rewrite app_length in *. lia. 
  ...
  - (* Case "Get". *)
    ...
    eapply valq_alt2 in HQX as HQX'; eauto.
    
    assert (~ por K
              (por (vars_locs E (pand (plift p) (plift k)))
                 (por (pdiff (vars_locs E (pand (plift p) (plift m))) (val_locs (vref i)))
                    (pdiff (pdiff (pdom (M1 ++ M)) (pdom M)) (val_locs (vref i)))))
      i) as NK. {
      intros IO. rewrite val_locs_ref in IO.
      destruct HQX' as [HQ_old | [HQ_moved | HQ_fresh]].
      ...
      assert (x = x0). { eapply WFE. eapply IO. eapply IO. eapply HQ. eapply HQ. }
      ...
      (* - fresh *)
      destruct IO as [IO_K | [IO_k | [IO_m | IO_fresh]]].
      (* K *) eapply HQ_fresh. eapply ST. eauto.
      (* k *) eapply env_type_store_wf in IO_k; eauto. 
      unfoldq. rewrite app_length in *. lia. 
      (* m *) destruct IO_m. unfoldq. intuition.
      (* fresh *) destruct IO_fresh. unfoldq. intuition.
    }
    ...
    + (* result well typed *)
      ...
      rewrite val_locs_ref, val_locs_bool.
      ...
      intros ? [XX_K|[XX_k|[XX_m|XX_fresh]]].
      eapply ST in XX_K. unfoldq. rewrite app_length. lia.
      eapply env_type_store_wf in XX_k. 2: eapply WFE. unfoldq. rewrite app_length. lia.
      destruct XX_m as [XX_m ?]. eapply env_type_store_wf in XX_m. 2: eapply WFE. unfoldq. rewrite app_length. lia.
      eapply XX_fresh. 
  ...
  - (* Case "Free". *)
     ...
     *)
      ...
      assert (store_type S1 (M1 ++ M)
                (por K
                   (por (vars_locs E (pand (plift p) (plift (qor k q))))
                      (por (pdiff (vars_locs E (pand (plift p) (plift m))) (val_locs (vbool false)))
      ...
      eapply storet_free. eauto. {
        ...
        destruct HQX' as [HQ_old|[HQ_moved|HQ_fresh]].
        (* - old *)
        right. left. destruct HQ_old. exists x. unfoldq. intuition.
        (* - moved *)
        right. right. left. unfoldq. intuition.
        (* - fresh *)
        right. right. right. unfoldq. intuition. 
  ...
  - (* Case "Move". *)
    ...
    + (* result well typed *)
      ...
      assert (store_type S1 (M1 ++ M)
                (por K
                   (por (vars_locs E (pand (plift p) (plift k)))
                      (por (pdiff (vars_locs E (pand (plift p) (plift (qor m q)))) (val_locs (vref i)))
      ...
      destruct HQX' as [HQ_old|[HQ_move|HQ_fresh]].
      (* - old *)
      unfoldq. intros. rewrite val_locs_ref in H3. unfoldq. destruct H3. subst x.
      destruct HQ_old. exists x. split. split. eapply H4. right. rewrite plift_or. right. eapply H4. eapply H4.
      (* - move *)
      unfoldq. intros. rewrite val_locs_ref in H3. unfoldq. destruct H3. subst x.
      destruct HQ_move. exists x. split. split. eapply H4. right. rewrite plift_or. left. eapply H4. eapply H4.
      (* - fresh *)
      unfoldq. intros. rewrite val_locs_ref in H3. unfoldq. destruct H3. subst x.
      lia. 
  ...
  - (* Case "App". *)
    ...
    assert (forall a b,
        ...
        vars_locs E (pand (plift p) a) x ->
        vars_locs E (pand (plift p) b) x -> False) as vars_locs_sep.
    {
    ...
    assert (forall a b,
        ...
        psub (pand (vars_locs E (pand (plift p) (plift a))) 
          (vars_locs E (pand (plift p) (plift b)))) pempty) as vars_locs_sep1.
    {
      intros. unfoldq. intros. eapply vars_locs_sep. eapply H. eapply H0. eapply H0. 
    }
    
    assert (forall a x,
               vars_locs E (pand (plift p) (plift a)) x -> forall (M M1: stty),
                 env_type M E env (plift p) ->
        x < length (M1++M) /\ ~ (x < length M) -> False) as vars_locs_not_fresh.
    {
      intros. eapply H1. eapply env_type_store_wf. eauto. eauto. 
    }
    (* eapply vars_locs_sep1 in SEP_KFE1; eauto. *)

    assert (forall a, psub a (plift p) ->
       psub (vars_locs E a)
         (vars_locs E (pand (plift p) a))) as vars_locs_up.
    {
    ...
    remember (por K
                (por (vars_locs E (pand (plift p) (plift kf)))
                   (por (pdiff (vars_locs E (pand (plift p) (plift mf))) (val_locs vf))
    ...
    (* K1 sep e1 *) {
      ...
      (* fresh *) destruct (vars_locs_not_fresh _ _ HY_e M M1); eauto.
    }
    ...
    (* K1 sep m1 *) {
      ...
      (* fresh *) destruct (vars_locs_not_fresh _ _ HY_m M M1); eauto.
    }
    ...
    remember (por K1
                (por (vars_locs E (pand (plift p) (plift k1)))
                   (por (pdiff (vars_locs E (pand (plift p) (plift m1))) (val_locs vx))
    ...
    assert (exists S3 M3 vy,
      ...
      (* K2 sep e2/e2x *) {
        ...
        (* K  *) eapply KSEPE. split. eauto. right. right. left. eapply vars_locs_up; eauto. 
        (* kf *) eapply vars_locs_sep. eapply SEP_KFE2. eapply H_kf. eapply vars_locs_up; eauto. 
        (* mf *) eapply vars_locs_sep. eapply SEP_MFE2. eapply H_mf. eapply vars_locs_up; eauto.
        (* fresh f *) eapply vars_locs_not_fresh. eapply vars_locs_up. 2: eauto. eauto. unfoldq. eapply WFE.  eapply H_freshf.
        (* k1 *) eapply vars_locs_sep. eapply SEP_K1E2. eapply H_k1. eapply vars_locs_up; eauto.
        (* m1 *) eapply vars_locs_sep. eapply SEP_M1E2. eapply H_m1. eapply vars_locs_up; eauto.
        (* fresh x *) eapply vars_locs_not_fresh. eapply vars_locs_up. 2: eauto. eauto. unfoldq. eapply WFE1. eapply H_fresh1. 
        
        (* -- e2x -> vx -- *)

        destruct e2x. 2: destruct H. 
        eapply valq_alt in HQX as HQX'; eauto.
        ...
        (* fresh f *) eapply vars_locs_not_fresh. eauto. eapply WFE. eapply H_freshf.
        ...
        (* k1 *) eapply vars_locs_not_fresh. eapply H_k1. eapply WFE1. eapply HV_fresh. (* can't return fresh kill *)
        (* m1 *) eapply H_m1. eauto. 
        (* fresh x *) eapply H_fresh1. eauto.
      }
      ...
      (* K2 sep m2/m2x *) {
        ...
        (* K  *) eapply KSEPM. split. eauto. right. right. left. eapply vars_locs_up; eauto. 
        (* kf *) eapply vars_locs_sep. eapply SEP_KFM2. eapply H_kf. eapply vars_locs_up; eauto. 
        (* mf *) eapply vars_locs_sep. eapply SEP_MFM2. eapply H_mf. eapply vars_locs_up; eauto.
        (* fresh f *) eapply vars_locs_not_fresh. eapply vars_locs_up. 2: eauto. eauto. unfoldq. eapply WFE.  eapply H_freshf.
        (* k1 *) eapply vars_locs_sep. eapply SEP_K1M2. eapply H_k1. eapply vars_locs_up; eauto.
        (* m1 *) eapply vars_locs_sep. eapply SEP_M1M2. eapply H_m1. eapply vars_locs_up; eauto.
        (* fresh x *) eapply vars_locs_not_fresh. eapply vars_locs_up. 2: eauto. eauto. unfoldq. eapply WFE1. eapply H_fresh1. 
    ...
    + (* result well typed *)
      ...
      eapply vars_locs_sep. eapply SEP_MFM1. eauto. exists x0. intuition.
      ...
      eapply vars_locs_not_fresh. exists x0. eauto. eapply WFE. rewrite app_length. eapply H_freshf.
      eapply vars_locs_not_fresh. exists x0. eauto. eapply WFE. rewrite app_length. eapply H_freshf. 
      destruct m2x. 2: contradiction.
      destruct (HQX x). rewrite app_length. intuition.
      destruct H1. destruct H1. destruct H3. 
      ...
      eapply vars_locs_sep. eapply SEP_MFM1. exists x0. eauto. eauto.
      ...
      eapply valq_alt in HQX as HQX'. 2: { eapply valt_wf. eapply HVX. }
      ...
      eapply valt_wf. eapply HVX. eauto.
        ...
        (* vy is vf *)
        eapply valq_alt1 in HQF. 2: eauto.
        eapply HQF in H0. destruct H0 as [HF_old | [HF_moved | HF_fresh]].
          ...
          (* vf fresh *)
          eapply vars_locs_not_fresh. eapply HX_moved. eapply WFE. eapply HF_fresh.
        (* vy is q2x/vx *)
      ...
      split. split. intuition. intuition. intuition.
      destruct (HQY2 x) as [? | [? | ?]]. intuition. repeat rewrite app_length. eauto.
      eapply H1. rewrite <-app_length. eapply valt_wf. eapply HVF. eauto. 
      rewrite H2 in H4. (* q2x = false *) contradiction.
      rewrite H3 in H4. (* m2x = false *) contradiction.
      ...
      destruct EXX as [?|[?|[?|?]]].
      (* K  *) repeat rewrite <-app_length. eapply SW3. left. subst K2. left. subst K1. left. eauto.
      (* k2 *) destruct H. destruct H. destruct H. eapply envt_var_store_wf in H0; eauto. unfoldq. rewrite <-app_length. lia.
      (* m2 *) destruct H. destruct H. destruct H. eapply envt_var_store_wf in H1; eauto. unfoldq. rewrite <-app_length. lia.
      (* fresh *) lia. 
  ...
  - (* Case "Abs". *)
    ...
    + (* result well typed *)
      ...
      eapply ST. eauto. destruct H6 as [? [[? []] ?]]. destruct H6 as [? [[? []] ?]]. 
