...
- effects
  - observable write effect qualifiers
...
Fixpoint val_type (M:stty) (H:venv) v T : Prop :=
  ...
  | vref l, TRef T fr q =>
     ...
     (* st_locs M l /\ (* redundant with line above *) *)
      exists vt qt,
        indexr l M = Some (vt, qt) /\ (* no distinction in stty *)
        (forall M',
  ...
  | vabs G py ty, TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2 =>
            ...
            ->
              exists S'' M'' vy,
                ...
                /\
                  store_type S'' M''
                /\
                  store_effect S' S'' (por (locs_locs_stty M' (vars_locs H (plift e2))) (por (pif e2x (locs_locs_stty M' (val_locs vx)))(pif e2f (locs_locs_stty M' (val_locs v)))))
                /\
                  val_type
                    M''
                    H
...
Definition exp_type S M H t T p fr q e :=
  exists S' M' v,
    tevaln S H t S' v /\
    st_chain M M' (st_locs M) /\
    store_type S' M' /\
    store_effect S S' (locs_locs_stty M (vars_locs H (pand p e))) /\  (* p and e *)
    val_type M' H v T /\
...
Definition sem_type G t T p fr q e :=
  forall S M E,
    env_type M E G p ->
    store_type S M  ->
    exp_type S M E t T p fr q e. 
...
(* has_type *)
(*
Lemma has_type_wf_e: forall G t T p fr q e,
  has_type G t T p fr q e ->
  psub (plift e) (plift p).
Proof.
  intros. induction H; auto. 1-3,6: rewrite plift_empty. 1-4: intros ? ?; intuition.
  repeat rewrite plift_or. 
  intros ? ?. destruct H2. eapply IHhas_type1 in H2. auto.
  destruct H2.  eapply IHhas_type2 in H2. auto. eapply H1 in H2. auto.
  repeat rewrite plift_or. rewrite plift_if.
  intros ? ?. destruct H5. eapply IHhas_type1. auto. destruct H5. eapply IHhas_type2. auto.
  destruct H5. eapply H4 in H5. auto.
  rewrite plift_and in H5. 
  destruct e2x; simpl in H5. destruct H5. auto. contradiction. 
Qed.
*)
...
Proof. 
  intros T. induction T; intros; destruct v; simpl in *; intuition.
  + econstructor.
  + destruct b; intuition. repeat econstructor; eauto.
  + econstructor; eauto. 
Qed.
...
Proof. 
  ...
  - (* Abs shrink *)
    inversion H0. subst.
    destruct (H8 S' M' vx) as [S'' [M'' [vy [HEY HVY]]]]. eauto. eauto. 
    eapply IHT1; eauto.
    rewrite vars_locs_extend; auto.
    exists S'', M'', vy. intuition.
    erewrite <- vars_locs_extend. eauto. eauto. 
    eapply IHT2; eauto.
  ...
  - (* Abs grow *)
    inversion H0. subst.
    destruct (H8 S' M' vx) as [S'' [M'' [vy [HEY [ST2 [HVY HQY]]]]]]. eauto. eauto.
      eapply IHT1; eauto. 
      rewrite vars_locs_extend in H11; auto.
    exists S'', M'', vy. intuition.
    rewrite vars_locs_extend. eauto. eauto. 
    eapply IHT2; eauto.
...
Proof. 
  ...
  - intros.
    ...
    + rewrite indexr_skip in H9. 
      ...
      exists v. repeat split; intuition.
      eapply valt_extend1. eapply valt_closed in VALT; eauto. unfoldq. intuition. 
...
Lemma sem_true: forall G p,
    sem_type G ttrue TBool p false pempty pempty.
Proof.
  ...
  - eapply stchain_refl. 
  - eauto.
  - rewrite pand_empty_r. rewrite vars_locs_empty. rewrite lls_empty. intros ? ?. intuition.
  - simpl. eauto. 
  - eapply valq_bool.
Qed.
  
Lemma sem_false: forall G p,
    sem_type G tfalse TBool p false pempty pempty.
Proof.
  intros. intros S M H WFE ST.
...
Lemma sem_var: forall x G T (p:pl) fr q,
    indexr x G = Some (T, fr, q) ->
    p x ->
    sem_type G (tvar x) T p false (pone x) pempty.
Proof.
...
Lemma sem_ref: forall t G T p q e,
    sem_type G t T p false (plift q) (plift e) ->
    psub (plift q) p ->
    sem_type G (tref t) (TRef T false q) p true (plift q) (plift e).
Proof.
  intros. intros ? ? ? WFE ST. 
...
Lemma sem_get1: forall t env T p q1 fr q e,
    sem_type env t (TRef T false q1) p fr q e ->
    psub (plift q1) p ->
    sem_type env (tget t) T p false (plift q1) e.
Proof.
  intros. intros ? ? ? WFE ST.
  destruct (H S M E WFE ST) as [S1 [M1 [vx [IW1 [SC1 [ST1 [SE1 [HVX HQX]]]]]]]]. 
...
Lemma sem_get2: forall t env T p rf1 q1 fr q e,
    sem_type env t (TRef T rf1 q1) p fr q e ->
    sem_type env (tget t) T p fr q e.
Proof.
  intros. intros ? ? ? WFE ST.
  destruct (H S M E WFE ST) as [S1 [M1 [vx [IW1 [SC1 [ST1 [SE1 [HVX HQX]]]]]]]]. 
  destruct vx; try solve [inversion HVX]. simpl in HVX.
  ...
  + auto. 
  + (* result well typed *)
    eapply valt_store_extend. eapply VTM. 2: eapply VLS. 2: eauto. 
    eapply stchain_tighten. eapply stchain_refl.
...
Lemma sem_get: forall t env T p q1 rf1 fr q e,
    sem_type env t (TRef T rf1 q1) p fr q e ->
    psub (plift q1) p ->
    sem_type env (tget t) T p (if rf1 then fr else false) (por (plift q1) (pif rf1 q)) e.
Proof.
  intros. intros ? ? ? WFE ST.
  destruct (H S M E WFE ST) as [S1 [M1 [vx [IW1 [SC1 [ST1 [SE1 [HVX HQX]]]]]]]]. 
  destruct vx; try solve [inversion HVX]. simpl in HVX.
  ...
  + (* qualifier *)
    ...
    * assert (val_qual M M1 E v p false (plift q1)). {
        intros ? ?. left. eapply VLS in H4; eauto. rewrite VTQ in H4.
...
Lemma sem_put: forall t1 t2 env T p fr1 q1 e1 rf2 q2 e2,
    sem_type env t1 (TRef T rf2 q2) p fr1 q1 e1 ->
    sem_type env t2 T p false (plift q2) e2 ->
    psub (plift q2) p -> 
    sem_type env (tput t1 t2) TBool p false pempty (por e1 (por e2 q1)).
Proof.
  intros. intros ? ? ? WFE ST.
  ...
  + (* store type *)
    ...
    * exists vt, qt, vx. subst i. 
      ...
      -- unfold val_qual in HQX.
         ...
         intros ? ?. eapply HQX in H15. destruct H15. 2: contradiction.
         rewrite VQT1.
         eapply lls_mono; eauto. eapply vars_locs_monotonic. unfoldq. intuition.
         intuition.
      -- intros. intros ? ?. eapply HQX in H15. destruct H15. 2: contradiction.
         eapply lls_mono; eauto. rewrite VQT1.
  ...
  + (* store preservation *)
    ...
    destruct (HQR l).
      ...
      destruct H15.  simpl. 
      erewrite lls_change with (q := (vars_locs E (pand p (por e1 (por e2 q1))))). eapply lls_mono; eauto.
      eapply vars_locs_monotonic. intros ? ?. unfoldq; intuition. 
      eapply stchain_tighten; eauto.  eapply env_type_store_deep_wf; eauto.
      intros ? ?. unfoldq; intuition. 
      destruct fr1. simpl in *. unfold pstdiff, pdiff in H3. apply indexr_var_some' in H16.
      unfold st_locs in H3. unfoldq; intuition.
      simpl in H3. contradiction. 
    }{ rewrite update_indexr_miss.
       assert ((locs_locs_stty M (vars_locs E (pand p e1)) l) = locs_locs_stty  M1 (vars_locs E (pand p e1)) l) as LLS1. {
         erewrite lls_change; eauto. 
         eapply stchain_tighten; eauto. eapply env_type_store_deep_wf; eauto. intros ? ?. unfoldq; intuition.
      }
      unfold store_effect in *.
      eapply SE1 in H16 as A; eauto. 
      2: {
        intros ?. eapply H15. eapply lls_mono; eauto. eapply vars_locs_monotonic. intros ? ?. unfoldq; intuition.
      }
      eapply SE2 in A  as B; eauto.
      intros ?. eapply H15. erewrite <- lls_change in H18.
      eapply lls_mono with (q := (vars_locs E (pand p e2))); eauto. eapply vars_locs_monotonic. intros ? ?. unfoldq; intuition.
      eapply stchain_tighten; eauto. eapply env_type_store_deep_wf; eauto. unfoldq; intuition. lia.
     }
...
Lemma eff_abs: forall M' vx E p e2f e2 e2x l,
...
Lemma sem_abs: forall env t T1 T2 p fn1 fr1 q1 fn2 ar2 fr2 q2 qf e2 e2f e2x,
...
Proof.
  ...
  + (* store typing *)
    eauto.
  + rewrite pand_empty_r. rewrite vars_locs_empty, lls_empty. intros ? ?. intuition. 
  + (* result well typed *)
    simpl. 
    ...
    eapply WFE' in H7; eauto. unfoldq; intuition.

    (* INDUCTION *)
    destruct (H S' M' (vx::E)) as [S2 [M2 [vy IHW1]]].
    
    (* ENV_TYPE*) {
    rewrite <-plift_and in H7. rewrite <-val_locs_abs with (t:=t) in H7.
    ...
    intros. subst fr1. intros ? ?. eapply H10 in H11.
    ...
    destruct fn1; eapply H5; unfoldq. eapply H11. contradiction.

    intros ? ?. eapply H11. eapply H12. 

    eapply stchain_tighten. eapply H7. 
    ...
    (* store preservation *)
    intros ? ? PV ?. eapply H13.
    intros ?. eapply PV. rewrite <- plift_and in *. eapply eff_abs; auto.
    ...
    (* vy < vf \/ vx *)
    apply valt_wf in H9(*HVX*). apply valt_wf in H15(*HVY*).
    rename H17 into HVY.
    ...
    - (* q *)
      ...
      * (* from arg *)
        subst x0. eapply lls_mono in H19. 2: { intros ? ?. eapply var_locs_head. eauto. }
        right. right. left.
        destruct ar2. eauto. 
        destruct H18. { eapply H4 in H18. lia. }
        destruct H18. contradiction.
        destruct fn2. simpl in H18. rewrite plift_and in H18. destruct H18. eapply H5 in H20. lia.
        simpl in H18. contradiction.
      * (* from func *)
        assert (x0 < length (vx::E)). eapply lls_var in H19. destruct H19. rewrite indexr_extend1 in H19. eapply H19. simpl in H21.
        eapply lls_mono in H19. 2: { intros ? ?. eapply var_locs_shrink. eauto. lia. }
        destruct H17. 2: { contradiction. }
        destruct H18.         
        left. split. 
        eapply lls_mono. 2: eapply H19. intros ? ?. exists x0. intuition. 
        eapply lls_vars'. eapply H19. eauto.
        destruct H18. destruct ar2; contradiction. 
        destruct fn2. right. left. simpl. eapply lls_vars'. eapply H19. auto. 
        simpl in H18. contradiction.
...
Lemma sem_app: forall env f t T1 T2 p frf qf frx qx fn1 fr1 q1 fn2 ar2 fr2 q2 e2f e2x e2 ex ef
    (E2:  psub (plift e2) p),
    sem_type env f (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2) p frf (plift qf) ef ->  
    sem_type env t T1 p frx (plift qx) ex ->
    (if fn1 then
       if fr1 then
         True
       else
    ...
    sem_type env (tapp f t) T2 p
      ...
      (por ef (por ex (por (plift e2) (por (pif e2f (plift qf)) (pif e2x (plift qx)))))).
Proof.
  ...
  destruct fn1. { (* arg may overlap with function? *)
    destruct fr1. { (* arg may be fresh? *)
      ...
      destruct HSEP as [SEP | [SEP | SEP]]. { (* fn *)
        ...
        eapply lls_var in H13. destruct H13 as (? & ? & ?).
    ...
    right. destruct fr1. {
      ...
      edestruct val_locs_stty_decide. eauto. { (* check to see if we're aliasing function *)
        ...
        rewrite plift_vt. eapply vt_mono. 2: eapply H11. unfoldq. intuition. eauto.
  ...
  + (* store preservation *)
    ...
    assert (st_chain M M1 (locs_locs_stty M (vars_locs E (pand p ex)))) as A.  {
      eapply stchain_tighten; eauto. eapply env_type_store_deep_wf in WFE; eauto. unfoldq; intuition.
    } 
    assert (st_chain M M2 (locs_locs_stty M (vars_locs E (pand p (plift e2))))) as B. {
      eapply stchain_tighten; eauto. eapply env_type_store_deep_wf in WFE; eauto.
      intros ? ?. unfoldq; intuition. 
    }
    assert (st_chain M M2 (locs_locs_stty M (vars_locs E (pand p (plift qx))))) as C. {
     eapply stchain_tighten in SCC; eauto. eapply env_type_store_deep_wf in WFE; eauto.
     intros ? ?. unfoldq; intuition.
    }
    assert (st_chain M M2 (locs_locs_stty M (vars_locs E (pand p (por ef (por ex (por (plift e2)(pif true (pand p (plift qx)))))))))) as D. {
       eapply stchain_tighten in SCC; eauto. eapply env_type_store_deep_wf in WFE; eauto.
       intros ? ?. unfoldq; intuition.
    }
    ...
    destruct H9. 
    erewrite <- lls_change with (M := M) in H9. eapply lls_mono; eauto.
    eapply vars_locs_monotonic; eauto. unfoldq; intuition. 
    eapply stchain_tighten; eauto.
    intros ? ?. eapply lls_mono; eauto. eapply vars_locs_monotonic; eauto.
    unfoldq; intuition.
     
    destruct e2x; simpl in H9; destruct H9; simpl in H9.  
    destruct (HQX l0); auto.
    erewrite <-lls_change with (M := M) in H11.
    eapply lls_mono; eauto. eapply vars_locs_monotonic. intros ? ?. unfoldq; intuition.
    eapply stchain_tighten; eauto. intros ? ?. eapply lls_mono; eauto. eapply vars_locs_monotonic; eauto.
    unfoldq; intuition.
    destruct frx; simpl in H11. unfold pstdiff, pdiff, st_locs, pdom in H11. intuition. contradiction. 
    
    destruct e2f; simpl in H9. 
    erewrite <-lls_change with (M := M1) in H9. 
    destruct (HQF l0); auto. 
    erewrite <-lls_change with (M := M) in H11. 
    eapply lls_mono; eauto. eapply vars_locs_monotonic; eauto. intros ? ?. unfoldq; intuition.
    eapply stchain_tighten. eapply SC1. eapply env_type_store_deep_wf; eauto. unfoldq; intuition.
    destruct frf; simpl in H11. unfold pstdiff, pdiff in H11. unfold st_locs in H11. unfoldq; intuition. contradiction.
    eapply stchain_tighten. eapply SC2. auto.
    ...
    rewrite SE2; eauto. intros ?. eapply PV. erewrite <- lls_change with (M := M) in H9; auto. 
    eapply lls_mono; eauto. intros ? ?. eapply vars_locs_monotonic; eauto. unfoldq; intuition.
    
    rewrite SE1; eauto. intros ?. eapply PV. 
    eapply lls_mono; eauto. intros ? ?. eapply vars_locs_monotonic; eauto. unfoldq; intuition.
...
Lemma sem_seq: forall env t1 t2 p1 p2 p q1 q2 e1 e2
  (E1: sem_type env t1 TBool p1 false q1 e1)
  (E2: sem_type env t2 TBool p2 false q2 e2)
  (SUB1: psub p1 p)
  (SUB2: psub p2 p),
  sem_type env (tseq t1 t2) TBool p false pempty (por e1 e2).
Proof.
  ...
  + eapply se_trans; eapply se_sub; eauto. eapply lls_mono; eauto. eapply vars_locs_monotonic. unfoldq; intuition.
    intros ? ?. erewrite lls_change. eapply lls_mono; eauto. eapply vars_locs_monotonic; eauto. unfoldq; intuition.
    eapply stchain_tighten; eauto. eapply env_type_store_deep_wf. eapply WFE. unfoldq; intuition.
...
Lemma sem_sub: forall env y T p fr1 q1 fr2 q2 e1 e2,
    sem_type env y T p fr1 q1 e1 ->
    psub q1 q2 ->
    psub q2 (pdom env)  ->
    psub e1 e2 ->
    psub e2 (pdom env) ->
    sem_type env y T p (fr1 || fr2)  q2 e2.
Proof.
  intros. intros ? ? ? WFE ST.
  destruct (H S M E WFE ST) as [S1 [M1 [vx [IW1 [SC1 [ST1 [SE1 [HVX HQX]]]]]]]].
  ...
  eapply se_sub; eauto. eapply lls_mono. eapply vars_locs_monotonic. unfoldq; intuition.
  eauto.
  eapply valq_sub; eauto.
Qed.

Lemma sem_sub_var: forall env y T p fr1 q1 Tx qx x e1,
    sem_type env y T p fr1 q1 e1 ->
    q1 x ->
    indexr x env = Some (Tx, false, qx) ->
    psub (plift qx) p -> 
    sem_type env y T p fr1 (por (pdiff q1 (pone x)) (plift qx)) e1.
Proof.
...
Theorem fundamental_property : forall t G T p fr q e,
    has_type G t T p fr q e ->
    sem_type G t T (plift p) fr (plift q) (plift e).
Proof.
  intros ? ? ? ? ? ? ? W. 
  induction W.
  ...
  - rewrite plift_empty. rewrite plift_or. eapply sem_seq; eauto.
...
Corollary safety : forall t T fr q e,
  has_type [] t T qempty fr q e -> 
  exp_type [] st_zero [] t T (plift qempty) fr (plift q) (plift e).
Proof. 
...
Lemma encapsulation: forall G t T
  ...
  st_chain M M' pempty ->
  store_type S' M' ->
  exp_type S' M' H t T pempty false pempty pempty.
Proof.
