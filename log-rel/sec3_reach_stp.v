(*******************************************************************************
* Coq mechanization of the λ_$b<:$-calculus.
...
(* based on stlc_reach.v *)

(* 

Simply-typed lambda calculus with first-order mutable stores and reachability types, combined 
...
| q_cong: forall G q1 fr1 q2 fr2 q3 fr3,
    qtp G fr1 q1 fr2 q2 ->
    qtp G fr2 q2 fr3 q3 ->
    qtp G fr1 q1 fr3 q3
.
...
| s_ref: forall env T1 fr1 q1 T2 fr2 q2,
    ...
    stp env T2 T1 ->
    qtp env fr1 q1 fr2 q2 ->
    qtp env fr2 q2 fr1 q1 ->
    stp env (TRef T1 fr1 q1) (TRef T2 fr2 q2)
| s_fun: forall env T1a fn1a fr1a q1a T2a fn2a fx2a fr2a q2a
                    T1b fn1b fr1b q1b T2b fn2b fx2b fr2b q2b,
    stp env T1b T1a ->
    stp env T2a T2b ->            (* no deep argument dependency in T2 *)
    qtp env fr1b q1b fr1a q1a ->
    qtp ((T1b, fr1b, q1b) :: env) (* shallow arg dependency in q2 permitted *)
      fr2a (qor q2a (qif fx2a (qone (length env))))
      fr2b (qor q2b (qif fx2b (qone (length env)))) ->
    closed_ql (length env) q2a ->
    closed_ql (length env) q2b ->
    bsub fn1b fn1a ->
    bsub fn2a fn2b ->
    bsub fn1b fr1b -> (* if arg has self it also must be fresh *)
    stp env (TFun T1a fn1a fr1a q1a T2a fn2a fx2a fr2a q2a)
            (TFun T1b fn1b fr1b q1b T2b fn2b fx2b fr2b q2b)
| stp_id: forall env T1,
    closed_ty (length env) T1 ->
    stp env T1 T1.

Inductive sqtp : tenv -> ql -> ty -> bool -> ql -> ty -> bool -> ql -> Prop :=
| sq_sub: forall env p T1 fr1 q1 T2 fr2 q2,
    stp env T1 T2 ->
    qtp env fr1 q1 fr2 q2 ->
    sqtp env p T1 fr1 q1 T2 fr2 q2.
...
| t_sub_stp: forall env y T1 T2 p fr1 q1 fr2 q2,
...
* - Semantic Subqualifier 'sem_qtp'
* - Semantic Subpretyping 'sem_stp'
* - Semantic Subtyping 'sem_sqtp'
...
Fixpoint val_type (M:stty) (H:venv) v T : Prop :=
  ...
  | vabs G py ty, TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 =>
        ...
        (psub (val_locs v) (st_locs M)) /\
        (psub (vars_locs H (plift q1)) (val_locs v)) /\  
        (forall S' M' vx,
...
Definition exp_type S M H t T p fr q :=
  exists S' M' v,
    ...
    st_chain M M' (st_locs M) /\
    store_type S' M' /\
    store_effect S S' (vars_locs H p) /\  (* p and e *)
    val_type M' H v T /\
...
Proof. 
  intros T. induction T; intros; destruct v; simpl in *; intuition.
  + econstructor.
  + destruct b; intuition. econstructor; eauto. 
  + econstructor; eauto. 
Qed.
...
Proof.
  ...
  + rewrite val_locs_ref in *. destruct b; intuition.  intros ? ?. eapply H1. auto.
    ...
    assert (st_chain M M' (pempty)). {
      eapply stchain_tighten. eauto.  unfoldq; intuition.
    }
    exists vt.
    split. 
...
Proof. 
  ...
  - (* Ref shrink *)
    destruct b; intuition.
    inversion H0. eauto. 
    rewrite vars_locs_extend in H1. eauto.
    inversion H0. eauto.
    inversion H0. eauto.
    destruct H6 as [vt [IX VT ]].
    exists vt. intuition.
  ...
  - (* Ref grow *)
    ...
    inversion H0. eauto.
    inversion H0. eapply closedq_extend. eauto. rewrite app_length. lia. 
    destruct H6 as [vt [IX VT]].
    exists vt. intuition.
  ...
  - inversion H0. auto.
  - inversion H0. auto.
  - intros ? ?. eapply H6. rewrite vars_locs_extend. eauto.
    inversion H0. auto.
  - (* Abs shrink *)
    inversion H0. subst.
    ...
    eapply IHT2; eauto.
    rewrite vars_locs_extend in H18.
    eauto. eauto.
  - eapply closedty_extend; eauto.
  - eapply closedq_extend; eauto.
  - eapply closedty_extend; eauto.
  - unfoldq. rewrite app_length. intuition. eapply H4 in H7. lia.
  - intros ? ?. eapply H6. rewrite vars_locs_extend in H7. eauto.
    inversion H0. auto.
  - (* Abs grow *)
    inversion H0. subst.
...
Proof.
  ...
  - intros.
    ...
    exists vx; intuition.
    eapply valt_store_change. eauto. eapply stchain_tighten. eauto.
    intros ? ?. exists x. split. eauto. exists vx. intuition. 
  - intros. 
    destruct W as [W' W].
...
Lemma envt_store_change_empty: forall M M' H G,
...
Proof. 
  ...
  - intros.
    ...
    + rewrite indexr_skip in H9. 
      ...
      rewrite indexr_skip.
      destruct W1 as [v [TL [IH [VALT FR]]]].
...
Proof.
  ...
  + (* result well typed *)
    ...
    * rewrite plift_empty. eapply functional_extensionality. intros.
      eapply propositional_extensionality. unfoldq; intuition.
    ...
    * exists vt.
      ...
      -- intros. subst vt.
         split; intros.
         eapply valt_store_change. eauto. eapply stchain_tighten. 2: eauto. eapply stchain_empty.
         eapply valt_store_change. eauto. eapply stchain_tighten. 2: eauto. eapply stchain_empty. 
  + (* qualifier *)
    (* eapply valq_fresh. *)    
    intros ? ?. 
    right. simpl. rewrite val_locs_ref in H1. unfold pone in H1. subst x0.  unfold pstdiff, st_locs, st_extend. unfoldq.
...
Lemma sem_get: forall t env p T fr q,
...
Proof.
  ...
  + (* result well typed *)
    ...
    unfoldq; intuition. 
    intros ? ?. eapply VLS in H3. contradiction.
    eapply stchain_refl. 
  + (* qualifier *)
...
Lemma sem_put: forall t1 t2 env p T fr1 q1,
...
Proof.
  ...
  destruct HVR as (? & ? & ? & ? & vt & ? & ?).
  destruct ST2 as (? & ST2).
  assert (st_locs M1 i) as B. { apply indexr_var_some' in H5. unfold st_locs. unfoldq; intuition. }
  destruct (ST2 i) as (vt' & qt' & vz & ? & ? & ? & ? & ?). eapply SC2. eauto. 
  ...
  + (* store preservation *)
    ...
    destruct (HQR l).
      ...
      destruct fr1. simpl in *. unfold pstdiff, pdiff in H16. apply indexr_var_some' in H14.
...
Proof.
  ...
  destruct fn1. { (* arg may overlap with function? *)
    destruct fr1. { (* arg may be fresh? *)
      (* fn, fr: anything goes *)
      edestruct val_locs_decide. left. simpl. eauto. right. left. eauto.
    } {
      (* fn, not fr *) 
  ...
  + (* store preservation *)
    ...
    eapply vars_locs_monotonic. 2: eapply H11.  intros ? ?. unfoldq; intuition.
    ...
    eapply vars_locs_monotonic. 2: eapply H11.  intros ? ?. unfoldq; intuition.
    destruct frx; simpl in H11. unfold pstdiff, pdiff in H11. unfold st_locs in H11. unfoldq; intuition. contradiction.
...
(* ---------- main subtyping lemmas ---------- *)

Definition sem_qtp G q1 q2 :=
  forall M E,
    env_type M E G pempty ->
    psub q1 (pdom G) /\
    psub q2 (pdom G) /\
    psub (vars_locs E (q1)) (vars_locs E (q2)).

Definition sem_stp G T1 T2 :=
  forall M E,
    env_type M E G pempty ->
...
Lemma sem_qtp_sub: forall G q1 q2,
    psub q1 q2 ->
    psub q2 (pdom G) ->
    sem_qtp G q1 q2.
Proof.
...
Lemma sem_qtp_var: forall G (q1:pl) Tx qx x,
    q1 x ->
    indexr x G = Some (Tx, false, qx) ->
    psub q1 (pdom G) ->
    sem_qtp G q1 (por (pdiff q1 (pone x)) (plift qx)).
Proof.
  intros. intros ? ? WFE.
  eapply WFE in H0.
  destruct H0 as (v & ? & ? & ? & ?).
  ...
  split. {
  intros ? ?. unfoldq. intuition.
  assert (x0 < x). eapply H0. auto. apply indexr_var_some' in H2.  destruct WFE as [? ?]. lia.
   } {
  intros ? (? & ? & ?).
  bdestruct (x =? x1).
  - subst x1. intuition.
    eapply vars_locs_monotonic. 2: eapply H4.
    unfoldq. intuition.
    destruct H6. destruct H6. rewrite H6 in H2.
    inversion H2. subst. eauto.
  - exists x1. unfoldq. intuition. }
Qed.

Lemma sem_qtp_cong: forall G (q1 q2 q3: pl),
  sem_qtp G q1 q2 ->
  sem_qtp G q2 q3 ->
  sem_qtp G q1 q3 .
Proof.
  intros. intros ? ? WFE.
  destruct (H M E WFE) as [P1 [P2 P3]].
  destruct (H0 M E WFE) as [Q1 [Q2 Q3]].
  split. 2: split.
  + auto.
  + auto.
  + unfoldq; intuition.
...
Lemma sem_stp_bool: forall env,
    sem_stp env TBool TBool.
Proof.
  intros. intros M E p.
  split. constructor.
  split. constructor.
...
Lemma sem_stp_ref: forall env T1 fr1 q1 T2 fr2 q2, 
    sem_stp env T1 T2 ->
    sem_stp env T2 T1 ->
    sem_qtp env (plift q1) (plift q2) ->
    sem_qtp env (plift q2) (plift q1) ->
    bsub fr1 fr2 ->
    bsub fr2 fr1 ->
    sem_stp env (TRef T1 fr1 q1) (TRef T2 fr2 q2).
Proof.
  intros. intros M E WFE. split. 2: split. 
  constructor. eapply H. eauto. eapply H2. eauto.
  constructor. eapply H. eauto. eapply H1. eauto. 
  
  intros v V. destruct v, fr1; intuition. simpl in *.
  destruct fr2. { unfold bsub in H3. intuition. }
  destruct V as (? & ? & ? & ? & vt & IX & ?). 
  assert (forall M' : stty,
      st_chain M M' pempty ->
      forall v : vl, psub (val_locs v) pempty -> vt v <-> val_type M' E v T2) as EX. {
    intros. split.
    - intros. eapply H9 in H11; eauto.
      assert (val_type M' E v T2). eapply H. eapply envt_store_change_empty. eauto.
      eapply H11. eauto. eauto. 
    - intros. eapply H9. eauto. eauto. eapply H0. eapply envt_store_change_empty. eauto. eauto. }
  split. 2: split. 3: split. 4: split. 
  - replace (length E) with (length env). 2: { symmetry. eapply WFE. } eapply H0. eauto. 
  - eapply functional_extensionality. intros.
    eapply propositional_extensionality. split.
    rewrite <-H6. eapply H2. eauto.
    rewrite <-H6. eapply H1. eauto.
  - replace (pdom E) with (pdom env). eapply H1. eauto. symmetry. eapply wf_env_type. eauto.
  - eauto.
  - eauto. 
Qed.

Lemma sem_stp_fun0: forall env T1a fn1a fr1a q1a T2a fn2a fx2a fr2a q2a
                    T1b fn1b fr1b q1b T2b fn2b fx2b fr2b q2b,
    sem_stp env T1b T1a ->
    sem_stp env T2a T2b -> 
    sem_qtp env (plift q1b) (plift q1a) ->
    sem_qtp env (plift q2a) (plift q2b) ->
    bsub fn1b fn1a ->
    bsub fr1b fr1a ->
    bsub fn2a fn2b ->
    bsub fx2a fx2b -> 
    bsub fr2a fr2b ->
    sem_stp env (TFun T1a fn1a fr1a q1a T2a fn2a fx2a fr2a q2a)
            (TFun T1b fn1b fr1b q1b T2b fn2b fx2b fr2b q2b).
Proof.
  intros. intros M E WFE. split. 2: split.
  constructor. eapply H. eauto. eapply H0. eauto. eapply H1. eauto. eapply H2. eauto.
  constructor. eapply H. eauto. eapply H0. eauto. eapply H1. eauto. eapply H2. eauto. 
  intros v V. destruct v; intuition. simpl in *.
  destruct V as (? & ? & ? & ? & ? & ? & V).
  replace (length E) with (length env). 2: { symmetry. eapply WFE. }
  replace (pdom E) with (pdom env). 2: { symmetry. eapply wf_env_type. eapply WFE. }
  intuition.
  eapply H. eauto. eapply H1. eauto. eapply H0. eauto. eapply H2. eauto. (* closed *)

  intros ? ?. eapply H13. eapply H1. eauto. eauto. 
  destruct (V S' M' vx) as (S'' & M'' & vy & ? & ? & ? & ? & ? & ?); eauto.
  (* val_type of arg -- automatic! *) {
    eapply H. eapply envt_store_change_empty. eauto. eauto. }
  (* val_locs of arg *) {
    intros ? VX. destruct (H17 _ VX) as [H_fn | [H_fr | H_q]].
    + destruct fn1b. 2: contradiction. left. rewrite H3; eauto. 
    + destruct fr1b. 2: contradiction. right. left. rewrite H4; eauto.
    + right. right. eapply H1. eauto. eauto. 
  }
  exists S'', M'', vy. intuition.
  (* val_type of res *) {
    eapply H0. eapply envt_store_change_empty. eauto. eauto. }
  (* val_locs of res *) {
    intros ? VY. destruct (H23 _ VY) as [(H_q & ?) | [H_fn | [H_fx | H_fr]]].
    + left. eapply H2. eauto. eexists. eapply H24.
    + right. left. destruct fn2a. 2: contradiction. rewrite H5; eauto.
    + right. right. left. destruct fx2a. 2: contradiction. rewrite H6; eauto.
    + right. right. right. destruct fr2a. 2: contradiction. rewrite H7; eauto.
  }
Qed.

Lemma envt_extend_stub: forall M M1 H G vx T1 fr1 q1,
    env_type M H G pempty ->
    val_type M1 H vx T1 ->
    (fr1 = false -> psub (val_locs vx) (vars_locs H (plift q1))) ->
    closed_ty (length H) T1 ->
    closed_ql (length H) q1 ->
    env_type M1 (vx :: H) ((T1, fr1, q1) :: G) pempty.
Proof.
  intros.
  destruct H0 as (? & ? & ? & ?).
  ...
  - intros. bdestruct (x =? length G).
    + subst. rewrite indexr_head in H8. inversion H8. subst T1 fr1 q1.
      exists vx. repeat split.
      rewrite <-H0. eauto.
      rewrite <-H0. rewrite indexr_head. eauto.
      intros. contradiction.
      intros. intros ? ?. eapply vars_locs_extend1. eapply H2. eauto. eauto.
    + destruct (H6 x T fr q) as (v & ? & ? & ? & ?).
      rewrite indexr_skip in H8. eauto. eauto.
      exists v. intuition.
      rewrite indexr_extend1 in H11. eapply H11.
      intros ? ?. eapply vars_locs_extend1. eauto. 
  - intros. intros ? ?.
    destruct H11 as (? & ? & ? & ? & ?). eapply H9 in H12. contradiction.
Qed.


Lemma sem_stp_fun: forall env T1a fn1a fr1a q1a T2a fn2a fx2a fr2a q2a
                    T1b fn1b fr1b q1b T2b fn2b fx2b fr2b q2b,
    sem_stp env T1b T1a ->
    sem_stp env T2a T2b -> 
    sem_qtp env (plift q1b) (plift q1a) ->
    sem_qtp ((T1b, fr1b, q1b) :: env)
      (por (plift q2a) (pif fx2a (pone (length env))))
      (por (plift q2b) (pif fx2b (pone (length env)))) ->
    closed_ql (length env) q2a ->
    closed_ql (length env) q2b ->
    bsub fn1b fn1a ->
    bsub fr1b fr1a ->
    ...
    sem_stp env (TFun T1a fn1a fr1a q1a T2a fn2a fx2a fr2a q2a)
            (TFun T1b fn1b fr1b q1b T2b fn2b fx2b fr2b q2b).
Proof.
  intros. intros M E WFE.
  assert (fr1b = false -> fn1b = false) as H10. { destruct fr1b, fn1b; intuition. }
  split. 2: split.
  constructor. eapply H. eauto. eapply H0. eauto. eapply H1. eauto. eapply H3. eauto.
  constructor. eapply H. eauto. eapply H0. eauto. eapply H1. eauto. eapply H4. eauto. 
  intros v V. destruct v; intuition. simpl in *.
  destruct V as (? & ? & ? & ? & ? & ? & V).
  replace (length E) with (length env). 2: { symmetry. eapply WFE. }
  replace (pdom E) with (pdom env). 2: { symmetry. eapply wf_env_type. eapply WFE. }
  intuition.
  eapply H. eauto. eapply H1. eauto. eapply H0. eauto. 

  intros ? ?. eapply H16. eapply H1. eauto. eauto.
  
  destruct (V S' M' vx) as (S'' & M'' & vy & ? & ? & ? & ? & ? & ?); eauto.
  (* val_type of arg -- automatic! *) {
    eapply H. eapply envt_store_change_empty. eauto. eauto. }
  (* val_locs of arg *) {
    intros ? VX. destruct (H20 _ VX) as [H_fn | [H_fr | H_q]].
    + destruct fn1b. 2: contradiction. left. rewrite H5; eauto. 
    + destruct fr1b. 2: contradiction. right. left. rewrite H6; eauto.
    + right. right. eapply H1. eauto. eauto. 
  }
  exists S'', M'', vy. intuition.
  (* val_type of res *) {
    eapply H0. eapply envt_store_change_empty. eauto. eauto. }
  (* val_locs of res *) {
    intros ? VY. destruct (H26 _ VY) as [H_q (* & ?)*) | [H_fn | [H_fx | H_fr]]].
    + eapply vars_locs_extend1 in H_q. eapply vars_locs_monotonic in H_q. eapply H2 in H_q.
      3: { unfoldq. intuition. }
      destruct H_q as (? & [? | ?] & ?). {
        left. 
        rewrite <-vars_locs_extend with (H':=[vx]). eexists. split. eauto. eauto.
        rewrite plift_closed' in H4. 
        replace (pdom E) with (pdom env). eauto.
        destruct WFE. unfoldq. rewrite H29. intuition.
      } {
        destruct fx2b. 2: contradiction.
        right. right. left.
        replace x0 with (length E) in H28. 2: { destruct WFE. unfoldq. intuition. }
        eapply var_locs_head in H28. eauto.
      }

      eapply envt_extend_stub. eauto. eauto. 
      intros ? ? ?. subst fr1b. eapply H20 in H30. 
      destruct H30. rewrite H10 in H29. contradiction. eauto. (* fn1b !*)
      destruct H29. contradiction. eauto.
      replace (length E) with (length env). eapply H. eauto. symmetry. eapply WFE.
      replace (length E) with (length env). eapply H1. eauto. symmetry. eapply WFE. 
      
    + right. left. destruct fn2a. 2: contradiction. rewrite H7; eauto.
    + destruct fx2a. 2: contradiction.

      edestruct H2 as (? & ? & QSUB). 
      eapply envt_extend_stub. eauto. eauto. 
      intros ? ? ?. subst fr1b. eapply H20 in H28. 
      destruct H28. rewrite H10 in H27. contradiction. eauto. (* fn1b !*)
      destruct H27. contradiction. eauto.
      replace (length E) with (length env). eapply H. eauto. symmetry. eapply WFE.
      replace (length E) with (length env). eapply H1. eauto. symmetry. eapply WFE. 

      assert (vars_locs (vx :: E) (por (plift q2a) (pif true (pone (length env)))) x) as VT. {
        eexists. intuition. right. unfoldq. intuition.
        replace (length env) with (length E). 2: { destruct WFE. unfoldq. intuition. }
        exists vx. rewrite indexr_head. intuition. }

      eapply QSUB in VT. destruct VT as (? & [H_q | H_x] & ?).
      * left. eapply var_locs_shrink in H29. 2: { eapply H4 in H_q. destruct WFE. intuition. }
        eexists. split; eauto.
      * destruct fx2b. 2: contradiction.
        replace x0 with (length E) in H29. 2: { destruct WFE. unfoldq. intuition. }
        eapply var_locs_head in H29.
        right. right. left. eauto. 

    + right. right. right. destruct fr2a. 2: contradiction. rewrite H8; eauto.
  }
Qed.
...
Proof.
  intros. induction H.
  - eapply sem_qtp_sub; eauto. 
  - rewrite plift_or, plift_diff, plift_one.
    eapply sem_qtp_var; eauto.
  - eapply sem_qtp_cong; eauto.
Qed.

Theorem qtp_fr_fundamental: forall env fr1 q1 fr2 q2,
    qtp env fr1 q1 fr2 q2 ->
    bsub fr1 fr2.
Proof.
  intros. induction H.
...
Theorem stp_fundamental: forall env T1 T2,
    stp env T1 T2 ->
    sem_stp env T1 T2.
Proof.
  intros. induction H.
  ...
  - eapply sem_stp_ref; eauto.
    ...
    eapply qtp_fundamental; eauto.
    eapply qtp_fr_fundamental; eauto.
    eapply qtp_fr_fundamental; eauto.
  - eapply sem_stp_fun; eauto.
    eapply qtp_fundamental; eauto.
    rewrite <-plift_one, <-plift_if, <-plift_or.
    rewrite <-plift_if, <-plift_or. 
    eapply qtp_fundamental; eauto.
    eapply qtp_fr_fundamental; eauto.
    eapply qtp_fr_fundamental; eauto.
...
Theorem sqtp_fundamental: forall env p T1 fr1 q1 T2 fr2 q2,
    sqtp env p T1 fr1 q1 T2 fr2 q2 ->
    psub (plift q2) (plift p) ->
    sem_sqtp env (plift p) T1 fr1 (plift q1) T2 fr2 (plift q2).
Proof.
  intros. induction H.
  - intros t ? ? ? ? ? ?.
    destruct (H2 S M E) as (S' & M' & v & ? & ? & ? & ? & ? & ?); eauto.
    exists S', M', v. split. 2: split. 3: split. 4: split. 5: split.
    eauto. eauto. eauto. eauto.
    + eapply stp_fundamental; eauto.
      eapply envt_store_change_empty. eapply envt_tighten. eauto. unfoldq. contradiction.
    + eapply qtp_fundamental in H1 as H1'. 
      eapply valq_sub''; eauto. eapply H1'.
      eapply envt_store_change_empty. eapply envt_tighten. eauto. unfoldq. contradiction.
      eapply qtp_fr_fundamental. eauto. 
Unshelve.
  apply [].
