(*******************************************************************************
* Coq mechanization of the λ$^{\diamond}_{\varepsilon}-calculus.
...
(* based on sec5_reach_nested_effs.v *)
...
(*
   ...
   TFun
        ...
        e2f     : write effect may include the function
        e2x     : write effect may include the argument
        e2
*)
...
| c_fun: forall m T1 T2 fn1 fr1 q1 fn2 ar2 fr2 q2 e2f e2x e2,
    closed_ty m T1 ->
    closed_ty m T2 ->   (* not dependent *)
    closed_ql m q1 ->
    closed_ql m q2 ->  
    closed_ql m e2 -> 
    closed_ty m (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2)
.
...
| t_true: forall env p,
    has_type env ttrue TBool p false qempty qempty
| t_false: forall env p,
    has_type env tfalse TBool p false qempty qempty
| t_var: forall x env T p fr q,
    indexr x env = Some (T, fr, q) ->
    (plift p) x ->
    has_type env (tvar x) T p false (qone x) qempty
| t_ref: forall t env T p q e,
    has_type env t T p false q e ->
    psub (plift q) (plift p) ->
    has_type env (tref t) (TRef T false q) p true q e
| t_get1: forall t env T p q1 fr q e, (* special case 1 *)
    has_type env t (TRef T false q1) p fr q e ->
    psub (plift q1) (plift p) -> 
    has_type env (tget t) T p false q1 e
| t_get2: forall t env T p q1 fr q e, (* special case 2 *)
    has_type env t (TRef T false q1) p fr q e ->
    has_type env (tget t) T p fr q e
| t_get: forall t env T p q1 rf1 fr q e,
    has_type env t (TRef T rf1 q1) p fr q e ->
    psub (plift q1) (plift p) ->
    has_type env (tget t) T p (if rf1 then fr else false) (qor q1 (qif rf1 q)) e

| t_put: forall t1 t2 T env p fr1 q1 rf2 q2 e1 e2,
    has_type env t1 (TRef T rf2 q2) p fr1 q1 e1 ->
    has_type env t2 T p false q2 e2 ->
    (* psub (plift q2)(plift p) -> *)
    has_type env (tput t1 t2) TBool p false qempty (qor e1 (qor e2 q1))
| t_app: forall env f t T1 T2 p frf qf frx qx fn1 fr1 q1 fn2 ar2 fr2 q2 e2 e2f e2x ef ex,
    has_type env f (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2) p frf qf ef->  
    has_type env t T1 p frx qx ex ->
    (* ---- different app cases: *)
    (if fn1 then
       if fr1 then
         True
         ...
         (frx = false /\ psub (plift qx) (plift q1)) (* keep it *)
     else
       if fr1 then
         psub (pand 
                 (vars_trans' env qf)
    ...
    has_type env (tapp f t) T2 p
      ...
      (qor ef (qor ex (qor e2 (qor (qif e2f qf)(qif e2x qx)))))
| t_abs: forall env t T1 T2 p fn1 fr1 q1 fn2 ar2 fr2 q2 e2f e2x e2 qf,
    has_type ((T1, fr1, (qand p (qor q1 (qif fn1 qf))))::env) t T2 
      ...
      (qor e2 (qor (qif e2x (qone (length env)))(qif e2f (qand p qf))))
      ->
...
| t_seq: forall env t1 t2 p1 p2 p q1 q2 e1 e2,
    ...
    has_type env (tseq t1 t2) TBool p false qempty (qor e1 e2)
| t_sub: forall env y T p fr1 q1 e1 fr2 q2 e2,
...
| t_sub_var: forall env y T p fr1 q1 e1 qx x Tx,
    has_type env y T p fr1 q1 e1 ->
    plift q1 x ->
    indexr x env = Some (Tx, false, qx) ->
    psub (plift qx) (plift p) -> (* necessary? *)
    has_type env y T p fr1 (qor (qdiff q1 (qone x)) qx) e1
.
...
Fixpoint val_type (M:stty) (H1 H2:venv) v1 v2 T1 T2 : Prop :=
  ...
  | vref l1, vref l2, TRef T1 fr1 q1, TRef T2 fr2 q2 => 
    ...
    strel M l1 l2 /\ 
    st_filter M (locs_locs_stty (st1 M) (val_locs v1)) (locs_locs_stty (st2 M) (val_locs v2)) /\
    (* psub (locs_locs_stty (st1 M) (val_locs v1)) (st_locs1 M) /\ (* l < M, q** < M *)
    psub (locs_locs_stty (st2 M) (val_locs v2)) (st_locs2 M) /\ (* l < M, q** < M *)
    st_locs1 M l1 /\ (* redundant with line above *)
    st_locs2 M l2 /\ (* redundant with line above *)
    strel M l1 l2 /\ 
    (forall l1 l2, strel M l1 l2 -> (locs_locs_stty (st1 M) (val_locs v1)) l1 <-> (locs_locs_stty (st2 M) (val_locs v2) l2)) /\  (* A *)  *)
     exists vt qt1 qt2,
        indexr l1 (st1 M) = Some qt1 /\ (* no distinction in stty *)
        indexr l2 (st2 M) = Some qt2 /\
        st_valtype M l1 l2 = vt /\
        (forall M' S1' S2',
        ...
        psub (locs_locs_stty (st1 M) (plift qt1)) (locs_locs_stty (st1 M) (pone l1)) /\ (* a bit redundant ... could rely on storet ? *)
        psub (locs_locs_stty (st2 M) (plift qt2)) (locs_locs_stty (st2 M) (pone l2)) (* a bit redundant ... could rely on storet ? *)
  |  vabs G1 py1 ty1, vabs G2 py2 ty2, TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2, TFun T1' fn1' fr1' q1' T2' fn2' ar2' fr2' q2' e2f' e2x' e2' =>
        closed_ty (length H1) T1 /\
        (psub (plift q1) (pdom H1)) /\
       ...
       (* (psub (locs_locs_stty (st1 M) (val_locs v1)) (pnat (length1 M))) /\
        (psub (locs_locs_stty (st2 M) (val_locs v2)) (pnat (length2 M))) /\
        (forall l1 l2, strel M l1 l2 -> (locs_locs_stty (st1 M) (val_locs v1)) l1 <-> (locs_locs_stty (st2 M) (val_locs v2)) l2) /\  (* A *) *)
        (st_filter M (locs_locs_stty (st1 M)(val_locs v1))(locs_locs_stty (st2 M)(val_locs v2))) /\
        (forall S1' S2' M' vx1 vx2,
            ...
            store_type S1'' S2'' M''
            /\
            store_effect S1' S1'' (por (locs_locs_stty (st1 M') (vars_locs H1 (plift e2))) (por (pif e2x (locs_locs_stty (st1 M') (val_locs vx1)))(pif e2f (locs_locs_stty (st1 M') (val_locs v1)))))
            /\
            store_effect S2' S2'' (por (locs_locs_stty (st2 M') (vars_locs H2 (plift e2'))) (por (pif e2x' (locs_locs_stty (st2 M') (val_locs vx2)))(pif e2f' (locs_locs_stty (st2 M') (val_locs v2)))))
            /\
            val_type
              M''
              H1
...
Definition exp_type S1 S2 M H1 H2 t1 t2 S1' S2' M' v1 v2 T p  fr q (e: pl) :=
  ...
  st_filter M' (st_locs1 M') (st_locs2 M') /\
  store_type S1' S2' M' /\
  store_effect S1 S1' (locs_locs_stty (st1 M) (vars_locs H1 (pand p e))) /\
  store_effect S2 S2' (locs_locs_stty (st2 M) (vars_locs H2 (pand p e))) /\
      ...
      - intros. eapply lls_s in H4 as HU. 3: eapply IX1. eapply HV in HU. 2: intuition.
        inversion HU.
          * subst. inversion H5. subst l2.
            (* TODO: add store_type as premise? 
               from
                 H10 : strel M i i0
                 H13 : strel M l1 i0
               follows via store_type i=l1 and then
                 H19 : locs_locs_stty (st1 M) (plift qt1) l1
               is a contradiction (store_type telescope -> qt1 < i
...
Proof. 
  ...
  + { split. 2: split.
      ...
      * intros. split; intros. erewrite <- lls_change1 in H15; eauto.
        erewrite <- lls_change2; eauto.
        destruct H0 as (? & (? & ? & ?) & ? & ? & ?). 
        eapply H18 in H13 as A. intuition.    
        erewrite <- lls_change2 in H15; eauto.
...
Proof. 
  ...
  - (* Abs shrink *)
    ...
    rewrite vars_locs_extend in H25; eauto.
    rewrite vars_locs_extend in H26; eauto.
    eapply IHT2; eauto.
    rewrite vars_locs_extend in H30; eauto.
    rewrite vars_locs_extend in H32; eauto.  
  - eapply closedty_extend. apply H4. auto.
  - unfoldq. rewrite app_length. intuition. eapply H3 in H14. lia.
  - eapply closedty_extend. apply H5. auto.
  - unfoldq. rewrite app_length. intuition. eapply H6 in H14. lia.
  - unfoldq. rewrite app_length. intuition. eapply H7 in H14. lia.
  - eapply closedty_extend; eauto. 
  - unfoldq. rewrite app_length. intuition. eapply H9 in H14. lia.
  - eapply closedty_extend; eauto.
  - unfoldq. rewrite app_length. intuition. eapply H11 in H14. lia.
  - unfoldq. rewrite app_length. intuition. eapply H12 in H14. lia.
  - (* Abs grow *)
    inversion H0. subst.
    destruct (H15 S1' S2' M' vx1 vx2) as [S1'' [S2'' [M'' [vy1 [vy2 [HEY [HVY]]]]]]].
      eauto.
      ...
      eauto.
      eapply IHT1; eauto.
      rewrite vars_locs_extend in H19; eauto. 
      rewrite vars_locs_extend in H20; eauto. 
...
Lemma has_type_closed: forall M H1 H2 G p q T e t fr,
  env_type M H1 H2 G (plift p) ->
...
Proof.
  ...
  + inversion H0. subst. eapply closedq_or; intuition.
  ...
  + inversion H6. subst. eapply closedq_or; intuition. eapply closedq_or; intuition. eapply closedq_or; intuition. 
    eapply closedq_or; intuition. intros ? ?. destruct e2f; unfoldq; intuition. intros ? ?. destruct e2x; unfoldq; intuition.
  ...
  + eapply closedq_or; intuition. 
    unfold closed_ql. intros. unfold qdiff in H9. apply andb_true_iff in H9.
    destruct H9. eapply H8 in H9. auto.
...
Lemma valq_store_change1: forall M M' M'' S1 S2 G H1 H2 v1 v2 T T' fr p q
    ...
    (A: psub (locs_locs_stty (st1 M') (val_locs v1)) (st_locs1 M'))
    (B: psub (locs_locs_stty (st2 M') (val_locs v2)) (st_locs2 M')), 
    st_chain M' M'' ->
    val_qual (st1 M) (st1 M'') H1 v1 fr (pand p q).
Proof.
  intros. intros ? ?.
...
Proof. 
  ...
  + (* 1st *)
    ...
    - destruct frf. 2: contradiction. 
      ...
      destruct Hf_fr. unfold length1 in Hx_q.  unfoldq; intuition. unfoldq; intuition.
      eapply stchain_tighten. eapply envt_filter_deep; eauto. unfoldq; intuition. 
...
Lemma sem_true: forall G p,
    sem_type G ttrue ttrue TBool p false pempty pempty.
Proof.
...
Lemma sem_false: forall G p,
    sem_type G tfalse tfalse TBool p false pempty pempty.
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST.
...
Lemma sem_var: forall G x T1 (p:pl) fr q,
  ...
  p x ->
  sem_type G (tvar x) (tvar x) T1 p false (pone x) pempty.
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST. 
...
Lemma sem_ref: forall t1 t2 G T p q e,
    sem_type G t1 t2 T p false (plift q) (plift e) ->
    psub (plift q) p ->
    sem_type G (tref t1) (tref t2)(TRef T false q) p true (plift q) (plift e).
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST.
  ...
  + (* valq 1 *)
    ...
    * right. simpl. subst x1 q0 M0. rewrite val_locs_ref in *.
      unfold pone in H5. subst x x0. unfold pnot. unfoldq.
      simpl. rewrite SL1. 
...
Lemma sem_get1: forall t1 t2 env T p q1 fr q e,
    sem_type env t1 t2 (TRef T false q1) p fr q e ->
    psub (plift q1) p ->
    sem_type env (tget t1) (tget t2) T p false (plift q1) e.
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST.
  destruct (H M H1 H2 WFE STF S1 S2 ST) as [S1' [S2' [M' [v1 [v2 IE]]]]].
...
Lemma sem_get2: forall t1 t2 env T p rf1 q1 fr q e,
    sem_type env t1 t2 (TRef T rf1 q1) p fr q e ->
    sem_type env (tget t1) (tget t2) T p fr q e.
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST.
  destruct (H M H1 H2 WFE STF S1 S2 ST) as [S1' [S2' [M' [v1 [v2 IE]]]]].
  destruct IE as [IE1 [IE2 [STC [STF' [ST' [SE1 [SE2 [HV [HQ1 HQ2]]]]]]]]]. 
...
Lemma sem_get: forall t1 t2 env T p q1 rf1 fr q e,
    sem_type env t1 t2 (TRef T rf1 q1) p fr q e ->
    psub (plift q1) p ->
    sem_type env (tget t1) (tget t2) T p (if rf1 then fr else false) (por (plift q1) (pif rf1 q)) e.
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST.
  destruct (H M H1 H2 WFE STF S1 S2 ST) as [S1' [S2' [M' [v1 [v2 IE]]]]].
  destruct IE as [IE1 [IE2 [STC [STF' [ST' [SE1 [SE2 [HV [HQ1 HQ2]]]]]]]]]. 
  ...
  + destruct STL1 as (X & Y). destruct (Y i) as [qt1' [v' [IM1'' [IS1' [? ?]]]]].
    ...
    * assert (val_qual (st1 M) (st1 M') H1 v1 fr (pand p q)). {
      ...
      intros ? ?. destruct (H10 x). (* valq_sub, but don't have q < E *)
      -- eauto.
...
Lemma sem_put: forall t1 t2 t3 t4 env T p fr1 q1 e1 rf2 q2 e2,
    sem_type env t1 t2 (TRef T rf2 q2) p fr1 q1 e1 ->
    sem_type env t3 t4 T p false (plift q2) e2 ->
    (* psub q1 p -> *)
    sem_type env (tput t1 t3) (tput t2 t4) TBool p false pempty (por e1 (por e2 q1)).
Proof.
  intros. intros M H1 H2 WFE STF S1 S2 ST.
  destruct (H M H1 H2 WFE STF S1 S2 ST) as [S1' [S2' [M' [v1 [v2 IE]]]]].
  ...
  * (* store preservation 1 *)
    ...
    bdestruct (i =? l). { subst. destruct (HQ1 l).
      ...
      destruct fr1. simpl in *. unfold pnot, pdom in H3. apply indexr_var_some' in H10.
      rewrite LS1 in H10. destruct H3. auto. 
      simpl in H3. contradiction.
      
    }{ rewrite update_indexr_miss.
       assert ((locs_locs_stty (st1 M) (vars_locs H1 (pand p e2)) l) =
               locs_locs_stty (st1 M') (vars_locs H1 (pand p e2)) l) as LLS1. {
          assert (st_chain_partial M M' (locs_locs_stty (st1 M) (vars_locs H1 (pand p e2))) (locs_locs_stty (st2 M) (vars_locs H2 (pand p e2)))).
          eapply stchain_tighten; eauto. eapply envt_filter_deep; eauto. unfoldq; intuition.
          eapply envt_store_deep_wf; eauto. unfoldq; intuition. eapply envt_store_deep_wf; eauto. unfoldq; intuition.
          eapply lls_change1 in H12. rewrite H12. auto.
        }

        assert ( ~ locs_locs_stty (st1 M) (vars_locs H1 (pand p e1)) l) as X. { 
  ...
  * (* store preservation 2 *)
    ...
    bdestruct (i0 =? l). { subst. destruct (HQ2 l).
      ...
      destruct fr1. simpl in *. unfold pnot, pdom in H3. apply indexr_var_some' in H10.
      rewrite LS2 in H10. destruct H3. auto.
      simpl in H3. contradiction.
      
    }{ rewrite update_indexr_miss.
       assert ((locs_locs_stty (st2 M) (vars_locs H2 (pand p e2)) l) =
               locs_locs_stty (st2 M') (vars_locs H2 (pand p e2)) l) as LLS2. {
          assert (st_chain_partial M M' (locs_locs_stty (st1 M) (vars_locs H1 (pand p e2))) (locs_locs_stty (st2 M) (vars_locs H2 (pand p e2)))).
          eapply stchain_tighten. eapply envt_filter_deep; eauto. unfoldq; intuition. eauto.
          eapply envt_store_deep_wf; eauto. unfoldq; intuition.
          eapply envt_store_deep_wf; eauto. unfoldq; intuition.
        ...
        assert ( ~ locs_locs_stty (st2 M) (vars_locs H2 (pand p e1)) l) as X. { 
          intros ?. eapply H9. eapply lls_mono; eauto. eapply vars_locs_mono; eauto. unfoldq; intuition.
        }
...
Lemma eff_aux: forall M' vx E p e2f e2 e2x l,
  closed_ql (length E) e2 ->
  closed_ql (length E) p ->
  locs_locs_stty M' (vars_locs (vx :: E) (pand(por (plift p) (pone (length E)))
    (por (plift e2) (por (pif e2x (pone (length E))) (pif e2f (plift p)))))) l
  -> 
  por (locs_locs_stty M' (vars_locs E (plift e2))) (por (pif e2x (locs_locs_stty M' (val_locs vx)))
    (pif e2f (locs_locs_stty M' (vars_locs E (plift p))))) l.
Proof.
  intros. inversion H1; subst.
  - destruct H2 as [? [? ?]]. destruct H2 as [? ?].
    destruct H2; destruct H4.
    left. left. exists x; intuition. eapply var_locs_shrink; eauto.
    right. destruct H4; destruct e2x; destruct e2f; simpl in *; intuition. 
    unfoldq; intuition. subst x. eapply H0 in H2. lia.
    unfoldq; intuition. subst x. eapply H0 in H2. lia.
    right. simpl. left. exists x; intuition. eapply var_locs_shrink; eauto.
    right. simpl. left. exists x; intuition. eapply var_locs_shrink; eauto.
    left. left. exists x; intuition. eapply var_locs_shrink; eauto.
    unfold pone in H2. subst. destruct H4; destruct e2x; intuition; simpl in *. 
    right. left. simpl. left. eapply var_locs_head in H3. auto.
    destruct e2f; simpl in H2. eapply H0 in H2. lia. contradiction.
    destruct e2f; simpl in H2. eapply H0 in H2. lia. contradiction.
  - subst. destruct H2 as [? [? ?]]. destruct H2 as [? ?].
    destruct H2; destruct H6.
    left. eapply lls_s; eauto. exists x; intuition. eapply var_locs_shrink; eauto.
    right. destruct H6; destruct e2x; destruct e2f; simpl in *; intuition.
    unfoldq; intuition. subst x. eapply H0 in H2. lia.
    unfoldq; intuition. subst x. eapply H0 in H2. lia.
    right. simpl. eapply lls_s; eauto. exists x; intuition. eapply var_locs_shrink; eauto.
    right. simpl. eapply lls_s; eauto. exists x; intuition. eapply var_locs_shrink; eauto.
    unfoldq; intuition. subst x. eapply H in H6. lia.
    unfold pone in H2. subst. destruct H6; destruct e2x; intuition; simpl in *. 
    right. left. simpl. destruct H5 as [? [? ?]]. rewrite indexr_head in H5. inversion H5. subst vx.
    eapply lls_s; eauto.
    destruct e2f; simpl in H2. eapply H0 in H2. lia. contradiction.
    destruct e2f; simpl in H2. eapply H0 in H2. lia. contradiction.
Qed.

Lemma sem_abs: forall env t1 t2 T1 T2 p fn1 fr1 q1 fn2 ar2 fr2 q2 qf e2 e2f e2x,
    sem_type ((T1, fr1, (qand p (qor q1 (qif fn1 qf))))::env) t1 t2 T2 
      ...
      (por (plift e2) (por (pif e2x (pone (length env)))(pif e2f (plift (qand p qf)))))
      ->
    ...
    sem_type env (tabs (qand p qf) t1) (tabs (qand p qf) t2)
      (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2)
      (plift p) false (plift qf) pempty.
Proof.
  intros. intros ? ? ? WFE STF ? ? ST.
  exists S1, S2.
  exists M. (* (st_tighten M (val_max (vabs E (qand p qf) t))). *)
  exists (vabs H7 (qand p qf) t1), (vabs H8 (qand p qf) t2).
  split. 2: split. 3: split. 4: split. 5: split. 6: split. 7: split. 8: split. 9: split.
  + (* term evaluates *)
  ...
  + (* result well typed *)
   ...
   repeat rewrite val_locs_abs. eapply envt_filter_deep; eauto. rewrite plift_and. unfoldq; intuition.

      (* INDUCTION *)
   rename H13 into HQX1. rename H14 into HQX2. 
   rewrite val_locs_abs in *. rewrite val_locs_abs in *.
   rewrite plift_and in *. 
   assert (env_type M' (vx1 :: H7) (vx2 :: H8) ((T1,fr1, (qand p (qor q1 (qif fn1 qf)))) :: env) 
    ...
    - intros. rewrite plift_and in *. rewrite plift_or in *. 
      subst fr1. intros ? ?. eapply HQX1 in H13.
    ...
    - intros. rewrite plift_and in *. rewrite plift_or in *. 
      subst fr1. intros ? ?. eapply HQX2 in H13.
      destruct H13 as [ A | [B | C]]. {
        destruct fn1. 2: contradiction.
        simpl in A. eapply lls_mono. 2: eauto. eapply vars_locs_mono. 
        unfoldq. intuition.
    ...
    - rewrite plift_and, plift_or, plift_if. rewrite pand_or_distribute2.
      intros ? ?. destruct H13. destruct H13. eapply H0 in H14. unfoldq; intuition. destruct fn1; unfoldq; intuition. 
    - eapply closedq_and; eauto.
  }    
    
   destruct (H M' (vx1::H7) (vx2::H8) WFE' H10  S1' S2' H11) as [S1'' [S2'' [M'' [vy1 [vy2 IHW1]]]]].
  ...
  - (* store preserve 1 *)
    rewrite <- plift_and in *.
    intros ? ? PV ?. eapply SE3. intros VL. eapply PV.
    replace (length env) with (length H7) in VL.
    eapply eff_aux; eauto. rewrite L1. auto. rewrite L1. eapply closedq_and; eauto. auto.
   
   
  - (* store preserve 2 *)
    rewrite <- plift_and in *.
    intros ? ? PV ?. eapply SE4. intros VL. eapply PV.
    replace (length env) with (length H8) in VL.
    eapply eff_aux; eauto. rewrite L2. auto. rewrite L2. eapply closedq_and; eauto. auto.

  - (* valt *)
    eapply valt_extend; eauto.
    rewrite L1. auto. rewrite L2. auto.
    replace (vx1::H7) with ([vx1]++H7) in HVY; auto.
    replace (vx2::H8) with ([vx2]++H8) in HVY; auto.
    eapply HVY.
  ...
  - (* valq 1 *)
   ...
   --  (* q *)
     ...
     bdestruct (x0 =? length H7).
      (* from arg *)
     * subst x0. eapply lls_mono in H16. 2: { intros ? ?. eapply var_locs_head. eauto. }
       right. right. left.
       destruct ar2. eauto. 
       destruct H14. { destruct H14. eapply H5 in H17. lia.  }
       destruct H15. eapply H4 in H15. lia.
       destruct fn2. destruct H15; simpl in H15. contradiction. 
       destruct H15. eapply H5 in H17. lia. 
       destruct H15; simpl in H15; contradiction.
     * (* from func *)
       assert (x0 < length (vx1:: H7)). eapply lls_var in H16. destruct H16. rewrite indexr_extend1 in H16. eapply H16. simpl in H18.
       eapply lls_mono in H16. 2: { intros ? ?. eapply var_locs_shrink. eauto. lia. }
       destruct H14 as [[? ?] | ?]; destruct H15 as [? | [? | ?] ].
       left. split. 
       eapply lls_mono. 2: eapply H16. intros ? ?. exists x0. intuition. 
       eapply lls_vars'. eapply H16. unfoldq; intuition.
       destruct ar2; contradiction. 
       destruct fn2. right. left. simpl. eapply lls_vars'. eapply H16. auto. 
       simpl in H15. contradiction. contradiction. contradiction. contradiction.
  ...
  - (* valq 2 *)
    ...
    -- (* q *)
      eapply lls_vars in HY_q. 
      destruct HY_q as (? & (? & ?) & ?).
      replace (length env) with (length H8) in *.
      bdestruct (x0 =? length H8).
      * (* from arg *)
        subst x0. eapply lls_mono in H16. 2: { intros ? ?. eapply var_locs_head. eauto. }
        right. right. left.
        destruct ar2. eauto. 
        destruct H14. { destruct H14. eapply H5 in H17. lia.  }
        destruct H15. eapply H4 in H15. lia.
        destruct fn2. simpl in H15.  destruct H15; simpl in H15. contradiction. 
        destruct H15. eapply H5 in H17. lia. 
        destruct H15; simpl in H15; contradiction.
      * (* from func *)
        assert (x0 < length (vx2:: H8)). eapply lls_var in H16. destruct H16. rewrite indexr_extend1 in H16. eapply H16. simpl in H18.
        eapply lls_mono in H16. 2: { intros ? ?. eapply var_locs_shrink. eauto. lia. }
        destruct H14 as [[? ?] | ?]; destruct H15 as [? | [? | ?] ].
        left. split. 
        eapply lls_mono. 2: eapply H16. intros ? ?. exists x0. intuition. 
...
Lemma sem_app: forall env f1 f2 t1 t2 T1 T2 p frf qf frx qx fn1 fr1 q1 fn2 ar2 fr2 q2 e2f e2x e2 ex ef
    (E2:  psub (plift e2) (plift p)),
    sem_type env f1 f2 (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2) (plift p) frf (plift qf) ef ->  
    sem_type env t1 t2 T1 (plift p) frx (plift qx) (plift ex) ->
    (if fn1 then
       if fr1 then
         True
       else
    ...
    sem_type env (tapp f1 t1) (tapp f2 t2) T2 (plift p)
      ...
      (por ef (por (plift ex) (por (plift e2) (por (pif e2f (plift qf)) (pif e2x (plift qx)))))).
Proof. 
  ...
  eapply stchain_tighten. eapply valt_filter; eauto. eauto. eapply H32. eapply H32.
  ...
  destruct fn1. { (* arg may overlap with function? *)
    destruct fr1. { (* arg may be fresh? *)
      ...
      destruct HSEP as [SEP | [SEP | SEP]]. { (* fn *)
        ...
        eapply lls_var in H37. destruct H37 as (? & ? & ?).
    ...
    right. destruct fr1. {
      ...
      edestruct val_locs_stty_decide. destruct ST2; eauto. { (* check to see if we're aliasing function *)
        ...
        rewrite plift_vt. eapply vt_mono. 2: eapply H34. unfoldq. intuition. eauto. 
        rewrite plift_vt. eapply vt_mono. 2: eapply H35. unfoldq. intuition. eauto.
        unfoldq. intuition eauto.
      }{ 
        left. eauto.
      }
  ...
  + (* store preservation 1 *)  
    ...
    destruct H33 as [EE2 | [E2X | E2F]]. {
      ...
      destruct frx; unfold not, pdom in H33. destruct H33. unfold length1 in H34. 
      ...
      eapply envt_store_deep_wf; eauto. unfoldq; intuition.
      eapply envt_store_deep_wf; eauto. unfoldq; intuition.
      destruct frf; simpl in H33. unfold pnot, pdom in H33. destruct H33. unfold length1, pnat in *. lia.
      contradiction. contradiction.
    }
  ...
  destruct H33 as [EE2 | [E2X | E2F]]. {
    ...
    destruct frx; unfold pnot, pdom in H33. destruct H33. 
    ...
    eapply envt_store_deep_wf; eauto. unfoldq; intuition.
    eapply envt_store_deep_wf; eauto. unfoldq; intuition.
    destruct frf; simpl in H33. unfold pnot, pdom in H33. destruct H33. unfold length2, pnat in *. lia.
    contradiction. contradiction.
  }
  ...
  + remember (vabs l q t) as vf.
    assert (val_qual (st1 M) (st1 M3) H5 vf frf (pand (plift p) (plift qf))) as HQF'. {
      eapply valq_store_change1. eauto. eauto. eauto. eauto. 
      eapply valt_deep_wf; eauto. eapply valt_deep_wf; eauto. eapply st_trans; eauto.  }
    assert (val_qual (st1 M1) (st1 M3) H5 vx1 frx (pand (plift p) (plift qx))) as HQX'. {
      eapply valq_store_change1. eapply envt_store_extend; eauto. eauto. eauto. eauto. 
      eapply valt_deep_wf; eauto. eapply valt_deep_wf; eauto. eauto. }
  ...
  + remember (vabs l0 q0 t0) as vf.
    assert (val_qual (st2 M) (st2 M3) H6 vf frf (pand (plift p) (plift qf))) as HQF'. {
      eapply valq_store_change2. eauto. eauto. eauto. eauto. 
      eapply valt_deep_wf; eauto. eapply valt_deep_wf; eauto.  eapply st_trans; eauto.  }
    assert (val_qual (st2 M1) (st2 M3) H6 vx2 frx (pand (plift p) (plift qx))) as HQX'. {
      eapply valq_store_change2. eapply envt_store_extend; eauto. eauto. eauto. eauto. 
      eapply valt_deep_wf; eauto. eapply valt_deep_wf; eauto. eauto. }
...
Lemma sem_sub: forall env y1 y2 T p fr1 q1 fr2 q2 e1 e2,
    sem_type env y1 y2  T p fr1 q1 e1 ->
    psub q1 q2 ->
    psub q2 (pdom env)  ->
    psub e1 e2 ->
    psub e2 (pdom env) ->
    sem_type env y1 y2 T p (fr1 || fr2)  q2 e2.
Proof.
  intros. intros ? ? ? WFE  STF S1 S2 ST.
  destruct (H M H4 H5 WFE STF S1 S2 ST) as [S1' [S2' [M1 [vx1 [vx2 IE]]]]]. 
  destruct IE as [IE1 [IE2 [SC1 [STF1 [ST1 [SE1 [SE2 [HVX [HQX1 HQX2]]]]]]]]].
  assert (psub q2 (pdom H4)). {
    unfoldq. destruct WFE. rewrite H6. intuition. } 
  assert (psub q2 (pdom H5)). {
    unfoldq. destruct WFE as [? [? ?]]. rewrite H8. intuition. }   
...
Lemma sem_sub_var: forall env y1 y2 T p fr1 q1 Tx qx x e1,
    sem_type env y1 y2 T p fr1 q1 e1 ->
    q1 x ->
    indexr x env = Some (Tx, false, qx) ->
    psub (plift qx) p -> 
    sem_type env y1 y2 T p fr1 (por (pdiff q1 (pone x)) (plift qx)) e1.
Proof.
...
(*
t_seq: forall env t1 t2 T1 T2 p1 p2 fr1 fr2 p q1 q2 e1 e2,
  has_type env t1 T1 p1 fr1 q1 e1 ->
  has_type env t2 T2 p2 fr2 q2 e2 ->
  psub (plift p1) (plift p) ->
  psub (plift p2) (plift p) ->
...
Lemma sem_seq: forall env t1 t2 t3 t4 p1 p2 p q1 q2 e1 e2
  (E1: sem_type env t1 t2 TBool p1 false q1 e1)
  (E2: sem_type env t3 t4 TBool p2 false q2 e2)
  (SUB1: psub p1 p)
  (SUB2: psub p2 p),
  sem_type env (tseq t1 t3) (tseq t2 t4) TBool p false pempty  (por e1 e2).
Proof.
  intros. intros ? ? ? WFE STF S1 S2 ST.
  (* E1 *)
...
Proof.
  ...
  - rewrite plift_one, plift_empty. eapply sem_var; eauto.
...
| cx_ref: forall G t p T  q e,
    has_type G t T p false q e ->
    psub (plift q)(plift p) ->
    ctx_type (fun x => tref x) G T (plift p) false (plift q) (plift e) G (TRef T false q) (plift p) true (plift q) (plift e)
| cx_get1: forall G T p q fr e q1,
    psub (plift q1) p ->
    ctx_type (fun x => tget x) G (TRef T false q1) p fr q e G T p false (plift q1) e
| cx_get2: forall G T p q fr e q1,
    ctx_type (fun x => tget x) G (TRef T false q1) p fr q e G T p fr q e    
| cx_get: forall G T p q fr fr1 e q1,
    psub (plift q1) p ->
    ctx_type (fun x => tget x) G (TRef T fr1 q1) p fr (plift q) e G T p (if fr1 then fr else false) (por (plift q1) (pif fr1 (plift q))) e     
| cx_put1: forall G t1 T p fr1 q1 e1 e2 fr2 q2,
    has_type G t1 (TRef T fr2 q2) p fr1 q1 e1 ->
    psub (plift q2)(plift p) ->
    ctx_type (fun x => tput t1 x) G T (plift p) false (plift q2) e2 G TBool (plift p) false pempty (por (plift e1) (por e2 (plift q1)))
| cx_put2: forall G t2 T p fr1 q1 e1 e2 fr2 q2,
    has_type G t2 T p false q2 e2 ->
    psub (plift q2)(plift p) ->
    ctx_type (fun x => tput x t2) G (TRef T fr2 q2) (plift p) fr1 (plift q1) e1 G TBool (plift p) false pempty (por e1 (por (plift e2) (plift q1)))
| cx_app1: forall t2 G T1 (fr1:bool) T2 frx q1 fr2 q2 e2 ef p frf qf ar2 e2f e2x ex qx (fn1:bool) fn2,
    has_type G t2 T1 p frx qx ex ->
    (if fn1 then
       if fr1 then
         True
       else
         (* only support truly parametric cases here *)
         (*(frx = false /\ (exists x0, f = tvar x0 /\ psub (plift qx) (pone x0))) \/ (* f : µz. (x: A^z); f(f); val x: A^f; f(x)*)
         (frx = false /\ (exists p0 t0, f = tabs p0 t0 /\ psub (plift qx) (plift p0))) \/ *)
         (frx = false /\ psub (plift qx) (plift q1)) (* keep it *)
     else
    ...
    psub (plift q2) (plift p) ->
    psub (plift e2) (plift p) ->
    ctx_type (fun x => tapp x t2) G (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2) (plift p) frf (plift qf) ef G T2 (plift p) (fn2 && frf || ar2 && frx || fr2) (por (pif fn2 (plift qf)) (por (pif ar2 (plift qx)) (plift q2))) (por ef (por (plift ex) (por (plift e2) (por (pif e2f (plift qf))(pif e2x (plift qx))))))
| cx_app2: forall t1 G T1 T2 p frf qf frx qx fn1 fr1 q1 fn2 ar2 fr2 q2 e2 e2f e2x ef ex,
    has_type G t1 (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2) p frf qf ef ->
    (if fn1 then
       if fr1 then
         True
       else
         (frx = false /\ (exists x0, t1 = tvar x0 /\ psub (plift qx) (pone x0))) \/ (* f : µz. (x: A^z); f(f); val x: A^f; f(x)*)
    ...
    psub (plift q2) (plift p) ->
    psub (plift e2) (plift p) ->
    ctx_type (fun x => tapp t1 x) G T1 (plift p) frx (plift qx) (plift ex)  G T2 (plift p) (fn2 && frf || ar2 && frx || fr2) (plift (qor (qif fn2 qf) (qor (qif ar2 qx) q2)))(plift ((qor ef (qor ex (qor e2 (qor (qif e2f qf)(qif e2x qx)))))))
| cx_abs: forall G T1 T2 p fn1 fr1 q1 fn2 ar2 fr2 q2 e2x e2f e2 qf,
    closed_ty (length G) T1 ->
    ...
    ctx_type (fun x => tabs (qand p qf) x) ((T1,fr1, (qand p (qor q1 (qif fn1 qf))))::G) T2 (plift (qor (qand p qf)(qone (length G)))) fr2 (plift (qor q2 (qor (qif ar2 (qone (length G)))(qif fn2 (qand p qf)))) ) (plift (qor e2 (qor (qif e2x (qone (length G)))(qif e2f (qand p qf))))) G (TFun T1 fn1 fr1 q1 T2 fn2 ar2 fr2 q2 e2f e2x e2) (plift p) false (plift qf) pempty
| cx_seq1: forall G t1 p p1 p2 q1 e1 e2 q2,
    has_type G t1 TBool p1 false q1 e1 ->
    psub (pand (plift p1)(plift p2)) pempty ->
    psub (plift p1) (plift p) ->
    psub (plift p2) (plift p) ->
    ctx_type (fun x => tseq t1 x) G TBool (plift p2) false q2 e2 G TBool (plift p) false  pempty (por (plift e1) e2)
| cx_seq2: forall G t2 p p1 p2 q1 e1 e2 q2,
    has_type G t2 TBool p2 false q2 e2 ->
    psub (pand (plift p1)(plift p2)) pempty ->
    psub (plift p1) (plift p) ->
    psub (plift p2) (plift p) ->
    ctx_type (fun x => tseq x t2) G TBool (plift p1) false q1 e1 G TBool (plift p) false pempty (por e1 (plift e2))
...
| cx_sub_var: forall G y T p fr1 q1 e1 qx x Tx,
    has_type G y T p fr1 q1 e1 ->
    plift q1 x ->
    indexr x G = Some (Tx, false, qx) ->
    psub (plift qx)(plift p) ->
    ctx_type (fun x => x) G T (plift p) fr1 (plift q1) (plift e1) G T (plift p) fr1 (por (pdiff (plift q1)(pone x)) (plift qx)) (plift e1)
| cx_trans: forall f g G1 p1 T1 fr1 q1 e1 G2 p2 T2 fr2 q2 e2 G3 p3 T3 fr3 q3 e3,
    ctx_type f G1 T1 p1 fr1 q1 e1 G2 T2 p2 fr2 q2 e2 ->
    ctx_type g G2 T2 p2 fr2 q2 e2 G3 T3 p3 fr3 q3 e3 ->
    ctx_type (fun x => g (f x)) G1 T1 p1 fr1 q1 e1 G3 T3 p3 fr3 q3 e3
.
...
Lemma encapsulation: forall G t T
  (W: has_type G t T qempty false qempty qempty),
  ...
  st_filter M' (st_locs1 M') (st_locs2 M') ->
  exists S1'' S2'' M'' v1 v2,
  exp_type S1' S2' M' H1 H2 t t S1'' S2'' M'' v1 v2 T pempty false pempty pempty.
Proof.
