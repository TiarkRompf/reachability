(**********************************************************************************************
* Coq mechanization of the re-ordering rule with effect qualifier in the λ^♦_{\varepsilon}-caluclus.
***********************************************************************************************)

(* based on sec6_reach_binary_effs.v *)
(* based on sec7_store_invariants_effs.v *)
...
Lemma separation1: forall M H1 H2 G p p1 p2 e2 
   (WFE: env_type M H1 H2 G p) 
   (SEP: psub (pand (vars_trans' G p1)(vars_trans' G e2)) pempty)
   (SUB1: psub (plift p2) p)
   (SUB2: psub (plift p1) p),
   psub (pand (locs_locs_stty (st1 M) (vars_locs H1 (plift p1)))(locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2) (plift e2))))) pempty.
Proof.
  ...
  assert (psub (pand (vars_trans G (plift p1))(vars_trans G (pand (plift p2) (plift e2)))) pempty). {
    intros ? [? ?]. eapply SEP. split. auto. eapply vt_mono; eauto. unfoldq; intuition.
  }
  assert (psub (pand (vars_trans G (plift p1))(vars_trans G (pand (plift p2) (plift e2)))) p). {
    intros ? ?. eapply H in H0. unfoldq; intuition.
  }
  eapply vars_locs_mono with (H := H1) in H. eapply lls_mono with (M := (st1 M)) in H.
  rewrite <- lls_empty with (M := (st1 M)). rewrite <- vars_locs_empty with (H := H1). 
  intros ? ?.  eapply W2 in H3; eauto. unfoldq; intuition.
  eapply envt_telescope; eauto. eapply envt_telescope; eauto.
Qed.


Lemma separation2: forall M H1 H2 G p p1 p2 e2 
   (WFE: env_type M H1 H2 G p) 
   (SEP: psub (pand (vars_trans' G p1)(vars_trans' G e2)) pempty)
   (SUB1: psub (plift p2) p)
   (SUB2: psub (plift p1) p),
   psub (pand (locs_locs_stty (st2 M) (vars_locs H2 (plift p1)))(locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2) (plift e2))))) pempty.
Proof.
  intros. remember WFE as WFE'. clear HeqWFE'. destruct WFE as [L1 [L2 [P1 [P2 [W1 [W2 W3]]]]]].
  unfold vars_trans' in SEP. rewrite plift_vt in SEP. rewrite plift_vt in SEP. 
  assert (psub (pand (vars_trans G (plift p1))(vars_trans G (pand (plift p2) (plift e2)))) pempty). {
  ...
  eapply vars_locs_mono with (H := H2) in H. eapply lls_mono with (M := (st2 M)) in H.
  rewrite <- lls_empty with (M := (st2 M)). rewrite <- vars_locs_empty with (H := H2). 
  intros ? ?.  eapply W3 in H3; eauto. unfoldq; intuition.
  eapply envt_telescope; eauto. eapply envt_telescope; eauto.
Qed.
...
Theorem reorder_seq : forall G t1 t2 p1 p2  q1 q2 e1 e2 p
  (W1: has_type G t1 TBool p1 false q1 e1)
  (W2: has_type G t2 TBool p2 false q2 e2)
  (SEPA: psub (pand (vars_trans' G p1)(vars_trans' G e2)) pempty)  (* t1's observation is separate from t2's effects *)
  (SEPB: psub (pand (vars_trans' G p2)(vars_trans' G e1)) pempty)  (* t2's observation is separate from t1's effects *)
  (SUB1: psub (plift p1) (plift p))
  (SUB2: psub (plift p2) (plift p)),
  sem_type G (tseq t1 t2) (tseq t2 t1) TBool (plift p) false pempty (por (plift e1) (plift e2)).
Proof.
  ...
  2: { eapply separation1. eapply WFE. intros ? ?.  eapply SEPB. 
      destruct H. split; auto. unfold vars_trans' in *. rewrite plift_vt in *. auto. 
      1-2: eapply envt_telescope; eauto.  eauto. }
  ...
  2: { unfold st2, st1. simpl. eapply separation2. eapply WFE. intros ? ?.  eapply SEPA. 
       destruct H. split; auto. unfold vars_trans' in *. rewrite plift_vt in *. auto. 
       1-2: eapply envt_telescope; eauto.  eauto. }
  ...
  (* separation *)
  assert (forall l, locs_locs_stty (st1 M) (vars_locs H1 (plift p1)) l -> locs_locs_stty (st1 M)  (vars_locs H1 (pand (plift p2) (plift e2))) l -> False) as SEP1. {
    intros. eapply separation1 with (p1 := p1). eauto. intros ? [? ?]. eapply SEPA; eauto. split. auto. eapply H7.
    eapply SUB2. auto. 
    split; eauto.
  }

  
  assert (forall l, locs_locs_stty (st2 M) (vars_locs H2 (plift p1)) l -> locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l -> False) as SEP1X. {
    intros. eapply separation2 with (p1 := p1). eauto. intros ? [? ?]. eapply SEPA. split. auto. eapply H7. eauto. eauto.
    split. eauto. eauto.
  }
  
  assert (forall l1 l2, strel M l1 l2 -> locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))) l1 -> ~ locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))) l2 -> False) as SEP2. {
    intros. eapply H6. edestruct envt_same_locs with (p1:=(pand (plift p1)(plift e1))). 2: eapply WFE. eauto. eauto. unfoldq; intuition.
    intuition. 
    
  }

  assert (forall l1 l2, strel M l1 l2 -> locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2)(plift e2))) l1 -> ~ locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l2 -> False) as SEP3. {
    intros. eapply H6. edestruct envt_same_locs with (p1:=(pand (plift p2)(plift e2))). 2: eapply WFE. eauto. eauto. unfoldq; intuition. eauto.
  }    
  
  
  assert (forall l1 l2, strel M l1 l2 -> locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))) l2 -> ~ locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))) l1 -> False) as SEP4. {
    intros. eapply H6. edestruct envt_same_locs with (p1:=(pand (plift p1)(plift e1))). 2: eapply WFE. eauto. eauto. unfoldq; intuition. eauto. 
  }
  

  assert (forall l1 l2, strel M l1 l2 -> locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l2 -> ~ locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2)(plift e2))) l1 -> False) as SEP5. {
    intros. eapply H6. edestruct envt_same_locs with (p1:=(pand (plift p2)(plift e2))). 2: eapply WFE. eauto. eauto. unfoldq; intuition. eauto. 
  }


  assert (st_chain_partial M M''' (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1)))) (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))))) as SXMP2. {
     eapply stchain_tighten with (p1' := (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1)))))(p2':= (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))))) in STC''' as R.
     2: {
         unfold st_filter. split. 2: split.
       + unfold st_locs1, pnat, length1, st1. simpl. eapply envt_store_deep_wf; eauto. unfoldq; intuition. 
       + unfold st_locs2, pnat, length2, st2. simpl. intros ? ?. eapply envt_store_deep_wf in H4; eauto. unfold pnat, length2, st2 in *. lia. unfoldq; intuition. 
       + intros. unfold strel in H4. simpl in H4. destruct H4 as [? [? ?]].
         split. intros. eapply envt_filter_deep in H7; eauto. unfoldq; intuition.
         intros ?. eapply envt_filter_deep in H7; eauto. unfoldq; intuition.
     }
     2: { unfold st_locs1, pnat, length1, st1. simpl. eapply envt_store_deep_wf; eauto. unfoldq; intuition. }
     2: {
     ...
     + unfold st2 in D. simpl in D. unfold st2. eapply sst_trans'' with (M2 := (st2 M'')).
       eapply sstchain_tighten. eapply STC''; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition. 
       destruct STC''' as [? [? [? [XX ?]]]]. eapply sstchain_tighten. eapply XX.
       unfold st_locs1, st_locs2, pnat, length1, length2, st1, st2 in *. simpl in *.
       intros ? ?. eapply envt_store_deep_wf in H8; eauto. unfold pnat, length2, st2 in H8. lia. unfoldq; intuition. 
     ...
     + intros. simpl in E. specialize (E l1 l2); intuition. unfold strel in H8. simpl in H8. rewrite <- H8. 
       ...
       ++ eapply SEP1X; eauto. eapply lls_mono. 2: eapply H5. eapply vars_locs_mono; eauto. unfoldq; intuition. 
       ++ intros ? ?. destruct H8 as [[? | ?] ?]. 
          assert (l2 < length (st2 M)) as X. {  eapply envt_store_deep_wf in H5; eauto. unfoldq; intuition. }
          destruct H8.
          assert (x1 < length (st2 M)). {  
            inversion H10; subst. unfold pone in H12. subst x1. eapply envt_store_deep_wf. eapply WFE. 2: eapply H5. unfoldq; intuition.
          ...
          eapply lls_filter in H10. eapply H10 in H11. eapply lls_filter in H4. eapply H4 in H11. 
          eapply H8. eapply lls_mono. 2: eapply H11. eapply vars_locs_mono; eauto. unfoldq; intuition.
          unfoldq; intuition.
        ++ split. 2: split. intros. 
          ...
          assert (locs_locs_stty (st2 M) (pone x) x). left. unfold pone. auto.
          eapply lls_filter in H9. eapply H9 in H10. eapply lls_filter in H5. eapply H5 in H10. 
          eapply H7. eapply lls_mono. 2: eapply H10. eapply vars_locs_mono; eauto. unfoldq; intuition.
          auto.

     + intros. unfold strel in F. simpl in F. specialize (F l1 l2); intuition. eapply H4; auto.
       intros ?. eapply SEP1X; eauto. eapply lls_mono. 2: eapply H6. eapply vars_locs_mono; eauto. unfoldq; intuition. 
       split.
       ...
       assert (locs_locs_stty (st2 M) (pone x) x). left. unfold pone. auto.
       eapply lls_filter in H6 as H6'. eapply H6' in H10. eapply lls_filter in H10.
       eapply H10 in H11. 
       eapply H9. eapply lls_mono. 2: { eapply H11. } eapply vars_locs_mono; auto. unfoldq; intuition.
       auto.
  }

  assert (st_chain_partial M M'' (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2)(plift e2)))) (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))))) as SXMP3. {
      eapply stchain_tighten with (p1' := (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2)(plift e2)))))(p2':= (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))))) in STC'' as R.
     2: {
       unfold st_filter. split. 2: split.
       + unfold st_locs1, pnat, length1, st1. simpl. intros ? ?. eapply envt_store_deep_wf in H4; eauto. unfold pnat, length1, st1 in *. lia. unfoldq; intuition. 
       + unfold st_locs2, pnat, length2, st2. simpl. intros ? ?. eapply envt_store_deep_wf in H4; eauto. unfoldq; intuition.
       + intros. unfold strel in H4. simpl in H4. destruct H4 as [? [? ?]].
         split. intros. eapply envt_filter_deep; eauto. unfoldq; intuition.
         intros. eapply envt_filter_deep; eauto. unfoldq; intuition.
     }
     2: {
      unfold st_locs1, pnat, length1, st1. simpl. intros ? ?. eapply envt_store_deep_wf in H4; eauto.  unfold pnat, length1, st1 in *. lia. unfoldq;intuition. 
     }
     ...
     + eapply envt_filter_deep; eauto. unfoldq; intuition.
     + auto.
     + unfold st1 in C. simpl in C. unfold st1. eapply sst_trans''; eauto. eapply sstchain_tighten; eauto. eapply STC'; eauto. eapply envt_store_deep_wf; eauto. 
       unfoldq; intuition. eapply envt_store_deep_wf; eauto. unfoldq; intuition. 
     ...
     + intros ? ?. simpl in E. specialize (E l1 l2); intuition. rewrite <- H8. 
       ...
       eapply lls_mono. 2: eapply H9. eapply vars_locs_mono; eauto. unfoldq; intuition.
       intros ? ?. destruct H9 as [[? | ?] ?]. 
       ...
       eapply lls_filter in H10. eapply H10 in H11. eapply lls_filter in H4. eapply H4 in H11. 
       eapply H9. eapply lls_mono. 2: eapply H11. eapply vars_locs_mono; eauto. unfoldq; intuition.

       unfold strel in *. simpl in *.  split. intros ?. eapply SEP1; eauto. eapply lls_mono; eauto. eapply vars_locs_mono; eauto. unfoldq; intuition. 
       split.
       ...
       eapply lls_filter in H9. eapply H9 in H10. eapply lls_filter in H4. eapply H4 in H10. 
       eapply H7. eapply lls_mono. 2: eapply H10. eapply vars_locs_mono; eauto. unfoldq; intuition.
       auto.
       
     + intros. unfold strel in F. simpl in F. specialize (F l1 l2); intuition. eapply H4; auto.
       intros ?. eapply SEP1; eauto. eapply lls_mono; eauto. eapply vars_locs_mono; eauto. unfoldq; intuition. 
       split.
       intros ? ?. destruct H9 as [[? | ?] ?].
       ...
       assert (locs_locs_stty (st1 M) (pone x) x). left. unfold pone. auto.
       eapply lls_filter in H10. eapply H10 in H11. eapply lls_filter in H5. eapply H5 in H11. 
       eapply H9. eapply lls_mono. 2: eapply H11. eapply vars_locs_mono; eauto. unfoldq; intuition.
  ...
  assert (forall l1 l2,
               ...
               strel M''' l1 l2 ->
               ~ (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2) (plift e2))) l2) /\ 
  ...
  assert (forall l1 l2,
               l1 < length S1' /\ l2 < length S2 ->
               strel M'' l1 l2 ->
               ~ (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))) l1) /\ 
  ...
  fun l1 l2 =>
    strel M l1 l2 /\
      ~ (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1) (plift e1)))) l1 /\
      ~ (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1) (plift e1)))) l2 /\
      ~ (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2) (plift e2)))) l1 /\
      ~ (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2) (plift e2)))) l2 \/
    strel M''' l1 l2 /\
       ((locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1) (plift e1)))) l1 /\
       (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1) (plift e1)))) l2 \/
       ~ l1 < length1 M /\ l1 < length1 M' /\ l1 < length1 M''' /\ 
       (psub(pand (por (pdiff (pnat (length1 M')) (pnat (length1 M))) (pnot (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2) (plift e2))))))(locs_locs_stty (st1 M) (pone l1)))) pempty /\
    ...
    strel M'' l1 l2 /\
       ...
       psub(pand (por (pdiff (pnat (length2 M'')) (pnat (length2 M))) (pnot (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))))))(locs_locs_stty (st2 M) (pone l2))) pempty     
       ),
   fun l1 l2 v1 v2 =>
  ...
  assert (st_chain M MM). {
    ...
    - repeat split.
      ...
      * intros X. unfold strel in H4. simpl in H4. unfold st_locs2, pnat, length2, st2. 
        ...
        eapply envt_store_deep_wf in H6; eauto. unfoldq; intuition. 
      ...
      * intros X. unfold strel in H4. simpl in H4. unfold st_locs1, pnat, length1, st1. 
        destruct H4. destruct H4 as [? [? [? [? ?]]]]. eapply STF; eauto.
        destruct H4. destruct H4 as [? [[? ?] | [? [? [? [? [? ?]]]]]]].  
        eapply envt_store_deep_wf in H5; eauto. unfoldq; intuition. 
    ...
    - (* st_valtype *)    
      ...
      split; intros. 
      * assert (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))) l1 \/ ~ locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))) l1). {  rewrite <- plift_and. rewrite plift_vars_locs.  eapply val_locs_stty_decide. eapply ST; eauto. }
        assert (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))) l2 \/ ~locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))) l2). { rewrite <- plift_and. rewrite plift_vars_locs.  eapply val_locs_stty_decide. eapply ST; eauto. }
        assert (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2)(plift e2))) l1 \/ ~locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p2)(plift e2))) l1). { rewrite <- plift_and. rewrite plift_vars_locs.  eapply val_locs_stty_decide. eapply ST; eauto. }
        assert (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l2 \/ ~locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l2). { rewrite <- plift_and. rewrite plift_vars_locs.  eapply val_locs_stty_decide. eapply ST; eauto. } 
        
        destruct H8; destruct H9; destruct H10; destruct H11.
        ** (*   p1 l1,   p1 l2,   p2 l1,   p2 l2 *)
           edestruct (SEP1 l1); eauto. eapply lls_mono. 2: eapply H8. eapply vars_locs_mono; eauto. unfoldq; intuition.
        ** (*   p1 l1,   p1 l2,   p2 l1, ~ p2 l2 *)
           edestruct (SEP1 l1); eauto. eapply lls_mono. 2: eapply H8. eapply vars_locs_mono; eauto. unfoldq; intuition.
        ** (*   p1 l1,   p1 l2, ~ p2 l1,   p2 l2 *)
           edestruct (SEP1X l2); eauto. eapply lls_mono. 2: eapply H9. eapply vars_locs_mono; eauto. unfoldq; intuition.
        ** (*   p1 l1,   p1 l2, ~ p2 l1, ~ p2 l2 *)
           right. left. 
           repeat split; auto. 
           destruct STC''' as [? [? [? [? [STY ?]]]]]. 
           ...
           unfoldq; intuition.
           assert (locs_locs_stty (st2 M) (pone x1) x1). left. unfold pone. auto.
           eapply lls_filter in H18. eapply H18 in H19. eapply lls_filter in H9. eapply H9 in H19. 
           eapply H17. eapply lls_mono. 2: eapply H19. eapply vars_locs_mono; eauto. unfoldq; intuition.
           auto.
           eapply envt_store_deep_wf in H8; eauto. unfoldq; intuition. 
        ...
        ** (* ~ p1 l1, ~ p1 l2,   p2 l1,   p2 l2 *)
           ...
           assert (locs_locs_stty (st1 M) (pone x1) x1). left. unfold pone. auto.
           eapply lls_filter in H18. eapply H18 in H19. eapply lls_filter in H10. eapply H10 in H19.
           eapply H17. eapply lls_mono. 2: eapply H19. eapply vars_locs_mono; eauto. unfoldq; intuition. 
           
           eapply envt_store_deep_wf in H10; eauto. unfold pnat, length1, st1 in *. lia. unfoldq; intuition.
           eapply envt_store_deep_wf in H11; eauto. unfoldq; intuition.
      ...
      * destruct H7 as [[? [? [? [? ?]]]] | [[? ?] | [? ?]]]. 
        ...
        ** destruct STC''' as [? [? [? [? [STY ?]]]]]. 
           ...
           split. intros ?. eapply SEP1X; eauto. eapply lls_mono. 2: eapply H14. eapply vars_locs_mono; eauto.
           unfoldq; intuition. 
           split.
           ...
           assert (locs_locs_stty (st2 M) (pone x1) x1). left. unfold pone. auto. unfoldq; intuition.
           eapply H16.
           eapply lls_filter in H17. 
           assert (locs_locs_stty (snd (fst(fst M))) (pone x1) x1) as X. { left. unfoldq; intuition. }
           eapply H17 in X. eapply lls_filter in H14. eapply H14 in X. 
           eapply lls_mono. 2: eapply X. eapply vars_locs_mono; eauto. unfoldq; intuition.
           auto.
        ...
        ** destruct STC'' as [? [? [? [? [STY ?]]]]]. 
           ...
           assert (locs_locs_stty (st1 M) (pone x1) x1). left. unfold pone. auto.
           eapply lls_filter in H17. eapply H17 in H18. eapply lls_filter in H8. eapply H8 in H18.
           eapply H16. eapply lls_mono. 2: eapply H18. eapply vars_locs_mono; eauto. unfoldq; intuition.
           auto.
           intros ?. eapply envt_store_deep_wf in H19; eauto.  unfoldq; intuition.
           split.
    ...
    - (* strel *)
      ...
      assert (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l2 \/ ~locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))) l2). { rewrite <- plift_and. rewrite plift_vars_locs.  eapply val_locs_stty_decide. eapply ST; eauto. } 
        ...
        * (*   p1 l1,   p1 l2,   p2 l1,   p2 l2 *)
        edestruct (SEP1 l1); eauto. eapply lls_mono. 2: eapply H6. eapply vars_locs_mono; eauto. unfoldq; intuition.
        * (*   p1 l1,   p1 l2,   p2 l1, ~ p2 l2 *)
          edestruct (SEP1 l1); eauto. eapply lls_mono. 2: eapply H6. eapply vars_locs_mono; eauto. unfoldq; intuition.
        * (*   p1 l1,   p1 l2, ~ p2 l1,   p2 l2 *)
          edestruct (SEP1X l2); eauto. eapply lls_mono. 2: eapply H7. eapply vars_locs_mono; eauto. unfoldq; intuition.
        * (*   p1 l1,   p1 l2, ~ p2 l1, ~ p2 l2 *)
          ...
          assert (locs_locs_stty (st2 M) (pone x) x). left. unfold pone. auto.
          eapply lls_filter in H11. eapply H11 in H12. eapply lls_filter in H7. eapply H7 in H12. 
          eapply H10. eapply lls_mono. 2: eapply H12. eapply vars_locs_mono; eauto. unfoldq; intuition.
        ...
        * (* ~ p1 l1, ~ p1 l2,   p2 l1,   p2 l2 *)
          ...
          assert (locs_locs_stty (st1 M) (pone x) x). left. unfold pone. auto.
          eapply lls_filter in H11. eapply H11 in H12. eapply lls_filter in H8. eapply H8 in H12. 
          eapply H10. eapply lls_mono. 2: eapply H12. eapply vars_locs_mono; eauto. unfoldq; intuition.
  ...
  assert (st_chain_partial M MM (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1)))) (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p1)(plift e1))))) as SRMP1. {
     eapply stchain_tighten; eauto. eapply envt_filter_deep; eauto.
     unfoldq; intuition. 
     1,2: eapply envt_store_deep_wf; eauto; unfoldq; intuition. 
  } 

  assert (store_effect S1 S1'' (locs_locs_stty (st1 M) (vars_locs H1 (por (pand (plift p1)(plift e1))(pand (plift p2) (plift e2)))))) as SSE1.  {
    ...
    eapply sstchain_tighten; eauto. eapply STC'; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition.
  }

  assert (store_effect S2 S2'' (locs_locs_stty (st2 M) (vars_locs H2 (por (pand (plift p1) (plift e1))(pand (plift p2) (plift e2)))))) as SSE2. {
  ...
  assert (st_filter MM (st_locs1 MM)(st_locs2 MM)) as SRF. {
    ...
    + intros. unfold strel in H5. simpl in H5. unfold st_locs1, st_locs2, pnat, length1, length2, st1, st2. simpl.
      ...
      - destruct B as [? [B1 | B2]]. destruct B1. eapply envt_store_deep_wf in H6, H7; eauto.
        unfold pnat, length1, length2, st1, st2 in *. split; intros; lia. 1,2: unfoldq; intuition.
  ...
  assert (sst_chain_partial (st1 M')(st1 M'')(locs_locs_stty (st1 M')(vars_locs H1 (pand (plift p1)(plift e1))))) as XXYY. {
    repeat split.
    ...
    + intros.  erewrite <- lls_change with (M := st1 M) in H5. 
      2: { eapply sstchain_tighten. eapply STC'; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition. }
      assert (indexr l (st1 M) = indexr l (st1 M')). { eapply STC'; eauto. eapply envt_store_deep_wf in H5; eauto. unfoldq; intuition. }
      assert (indexr l (st1 M) = indexr l (st1 M'')). { eapply sstchain_tighten. eapply STCMM''1. 2: eauto.  eapply envt_store_deep_wf; eauto. unfoldq; intuition. }
      congruence.
  }

  assert (sst_chain_partial (st1 M'') (st1 M''') (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))))) as X1Y1. {
    repeat split.
    + intros ? ?. eapply envt_store_deep_wf in H5; eauto. unfold pnat, length1, st1, pdom in *. lia. unfoldq; intuition.
    + intros ? ?. eapply envt_store_deep_wf in H5; eauto. unfold pnat, length1, st1, pdom in *. lia. unfoldq; intuition.
    + intros ? ?. 
      assert (indexr l (st1 M) = indexr l (st1 M'')). { 
        ...
        3: { intros ? ?. eauto. } 
        erewrite <- lls_change. 2: { eapply sstchain_tighten. eapply STC'. eapply envt_store_deep_wf; eauto. unfoldq; intuition. } eapply envt_store_deep_wf; eauto. unfoldq; intuition.
        erewrite <- lls_change. 2: { eapply sstchain_tighten. eapply STC'. eapply envt_store_deep_wf; eauto. unfoldq; intuition. } eapply envt_store_deep_wf; eauto. unfoldq; intuition.
        intros ? ?. erewrite <- lls_change; eauto. eapply sstchain_tighten. eapply STC'. eapply envt_store_deep_wf; eauto. unfoldq; intuition.  }
  ...
  assert (store_type S1'' S2'' MM). {
    ...
    - intros. destruct H5 as [A | [B | C]].
     ...
     + (* strel M''' l1 l2 and l1 l2 are in p1 *)
       ...
       * unfold st1. simpl. assert (snd (fst M) l1 l2). { eapply SXMP2; eauto. }
         ...
         assert (l1 < length (fst(fst(fst M)))) as X. { eapply envt_store_deep_wf in HA; eauto.  unfoldq; intuition. }
       ...
       * erewrite SE2'; eauto. unfold st1. simpl. intros ?. erewrite <- lls_change with (M := (fst (fst (fst M)))) in H13.
         eapply SEP1; eauto. eapply lls_mono. 2: eapply HA.  eapply vars_locs_mono; eauto. unfoldq; intuition.
         eapply stchain_tighten. 2: eapply STC'; eauto. eapply envt_filter_deep; eauto. unfoldq; intuition.
       ...
       * (* st_valtype *)
         ...
         ** destruct H10. destruct H10 as [? [? ?]]. contradiction. 
            ...
            destruct H13 as [? [? [? ?]]]. eapply envt_store_deep_wf in HA; eauto. unfoldq; intuition. 
            destruct H10 as [? [? | ?]]. destruct H13. assert False. eapply SEP1; eauto. 
            eapply lls_mono. 2: eapply HA. eapply vars_locs_mono; eauto. unfoldq; intuition. contradiction.
       ...
       * (* st_filter *)
         ...
         assert (indexr l1 S1'' = Some v1'x) as ISV0. { 
          erewrite SE2'; eauto. intros ?. eapply SEP1; eauto. erewrite lls_change. 
          eapply lls_mono. 2: eapply HA. eapply vars_locs_mono; eauto. unfoldq; intuition.
          assert (st_chain M M). { eapply st_refl; eauto. }
          eapply sstchain_tighten. eapply H13; eauto. eapply envt_store_deep_wf; eauto.
          erewrite lls_change; eauto. eapply sstchain_tighten. eapply STC'; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition.
         }
         assert (indexr l1 (st1 M) = Some qt1x) as IM1x. { rewrite <- H6. eapply SXMP2; eauto.  }
         assert (indexr l1 (st1 M') =  Some qt1x) as IM2x. { rewrite <- IM1x. symmetry.  eapply STC'. unfold st_locs1. unfoldq. eapply envt_store_deep_wf in HA; eauto. unfoldq; intuition. }
         assert (indexr l1 (st1 M') = indexr l1 (st1 M'')) as IM3x. { eapply XXYY. erewrite <- lls_change; eauto. eapply sstchain_tighten. eapply STC'; eauto. eapply  envt_store_deep_wf; eauto. unfoldq; intuition.  }
         ...
         ** unfold strel in H12. simpl in H12. destruct H12 as [XX | [YY | ZZ]]. 
            *** destruct XX as [Ax [Bx [Cx [Dx Ex]]]].  
                ...
                2: { erewrite <- lls_change; eauto. eapply sstchain_tighten; eauto.  eapply envt_store_deep_wf; eauto. unfoldq; intuition. } 2: eauto.
            ...
            *** destruct YY as [Ax [Bx | Cx]]. destruct Bx. intros.  
            ...
            (* (S1, M1) ->p1 (S1', M1''') ->p2 (S1'', M1'') *)
               erewrite <- lls_change; eauto.     
               eapply sstchain_tighten with (p := (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))))). 
               ...
               unfoldq; intuition.
               eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition.
               destruct Cx as [Cx1 [Cx2 [Cx3 [Cx4 Cx5]]]]. intros.
               eapply C; eauto.
               erewrite <- lls_change; eauto.     
               eapply sstchain_tighten with (p := (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))))); eauto. 
               intros ? ?. erewrite lls_change with (M' := (st1 M'')); eauto.
           ...
           *** destruct ZZ as [ZA [ZB | ZC]]. destruct ZB as [ZB1 ZB2]. intros.
               assert (psub (locs_locs_stty (st1 M'') (val_locs v1'x)) (locs_locs_stty (st1 M'')(vars_locs H1 (pand (plift p1)(plift e1))))).  {
               ...
               eapply SEP1 in ZB1. contradiction.
               eapply lls_mono. 2: eapply H12. eapply vars_locs_mono; eauto. unfoldq; intuition.
         ...
         ** intros. unfold strel in H12. simpl in H12. destruct H12 as [XX | [YY | ZZ]].
            ...
            *** destruct YY as [Ax [Bx | Cx]]. destruct Bx as [Bx1 Bx2].
                ...
                erewrite lls_change; eauto. eapply sstchain_tighten with (p := (locs_locs_stty (st1 M) (vars_locs H1 (pand (plift p1)(plift e1))))); eauto.
           ...
           *** destruct ZZ as [ZA [[ZB1 ZB2 ] | ZC]]. 
               assert (psub (locs_locs_stty (st2 M''') (val_locs v2'x)) (locs_locs_stty (st2 M''')(vars_locs H2 (pand (plift p1)(plift e1))))).  {
                 eapply val_reach_p; eauto. erewrite <- lls_change with (M := st2 M); eauto. eapply SXMP2; eauto.
               } 
               eapply H12 in H13. rewrite <- lls_change with (M := st2 M) in H13; eauto. 
               eapply SEP1X in ZB2. contradiction.
               eapply lls_mono. 2:eapply H13. eapply vars_locs_mono; eauto. unfoldq; intuition.
               eapply SXMP2; eauto.
     ...
     + (* strel M'' l1 l2 and l1 l2 are in p2 *) 
       ...
       * unfold st2. simpl. assert (snd (fst M) l1 l2). { eapply SXMP3; eauto. }
         ...
         assert (l2 < length (snd(fst(fst M)))) as X. { eapply envt_store_deep_wf in HB; eauto. unfoldq; intuition.  }
       ...
       * erewrite SE2''; eauto. unfold st2. simpl. intros ?. erewrite <- lls_change with (M := (snd (fst (fst M)))) in H13.
         eapply SEP1X; eauto. eapply lls_mono; eauto. eapply vars_locs_mono; eauto. unfoldq; intuition. 
         eapply sstchain_tighten. eapply STC''; eauto.  eapply envt_store_deep_wf; eauto. unfoldq; intuition.
       * (* st_valtype *)
         simpl. subst vtx. eapply functional_extensionality. intros. eapply functional_extensionality. intros.
         eapply propositional_extensionality. split; intros.
         ** destruct H10. destruct H10 as [? [? [? [? ?]]]]. contradiction. 
            destruct H10. destruct H10 as [? [? | ?]]. destruct H13. assert False. eapply SEP1; eauto. 
            eapply lls_mono. 2:eapply H13. eapply vars_locs_mono; eauto. unfoldq; intuition.
            contradiction.
            destruct H13 as [? [? [? [? [? [? ?]]]]]]. eapply envt_store_deep_wf in HA; eauto. unfold pnat in HA. contradiction.
            unfoldq; intuition.
       ...
       *(* st_filter *)
         ...
         assert (indexr l2 (st2 M') =  Some qt2x) as IM2x. { rewrite <- IM1x. symmetry. eapply STC'; eauto. unfold st_locs2. unfold pnat. eapply envt_store_deep_wf in HB; eauto. unfoldq; intuition. }
         assert (indexr l2 (st2 M) = indexr l2 (st2 M''')) as IM3x. { eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition.  }
         ...
         ** intros. unfold strel in H12. simpl in H12. destruct H12 as [XX | [YY | ZZ]].
           ...
           *** destruct YY as [Ax [Bx | Cx]].
               destruct Bx.
               assert (psub (locs_locs_stty (st1 M'') (val_locs v1'x)) (locs_locs_stty (st1 M'')(vars_locs H1 (pand (plift p2)(plift e2))))).  {
                 eapply val_reach_p; eauto. erewrite <- lls_change with (M := (st1 M)); auto.  
                 eapply SXMP3; eauto. 
               }
               eapply H15 in H13. erewrite <- lls_change with (M := st1 M) in H13. 
               eapply SEP1 in H13. contradiction.
               eapply lls_mono. 2: eapply H12. eapply vars_locs_mono; eauto. unfoldq; intuition.
               eapply SXMP3; eauto.
           ...
           *** destruct ZZ as [Ax [Bx | Cx]]. destruct Bx as [Bx1 Bx2].
               ...
               assert (sst_chain_partial (st2 M) (st2 M'') (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))))). { eapply SXMP3. }
               assert (sst_chain_partial (st2 M) (st2 M''') (locs_locs_stty (st2 M) (vars_locs H2 (pand (plift p2)(plift e2))))). { eapply sstchain_tighten. eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition. }
               assert (sst_chain_partial (snd (fst (fst M''))) (snd (fst (fst M''')))(locs_locs_stty (snd (fst (fst M''))) (vars_locs H2 (pand (plift p2)(plift e2))))). {
       ...
       ** intros.  unfold strel in H12. simpl in H12. destruct H12 as [XX | [YY | ZZ]].
         *** destruct XX as [Ax [Bx [Cx [Dx Ex]]]].
             ...
             2: { erewrite lls_change; eauto. eapply sst_chain_partial_symmetry. erewrite <- lls_change with (M := st2 M). eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition. 
                  eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition. }
             eapply ISV0' in H13. erewrite <- lls_change with (M := st2 M) in H13. 2: auto. specialize (SEP1X l3); intuition.
             eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition. 
         *** destruct YY as [Ax [Bx | Cx]].
             eapply val_reach_p in ISV0 as ISV0'. 3: eapply IM4x. 3: eauto. 2: { erewrite <- lls_change; eauto. eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition.  }
             eapply ISV0' in H13. erewrite <- lls_change with (M := (st2 M)) in H13; eauto.
             eapply SEP1X in H13. contradiction. 
             destruct Bx as [BX1 BX2].
             eapply lls_mono. 2: eapply BX2. eapply vars_locs_mono; eauto. unfoldq; intuition.
             eapply sstchain_tighten; eauto. eapply envt_store_deep_wf; eauto. unfoldq; intuition.
             destruct Cx as [Cx1 [Cx2 [Cx3 [Cx4 [Cx5 [Cx6 Cx7]]]]]].
   ...
   - unfold strel. simpl. intros.
     ...
     + destruct H5; destruct H6.
       ++ destruct H5. destruct H6.
          eapply SEP1 in H6. contradiction. eapply lls_mono. 2: eapply H5. eapply vars_locs_mono; eauto. unfoldq; intuition.  
     ...
     + destruct H5; destruct H6.
       ++ destruct H5. destruct H6. 
          eapply SEP1 in H5. contradiction.  eapply lls_mono. 2: eapply H6. eapply vars_locs_mono; eauto. unfoldq; intuition.
       ++ destruct H5. eapply envt_store_deep_wf in H5; eauto. unfold pnat in H5. intuition.
          unfoldq; intuition.
       ++ destruct H6. eapply envt_store_deep_wf in H6; eauto. unfold pnat in H6. intuition.
          unfoldq; intuition.
       ++ intuition.
     + destruct H5; destruct H6. 
       ++ destruct H5. destruct H6. eauto.
       ++ destruct H5. eapply envt_store_deep_wf in H5; eauto. unfoldq; intuition. 
       ++ destruct H6. eapply envt_store_deep_wf in H6; eauto. unfoldq; intuition.
       ++ eauto.
   ...
   - unfold strel. simpl. intros.
     ...
     + destruct H5; destruct H6. 
       ++ destruct H5. destruct H6.
          eapply SEP1X in H8. contradiction. eapply lls_mono. 2: eapply H7. eapply vars_locs_mono; eauto. unfoldq; intuition.
     ...
     + destruct H5; destruct H6.
       ++ destruct H5. destruct H6. 
          eapply SEP1X in H7. contradiction. eapply lls_mono. 2: eapply H8. eapply vars_locs_mono; eauto. unfoldq; intuition.
       ++ destruct H5. eapply envt_store_deep_wf in H7; eauto. unfold pnat in H7. intuition.
          unfoldq; intuition.
       ++ destruct H6. eapply envt_store_deep_wf in H7; eauto. unfold pnat in H7. intuition.
          unfoldq; intuition.
     ...
     + destruct H5; destruct H6. 
       eauto.
       destruct H5. eapply envt_store_deep_wf in H7; eauto. unfoldq; intuition.
       destruct H6. eapply envt_store_deep_wf in H6; eauto. unfoldq; intuition.
       eauto.
  }
