(***************************************************************************************************
* Store invariants used for the proof of re-ordering  without effect qualifiers in the λ^♦-caluclus.
****************************************************************************************************)
...
Lemma val_locs_empty:  forall t G T  p 
  (W: has_type G t T p false qempty),
...
Theorem store_invariance : forall t G T q fr
  (W: has_type G t T qempty fr q),
  ...
  exp_type S1 S2 (st_empty MS1 MS2) H1 H2 t t S1' S2' M' v1 v2 T (plift qempty) fr (plift q).
Proof.
    intros.
...
Theorem store_invariance2 : forall t G T p p' fr q
  (W: has_type G t T p fr q),
  ...
  forall S1' MS1', store_effect S1 S1' p' ->
                 ...
                 sstore_type S1' MS1' ->
  exists S1'' S2' M' v1 v2,
  exp_type S1' S2 (MS1', st2 M, strel M, st_valtype M) H1 H2 t t S1'' S2' M' v1 v2 T (plift p) fr (plift q).
Proof.
  intros. 
...
Theorem store_invariance3 : forall t G T p p' fr q
  (W: has_type G t T p fr q),
  forall M H1 H2, env_type M H1 H2 G (plift p) ->
  forall S1 S2, store_type S1 S2 M ->
  ...
  forall S2' MS2', store_effect S2 S2' p' ->
                 ...
                 sstore_type S2' MS2' ->
  exists S1' S2'' M' v1 v2,
  exp_type S1 S2' (st1 M, MS2', strel M, st_valtype M) H1 H2 t t S1' S2'' M' v1 v2 T (plift p) fr (plift q).
Proof. 
  intros. 
...
Theorem store_invariance2' : forall t G T p fr q 
  (W: has_type G t T p fr q),
  forall M H1 H2, env_type M H1 H2 G (plift p) ->
  forall S1 S2, store_type S1 S2 M ->
  ...
  exp_type S1' S2 (MS1', st2 M, fun l1 l2 => 
                  ...
                  H1 H2 t t S1'' S2' M' v1 v2 T (plift p) fr (plift q).
Proof.
  intros.
...
Theorem store_invariance3' : forall t G T p fr q 
  (W: has_type G t T p fr q),
  forall M H1 H2, env_type M H1 H2 G (plift p) ->
  forall S1 S2, store_type S1 S2 M ->
