(****************************************
*  This version is for the technical report on self references
 ****************************************)
...
| s_sref : forall Γ Σ T1 T2 q1 q2 d1 d2,
    qstp Γ Σ d1 d2 ->
    stp Γ Σ T1 ∅ T2 ∅ ->
    stp Γ Σ T2 ∅ T1 ∅ ->
    closed_Qual 1 (‖Γ‖) (‖Σ‖) q1 ↑ ->
    closed_Qual 1 (‖Γ‖) (‖Σ‖) q2 ↑ ->
    qbvs q1 0 = qbvs q2 0 ->
    qstp Γ Σ ([[0 ~> ∅ ]]ᵈ q1) ([[0 ~> ∅ ]]ᵈ q2) ->
    qstp Γ Σ ([[0 ~> ∅ ]]ᵈ q2) ([[0 ~> ∅ ]]ᵈ q1) ->
    stp Γ Σ (TSRef q1 T1) d1 (TSRef q2 T2) d2
...
| t_sderef: forall Γ φ Σ T d d1 t,
		has_type Γ φ Σ t (TSRef d1 T) d ->
		♦∉ d1 ->
		([[0 ~> d]]ᵈ d1) ⊑↑ φ ⊔ {♦} ->
		has_type Γ φ Σ !t T ([[0 ~> d]]ᵈ d1)
...
| t_sassign: forall Γ φ Σ T t1 d d1 t2,
    has_type Γ φ Σ t1 (TSRef d1 T) d ->
    has_type Γ φ Σ t2 T ([[0 ~> ∅]]ᵈ d1) ->
    ♦∉ d1 ->
    has_type Γ φ Σ (tassign t1 t2) TUnit ∅

| t_sassign_v: forall Γ φ Σ T f d d1 t2,
    has_type Γ φ Σ (tvar (varF f)) (TSRef d1 T) d ->
    has_type Γ φ Σ t2 T ([[0 ~> $!f ]]ᵈ d1) ->
    ♦∉ d1 ->
    has_type Γ φ Σ (tassign (tvar (varF f)) t2) TUnit ∅

| t_sassign_l: forall Γ φ Σ T l d d1 t2,
    has_type Γ φ Σ (tloc l) (TSRef d1 T) d ->
    has_type Γ φ Σ t2 T ([[0 ~> &!l ]]ᵈ d1) ->
    ♦∉ d1 ->
...
Definition CtxOK (Γ : tenv) (φ : qual) (Σ : senv) (σ : store) : Prop :=
  qdom' σ ⊑↑ qdom Σ /\ ‖ σ ‖ = ‖ Σ ‖ /\
  φ ⊑↑ (qdom' σ) /\ forall b l v T q,
  l ∈ₗ φ ->
  indexr l Σ = Some (b,T,q) ->
  indexr l σ = Some (Some v) ->
  value v /\ 
  (b = false -> has_type Γ φ Σ v T q) /\ 
  (b = true -> has_type Γ φ Σ v T ([[0 ~> &!l ]]ᵈ q)).
...
Fixpoint subst_ty (T : ty) (v : nat) (U: ty) (q : qual) : ty :=
  ...
  | TSRef q1 T       => TSRef (subst_qual q1 v q) (subst_ty T v U q)
  | TInt             => TInt
  | TBool            => TBool
  end.
...
| vtp_sloc:  forall Σ φ l T U q1 q2 d,
  closed_Qual 0 0 (‖Σ‖) d↑ ->
  closed_ty 0 0 (‖Σ‖) T ->
  closed_Qual 1 0 (‖Σ‖) q1↑ ->
  closed_Qual 1 0 (‖Σ‖) q2↑ -> (*GW*)
  l ∈ₗ φ ->
  indexr l Σ = Some (true,T,q1) ->
  stp [] Σ T ∅ U ∅ ->
  stp [] Σ U ∅ T ∅ ->
  qbvs q1 0 = qbvs q2 0 ->
  qstp [] Σ &!l d -> (* GW *)
  ♦∉ q1 ->
  qstp [] Σ ([[0 ~> ∅ ]]ᵈ q1) ([[0 ~> ∅ ]]ᵈ q2) -> (* GW*)
  qstp [] Σ ([[0 ~> ∅ ]]ᵈ q2) ([[0 ~> ∅ ]]ᵈ q1) -> (* GW*)
  vtp Σ φ &l (TSRef q2 U) d
...
| vtp'_sloc:  forall Γ Σ φ l T U q1 q2 d,
  closed_Qual 0 (‖Γ‖) (‖Σ‖) d↑ ->
  closed_ty 0 (‖Γ‖) (‖Σ‖) T ->
  closed_Qual 1 (‖Γ‖) (‖Σ‖) q1↑ ->
  closed_Qual 1 (‖Γ‖) (‖Σ‖) q2↑ -> (*GW*)
  l ∈ₗ φ ->
  indexr l Σ = Some (true,T,q1) ->
  (* GW: another sensible choice is to bake qstp of q1/q2 into stp here too.
     In this version, because we have stp_qual_irrelevance, it doesn't matter *)
  ...
  qstp Γ Σ ([[0 ~> ∅ ]]ᵈ q1) ([[0 ~> ∅ ]]ᵈ q2) ->
  qstp Γ Σ ([[0 ~> ∅ ]]ᵈ q2) ([[0 ~> ∅ ]]ᵈ q1) ->
  vtp' Γ Σ φ &l (TSRef q2 U) d
| vtp'_top: forall Γ Σ φ t T d,
  vtp' Γ Σ φ t T d ->
  (* senv_saturated Σ d ->   (1* due to vtp'_saturated *1) *)
  vtp' Γ Σ φ t TTop d
  .
...
| SLoc   : forall Γ Σ Tx dx l,
    (* ♦∉ dx -> (1* not necessarily true for growing substitution *1) *)
    (Tx = TTop \/ exists T d, Tx = TRef d T \/ Tx = TSRef d T) ->
    &!l ⊑↑ dx -> (* needed for subst_qstp *)
    Substq Γ Σ Tx dx &!l (* substituting using a location *)
| SLocGrow   : forall Γ Σ Tx df dx l,
    (Tx = TTop \/ exists T d, Tx = TRef d T \/ Tx = TSRef d T) ->
    ♦∉ dx ->
    closed_Qual 0 (‖ Γ ‖) (‖ Σ ‖) dx ↑ -> (* for Substq_weaken *)
    closed_Qual 0 (‖ Γ ‖) (‖ Σ ‖) df ↑ ->
    &!l ⊑↑ dx -> (* needed for subst_qstp *)
    Substq Γ Σ Tx (q_trans_tenv Γ df ⋒ q_trans_tenv Γ dx) &!l (* substituting using a location *)
.
...
simpl. eapply closed_qual_open'; eauto. eapply closed_qual_open'; eauto. 1-3: apply Q_lift_closed; Qcrush. intuition.
  ...
  - (*TSRef*) constructor; eauto.
    1,2: apply IHn; auto; lia. 1,2: apply qstp_refl; eapply closed_qual_open'; eauto.
  - constructor; auto.
  - constructor; auto.
Qed.
...
Lemma closed_ty_not_tfvs : forall T b f f' l,
  closed_ty b f l T ->
  f <= f' ->
  ~Tfvs T f'.
intros. intuition. induction H; eauto. 
  - inversion H1. subst. lia.
  - inversion H. inversion H2. intuition. destruct H1 as [H1 | [H1 | [H1 | H1]]]; intuition.
    + unfold closed_Nats in H5. apply H5 in H1. lia. 
    + unfold closed_Nats in H7. apply H7 in H1. lia. 
  - intuition. apply H3. inversion H1; intuition. inversion H2. intuition. unfold closed_Nats in H5. apply H5 in H4. lia.
  - intuition. apply H3. inversion H1; intuition. inversion H2. intuition. unfold closed_Nats in H5. apply H5 in H4. lia.
  - inversion H. inversion H2. intuition. destruct H1 as [H1 | [H1 | [H1 | H1]]]; intuition.
    + unfold closed_Nats in H5. apply H5 in H1. lia. 
    + unfold closed_Nats in H7. apply H7 in H1. lia. 
...
Lemma closed_Qual_subst1_inv : forall d d' b f l,
  closed_Qual b f l ({0 |-> d' }ᵈ d) ↑ ->
  closed_Qual b (S f) l d ↑.
intros. unfold_q. ndestruct (snd (fst (fst d)) 0).
  - Qcrush. bdestruct (x =? 0). lia. specialize (H1 (pred x)). destruct H1; auto. left. intuition. rewrite Nat.succ_pred_pos; auto. all: lia.
  - Qcrush. bdestruct (x =? 0). lia. specialize (H1 (pred x)). intuition. rewrite Nat.succ_pred_pos in H1; intuition.
Qed.
...
Lemma tfvs_subst_id : forall T n T' q' T'' q'',
  ~Tfvs T n -> ({ n |-> T' ~ q' }ᵀ T) = ({ n |-> T'' ~ q'' }ᵀ T).
intros. induction T; auto. 
  - destruct v; auto. unfold subst_ty. apply Nat.eqb_neq in H. rewrite H; auto.
  - intuition. simpl. rewrite IHT1,IHT2. repeat rewrite qfvs_subst_id with (q':=q') (q'':=q''). all: Qcrush.
  - intuition. simpl. rewrite IHT1,IHT2. repeat rewrite qfvs_subst_id with (q':=q') (q'':=q''). all: Qcrush.
  - intuition. simpl. rewrite IHT. repeat rewrite qfvs_subst_id with (q':=q') (q'':=q''). all: Qcrush.
  - intuition. simpl. rewrite IHT. repeat rewrite qfvs_subst_id with (q':=q') (q'':=q''). all: Qcrush.
Qed.
...
Lemma qbvs_splice: forall q l n,
  (qbvs q n) = (qbvs (q ↑ᵈ l) n).
intros. destruct (qbvs q n) eqn:Eq; Qcrush.
Qed.
...
Proof. intros Γ1 Γ2 Σ T1 d1 T2 d2  Hstp T'. remember (Γ1 ++ Γ2)  as Γ. generalize dependent Γ1.  induction Hstp. intros Γ1.
  ...
  - intros. assert (stp Γ Σ (TSRef q1 T1) d1 (TSRef q2 T2) d2). { constructor; intuition. } subst.
    apply stp_closed in H5 as Hcl. intuition.
    inversion H6. inversion H7. subst. simpl.
    specialize (IHHstp1 Γ1). intuition.
    specialize (IHHstp2 Γ1). intuition.
    constructor. apply weaken_qstp_gen. subst; auto.
    1,2 : rewrite splice_qual_empty in H9, H11; auto.
    1,2: apply splice_qual_closed'; rewrite app_length in *; rewrite splice_env_length; auto.
    repeat erewrite <- qbvs_splice; auto. 
    1, 2: erewrite <- @splice_qual_id with (d:=∅) (f:=(‖ Γ2 ‖)); auto; repeat rewrite <- splice_qual_open'''; eapply weaken_qstp_gen; intuition.
...
Proof. intros Γ1 b U du Γ2 Σ T1 d1 T2 d2 HST Hb. remember (Γ1 ++ (bind_tm, b,U,du) :: Γ2) as Γ.
  ...
  - (* TSRef *) subst. constructor. eapply narrowing_qstp_gen; eauto. auto. auto.
    1, 2: rewrite app_length in *; simpl in *; auto. auto.
    1, 2: eapply narrowing_qstp_gen; subst; eauto.
...
Proof. intros Γ1 b U du Γ2 Σ T1 d1 T2 d2 HST. remember (Γ1 ++ ((bind_ty, b, U, du)) :: Γ2) as Γ.
  ...
  - (* TSRef *) subst. constructor. eapply narrowing_qstp_ty_gen; eauto. auto. auto.
    1,2 : rewrite app_length in *; simpl in *; auto. auto.
    1,2: eapply narrowing_qstp_ty_gen; subst; eauto.
...
intros. unfold_q. ndestruct (snd (fst d) 0).
  - Qcrush. bdestruct (x =? 0). lia. eapply lt_le_trans with (m:=b); auto.
  - Qcrush.
Qed.
...
Proof.
  ...
  - econstructor. eapply closed_ty_monotone; eauto; lia. eapply closed_Qual_monotone; eauto. lia.
  - econstructor. eapply closed_ty_monotone; eauto; lia.
...
Lemma wf_senv_saturated_qplus : forall {Σ}, wf_senv Σ -> forall {l b T q}, indexr l Σ = Some (b, T, q) -> senv_saturated Σ q -> senv_saturated Σ (q ⊔ &!l).
  intros. specialize (wf_senv_prop H l b T q) as Hwf. intuition. eapply senv_saturated_qplus; eauto.
Qed.
...
Lemma closed_ty_open_same : forall (T : ty) (b bb f l : nat),
       closed_ty bb f l T ->
       forall (U : ty) (d : qual),
       closed_ty bb f l U ->
       closed_Qual bb f l d↑ -> closed_ty bb f l ([[b ~> U ~ d ]]ᵀ T).
intros. generalize dependent b. induction H; intros; simpl; auto.
  - bdestruct (b0 =? x). subst. eapply closed_ty_monotone; eauto. constructor. lia.
  - constructor; auto. apply closed_Qual_open_same; eauto. apply closed_Qual_open_same; eauto. eapply closed_Qual_monotone; eauto. 
    eapply IHclosed_ty2. eapply closed_ty_monotone; eauto. eapply closed_Qual_monotone; eauto. 
  - constructor; auto. apply closed_Qual_open_same; eauto.
  - constructor; auto. apply closed_Qual_open_same; eauto. eapply closed_Qual_monotone; eauto. 
  - constructor; auto. apply closed_Qual_open_same; eauto. apply closed_Qual_open_same; eauto. eapply closed_Qual_monotone; eauto. apply IHclosed_ty2. eapply closed_ty_monotone; eauto. eapply closed_Qual_monotone; eauto. 
Qed.

Lemma tbvs_splice_ty : forall T1 v k,
  Tbvs T1 v <-> Tbvs (T1 ↑ᵀ k) v.
intros. intuition.
  - generalize dependent v. induction T1; intros; simpl; auto. 
    + destruct v; auto. destruct (le_lt_dec k i); auto.
    + destruct H as [H | [ H | [ H | H ]]]; intuition.
    + destruct H as [H | [ H | [ H | H ]]]; intuition.
    + destruct H as [H | H]; intuition.
    + destruct H as [H | H]; intuition.
  - remember (T1 ↑ᵀ k). generalize dependent v. generalize dependent t. induction T1; intros; rewrite Heqt in H; simpl in *; auto. 
    + destruct v; auto. destruct (le_lt_dec k i); auto.
    + specialize (IHT1_1 (T1_1 ↑ᵀ k)). specialize (IHT1_2 (T1_2 ↑ᵀ k)). destruct H as [H | [ H | [ H | H ]]]; intuition.
    + specialize (IHT1_1 (T1_1 ↑ᵀ k)). specialize (IHT1_2 (T1_2 ↑ᵀ k)). destruct H as [H | [ H | [ H | H ]]]; intuition.
    + specialize (IHT1 (T1 ↑ᵀ k)). destruct H as [H | H]; intuition.
    + specialize (IHT1 (T1 ↑ᵀ k)). destruct H as [H | H]; intuition.
Qed.
...
Lemma wf_senv_splice_id : forall Σ n,
  wf_senv Σ ->
  Σ ↑ᴳ n = Σ.
intros. induction Σ; simpl; auto. inversion H. 
- subst. simpl. erewrite splice_ty_id. erewrite splice_qual_id. rewrite IHΣ; auto. eapply closed_qual_monotone; eauto. lia. eapply closed_ty_monotone; eauto. lia.
- subst. simpl. erewrite splice_ty_id. erewrite splice_qual_id. rewrite IHΣ; auto. eapply closed_qual_monotone; eauto. lia. eapply closed_ty_monotone; eauto. lia.
Qed.
...
- inversion H0. subst. specialize (IHT b' b). rewrite IHT; simpl; auto. erewrite qbvs_closed; eauto. eapply closed_Qual_monotone; eauto. lia.
Qed.
...
intuition.
    ...
    + (* (‖ Γ0 ‖) > f *)
      ...
      rewrite subqual_splice_lr'; auto. 
      rewrite <- not_free_splice_ty_iff. auto.
  - (* t_app_val *) simpl. rewrite splice_qual_open''. rewrite splice_ty_open''. rewrite splice_qual_empty. apply has_type_closed in HT2 as Hcl. intuition. apply t_app_val with (T1:=T1 ↑ᵀ ‖Γ2‖) (df:=df ↑ᵈ ‖Γ2‖); eauto.
    destruct H; simpl; auto. 
    intros. assert (t2 = &l). eapply splice_tm_loc_id; eauto. rewrite H0 with (l:=l); auto. apply Q_lift_eq. Qcrush. 
    specialize (@splice_qual_lub_dist φ ({♦}) (‖ Γ2 ‖)) as Hdist. rewrite splice_qual_fresh in Hdist.
    rewrite <- Hdist. rewrite <- @splice_qual_empty with (k := ‖Γ2‖); rewrite <- splice_qual_open''.
    rewrite subqual_splice_lr'; auto. 
auto. 
    rewrite <- not_free_splice_ty_iff. auto.
  (* - (1* t_app_loc *1) simpl. rewrite splice_qual_open''. rewrite splice_ty_open''. rewrite splice_qual_empty. apply t_app_loc with (T1:=T1 ↑ᵀ ‖Γ2‖) (df:=df ↑ᵈ ‖Γ2‖) (l:=l). *)
  (*   apply IHHT1; auto. apply IHHT2; auto. *)
  (*   specialize (@splice_qual_lub_dist φ ({♦}) (‖ Γ2 ‖)) as Hdist. rewrite splice_qual_fresh in Hdist. *)
  (*   rewrite <- Hdist. rewrite <- @splice_qual_empty with (k := ‖Γ2‖); rewrite <- splice_qual_open''. *)
(* rewrite subqual_splice_lr'; auto. apply Q_lift_eq. Qcrush. *) 
(* auto. *) 
  (*   rewrite <- not_fresh_splice_iff. auto. *)
  (*   rewrite <- not_free_splice_ty_iff. auto. *)
...
repeat rewrite splice_q_trans_tenv_comm. auto.
  ...
  - (* t_sassign *) simpl. specialize (IHHT1 wfs Γ2 wft Γ1). specialize (IHHT2 wfs Γ2 wft Γ1). intuition.
    remember (a, b1, b0, b) as X.
specialize (H0 X). specialize (H1 X). simpl in *. rewrite splice_qual_empty. eapply t_sassign with (d:=d ↑ᵈ (‖ Γ2 ‖)) (d1:=d1 ↑ᵈ (‖ Γ2 ‖)) (T:=T ↑ᵀ (‖ Γ2 ‖)); eauto. rewrite splice_qual_open''' in H0. rewrite splice_qual_empty in H0; auto.
  - (* t_sassign_v *) simpl. specialize (IHHT1 wfs Γ2 wft Γ1). specialize (IHHT2 wfs Γ2 wft Γ1). intuition.
    remember (a, b1, b0, b) as X.
    specialize (H0 X). specialize (H1 X). simpl in *. rewrite splice_qual_empty. destruct (le_lt_dec (‖ Γ2 ‖) f); eapply t_sassign_v; eauto.
    + (* f in the spliced portion *) replace ($! (S f)) with (($! f ↑ᵈ (‖ Γ2 ‖))); eauto. rewrite <- splice_qual_open'''; auto. apply Q_lift_eq. Qcrush.
    + (* f in unspliced portion *) replace ($! f) with (($! f ↑ᵈ (‖ Γ2 ‖))); eauto. rewrite <- splice_qual_open'''; auto. apply Q_lift_eq. Qcrush.
  - (* t_sassign_l *) simpl. specialize (IHHT1 wfs Γ2 wft Γ1). specialize (IHHT2 wfs Γ2 wft Γ1). intuition.
    remember (a, b1, b0, b) as X.
    specialize (H0 X). specialize (H1 X). simpl in *. rewrite splice_qual_empty. eapply t_sassign_l; eauto.
...
Lemma weaken_flt : forall {Γ φ Σ t T d},
  ...
  (* - assert (φ ⊔ {♦} ⊑↑ φ' ⊔ {♦}). eapply Subq_qor; eauto. *)
  (*   eapply t_app_loc; eauto. eapply Subq_trans; eauto. *)
  - assert (φ ⊔ {♦} ⊑↑ φ' ⊔ {♦}). eapply Subq_qor; eauto.
...
Proof.  intros Γ φ Σ t T d HT. induction HT; intros; intuition.
    ...
    - econstructor; eauto using closed_tm_monotone, closed_ty_monotone, closed_Qual_monotone, extends_length. apply IHHT; auto. simpl. apply closed_qenv_extend; auto. apply closed_qenv_extend; auto. eapply closed_qenv_monotone; eauto. Qcrush. inversion H0. Qcrush.
  (* - eapply t_app_loc; eauto using closed_Qual_monotone. *)
  - econstructor; eauto using closed_Qual_monotone.
...
Proof. intros t Γ1 bd b U du Γ2 φ Σ T d HT Hb. remember (Γ1 ++ (bd, b, U, du) :: Γ2) as Γ.
  ...
  - repeat rewrite app_length in *; simpl in *; auto.
    ...
    rewrite app_comm_cons. rewrite app_comm_cons.
    eapply IHHT; eauto. simpl. auto.
  - eapply t_app_empty; eauto.
  - eapply t_app_var; eauto. 
  - eapply t_app_val; eauto.
  (* - eapply t_app_loc; eauto. *)
  - eapply t_app; eauto.
...
Lemma closed_Qual_subq_qdom : forall q (Σ: senv),
  closed_Qual 0 0 (‖ Σ ‖) q ↑ ->
  q ⊑↑ (qdom Σ).
intros. unfold qdom,n_dom. Qcrush; eauto. unfold N_lift. apply Nat.ltb_lt. eauto.
Qed.
...
Proof.
  intros. unfold CtxOK in *. destruct H as [H [Hlen Hsub]]. 
  ...
  - (* l points to new location *)
    ...
    apply closed_Qual_qor; eauto. rewrite Hlen. Qcrush.
  - (* l points to old locations *)
    destruct Hsub as [Hsub HH]. specialize (HH b0 l v T0 q). 
    assert (N_lift (qlocs φ) l). { Qcrush. } 
    assert (Hneq: l =? ‖ Σ ‖ = false). { apply Nat.eqb_neq. lia. }
    rewrite Hneq in *. intuition.
    + eapply weaken_flt. eapply weaken_store. all: eauto. 
    apply closed_qenv_empty. apply [].
    apply closed_Qual_qor; eauto. rewrite Hlen. Qcrush.
    + intuition. eapply weaken_flt. eapply weaken_store. all: eauto. 
    apply closed_qenv_empty. apply [].
    apply closed_Qual_qor; eauto. rewrite Hlen. Qcrush.
Qed.
...
Lemma CtxOK_weaken'' : forall {Σ σ T b t d φ},
  ...
  value t ->
  (b = false -> has_type [] φ Σ t T d) ->
  (b = true -> has_type [] φ Σ t T ([[0 ~> &! (‖ σ ‖) ]]ᵈ d)) ->
  wf_senv Σ ->
  CtxOK [] (qdom ((b, T, d) :: Σ)) ((b, T, d) :: Σ) (Some t :: σ).
Proof.
  intros. replace (qdom ((b, T, d) :: Σ)) with (qdom Σ ⊔ &! (‖ σ ‖)). eapply CtxOK_weaken'; eauto; intros; subst; intuition. 
  - eapply @weaken_flt with (φ:=φ); eauto. apply has_type_closed in H4. intuition. apply closed_Qual_subq_qdom; auto. 
  - eapply @weaken_flt with (φ:=φ); eauto. eapply weaken_store; eauto. apply closed_qenv_empty. apply []. apply has_type_closed in H4. intuition. eapply Subq_trans. apply closed_Qual_subq_qdom; eauto. auto. apply closed_Qual_qor; eauto. inversion H. intuition. rewrite H6. Qcrush.
  - rewrite qdom_cons_qor. inversion H. intuition. rewrite H6. auto.
Qed.
...
Proof.
  ...
  - constructor. apply indexr_var_some' in H4; intuition.
  - constructor. apply stp_closed in H5. intuition. eapply closed_Qual_monotone; eauto.
Qed.
...
Lemma vtp_qual_ex: forall {Σ φ t T d d'}, vtp Σ φ t T d -> qstp ([]: tenv) Σ d d' -> vtp Σ φ t T d'.
Proof.
  intros. apply qstp_closed in  H0 as  Hcl_qstp; intuition.  induction  H.
  - assert  (qstp  [] Σ  df1 d'). { eapply qs_trans;  eauto. }
    econstructor; eauto.
  - econstructor;  eauto.
  - econstructor. Focus 6. eapply H7. all: eauto.
  - econstructor. Focus 6. eapply H7. all: eauto.
  - assert (qstp [] Σ df1 d').  { eapply qs_trans; eauto. }
    econstructor; eauto.
  - econstructor; eauto.
...
Lemma unopen_fresh : forall {q1 q2},
  [[0 ~> ∅ ]]ᵈ q1 = [[0 ~> ∅ ]]ᵈ q2 ->
  ♦∉ q1 <-> ♦∉ q2.
intros. qual_destruct q1. qual_destruct q2. autounfold in *. simpl in *.
unfold open_qual in *. unfold qbvs in *. simpl in *.
ndestruct (bvs 0); ndestruct (bvs0 0); autounfold in *; simpl in *; repeat rewrite qor_empty_right in H; inversion H; subst; intuition.
Qed.
...
Proof.
  intros. induction H.
  - eapply vtp'_loc. Focus 6. eapply H5. all: eauto.
    apply qstp_closed in H0. intuition.
  - eapply vtp'_sloc. Focus 6. eapply H5. all: eauto.
    apply qstp_closed in H0. intuition.
  - econstructor; eauto.
Qed.
...
Proof.
  ...
  - eapply qstp_empty in H10. eapply qstp_empty in H11.
    ...
    econstructor; eauto.
    eapply qstp_closed in H0. intuition.
  - eapply qstp_empty in H11. eapply qstp_empty in H12.
    assert ([[0 ~> ∅ ]]ᵈ q1 = [[0 ~> ∅ ]]ᵈ q2). eapply subQual_eq; eauto.
    try rewrite H12 in *.
    eapply vtp_sloc. 6: eauto 3. all: eauto 3.
    eapply qstp_closed in H0. intuition.
    1,2: rewrite H13; apply qstp_refl; eapply closed_qual_open'; eauto.
  - econstructor; eauto.
    eapply qstp_closed in H0. intuition.
  - econstructor; eauto.
...
Proof.
  ...
  - auto.
  - inversion Hvtp'; subst. intros. eapply vtp'_loc. Focus 6. eapply H11. all: eauto 3.
  - inversion Hvtp'; subst. intros. eapply vtp'_sloc. Focus 6. eapply H12. all: eauto 3.
  - inversion Hvtp'; subst; econstructor; eauto.
  - intuition.
  - inversion Hvtp'.
  - inversion Hvtp'.
...
Proof.
  ...
  - inversion HVtp; subst. intros. eapply vtp_sloc. Focus 6. eapply H12. all: eauto.
...
Proof.
  ...
  - (* tloc *) intros.  eapply vtp_loc; eauto.
    ...
    + apply stp_refl; auto.
    + apply stp_refl; auto.
  - (* tsloc *) intros.  eapply vtp_sloc with (q1:=q); eauto 2.
    + Qcrush.
    + apply stp_refl; auto.
    + apply stp_refl; auto.
    + apply qstp_refl. apply Q_lift_closed. Qcrush.
    + apply qstp_refl. eapply closed_qual_open'; eauto.
    + apply qstp_refl. eapply closed_qual_open'; eauto.
...
(* a tightly bounded vtp on qualifier (locations) *)
Inductive vlocs : senv -> qual -> tm -> ty -> qual -> Prop :=
| vlocs_tabs: forall Σ φ t T1 T2 d1 d2 df1,
  closed_tm 2 0 (‖Σ‖) t ->
  closed_ty 0 0 (‖Σ‖) (TAll d1 d2 T1 T2) -> (* subtype *)
  closed_Qual 0 0 (‖ Σ ‖) df1 ↑ ->
  has_type [(bind_ty, false, T1, d1); (bind_tm, true, (TAll d1 d2 T1 T2), df1)] (df1 ⊔ $!0 ⊔ $!1)  Σ
           (t <~²ᵗ ([] : tenv)) (T2 <~²ᵀ ([] : tenv)) (d2 <~²ᵈ ([] : tenv)) ->
  d1 ⊑↑ df1 ⊔ {♦} ->
  ♦∉ df1 ->
  vlocs Σ φ (ttabs t) (TAll d1 d2 T1 T2) df1

| vlocs_base: forall Σ φ,
  vlocs Σ φ tunit TUnit ∅

| vlocs_loc:  forall Σ φ l T U q1 q2,
  closed_ty 0 0 (‖Σ‖) T ->
  closed_Qual 0 0 (‖Σ‖) q1↑ ->
  l ∈ₗ φ ->
  indexr l Σ = Some (false,T,q1) ->
  ...
  vlocs Σ φ &l (TRef q2 U) &!l

| vlocs_sloc:  forall Σ φ l T U q1 q2,
  closed_ty 0 0 (‖Σ‖) T ->
  closed_Qual 1 0 (‖Σ‖) q1↑ ->
  l ∈ₗ φ ->
  indexr l Σ = Some (true,T,q1) ->
  ...
  vlocs Σ φ &l (TSRef q2 U) &!l

| vlocs_abs: forall Σ φ T1 d1 T2 d2 df1 t,
  closed_tm 2 0 (‖Σ‖) t ->
  closed_ty 0 0 (‖Σ‖) (TFun d1 d2 T1 T2) ->
  closed_Qual 0 0 (‖ Σ ‖) df1 ↑ ->
  has_type [(bind_tm, false, T1, d1); (bind_tm, true, (TFun d1 d2 T1 T2), df1)] (df1 ⊔ $!0 ⊔ $!1)  Σ
           (t <~²ᵗ ([] : tenv)) (T2 <~²ᵀ ([] : tenv)) (d2 <~²ᵈ ([] : tenv)) ->
  d1 ⊑↑ df1 ⊔ {♦} ->
  ♦∉ df1 ->
  vlocs Σ φ (tabs t) (TFun d1 d2 T1 T2) df1
| vlocs_top: forall Σ φ t T d,
  vlocs Σ φ t T d ->
  vlocs Σ φ t TTop d

| vlocs_int: forall Σ φ c,
  vlocs Σ φ (tnat c) TInt ∅

| vlocs_bool: forall Σ φ c,
  vlocs Σ φ (tbool c) TBool ∅
.

Lemma vlocs_vtp : forall Σ φ t T q,
  vlocs Σ φ t T q ->
  vtp Σ φ t T q.
Proof.
intros. induction H; auto.
  - inversion H0. subst. eapply vtp_tabs; eauto 3.
    apply stp_refl; auto.
    apply stp_refl; auto. apply closed_ty_open2; simpl.
    1,2,4: eapply closed_ty_monotone; eauto.
    1,2: apply Q_lift_closed; Qcrush.
    eapply qstp_refl. apply closed_Qual_open2; Qcrush.
  - apply indexr_var_some' in H2 as Hlt.
    apply qstp_empty in H7.
    eapply vtp_loc. 6: eauto. all: eauto 3. Qcrush.
    apply qstp_refl; auto. Qcrush.
  - apply indexr_var_some' in H2 as Hlt.
    apply qstp_empty in H8 as Hsub.
    eapply vtp_sloc. 6: eauto. all: eauto 3. Qcrush.
    eapply closed_qual_open' with (d':=∅)in H0; eauto. 
    eapply closed_Qual_open_inv with (d':= ∅); eauto. 
    eapply @closed_Qual_sub with (d:=[[0 ~> ∅ ]]ᵈ q1); eauto. 
    apply qstp_refl; auto. Qcrush. 
  - inversion H0. subst. eapply vtp_abs; eauto 3.
    apply stp_refl; auto. intuition.
    apply stp_refl; auto. apply closed_ty_open2; simpl.
    1,2,4: eapply closed_ty_monotone; eauto.
    1,2: apply Q_lift_closed; Qcrush.
    eapply qstp_refl. apply closed_Qual_open2; Qcrush.
...
Lemma stp_TSRef_super_canonical_form : forall Γ Σ T q1 q2 d1 T1,
  stp Γ Σ (TSRef d1 T1) q1 T q2 ->
  (exists d1' T1', T = TSRef d1' T1') \/ T = TTop.
intros. remember (TSRef d1 T1).
generalize dependent T1.
generalize dependent d1.
induction H; intros; subst; try solve [inversion Heqt].
  - right. auto.
...
Lemma has_type_ttabs_canonical_form: forall Γ Σ φ t T q,
  ...
  (exists d1 d2 T1 T2, T = (TAll d1 d2 T1 T2)) \/ (T = TTop).
intros. remember (ttabs t). induction H; try solve [inversion Heqt0].
...
Lemma has_type_tloc_canonical_form: forall Γ Σ φ l T q,
  ...
  (exists d1 T1, T = (TRef d1 T1) \/ T = (TSRef d1 T1)) \/ (T = TTop).
intros. remember (tloc l). generalize dependent l.
induction H; intros; subst; try solve [inversion Heqt].
  - left. exists q,T. auto.
  - left. exists q,T. auto.
  - specialize (IHhas_type l eq_refl). intuition.
    + (* T1 is not TTop *)
      destruct H2 as [d1'[T1' HH]]. intuition; subst.
      ++ apply stp_TRef_super_canonical_form in H0. intuition. left. Ex. subst. exists x,x0. auto.
      ++ apply stp_TSRef_super_canonical_form in H0. intuition. left. Ex. subst. exists x,x0. auto.
    + (* T1 is TTop *)
      right. rewrite H2 in H0. eapply stp_TTop_super_canonical_form; eauto.
Qed.

Lemma has_type_tref_canonical_form: forall Γ Σ φ t T q,
  has_type Γ φ Σ (tref t) T q ->
  (exists d1 T1, T = (TRef d1 T1)) \/ (T = TTop).
intros. remember (tref t). generalize dependent t.
induction H; intros; subst; try solve [inversion Heqt0].
  - left. exists d1,T. auto.
  - specialize (IHhas_type t eq_refl). intuition.
    + (* T1 is not TTop *)
      destruct H2 as [d1'[T1' HH]]. subst.
...
Lemma has_type_tvar_lookup_TSRef_canonical_form: forall Γ Σ φ x T q,
  ...
  forall bd fr T1 q1 q2,
  indexr x Γ = Some (bd, fr, TSRef q1 T1, q2) ->
  (exists q1' T1', T = TSRef q1' T1') \/ T = TTop.
...
induction H; intros; subst; try solve [inversion Heqt].
  ...
  - intuition. specialize (H3 T0). intuition. Ex. subst. apply stp_TSRef_super_canonical_form in H0. destruct H0.
...
Lemma has_type_vtp': forall {Γ Σ φ t T q U d},
  value t ->
  T = TRef q U \/ T = TSRef q U ->
  has_type Γ φ Σ t T d ->
  wf_senv Σ ->
  vtp' Γ Σ φ t T d.
Proof.
  intros Γ Σ φ t T q U d HV HeqT Ht Hwf.
  ...
  - (* tloc *) eapply vtp'_loc. 6: eauto. all: eauto.
    ...
    + apply qstp_refl. eapply closed_Qual_monotone; eauto. lia.
  - (* tsloc *) inversion HeqT. subst. eapply vtp'_sloc. 6: eauto. all: eauto.
    + eapply closed_ty_monotone; eauto. lia.
    + eapply closed_Qual_monotone; eauto. lia.
    + eapply closed_Qual_monotone; eauto. lia.
    + Qcrush.
    ...
    + apply stp_refl; auto. eapply closed_ty_monotone; eauto. lia.
    + apply qstp_refl. eapply closed_qual_open'; eauto. eapply closed_Qual_monotone; eauto. lia.
    + apply qstp_refl. eapply closed_qual_open'; eauto. eapply closed_Qual_monotone; eauto. lia.
  - inversion HV; subst.
...
Lemma vtp_has_type: forall {Σ t T d φ}, vtp Σ φ t T d -> has_type [] d Σ t T d.
  ...
  + (* tsloc *) eapply qstp_empty in H8.
    eapply t_sub; auto. eapply t_sloc; eauto. eapply s_sref; eauto 2.
...
Lemma subst_qstp :  forall {Γ bd b Tf df df' Σ d1 d2},
  ...
  -  bdestruct (f =? 0).
    * pose (H' := H). subst. rewrite indexr_skips in H'; auto. simpl in H'. inversion H'. subst.
      ...
      + destruct H5. inversion H5. Ex; discriminate.
      + destruct H5. inversion H5. Ex; discriminate.
...
Lemma subst_ty_qstp :  forall {Γ T d d' Σ d1 d2},
    ...
    Substq (Γ ++ [(bind_ty, false, T, d)]) Σ T d d' ->
...
apply Substq_weaken; eauto. simpl; rewrite app_length; simpl. eapply closed_Qual_monotone; eauto. lia.
  ...
  - specialize (stp_closed HS1). intuition. specialize (stp_closed HS2). intuition.
    ...
    1,2: repeat erewrite open_subst1_qual_comm'; eauto; rewrite splice_qual_empty; eapply subst_qstp; eauto.
...
intros. induction H; auto. 
  - simpl. rewrite IHwf_senv. erewrite subst1_ty_id; eauto. erewrite subst1_qual_id; eauto.
  - simpl. rewrite IHwf_senv. erewrite subst1_ty_id; eauto. erewrite subst1_qual_id; eauto.
Qed.
...
Lemma q_trans''_subst1_tenv_subq'' : forall Γ0 Σ bd Tx l dx dx0 df0 df bx,
  (* forall dx, (q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) df0 ⋒ q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) dx0) = dx -> *)
  wf_senv Σ ->
  wf_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) Σ ->
  &! l ⊑↑ dx0 ->
  closed_Qual 0 0 (‖ Σ ‖) &!l ↑ ->
  (* closed_Qual 0 0 (‖ Σ ‖) df0 ↑ -> *)
q_trans'' ({0 |-> Tx ~ &! l }ᴳ Γ0) ({0 |-> &! l }ᵈ df) (‖ Γ0 ‖) ⊑↑ 
{0 |-> &! l }ᵈ (q_trans'' (Γ0 ++ [(bd, bx, Tx, df0 ⋒ dx0)]) df (S (‖ Γ0 ‖))).
...
(* q_trans'' ({0 |-> Tx ~ &! l }ᴳ Γ0) ({0 |-> &! l }ᵈ df) (‖ Γ0 ‖) ⊑↑ *) 
(* {0 |-> &! l }ᵈ (q_trans'' (Γ0 ++ [(bd, bx, Tx, q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) df0 ⋒ q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) dx0)]) df (S (‖ Γ0 ‖))) *)
(* intros. erewrite <- q_trans''_subst1_tenv_comm' with (df0:=df0); eauto. apply subst_qual_subqual_monotone. eapply Subq_trans. apply q_trans''_incr_subq. eapply q_trans_tenv_narrowing_subq; eauto. *)
(* Qed. *)
...
- erewrite <- q_trans''_subst1_tenv_comm; eauto. apply subst_qual_subqual_monotone. apply @Subq_trans with (d2:=q_trans'' (Γ0 ++ [(bd, bx, Tx, &! l)]) df (S (‖ Γ0 ‖))); eauto. apply q_trans''_incr_subq. apply q_trans_tenv_narrowing_subq; auto. eapply wf_tenv_weaken'; eauto.
- eapply @Subq_trans with (d2:=
  ({0 |-> &!l }ᵈ (q_trans'' (Γ0 ++ [(bd, bx, Tx, q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) df0 ⋒ q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) dx0)]) df (S (‖ Γ0 ‖))))); eauto. eapply q_trans''_subst1_tenv_subq''; eauto. eapply @Subq_trans with (d2:=q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) &!l). apply q_trans''_subq'. apply q_trans''_subq; auto.
Qed.
...
Lemma q_trans_senv_wf_senv_non_fresh : forall Σ dx,
  ...
  ♦∉ dx ->
  ♦∉ q_trans_senv Σ dx.
intros. unfold q_trans_senv. apply q_trans''_non_fresh; auto. intros. destruct a as [ [ b T ] q ]. simpl. eapply wf_senv_prop with (l:=x) (T:=T) (q:=q) (b:=b) in H; intuition.
Qed.
...
Lemma vtp_value' : forall Γ Σ φ t T d,
  vtp' Γ Σ φ t T d -> value t.
intros. induction H; auto.
Qed.

Lemma vtp_loc_qual_shrink : forall Σ φ l Tx dx,
  vtp Σ φ & l Tx dx ->
  vtp Σ φ & l Tx &!l.
Proof.
  intros. remember &l. induction H; try inversion Heqt; subst.
  - apply indexr_var_some' in H4 as Hlt.
    eapply vtp_loc with (q1:=q1); eauto 3.
    + Qcrush.
    + apply qstp_refl. apply Q_lift_closed. Qcrush.
  - apply indexr_var_some' in H4 as Hlt.
    eapply vtp_sloc with (q1:=q1); eauto 3.
    + Qcrush.
    + apply qstp_refl. apply Q_lift_closed. Qcrush.
  - eapply vtp_top; eauto.
Qed.

Lemma vtp_vlocs_qual_shrink : forall Σ φ l Tx dx,
  vtp Σ φ & l Tx dx ->
  vlocs Σ φ & l Tx &! l.
Proof.
  intros. remember &l. induction H; try inversion Heqt; subst.
  - apply indexr_var_some' in H4 as Hlt.
    eapply vlocs_loc with (q1:=q1); eauto 3.
  - apply indexr_var_some' in H4 as Hlt.
    eapply vlocs_sloc with (q1:=q1); eauto 3.
  - eapply vlocs_top; eauto. 
Qed.

Lemma vtp_vlocs_ty : forall Σ φ l T,
  vtp Σ φ & l T &!l ->
  vlocs Σ φ & l T &!l.
intros. remember &l. remember &!l. induction H; try inversion Heqt; subst. 
- eapply vlocs_loc with (q1:=q1); eauto. 
- eapply vlocs_sloc with (q1:=q1); eauto. 
- eapply vlocs_top; eauto.
Qed.
...
Lemma closed_ty_not_tbvs : forall {b b' f l T1},
  closed_ty b f l T1 ->
  b <= b' ->
  ~ Tbvs T1 b'.
intros. generalize dependent b'. generalize dependent b. induction T1; intros; intuition.
- destruct v. inversion H1. inversion H. inversion H1. intuition. 
- inversion H. subst. destruct H1 as [ H1 | [ H1 | [ H1 | H1 ] ] ]. 1,3: Qcrush; eauto. 
  + inversion H10. intuition. specialize (H4 (S (S b'))). intuition.
  + eapply IHT1_2 with (b':=(S (S b'))); eauto. lia.
- inversion H. subst. destruct H1 as [ H1 | [ H1 | [ H1 | H1 ] ] ]. 1,3: Qcrush; eauto. 
  + inversion H10. intuition. specialize (H4 (S (S b'))). intuition.
  + eapply IHT1_2 with (b':=(S (S b'))); eauto. lia.
- inversion H. subst. inversion H1. inversion H8. intuition. specialize (H5 b'). intuition. eapply IHT1; eauto. 
- inversion H. subst. inversion H1. inversion H8. intuition. specialize (H5 (S b')). intuition. eapply IHT1; eauto. 
Qed.
...
Lemma not_tbvs_open_id : forall T b Tx dx,
  negb (tbvs T b) = true ->
  [[b ~> Tx ~ dx ]]ᵀ T = T.
intros. generalize dependent b. induction T; intros; simpl in *; auto. 
- destruct v; auto. bdestruct (b =? i); subst; auto. rewrite Nat.eqb_refl in H. inversion H.
- rewrite negb_true_iff in H. 
  apply orb_false_elim in H. intuition. 
  apply orb_false_elim in H0. intuition. 
  apply orb_false_elim in H. intuition. 
  rewrite IHT1, IHT2; auto. repeat rewrite not_qbvs_open_id; auto.
- rewrite negb_true_iff in H. 
  apply orb_false_elim in H. intuition. 
  apply orb_false_elim in H0. intuition. 
  apply orb_false_elim in H. intuition. 
...
- rewrite negb_true_iff in H. 
  apply orb_false_elim in H. intuition. 
  rewrite IHT; auto. repeat rewrite not_qbvs_open_id; auto.
- rewrite negb_true_iff in H. 
  apply orb_false_elim in H. intuition. 
  rewrite IHT; auto. repeat rewrite not_qbvs_open_id; auto.
Qed.

Lemma tbvs_subst1 : forall T b f l Tx dx,
  closed_Qual b f l dx ↑ ->
  closed_ty b f l Tx ->
  (tbvs ({0 |-> Tx ~ dx }ᵀ T) b) = (tbvs T b).
intros. generalize dependent b. induction T; intros; simpl; auto.
- destruct v. bdestruct (i =? 0); subst; auto. eapply tbvs_closed; eauto. bdestruct (i =? b); subst; simpl. 
  + apply Nat.eqb_refl. 
  + apply Nat.eqb_neq; auto. 
- rewrite IHT1, IHT2; auto. repeat erewrite qbvs_subst1; eauto 3. 1,2: eapply closed_Qual_monotone; eauto. eapply closed_ty_monotone; eauto. 
- rewrite IHT1, IHT2; auto. repeat erewrite qbvs_subst1; eauto 3. 1,2: eapply closed_Qual_monotone; eauto. eapply closed_ty_monotone; eauto. 
- rewrite IHT; auto. repeat erewrite qbvs_subst1; eauto 3.
- rewrite IHT; auto. repeat erewrite qbvs_subst1; eauto 3. eapply closed_Qual_monotone; eauto.
Qed.
...
Lemma vlocs_qual_loc : forall Σ φ T l d,
  vlocs Σ φ &l T d ->
  vlocs Σ φ &l T &!l.
intros. remember (&l). induction H; try inversion Heqt; subst.
- eapply vlocs_loc; eauto. 
- eapply vlocs_sloc; eauto. 
- eapply vlocs_top; eauto.
Qed.

Lemma vlocs_qual_eq : forall Σ φ T l d,
  vlocs Σ φ &l T d -> d = &!l.
intros. remember (&l). induction H; try inversion Heqt; subst; eauto.
Qed.
...
Lemma substitution_gen :
  ...
  Substq (Γ ++ [(bd, bx,Tx,dx)]) Σ Tx dx dx' ->
        ...
        ((exists l, tx = tloc l) -> vlocs Σ φ' tx Tx dx') ->
...
erewrite q_trans''_extend_closed_id' with (q:=$! (S (‖ Γ0 ‖))). replace (S (‖ Γ0 ‖)) with (‖ Γ0 ++ [(bd, bx, Tx, dx)] ‖). remember (Γ0 ++ [(bd, bx, Tx, dx)]) as l. pose proof q_trans_tenv_fv as Hfv. unfold q_trans_tenv in Hfv. erewrite q_trans''_fuel_max with (E:=((bind_tm, true, TFun d1 d2 T1 T2, df) :: l)); auto. rewrite Hfv. rewrite qand_qor_dist_l. erewrite qglb_disjoint_freshv; eauto. rewrite qor_empty_right. repeat erewrite q_trans''_extend_closed_id'. erewrite q_trans''_fuel_max with (E:=(l)); simpl; auto. subst l. eapply @Subq_trans with (d2:=(q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) dx' ⊓ q_trans_tenv (Γ0 ++ [(bd, bx, Tx, dx)]) φ)); eauto. apply Subq_qand_split; unfold q_trans_tenv; eapply q_trans''_subq; eauto.
  ...
  - (* t_app_var *) intuition. rename H6 into IHHT1. rename H5 into IHHT2. simpl in *.
    ...
    -- eapply t_app_val. eauto. eapply vtp_value; eauto.
       intros. subst. assert (vlocs Σ φ' & l Tx dx'). apply Hex. exists l. auto. 
       rewrite subst1_just_fv0. eapply vlocs_qual_eq in H4; auto.  
       apply IHHT2; auto.
       replace (∅) with ({0 |-> dx' }ᵈ ∅) by auto; unfold openq. repeat erewrite <- subst1_open_qual_comm; eauto. erewrite <- subst1_fresh_id. subst φs. rewrite <- subst1_qor_dist. apply subst_qual_subqual_monotone; auto.
       replace (∅) with ({0 |-> dx' }ᵈ ∅) by auto; unfold openq. repeat erewrite <- subst1_open_qual_comm; eauto.
       erewrite <- not_free_subst1_ty_iff; eauto.
  ...
  - (* t_app_val *) intuition. rename H9 into IHHT1. rename H8 into IHHT2. simpl.
    ...
    * specialize (H3 l); intuition; subst; auto. 
    * inversion H0.
    * replace (∅) with ({0 |-> dx' }ᵈ ∅) by auto; unfold openq. repeat erewrite <- subst1_open_qual_comm; eauto. erewrite <- subst1_fresh_id. subst φs. rewrite <- subst1_qor_dist. apply subst_qual_subqual_monotone; auto.
    * erewrite <- not_free_subst1_ty_iff; eauto.
    * replace (∅) with ({0 |-> dx' }ᵈ ∅) at 1; auto. unfold open_ty. repeat erewrite subst1_open_ty_comm; eauto.
  ...
  (* - (1* t_app_loc *1) intuition. rename H7 into IHHT1. rename H4 into IHHT2. simpl. *)
  (*   replace ({ 0 |-> dx' }ᵈ (openq df &!l d2)) with *)
  (*              (openq ({ 0 |-> dx' }ᵈ df) ({ 0 |-> dx' }ᵈ &!l) ({ 0 |-> dx' }ᵈ d2)). *)
  (*   replace ({0 |-> Tx ~ dx' }ᵀ (T2 <~ᵀ TUnit ~ ∅; T1 ~ &!l)) with *)
  (*              (({0 |-> Tx ~ dx' }ᵀ T2) <~ᵀ TUnit ~ ∅; ({0 |-> Tx ~ dx' }ᵀ T1) ~ ({0 |-> dx' }ᵈ &!l)). *)
  (*   apply t_app_loc with (T1:= { 0 |-> Tx ~ dx' }ᵀ T1) (df:=({0 |-> dx' }ᵈ df)) (l:=l); auto. *)
  (*   unfold openq; replace (∅) with ({0 |-> dx' }ᵈ ∅) by solve [apply subst1_qual_empty]; *)
  (*       erewrite <- subst1_open_qual_comm; eauto; erewrite <- subst1_open_qual_comm; eauto. *)
...
eapply closed_qual_subst1; eauto 3. eapply closed_Qual_q_trans''_monotone; eauto. replace (S (‖ Γ0 ‖)) with (‖ Γ0 ++ [(bd, bx, Tx, dx)] ‖). apply wf_tenv_closed_qenv; auto. rewrite app_length. simpl. lia.
  ...
  - (* t_sloc *) erewrite @subst1_qual_id with (q:=(&!l)); eauto. simpl. erewrite subst1_ty_id; eauto.
    erewrite subst1_qual_id; eauto. apply t_sloc; auto. eapply closed_qual_subst1'; eauto.
    erewrite <- @subst1_qual_id with (q:=(&!l)); eauto. eapply subst_qual_subqual_monotone; eauto.
    all : apply indexr_var_some' in H3; eapply just_loc_closed; eauto.
...
Qcrush. erewrite subst1_ty_id; eauto. apply subst1_non_fresh; eauto.
  ...
  - (* t_sderef *) simpl. erewrite subst1_open_qual_comm; eauto. apply t_sderef with (d := { 0 |-> dx' }ᵈ d); auto.
    apply subst1_non_fresh; eauto. erewrite <- subst1_open_qual_comm; eauto. subst φs. erewrite <- subst1_fresh_id. rewrite <- subst1_qor_dist. apply subst_qual_subqual_monotone; auto.
  ...
  - (* t_sassign_v *) intuition. rename H5 into IHHT2. rename H4 into IHHT1. specialize (IHHT2 Γ0). specialize (IHHT1 Γ0). intuition. rewrite subst1_qual_empty. simpl in *. bdestruct (f =? 0); subst.
    + (* fv is substituted. do normal assignment *)
      ...
      inversion Hvtp. subst. assert (exists l0 : loc, & l = & l0). exists l. auto. intuition. eapply t_sassign_l; eauto. inversion H6; subst.
      -- erewrite subst1_open_qual_comm in H3; auto. Qcrush.
      -- erewrite subst1_open_qual_comm in H3; auto. Qcrush.
      -- apply vlocs_qual_eq in H17; subst. erewrite subst1_open_qual_comm in H3; auto. Qcrush.
  ...
  - (* t_sassign_l *) intuition. rename H5 into IHHT2. rename H4 into IHHT1. specialize (IHHT2 Γ0). specialize (IHHT1 Γ0). intuition. rewrite subst1_qual_empty. simpl in *.
          eapply has_type_vtp' in H4 as Hvtp; eauto using vtp_value.
          inversion Hvtp. subst; try discriminate. eapply t_sassign_l; eauto. erewrite subst1_open_qual_comm in H3; auto. Qcrush.
...
Lemma subst1_qual_subq : forall q1 q2 d n,
  q1 ⊑↑ q2 ->
  ({n |-> q1 }ᵈ d) ⊑↑ ({n |-> q2 }ᵈ d).
...
Lemma closed_qual_open_bv1 : forall {d b f l},
    closed_Qual (S b) f l d↑ ->
    forall {x}, x <= f ->
...
Proof.
  ...
  rewrite Q_lift_open_qual. unfold_Q. unfold_N. ndestruct (bvs b); intuition; eauto 3 with arith. eapply closed_Nats_tighten; eauto.
Qed.
...
Lemma polar_subst_qbvs_same : forall q v n df m f l,
  closed_Qual 0 f l df↑ ->
  v <> m ->
  (qbvs q v) = (qbvs ({n |-> df }ᵈ ([[m ~> $! n ]]ᵈ q)) v).
intros. qual_destruct q. unfold_q. ndestruct (bvs m). ndestruct (bvs v); simpl; clift. 
  - ndestruct (n_or fvs (n_one n) n); simpl. 
    symmetry. fold (N_lift (n_or (n_or (n_diff bvs (n_one m)) n_empty) (snd (fst df))) v). Qcrush. 
    Qcrush.
  - ndestruct (n_or fvs (n_one n) n); simpl.
    symmetry. apply not_true_is_false. fold (N_lift (n_or (n_or (n_diff bvs (n_one m)) n_empty) (snd (fst df))) v). Qcrush; eauto.
    Qcrush. 
  - simpl. ndestruct (fvs n); auto; simpl. ndestruct (bvs v); symmetry. 
    fold (N_lift (n_or bvs (snd (fst df))) v). Qcrush.
    apply not_true_is_false. fold (N_lift (n_or bvs (snd (fst df))) v). Qcrush. eauto.
Qed.

Lemma polar_subst_tbvs_same : forall T v n Tf df m f l,
  closed_Qual 0 f l df↑ ->
  closed_ty 0 f l Tf ->
  v <> m ->
(tbvs T v) = (tbvs ({n |-> Tf ~ df }ᵀ ([[m ~> $ n ~ $! n ]]ᵀ T)) v).
intros. generalize dependent n. generalize dependent m. generalize dependent v. induction T; intros; auto.
  - destruct v; simpl. bdestruct (i =? n); subst; auto. erewrite tbvs_closed; eauto. eapply closed_ty_monotone; eauto. lia.
    destruct (lt_dec i n); auto.
    bdestruct (i =? v0 ); bdestruct (m =? i); subst; simpl. lia.
    rewrite Nat.eqb_refl. eauto.
    rewrite Nat.eqb_refl. erewrite tbvs_closed; eauto. eapply closed_ty_monotone; eauto. lia.
    symmetry. rewrite Nat.eqb_neq. auto. 
  - simpl. rewrite <- IHT1, <- IHT2; auto. repeat erewrite <- polar_subst_qbvs_same; eauto.
  - simpl. rewrite <- IHT1, <- IHT2; auto. repeat erewrite <- polar_subst_qbvs_same; eauto.
  - simpl. rewrite <- IHT; auto. erewrite <- polar_subst_qbvs_same; eauto.
  - simpl. rewrite <- IHT; auto. erewrite <- polar_subst_qbvs_same; eauto.
Qed.

Lemma subst_qual_subqual_monotone'' : forall {d1 d2}, d1 ⊑↑ d2 -> forall {df n}, ({n |-> d1 }ᵈ df) ⊑↑ ({n |-> d2 }ᵈ df).
intros. qual_destruct df. unfold_q. ndestruct (fvs n). Qcrush. auto.
Qed.

Lemma open_qual_qbvs_same : forall m n q' q f l,
  closed_Qual m f l q' ↑ ->
  n <> m ->
  (qbvs q m) = qbvs ([[ n ~> q' ]]ᵈ q) m.
intros. qual_destruct q. unfold_q. ndestruct (bvs n); auto. simpl. ndestruct (bvs m). 
- symmetry. fold (N_lift (n_or (n_diff bvs (n_one n)) (snd (fst q'))) m). Qcrush.
- symmetry. apply not_true_is_false. fold (N_lift (n_or (n_diff bvs (n_one n)) (snd (fst q'))) m). Qcrush. eauto.
Qed.

Lemma open_ty_tbvs_same : forall m n T' q' T f l,
  closed_Qual m f l q' ↑ ->
  closed_ty m f l T' ->
  n <> m ->
  (tbvs T m) = tbvs ([[ n ~> T' ~ q' ]]ᵀ T) m.
intros. generalize dependent m. generalize dependent n. induction T; intros; simpl; auto. 
- destruct v; auto. bdestruct (i =? m); bdestruct (n =? i); subst; simpl; try lia. rewrite Nat.eqb_refl; auto. symmetry.
  apply not_true_is_false. intuition. eapply closed_ty_not_tbvs; eauto. assert (reflect (Tbvs T' m) (tbvs T' m)). apply tbvs_reflect. destruct H4; eauto. inversion H3. bdestruct (i =? m); eauto. 
- rewrite <- IHT1, <- IHT2; auto. repeat erewrite <- open_qual_qbvs_same; eauto. 1,2: eapply closed_Qual_monotone; eauto. eapply closed_ty_monotone; eauto.
- rewrite <- IHT1, <- IHT2; auto. repeat erewrite <- open_qual_qbvs_same; eauto. 1,2: eapply closed_Qual_monotone; eauto. eapply closed_ty_monotone; eauto.
- rewrite <- IHT; auto. repeat erewrite <- open_qual_qbvs_same; eauto.
- rewrite <- IHT; auto. repeat erewrite <- open_qual_qbvs_same; eauto. eapply closed_Qual_monotone; eauto.
Qed.
...
Lemma closed_ty_open2' : forall {b f l d1 d2 T T1 T2}, closed_ty b f l T -> closed_ty 0 f l T1 -> closed_Qual 0 f l d1↑ -> closed_ty 0 f l T2 -> closed_Qual 0 f l d2↑ -> closed_ty b f l ([[1 ~> T1 ~ d1 ]]ᵀ ([[0 ~> T2 ~ d2 ]]ᵀ T)).
  intros. erewrite open_rec_ty_commute''; eauto. eapply closed_ty_open''; eauto. eapply closed_ty_open''; eauto.
Qed.
...
Lemma subst2_ty_id' : forall {b f l T}, closed_ty b f l T -> forall {T1 T2 d1 d2}, { f |-> T1 ~ d1 ; T2 ~ d2 }ᵀ T = T.
  intros. repeat erewrite subst_ty_id; eauto.
Qed.
...
Lemma substitution1 : forall {t bdx bdf bf Tf df bx Tx dx Σ T d},
     ...
     forall {vx φ'}, vtp Σ φ' vx Tx dx -> ♦∉ dx ->
       ...
       (forall l', vx = &l' -> (not_free 1 T \/ dx = &!l')) ->
         has_type [] (df ⊔ dx) Σ
         ({ 0 |-> vf ; vx }ᵗ (t <~²ᵗ ([]:tenv)))
...
Proof.
  ...
  (* ({0 |-> df1; d1 }ᵈ (d3 <~²ᵈ [])) *)
   ...
   - (* vf not location, but vx can be *)
     ...
     -- (* covariant case *)
        inversion H3. subst. 
        eapply @substitution_gen with (tx:=&x) (dx':=&!x) in H; eauto.
        eapply weaken_flt with (φ':=df ⊔ dx) in H; auto. 
        eapply t_sub with (T2:=({0 |-> Tf ~ df; Tx ~ dx }ᵀ (T <~²ᵀ []))) (d2:=({0 |-> df; dx }ᵈ (d <~²ᵈ []))) in H; auto.
        replace ({0 |-> Tf ~ df; Tx ~ &! x }ᵀ (T <~²ᵀ [])) with ({0 |-> Tf ~ df; Tx ~ dx }ᵀ (T <~²ᵀ ([]:tenv))). apply stp_refl.
        unfold open_ty', open_ty. unfold not_free in H8. simpl. repeat eapply closed_ty_subst1; eauto. 
        constructor. eapply subst_qual_subqual_monotone'; eauto. eapply vtp_loc_qual_contains; eauto. eapply closed_qual_subst1; eauto. eapply closed_qual_subst1; eauto. 
        ...
        assert (&! x ⊑↑ dx). eapply vtp_loc_qual_contains; eauto. Qcrush.
        simpl. apply closed_Qual_qor; eauto.
        unfold q_trans_tenv. rewrite q_trans''_closed_id. rewrite <- q_trans''_or_dist. simpl. 
        assert (&! x ⊓ df ⊑↑ &! x). Qcrush. 
        assert (&! x ⊓ dx ⊑↑ &! x). Qcrush. 
        assert (&! x ⊑↑ dx). eapply vtp_loc_qual_contains; eauto. 
        repeat rewrite qand_qor_dist_l. replace (&! x ⊓ $! 0) with (∅).
        rewrite qor_empty_left. ndestruct (qfvs df 0); eapply @Subq_trans with (d2:=(&!x ⊔ &!x) ⊔ &! x); Qcrush.
        apply Q_lift_eq. Qcrush. Qcrush.
        simpl. constructor; auto.
        constructor; auto.
        apply vtp_has_type, has_type_tloc_canonical_form in Hvtp2. intuition. right. Ex; exists x1, x0; intuition.
        eapply vtp_loc_qual_contains; eauto.
        eapply vtp_loc_qual_shrink. eauto.
        intros. eapply vtp_vlocs_qual_shrink; eauto.
     -- (* qualifier location case *)
        inversion H3. subst. 
        eapply @substitution_gen with (tx:=&x) (dx':=&!x) in H; eauto.
        replace ({0 |-> &! x }ᵈ (df ⊔ $! 0)) with (df ⊔ &!x) in H; auto. 
        repeat rewrite subst1_qor_dist. rewrite subst1_just_fv0. erewrite subst1_qual_id; eauto.
        unfold q_trans_tenv. rewrite q_trans''_closed_id. rewrite <- q_trans''_or_dist. simpl. 
        assert (&! x ⊓ df ⊑↑ &! x). Qcrush. 
        repeat rewrite qand_qor_dist_l. replace (&! x ⊓ $! 0) with (∅).
...
Lemma has_type_tvar_lookup_tsref_prop: forall Γ Σ φ x bd fr T T' q q',
  has_type Γ φ Σ (tvar (varF x)) T' q' ->
  indexr x Γ = Some (bd, fr, T, q) ->
  (forall T1 q1, T = TSRef q1 T1 -> bd = bind_tm).
intros. remember (tvar (varF x)). generalize dependent T1.
induction H; intros; subst; try solve [inversion Heqt].
  - inversion Heqt. subst. rewrite H in H0. inversion H0. subst. intuition.
  - intuition. eapply H4; eauto.
Qed.
...
erewrite q_trans''_extend_closed_id' with (q:=$! (S (‖ Γ0 ‖))). replace (S (‖ Γ0 ‖)) with (‖ Γ0 ++ [(bind_ty, bx, Tx, dx)] ‖). remember (Γ0 ++ [(bind_ty, bx, Tx, dx)]) as l. pose proof q_trans_tenv_fv as Hfv. unfold q_trans_tenv in Hfv. erewrite q_trans''_fuel_max with (E:=((bind_tm, true, TFun d1 d2 T1 T2, df) :: l)); auto. rewrite Hfv. rewrite qand_qor_dist_l. erewrite qglb_disjoint_freshv; eauto. rewrite qor_empty_right. repeat erewrite q_trans''_extend_closed_id'. erewrite q_trans''_fuel_max with (E:=(l)); simpl; auto. subst l. eapply @Subq_trans with (d2:=(q_trans_tenv (Γ0 ++ [(bind_ty, bx, Tx, dx)]) dx' ⊓ q_trans_tenv (Γ0 ++ [(bind_ty, bx, Tx, dx)]) φ)); eauto. apply Subq_qand_split; unfold q_trans_tenv; eapply q_trans''_subq; eauto.
  ...
  - (* t_app_val *) intuition. rename H5 into IHHT1. rename H4 into IHHT2. simpl.
    ...
    * specialize (H0 l); intuition; subst; auto. 
    * inversion H4.
    * replace (∅) with ({0 |-> dx' }ᵈ ∅) by auto; unfold openq. repeat erewrite <- subst1_open_qual_comm; eauto. erewrite <- subst1_fresh_id. subst φs. rewrite <- subst1_qor_dist. apply subst_qual_subqual_monotone; auto.
    * erewrite <- not_free_subst1_ty_iff; eauto.
    * replace (∅) with ({0 |-> dx' }ᵈ ∅) at 1; auto. unfold open_ty. repeat erewrite subst1_open_ty_comm; eauto.
  ...
  assert (Hsepf : DF ⊓ Φ ⊑↑ df1). {
   ...
   - (* vf not location, but vx can be *)
     ...
     -- (* covariant case *)
        ...
        replace ({0 |-> Tf ~ df1; Tx ~ &! x }ᵀ (T <~²ᵀ ([]:tenv))) with ({0 |-> Tf ~ df1; Tx ~ dx }ᵀ (T <~²ᵀ ([]:tenv))). apply stp_refl.
        ...
        rewrite Htrue. 
        assert (&! x ⊓ df1 ⊑↑ &! x ⊓ df). Qcrush. 
        assert (&! x ⊓ (df ⋒ dx) ⊑↑ &! x). Qcrush. 
        assert (&! x ⊑↑ dx). eapply vtp_loc_qual_contains; eauto. 
        rewrite qand_qor_dist_l. rewrite qand_qor_dist_l. 
        replace (&! x ⊓ $! 0) with (∅). rewrite qor_empty_right. 
        eapply @Subq_trans with (d2:=(&!x ⊓ df)). Qcrush.
        Qcrush.
        apply Q_lift_eq. Qcrush.
        simpl. constructor; auto.
        Qcrush.
        ...
        apply vtp_has_type in Hvtp2. apply has_type_tloc_canonical_form in Hvtp2; intuition. right. Ex; subst.
        exists x1, x0. intuition.
        exists x1, x0. intuition.
        subst. simpl. eapply closed_Qual_monotone; eauto. 
        subst. simpl. eapply closed_Qual_monotone; eauto. 
        eapply vtp_loc_qual_contains; eauto. 
        1-4: Qcrush. 
        apply vtp_vlocs_qual_shrink in Hvtp2. apply vlocs_vtp; eauto.
        intros. eapply vtp_vlocs_qual_shrink; eauto.
     -- (* qualifier location case *)
        inversion H12. subst.
        eapply @substitution_gen with (tx:=&x) (dx':=&!x) in H; eauto. simpl in H. 
        replace ({0 |-> &! x }ᵈ (df1 ⊔ $! 0)) with (df1 ⊔ &!x) in H; auto.  
        ...
        rewrite Htrue. 
        assert (&! x ⊓ df1 ⊑↑ &! x ⊓ df). Qcrush. 
        assert (&! x ⊓ (df ⋒ &!x) ⊑↑ &! x). Qcrush. 
        assert (&! x ⊑↑ &!x). eapply vtp_loc_qual_contains; eauto. 
        rewrite qand_qor_dist_l. rewrite qand_qor_dist_l. 
        replace (&! x ⊓ $! 0) with (∅). rewrite qor_empty_right. 
        repeat rewrite qand_qor_dist_l. replace (&! x ⊓ $! 0) with (∅).
        eapply @Subq_trans with (d2:=(&!x ⊓ df)). Qcrush.
        ...
        unfold q_trans_tenv. repeat rewrite q_trans''_closed_id; eauto. simpl. remember [(bdx, false, Tx, df ⋒ &!x)] as Γ'. 
        erewrite <- q_trans''_closed_id with (fuel:=(‖ Γ' ‖)) (E:=Γ') (d:=df); eauto. 
        erewrite <- q_trans''_closed_id with (fuel:=(‖ Γ' ‖)) (E:=Γ') (d:=&!x) at 1; eauto.
        fold (q_trans_tenv Γ' df). 
        fold (q_trans_tenv Γ' &!x). apply SLocGrow; auto.
        apply vtp_has_type in Hvtp2. apply has_type_tloc_canonical_form in Hvtp2; intuition. right. Ex; subst.
        exists x1, x0. intuition.
        exists x1, x0. intuition.
        subst. simpl. eapply closed_Qual_monotone; eauto. 
...
Lemma vtp_canonical_form_sloc : forall {Σ φ t T q d},
   vtp Σ φ t (TSRef q T) d -> value t -> exists (l : loc), t = tloc l.
Proof. intros. remember (TSRef q T) as R. remember t as tt. generalize dependent T.
       induction H; intuition; try discriminate; inversion H0; subst. exists l. intuition.
Qed.
...
Proof. intros. induction H.
  ...
  + inversion H2. inversion H3. intuition.
    assert (♦∉ ([[0 ~> ∅ ]]ᵈ q2)). eapply qstp_non_fresh; eauto. unfold open_qual. ndestruct (qbvs q1 0); Qcrush. 
    econstructor. Focus 6. eapply H5. all: eauto.
    apply qstp_delete_fresh; auto.
...
Lemma vtp_store_sloc_defined : forall {Σ φ σ l d T d'},
    CtxOK [] φ Σ σ ->
    vtp Σ φ (& l) (TSRef d T) d' ->
    exists v, indexr l σ = Some (Some v).
Proof.
  intros. inversion H. inversion H0. subst. intuition.
  assert (l ∈ₗ qdom' σ). Qcrush. unfold qmem, N_lift in H4. simpl in H4.
...
Lemma closed_ty_tfvs : forall f f' b l T, 
   f <= f' ->
   closed_ty b f l T ->
   ~Tfvs T f'.
intros. generalize dependent b. generalize dependent f. induction T; auto; intuition; simpl in *.
- destruct v; inversion H0; lia.
- inversion H0. subst. intuition. 1,2: Qcrush; eauto 3. eapply IHT1; eauto. eapply IHT2; eauto.
- inversion H0. subst. intuition. 1,2: Qcrush; eauto 3. eapply IHT1; eauto. eapply IHT2; eauto.
- inversion H0. subst. intuition. 1,2: Qcrush; eauto 3.
- inversion H0. subst. intuition. 1,2: Qcrush; eauto 3.
Qed.
...
Lemma vtp_loc_non_var : forall Σ φ l T v,
  vtp Σ φ & l T $! v -> False.
intros. remember &l. remember $!v. induction H; subst; try solve [inversion Heqt].
   - apply qstp_empty in H7. Qcrush. intuition eauto.
   - apply qstp_empty in H8. Qcrush. intuition eauto.
   - intuition.
Qed.

Lemma qbvs_open_subq : forall q1 d1,
  qbvs q1 = qbvs d1 ->
  [[0 ~> ∅ ]]ᵈ d1 ⊑↑ [[0 ~> ∅ ]]ᵈ q1 ->
  d1 ⊑↑ q1.
intros. qual_destruct q1. qual_destruct d1. unfold open_qual in *. repeat rewrite H in H0. unfold_q. ndestruct (bvs0 0); subst; auto. Qcrush. 
   - specialize (H0 x). intuition. 
   - specialize (H4 x). intuition.
Qed.

Lemma qbvs_0_qbvs : forall q1 d1, 
  qbvs q1 0 = qbvs d1 0 ->
  forall f l, closed_Qual 1 f l q1 ↑ ->
  forall f' l', closed_Qual 1 f' l' d1 ↑ ->
  qbvs q1 = qbvs d1.
intros. qual_destruct q1. qual_destruct d1. unfold open_qual in *. unfold_q. apply N_lift_eq. intros. bdestruct (x =? 0); subst. 
   - unfold N_lift. rewrite H. intuition. 
   - ndestruct (bvs0 0); Qcrush. 
     + specialize (H1 x). intuition.
     + specialize (H5 x). intuition.
     + specialize (H1 x). intuition.
     + specialize (H5 x). intuition.
Qed.
  ...
  - (* ttapp *) right. subst. intuition.
     ...
     + (* contraction *)
       ...
       * apply (Preserve Σ ∅ φ); auto.  rewrite qqplus_qbot_right_neutral.
         ...
         pose (VH' := H23). eapply vlocs_tabs with (φ:=df1) in VH'; eauto. assert (HT' : has_type [(bind_ty, false, T1, d1) ; (bind_tm, true, TAll d0 d3 T0 T3, df1)] (df1 ⊔ $!0 ⊔ $!1) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T3) (openq' ([]:tenv) d3)). {
         ...
         apply vlocs_vtp; eauto. Ex. discriminate. 
     + (* left congruence *)
  ...
  - (* t_tapp_fresh *) right. subst. intuition.
         ...
         -- destruct (♦∈? df) eqn:Hfresh.
            ** erewrite @openq_duplicate_eq_l with (f:=0) (l:=‖Σ'‖). 3,4 : eapply closed_qual_monotone; eauto. 2,3 : eauto.
               ...
               --- apply has_type_filter in H0. apply has_type_filter in H. apply Qqplus_bound.
                   ...
                   Qcrush.
            ** rewrite not_fresh_qqplus in H29; auto. apply t_sub with (T1:=(T2 <~ᵀ TUnit ~ ∅; T1 ~ ∅)) (d1:=d2 <~ᵈ df; ∅).
   ...
   eapply open_qual_Subq''; eauto. 
     ...
     + (* left congruence *)
         ...
         -- destruct (♦∈? df) eqn:Hfresh.
            ** erewrite @openq_duplicate_eq_l with (f:=0) (l:=‖Σ'‖). 3,4 : eapply closed_qual_monotone; eauto. 2,3 : eauto.
               ...
               --- eapply t_app_val with (T1:=T1) (df:=(df ⋓ d')); eauto.
                   eapply weaken_flt. eapply weaken_store. eauto. eauto.
                   apply closed_qenv_empty; apply []. eapply wf_senv_closed_qenv_qn; eauto. eauto.
                   eapply has_type_closed in H32. intuition. eapply Subq_trans; eauto.
               --- apply stp_refl. simpl. eapply closed_ty_monotone; eauto.
                   constructor; auto. apply closed_qual_qqplus; auto.
                   apply openq_closed. 2 : apply closed_qual_qqplus.
                   1,2,4 : eapply closed_qual_monotone; eauto; lia. all: eapply disjointq_closed; eauto.
               --- apply has_type_filter in H2. apply has_type_filter in H. apply Qqplus_bound.
                   eapply has_type_closed in H32. intuition. eapply closed_Qual_qor_fresh in H34.
                   eapply openq_subqual_trans'. eapply H6. all: eauto. apply Subqual_non_fresh; auto. Qcrush.
            ** rewrite not_fresh_qqplus in H32; auto. apply t_sub with (T1:=(T2 <~ᵀ TUnit ~ ∅; T1 ~ d1)) (d1:=d2 <~ᵈ df; d1).
               --- eapply t_app_val with (T1:=T1); eauto. eapply weaken_flt. eapply weaken_store. eapply H2. eauto.
                   apply closed_qenv_empty; apply []. eapply wf_senv_closed_qenv_qn; eauto. eauto.
       ...
       * (* d2 is not dependent on f, so we don't observe the growth in df *)
         ...
         replace (d2 <~ᵈ df; d1) with (d2 <~ᵈ df ⋓ d'; d1). (* since f doesn't occur in d2 *)
         eapply t_app_val with (T1:=T1); eauto. eapply weaken_flt.
   ...
   - (* tapp *) right. subst. intuition.
     ...
     + (* right congruence *)
       ...
       eauto. apply closed_qenv_empty. apply [].
       eapply wf_senv_closed_qenv_qn; eauto. eauto.
       eapply has_type_closed in H26. intuition. eapply Subq_trans; eauto.
     + (* left congruence *)
       apply has_type_closed in H0 as Hcl. intuition.
         ...
         -- destruct (♦∈? df) eqn:Hfresh.
            ** erewrite @openq_duplicate_eq_l with (f:=0) (l:=‖Σ'‖). 3,4 : eapply closed_qual_monotone; eauto. 2,3 : eauto.
               ...
               --- apply stp_refl. simpl. eapply closed_ty_monotone; eauto.
                   ...
                   1,2,4 : eapply closed_qual_monotone; eauto; lia. all: eapply disjointq_closed; eauto.
               --- apply has_type_filter in H0. apply has_type_filter in H. apply Qqplus_bound.
                   eapply has_type_closed in H30. intuition. eapply closed_Qual_qor_fresh in H35.
     ...
     + (* right congruence *)
       ...
       * (* d1 fresh *)
         ...
         -- (* d2 dependent on x *) apply (Preserve Σ' d' φ'); auto.
           ...
           **  apply has_type_filter in H0. apply has_type_filter in H. apply Qqplus_bound.
               apply has_type_closed in H27. intuition. eapply closed_Qual_qor_fresh in H39.
     ...
     + (* left congruence *)
       ...
       * (* df fresh *)
         ...
         -- (* d2 not dependent on f *) apply (Preserve Σ' ∅ φ'); auto. rewrite qqplus_qbot_right_neutral.
            ...
            eapply weaken_flt. eapply weaken_store; eauto.
            apply closed_qenv_empty. apply []. eauto.
            eapply has_type_closed in H35. intuition.
            eapply has_type_closed in H35. intuition. eauto using Subq_trans.
        ...
        * (* df not fresh *) rewrite not_fresh_qqplus in H35; auto. apply (Preserve Σ' ∅ φ'); auto.
          ...
          apply closed_qenv_empty. apply []. eapply has_type_closed in H35. intuition.
...
inversion H14. apply t_sub with (T1:=TSRef d1 T) (d1:=(&!‖σ‖)).
  ...
  - (*tderef*) subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H7 (TRef d1 T0) t). intuition.
    + (* contraction *) right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
      ...
      specialize (H25 q1). apply qstp_closed in H27 as Hcl. 
      apply t_sub with (T1 := T)(d1:= q1); auto. intuition.
  ...
  - (*tsderef*) subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H7 (TSRef d1 T0) t). intuition.
    + (* contraction *) right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
      ...
      rewrite <- H14 in HH14. apply indexr_var_some in HH14.
      destruct HH14 as [v HHH14].
      specialize (vtp_store_sloc_defined HH3 HV) as Hval. destruct Hval.
      exists x. exists σ. intuition. apply step_deref; intuition.
      apply (Preserve Σ ∅ φ); intuition. rewrite qqplus_qbot_right_neutral.
      specialize (H26 true l x T).
      assert (l ∈ₗ qdom Σ). { unfold qmem. unfold qdom. simpl. unfold N_lift.
        apply indexr_var_some' in H20. unfold n_dom.  apply Nat.ltb_lt in H20. auto. }
      specialize (H26 q1). apply qstp_closed in H28 as Hcl. 
      apply t_sub with (T1 := T)(d1:= ([[0 ~> &!l ]]ᵈ q1)); auto. intuition.
eapply stp_qual_irrelevance; eauto 3. constructor; auto. apply qstp_empty in H24, H28. eapply open_qual_Subq'''; eauto. 
    + (*congruence *) right. intros. specialize (H7 σ H10).
      destruct H7 as [t' [σ' HH8]]. exists (tderef t'). exists σ'. intuition. constructor; auto.
destruct H12. apply (Preserve Σ' d' φ'); eauto. rewrite open_qual_qqplus; auto. apply t_sderef; auto. rewrite <- open_qual_qqplus; auto. apply Qqplus_bound. all: eapply Subq_trans; eauto.
  ...
  - (*t_sassign *) subst. intuition. rename H into Ht1. rename H0 into Ht2. intuition.
      apply has_type_closed in Ht1 as Ht1C. intuition.
      apply has_type_closed in Ht2 as Ht2C. intuition.
      specialize (H5 (TSRef d1 T) t1). intuition.
      specialize (H8 T t2). intuition.
      + (* contraction *)
        right. intros.
        pose (Ht1' := Ht1). eapply has_type_vtp in Ht1'; eauto.
        ...
        inversion Ht1'. destruct Hloc. subst.
        pose (Ht2' := Ht2). apply has_type_vtp in Ht2'; auto.
        exists tunit. exists (update σ x (Some t2)). inversion H30. subst.
        inversion H13. subst. specialize (indexr_var_some' H22) as HH22. intuition.
        econstructor; eauto. rewrite H28; auto. auto. apply (Preserve Σ ∅ φ); auto.
        eapply CtxOK_update; eauto. rewrite H28; auto. Qcrush. intuition. intros. apply t_sub with (T1:=T) (d1:=([[0 ~> ∅ ]]ᵈ d1)); auto.
        eapply stp_qual_irrelevance; eauto 3.
        constructor. apply qstp_empty in H33. eapply @Subq_trans with (d2:=[[0 ~> ∅ ]]ᵈ q1); eauto. apply open_qual_Subq'; auto. 
        rewrite Q_lift_open_qual. eapply closed_Qual_open'; eauto 3. Qcrush. 
        eapply has_type_filter in Ht2.
        eapply qstp_empty in H31. eapply open_qual_subqual.
apply has_type_filter in Ht1. eapply @Subq_trans with (d2:=d); eauto. eapply qstp_empty; eauto. eapply Subq_trans; eauto.
      + (* right congruence *)
        right. intros. specialize (H8 σ H13). destruct H8 as [t' [σ' H4']].
        exists (tassign t1 t'). exists σ'. intuition. econstructor; eauto.
        pose (HV := Ht1). apply has_type_vtp in HV; intuition. inversion HV; subst.
        destruct H15. apply (Preserve Σ' ∅ φ'); eauto. rewrite not_fresh_qqplus in H33; auto. simpl.
        eapply t_sassign; eauto. eapply weaken_flt. eapply weaken_store; eauto 3.
        apply closed_qenv_empty. apply []. auto. eapply has_type_closed in H33. Qcrush.
        unfold_q. ndestruct (snd (fst d1) 0); Qcrush.
      + (* left congruence *)
  ...
  - (* t_sassign_v *) subst. intuition. rename H into Ht1. rename H0 into Ht2. intuition.
      apply has_type_closed in Ht1 as Ht1C. intuition.
      apply has_type_closed in Ht2 as Ht2C. intuition.
      specialize (H5 (TSRef d1 T) $f). intuition.
      specialize (H8 T t2). intuition.
      + (* contraction *)
  ...
  - (* t_sassign_l *) subst. intuition. rename H into Ht1. rename H0 into Ht2. intuition.
      apply has_type_closed in Ht1 as Ht1C. intuition.
      apply has_type_closed in Ht2 as Ht2C. intuition.
      specialize (H5 (TSRef d1 T) &l). intuition.
      specialize (H8 T t2). intuition.
      + (* contraction *)
        right. intros.
        pose (Ht1' := Ht1). eapply has_type_vtp in Ht1'; eauto.
        ...
        inversion H13. subst. specialize (indexr_var_some' H23) as HH22. intuition.
        econstructor; eauto. rewrite H17; auto. auto. apply (Preserve Σ ∅ φ); auto.
        eapply CtxOK_update with (b:=true). eauto 3. lia. eauto. Qcrush. intuition. intros. specialize (H29 true x &x T). intuition. apply t_sub with (T1:=T) (d1:=([[0 ~> &! x ]]ᵈ d1)); auto.
        eapply stp_qual_irrelevance; eauto 3.
        apply qs_sq. 
        apply open_qual_Subq. apply qbvs_open_subq; eauto. eapply qbvs_0_qbvs; eauto 3. eapply qstp_empty; eauto. 
        rewrite Q_lift_open_qual. eapply closed_Qual_open'; eauto 3. Qcrush.
apply has_type_filter in Ht2; eauto. eapply @Subq_trans with (d2:=([[0 ~> &! x ]]ᵈ d1)); auto.
        apply open_qual_Subq. apply qbvs_open_subq; eauto. eapply qbvs_0_qbvs; eauto 3. eapply qstp_empty; eauto. auto. 
      + (* right congruence *)
        right. intros. specialize (H8 σ H13). destruct H8 as [t' [σ' H4']].
