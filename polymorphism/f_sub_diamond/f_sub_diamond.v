...
| bind_tm : X -> binding  (* term typing x: T *)
| bind_ty : X -> binding  (* abstract tying X <: T*)
.

Definition tenv := list (@binding (bool * ty * qual)). (* (isFunctionSelfRef, Type, Qual) *)
...
Fixpoint subst_ty (T : ty) (v : nat) (U: ty) (q : qual) : ty :=
  ...
  | TVar (varB i)    => TVar (varB i)
  | TAll q1 q2 T1 T2 => TAll (subst_q q1 v q) (subst_q q2 v q) (subst_ty T1 v U q) (subst_ty T2 v U q)
  | TUnit            => TUnit
  | TFun q1 q2 T1 T2 => TFun (subst_q q1 v q) (subst_q q2 v q) (subst_ty T1 v U q) (subst_ty T2 v U q)
  | TRef q1 T        => TRef (subst_q q1 v q) (subst_ty T v U q)
  end.

Definition subst_tenv (Γ : tenv) (v : nat) (U: ty)(q1 : qual) : tenv :=
  map (fun p => match p with
             | bind_tm(b, T, q') => bind_tm(b, (subst_ty T v U q1) , (subst_q q' v q1))
             | bind_ty(b,  T, q') => bind_ty(b, (subst_ty T v U q1) , (subst_q q' v q1))
             end) Γ.
...
Lemma closed_ty_open : forall {T fr b f l}, closed_ty (S b) f l T -> forall {x}, x < f -> closed_ty b f l (open_rec_ty b (TVar (varF x)) ${fr}x T).
  induction T; intros; simpl; intuition; inversion H; subst; try constructor;
    try solve [apply IHT1; auto | apply IHT2; auto | apply IHT; auto ]; intuition.
  destruct (b =? x0) eqn: Hbi; intuition. constructor. apply Nat.eqb_neq in Hbi. lia.
  all: eapply closed_qual_open; eauto.
Qed.
...
Lemma closed_ty_open_ge : forall {T fr b f l}, closed_ty (S b) f l T ->
  forall {x}, f <= x ->  closed_ty b (S x) l ([[ b ~> (TVar (varF x)) ~ ${fr}x ]]ᵀ T).
  induction T; intros; simpl; intuition; inversion H; subst; try constructor;
    try solve [eapply IHT1; eauto | eapply IHT2; eauto | eapply IHT; eauto ].
  lia. destruct (b =? x0) eqn: Hbi. apply Nat.eqb_eq in Hbi. intuition. apply Nat.eqb_neq in Hbi; intuition.
  all: eapply closed_qual_open_ge; eauto.
Qed.
...
Lemma closed_ty_open_succ : forall {T fr b f l}, closed_ty b f l T -> forall {j}, closed_ty b (S f) l ([[ j ~>  (TVar (varF f))  ~  ${fr}f ]]ᵀ T).
  ...
  all: eapply closed_qual_open_succ; eauto.
Qed.
...
Lemma splice_ty_open : forall {T j fr n m}, ([[j ~> $(m + n) ~ ${fr}(m + n) ]]ᵀ T) ↑ᵀ n =
...
Lemma splice_qual_open_qual : forall {k df d},
    splice_qual k (open_qual 0  df d) =
    open_qual 0 (splice_qual k df) (splice_qual k d).
Proof.
...
Lemma splice_qual_open''': forall {k d i} {q}, splice_qual k ([[i ~> d ]]ᵈ q) = [[i ~> (d ↑ᵈ k)]]ᵈ (q ↑ᵈ k ).
Proof. destruct q. destruct d. simpl.
       destruct (NatSet.F.mem i t0) eqn:Hmem; simpl; f_equal.
       auto. rewrite splice_set_union_dist. auto.
Qed.
...
Lemma splice_ty_closed'' : forall {T x b l k}, closed_ty b x l T -> k <= x -> closed_ty b (S x) l (T ↑ᵀ k).
  ...
  1-2: eapply IHT2; eauto. eapply IHT; eauto.
Qed.
...
Lemma splice_qual_open'' : forall {k df d1 d2}, (d2 <~ᵈ df; d1) ↑ᵈ k = (d2 ↑ᵈ k) <~ᵈ (df ↑ᵈ k); (d1 ↑ᵈ k).
  intros. unfold openq. destruct d2; destruct d1; destruct df; simpl.
  destruct (mem 0 t0) eqn: H1; simpl.
  - destruct (mem 1 (union (remove 0 t0) t6)) eqn: H2; simpl; f_equal; repeat rewrite splice_set_union_dist; auto.
  - destruct (mem 1 t0) eqn:H2; simpl; f_equal. rewrite splice_set_union_dist. auto.
Qed.
...
Lemma weaken_qstp_store : forall {Γ Σ d1 d2}, qstp Γ Σ d1 d2 -> forall {Σ'}, qstp Γ (Σ' ++ Σ) d1 d2.
  ...
  - eapply qs_self_all; eauto. erewrite length_app. eapply closed_qual_monotone; eauto. lia.
  - eapply qs_qvar; eauto. all : erewrite length_app. eapply closed_ty_monotone; eauto. lia. eapply closed_qual_monotone; eauto. lia.
  - eapply qs_qvar_ty; eauto. all : erewrite length_app. eapply closed_ty_monotone; eauto. lia. eapply closed_qual_monotone; eauto. lia.
...
Proof. intros Γ1 Γ2 Σ T1 d1 T2 d2  Hstp T'. remember (Γ1 ++ Γ2)  as Γ. generalize dependent Γ1.  induction Hstp. intros Γ1.
  - (* TTop *) intros. subst. constructor.  rewrite length_app in *. rewrite splice_tenv_length in *. simpl.
  ...
  - (* TVarF x*)  intros; subst. (* specialize (IHHstp Γ1).  intuition. *)  apply stp_refl.
    apply indexr_var_some' in H.  rewrite length_app. rewrite splice_tenv_length.
    replace (‖ Γ1 ‖ + ‖ T' :: Γ2 ‖) with (S ((‖ Γ1 ‖) +  (‖ Γ2 ‖))).
    assert (closed_ty 0 ((‖ Γ1 ‖) +  (‖ Γ2 ‖)) (length Σ) $x). { constructor. rewrite length_app in *.  auto. }
  ...
  - (* TAll *) intros; subst.  assert (stp (Γ1 ++ Γ2) Σ (TAll d1 d2 T1 T2) d5 (TAll d3 d4 T3 T4) d6). { econstructor; eauto. }
    ...
    1,2,5,6: apply splice_qual_closed'; auto; eapply closed_qual_monotone; eauto; rewrite length_app in*.
    5-8: apply splice_ty_closed'; auto; eapply closed_ty_monotone; eauto; rewrite length_app in*.

    1-8: rewrite length_app; erewrite splice_tenv_length; lia.

    eapply weaken_qstp_gen; eauto.
    erewrite length_app in *; eauto.
    ...
    rewrite splice_set_empty in H3. auto.
...
Proof. intros Γ1 b U du Γ2 Σ T1 d1 T2 d2 HST Hb. remember (Γ1 ++ bind_tm(b,U,du) :: Γ2) as Γ.
  ...
  - (* TTop *) subst. constructor. rewrite length_app in *.  simpl in *. intuition. eapply narrowing_qstp_gen; eauto.
...
Proof. intros Σ Γ T1 d1 T2 d2 HSTP. induction HSTP; intros.
  + constructor; auto. eapply closed_ty_monotone; eauto. rewrite length_app. lia. apply weaken_qstp_store. auto.
  + econstructor; eauto. apply weaken_qstp_store. auto.
  + econstructor; eauto. eapply closed_ty_monotone; eauto. rewrite length_app. lia.
  + econstructor; eauto. eapply closed_ty_monotone in H; eauto. rewrite length_app. lia.
    eapply closed_ty_monotone in H0; eauto. rewrite length_app. lia.
...
Proof. intros Γ1 U du Γ2 Σ T1 d1 T2 d2 HST. remember (Γ1 ++ (bind_ty (false, U, du)) :: Γ2) as Γ.
  generalize dependent Γ1; induction HST; intros; intuition.
  - (* TTop *) subst. constructor. rewrite length_app in *.  simpl in *. intuition.
  ...
  - (* TRef *) subst. constructor. 1,4,5 : eapply narrowing_qstp_ty_gen; eauto. auto. auto.
...
Lemma stp_scale_qlub : forall {Γ Σ T1 d1 T2 d2}, stp Γ Σ T1 d1 T2 d2 -> forall {q}, closed_qual 0 (‖Γ‖) (‖Σ‖) q -> stp Γ Σ T1 (d1 ⊔ q) T2 (d2 ⊔ q).
  intros. eapply stp_qual_irrelevance; eauto. apply stp_qstp_inv in H. rewrite qlub_commute. rewrite @qlub_commute with (d1:=d2). eauto.
Qed.
...
Lemma saturated_cons : forall {Γ Σ q}, saturated Γ Σ q -> forall {b T q'}, saturated (bind_tm(b, T, q') :: Γ) Σ q.
  intros. constructor; intros; eauto. unfold tenv_saturated. intros. apply H in H0. inversion H0.
  * econstructor; eauto. rewrite indexr_skip; eauto. apply indexr_var_some' in H1. lia.
  * subst. eapply sat_tvar; eauto. rewrite indexr_skip; eauto. apply indexr_var_some' in H1. lia.
Qed.
...
Lemma saturated_conss : forall {Γ Σ q}, saturated Γ Σ q -> forall {T q'}, saturated Γ ((T, q') :: Σ) q.
  ...
  * econstructor; eauto. simpl. eapply closed_qual_monotone; eauto.
  * eapply sat_tvar; eauto. simpl. eapply closed_qual_monotone; eauto.
  * apply senv_saturated_conss; eauto.
Qed.

Lemma saturated_app : forall {Γ' Γ Σ q}, saturated Γ Σ q -> saturated (Γ' ++ Γ) Σ q.
  induction Γ'; intros; simpl; intuition;  destruct a; destruct p; destruct p.
  eapply saturated_cons; auto. eapply saturated_ty_cons; auto.
Qed.
...
Lemma saturated_qlub : forall {Γ Σ q1 q2}, saturated Γ Σ q1 -> saturated Γ Σ q2 -> saturated Γ Σ (q1 ⊔ q2).
  ...
  - inversion H5. subst. econstructor; eauto. eapply subqual_trans; eauto.
    eapply sat_tvar; eauto. eapply subqual_trans; eauto.
  - inversion H5. subst. econstructor; eauto. eapply subqual_trans; eauto.
    eapply sat_tvar; eauto. eapply subqual_trans; eauto.
Qed.
...
Lemma saturated_qglb : forall {Γ Σ q1 q2}, saturated Γ Σ q1 -> saturated Γ Σ q2 -> saturated Γ Σ (q1 ⊓ q2).
  ...
  inversion H5; inversion H1; subst.
  * rewrite H3 in H10. inversion H10. subst. econstructor; eauto. apply qglb_bound; auto.
  * rewrite H3 in H10. discriminate.
  * rewrite H3 in H10. discriminate.
  * rewrite H3 in H10. inversion H10. subst. eapply sat_tvar; eauto. apply qglb_bound; auto.
Qed.
...
Lemma weaken_store_tenv_saturated : forall {Γ Σ q}, tenv_saturated Γ Σ q -> forall {Σ'}, Σ' ⊇ Σ -> tenv_saturated Γ Σ' q.
  ...
  * econstructor; eauto. eapply closed_qual_monotone; eauto.
  * eapply sat_tvar; eauto. eapply closed_qual_monotone; eauto.
Qed.
...
Proof.
  destruct ht; intuition; try apply has_type_closed in ht; try apply has_type_closed in ht1;
    ...
    | [ |- closed_qual _ _ _ (_ ⊔ _)] => apply closed_qual_qlub; auto
    end.
  ...
  - constructor. eapply closed_ty_monotone; eauto. lia. eapply closed_qual_monotone; eauto. lia.
...
Lemma qstp_empty : forall {Σ q1 q2}, qstp [] Σ q1 q2 -> q1 ⊑ q2.
  intros. remember [] as Γ. induction H; subst; auto; try solve [simpl in H; discriminate].
...
Lemma open_qual_subqual': forall {d1 d2 d}, d1 ⊑ d2 -> forall {k}, ([[ k ~> d1 ]]ᵈ d) ⊑ ([[ k ~> d2 ]]ᵈ d).
Proof.
  intros. destruct d1. destruct d2. destruct d. simpl in *. intuition.
  destruct (mem k t6) eqn: Hmem; simpl in *; intuition; try fnsetdec. destr_bool. destr_bool.
Qed.
...
Lemma splice_openq: forall {q j d1 n}, ([[j ~> d1]]ᵈ q) ↑ᵈ n = ([[j ~> d1 ↑ᵈ n ]]ᵈ (q ↑ᵈ n)).
 destruct q; simpl; intuition; destruct d1.
destruct (mem j t0) eqn: Heq; simpl; f_equal.
rewrite splice_set_union_dist. auto.
Qed.

Lemma splice_open_rec_ty : forall {T j T1 d1 n}, ([[j ~> T1 ~ d1 ]]ᵀ T) ↑ᵀ n =
                [[j ~> (T1 ↑ᵀ n) ~ (d1 ↑ᵈ n)]]ᵀ (T ↑ᵀ n).
  induction T; intros; simpl; auto. destruct v; simpl.
  destruct (le_lt_dec n i) eqn:Heq. simpl. auto.
  simpl. auto.
  destruct (j =?i); auto.
  f_equal. erewrite splice_openq; eauto. erewrite splice_openq; eauto. eapply IHT1. eapply IHT2.
  f_equal. erewrite splice_openq; eauto. erewrite splice_openq; eauto. eapply IHT1. eapply IHT2.
  f_equal. erewrite splice_openq; eauto. rewrite IHT. auto.
Qed.
...
Lemma weaken_gen : forall {t Γ1 Γ2 φ Σ T d},
  ...
  - (* t_tabs *) rewrite length_app in *. simpl.  constructor.
    apply splice_closed'.
    ...
    specialize (IHHT Γ2 (bind_ty (false, T1, d1) :: bind_tm (true, TAll d1 d2 T1 T2, df) :: Γ1)). intuition.  rename H5 into IHHT.
    ...
    1-4 : repeat rewrite length_app; rewrite splice_tenv_length; auto.
  ...
  - (* t_tapp *) simpl. rewrite splice_qual_open''. rewrite splice_ty_open''. rewrite splice_qual_empty.
    apply t_tapp; eauto.
    1-2: erewrite length_app in *; erewrite splice_tenv_length; simpl in *;
  ...
  - (* t_ref *) simpl in *. specialize (IHHT Γ2 Γ1). intuition.
    specialize (H2 X). destruct d1. repeat rewrite empty_union_left.
    replace (qset true t0 t1 t2) with ({♦} ⊔ (qset b t0 t1 t2)); try qdec.
  ...
  - (* t_assign *) simpl. specialize (IHHT1 Γ2 Γ1). specialize (IHHT2 Γ2 Γ1). intuition.
    specialize (H0 X). specialize (H1 X). simpl in *. rewrite splice_set_empty in *.
...
Proof. intros t Γ1 b U du Γ2 φ Σ T d HT Hb. remember (Γ1 ++ bind_tm(b, U, du) :: Γ2) as Γ.
  ...
  induction HT; intros; subst.
  - (* t_tabs*) repeat  rewrite length_app in *. simpl in *.
  ...
  - econstructor; eauto. all : rewrite length_app in *; simpl in *; auto.
  - (* t_tapp_fresh*)
    eapply t_tapp_fresh; eauto.
    1,2: repeat rewrite length_app in *; simpl in *; auto.
...
Proof.   intros. induction  Γ; intros.
  ...
  + remember (length (Γ ++ [bind_ty(b, T, d)])) as L.
    ...
    - assert (x = L). eapply Nat.eqb_eq. eauto.
      ...
      right. split. rewrite length_app in HeqL. simpl in HeqL. lia.
       assert ((pred x) = (‖ ({ 0 |-> T ~  d }ᴳ Γ) ‖)).
       rewrite subst1_tenv_length. rewrite length_app in HeqL. simpl in HeqL.  lia.
       simpl. eapply Nat.eqb_eq in H1.  subst.
       destruct (Init.Nat.pred (length (Γ ++ [bind_ty (b, T, d)])) =? length ({0 |-> T ~ d }ᴳ Γ)); auto.
       inversion H1.
       subst. eauto.
    - assert (x <> L). eapply Nat.eqb_neq. eauto.
       ...
       simpl.
       assert ((pred x) <> ( ‖({ 0 |-> T ~  d }ᴳ Γ)‖)).
       rewrite length_app in HeqL. simpl in HeqL. rewrite subst1_tenv_length.
...
Proof.  intros. induction  Γ; intros.
  ...
  + remember (length (Γ ++ [bind_tm(b, T, d)])) as L.
    ...
    - assert (x = L). eapply Nat.eqb_eq. eauto.
      eapply indexr_hit in H.
      split. rewrite length_app in HeqL. simpl in HeqL. lia.
...
Lemma closed_qual_subst1' : forall {Γ0 X l Tf df φ b},
  ...
  rewrite length_app in *. simpl in *. replace (‖ Γ0 ‖ + 1) with (S (‖ Γ0 ‖) ) in H1.
  auto.  lia.
Qed.
...
Lemma closed_tm_subst1' : forall {Γ0 X l Tf df tx t b},
  ...
  rewrite length_app in *. simpl in *. replace (length Γ0 + 1) with (S (length Γ0)) in H0.
  auto. lia.
Qed.
...
Lemma saturated0 : forall {Γ Σ Tx frx fx bx lx fr ff bf lf},
    mem 0 ff = true -> saturated (Γ ++ [bind_tm(Tx, qset frx fx bx lx)]) Σ (qset fr ff bf lf) -> implb frx fr = true /\ fx [<=] ff /\ bx [<=] bf /\ lx [<=] lf.
  ...
  rewrite indexr_skips in H3; auto. simpl in H3. discriminate.
Qed.

Lemma saturated0_ty : forall {Γ Σ Tx frx fx bx lx fr ff bf lf},
    mem 0 ff = true -> saturated (Γ ++ [bind_ty(Tx, qset frx fx bx lx)]) Σ (qset fr ff bf lf) -> implb frx fr = true /\ fx [<=] ff /\ bx [<=] bf /\ lx [<=] lf.
  intros. inversion H0. specialize (H1 0). simpl in H1. apply NatSet.F.mem_2 in H.
  apply H0 in H. inversion H. rewrite indexr_skips in H3; auto. simpl in H3.
  inversion H3. subst. simpl in H4.
  rewrite indexr_skips in H3; auto. simpl in H3. inversion H3. subst. inversion H4. intuition.
Qed.
...
Lemma subst1_ty_preserves_separation : forall {df d1 sx Tx dx dx' Γ Σ φ},
    ...
    {0 |-> dx' }ᵈ df ⊓ {0 |-> dx' }ᵈ d1 = {0 |-> dx' }ᵈ (df ⊓ d1).
Proof. intros. destruct df as [frf ff bf lf]. destruct d1 as [fr1 f1 b1 l1]. destruct dx' as [frx' fx' bx' lx'].
  destruct φ as [frp fp bp lp]. inversion H0. subst. apply bound_0_empty in H11, H13. (* simpl in H5. *) subst.
  destruct dx as [frx fx bx lx]. simpl in H. intuition. rewrite inter_empty_left in H6.
  rewrite inter_empty_left in H.
  destruct (mem 0 ff) eqn:Hmem0ff; destruct (mem 0 f1) eqn:Hmem0f1; simpl; rewrite NatSetFacts.inter_b;
  ...
  - (* 0 ∈ df, 0 ∉ d1 *)
    (* the interesting bit is reasoning about the overlap, this requires the extra assumptions about
       saturation of the sets and the boundedness of the context, which imply Hlx: *)
    specialize (saturated0_ty Hmem0ff H4) as Hlx.
    f_equal; try fnsetdec.
    (* case frf = false, requires saturation/Hlx above: *)
    destruct frf; simpl; auto. intuition. destr_bool; intuition.
    repeat rewrite empty_union_right.
    ...
    fnsetdec.
  - (* 0 ∉ df, 0 ∈ d1, analogous to the previous case *)
    specialize (saturated0_ty Hmem0f1 H3) as Hlx.
    f_equal; try fnsetdec.
    (* case fr1 = false, requires saturation/Hlx above: *)
    destruct fr1; simpl; auto. intuition. destr_bool; intuition.
    repeat rewrite empty_union_right.
...
Lemma subst1_saturated : forall {Γ Σ bx Tx dx df dx'},
  ...
  - eapply subst1_mem in H5; eauto.
    ...
    * eapply @indexr_subst1 with (dx:=(qset fresh {}N {}N ls)) in H6; eauto.
      simpl in H6. econstructor. eauto. apply subst_qual_subqual_monotone. auto.
      eapply closed_qual_subst1; eauto. lia.
    * eapply @indexr_subst1_ty with (dx:=(qset fresh {}N {}N ls)) in H6; eauto.
      simpl in H6. eapply sat_tvar. eauto. apply subst_qual_subqual_monotone. auto.
      eapply closed_qual_subst1; eauto. lia.
...
Proof. intros Σ T1 d1 T2 d2 t HVtp HStp. remember t as tt. remember [] as Γ.  induction HStp; subst.
  ...
  - inversion HVtp; subst. eapply vtp_loc; eauto. apply qstp_closed in H. intuition.
    ...
    apply qstp_empty in H1. apply qstp_empty in H0.
    replace q2 with q1; auto. apply eq_if_eqqual; apply subqual_antisymm; auto.
    apply qstp_empty in H1. apply qstp_empty in H0.
    replace q2 with q1; auto. eauto. apply eq_if_eqqual; apply subqual_antisymm; auto.
    apply qstp_empty in H1. apply qstp_empty in H0.
    replace q2 with q1; auto. eauto. apply eq_if_eqqual; apply subqual_antisymm; auto.
  - inversion HVtp; subst. econstructor. 5: eapply H10. all : eauto.
    ...
    assert (stp [] Σ (TFun d1 d2 T1 T2) d5 (TFun d3 d4 T3 T4) d6).
       ...
       replace ([bind_tm(false,T3, d3); bind_tm(true,TFun d1 d2 T1 T2, {♦})]) with ([bind_tm(false,T3, d3)] ++ bind_tm(true,TFun d1 d2 T1 T2, {♦}) :: []) in HStp2; auto.
       replace ([bind_tm(false,T3, d3); bind_tm(true,TFun d0 d7 T0 T5, {♦})]) with ([bind_tm(false,T3, d3)] ++ bind_tm(true,TFun d0 d7 T0 T5, {♦}) :: []); auto.
       clear H4.
       eapply @narrowing_stp_gen with (U := (TFun d1 d2 T1 T2))(du:={♦})(Γ2 := []: tenv)  in HStp2; eauto.
    - intuition.
Qed.
...
Lemma has_type_vtp: forall {Σ φ t T d},
  ...
  wf_senv Σ ->
  vtp Σ t T d.
Proof. intros Σ φ t T d HV Ht HWF. remember [] as Γ.  induction Ht; eauto; subst; try inversion HV; subst;
       ...
       + (* tloc *) intros.  eapply vtp_loc; eauto.
          * eapply closed_qual_qlub; auto. apply just_loc_closed. apply indexr_var_some' in H0. lia.
          * apply stp_refl; auto.
          * apply stp_refl; auto.
          * eapply wf_senv_saturated_qplus; eauto.
...
intros. induction H.
  ...
  + (* tloc *) apply t_sub with (T1:=TRef q T) (d1:=(q ⊔ &!l)); auto.
    eapply t_loc; eauto. all :  apply qstp_empty in H5.
    all : eapply subqual_trans. 2,4 : eapply H5. all : auto.
...
Lemma subst_qstp :  forall {Γ b Tf df df' Σ d1 d2},
  ...
  -  bdestruct (f =? 0).
    * pose (H' := H). subst. rewrite indexr_skips in H'; auto. simpl in H'. inversion H'. subst.
      rewrite subst1_qlub_dist. rewrite subst1_just_fv0. erewrite subst1_qual_id; eauto. inversion H4; subst.
...
Lemma subst_stp : forall{T1 T2},
  ...
  - (* TVarF x *) simpl.  (bdestruct (x =? 0)).
    ...
    * (*x is in Γ0*) assert (Hx: 1 <= x); try lia. destruct x; try lia.
      ...
      intuition.
      simpl. specialize (@subst_qstp _ _  _ _ df' _ _ _ H0); intuition.   eapply stp_refl; eauto. constructor.
      erewrite <-  indexr_insert_ge in  H.  replace (Γ0 ++  [])  with Γ0 in H.
      apply indexr_var_some' in  H. erewrite subst1_tenv_length. intuition. intuition.
      intuition.
  - (* TVarF x  trans *)
  ...
  - (* TAll *) simpl. inversion H. inversion H0. subst.  econstructor; eauto.
    ...
    rewrite mem_empty in H6. rewrite unsplice_set_empty in H6.
  ...
  - specialize (stp_closed HS1). intuition. specialize (stp_closed HS2). intuition.
    simpl. constructor. eapply subst_qstp; eauto.
    specialize (IHHS1 Tf df Γ0). intuition. erewrite subst1_qual_empty in H14. auto.
    specialize (IHHS2 Tf df Γ0). intuition. erewrite subst1_qual_empty in H14. auto.
 ...
 Proof.   intros T1 T2 Γ T d d' Σ d1 d2 HS.
  ...
  - (* TVarF x *) simpl.  (bdestruct (x =? 0)).
    ...
    * (*x is in Γ0*) assert (Hx: 1 <= x); try lia. destruct x; try lia.
      ...
      apply indexr_var_some' in  H. intuition. intuition.
      intuition.
      simpl. specialize (@subst_ty_qstp  _ _ _ d' _ _ _ H0); intuition.   eapply stp_refl; eauto. constructor.
      erewrite <-  indexr_insert_ge in  H.  replace (Γ0 ++  [])  with Γ0 in H.
      apply indexr_var_some' in  H. erewrite subst1_tenv_length. intuition. intuition.
      intuition.
  ...
  - intuition. specialize (stp_closed HS1). intuition. specialize (stp_closed HS2). intuition.
    ...
    specialize (IHHS1 T Γ0). intuition. erewrite subst1_qual_empty in H14. auto.
    specialize (IHHS2 T Γ0). intuition. erewrite subst1_qual_empty in H14. auto.
    all : eapply subst_ty_qstp; eauto.
  - simpl. constructor. inversion H. subst. 2 : inversion H0. subst.
    1,2: constructor; try eapply closed_ty_subst1'; eauto; eapply closed_qual_subst1'; eauto.
    eapply subst_ty_qstp; eauto.  eapply IHHS1; eauto.
    unfold open_ty' in *. unfold open_ty in *.
...
Lemma substitution_gen :
  ...
  - (* ttabs *) simpl. eapply t_tabs; auto. eapply closed_tm_subst1'; eauto.
    ...
    Unshelve. auto.  eapply qglb_mono_lr; eauto.
  - (* t_tapp *) intuition. rename H10 into IHHT. simpl.
    ...
    1-2: rewrite length_app in *; simpl in *; rewrite subst1_tenv_length.
...
Proof.
Import NatSet.F.
    intros t Γ φ Σ Tx dx dx' T d Hsep Tx_Hcl Qx_Hcl Qx'_Hcl Hsenv_satu HT HSubst. specialize (has_type_closed HT) as Hcl. intuition.
  ...
  - (* ttabs *)  simpl. inversion H0. inversion H1. subst. apply has_type_closed in HT as Hcl_HT.
     ...
     assert (dx' ⊓ (df ⊔ $! (S (‖ Γ0 ‖)) ⊔ $! (S (S (‖ Γ0 ‖))) ⊔ {♦}) ⊑ dx). {
      repeat rewrite qglb_qlub_dist_l. erewrite qglb_disjoint_freshv; eauto. erewrite qglb_disjoint_freshv; eauto.
      erewrite qglb_disjoint_fresh; eauto. repeat rewrite qlub_empty_left. apply Substq_non_fresh in HSubst.
      rewrite HSubst. rewrite qlub_empty_right. eapply (subqual_trans _ Hsep).
     }
     intuition.
     ...
     repeat erewrite subst1_qlub_dist in  H15. erewrite length_app in H15.
     ...
     intuition.
     replace ({0 |-> Tx ~ dx' }ᴳ (bind_ty (false, T1, d1) :: bind_tm (true, TAll d1 d2 T1 T2, df) :: Γ0))
       with  (bind_ty (false, {0 |-> Tx ~ dx'}ᵀ T1, {0 |-> dx' }ᵈ d1)
             :: bind_tm (true, TAll ({0 |-> dx' }ᵈ d1) ({0 |-> dx' }ᵈ d2)
                ({0 |-> Tx ~ dx' }ᵀ T1) ({0 |-> Tx ~ dx' }ᵀ T2),
             {0 |-> dx' }ᵈ df) :: {0 |-> Tx ~ dx' }ᴳ Γ0) in H15.
     replace ({0 |-> dx' }ᵈ df ⊔ {0 |-> dx' }ᵈ $! (S (‖ Γ0 ‖))
           ⊔ {0 |-> dx' }ᵈ $! (S (S (‖ Γ0 ‖))) ⊔ {0 |-> dx' }ᵈ {♦})
       with   ({0 |-> dx' }ᵈ df ⊔ $! (‖ Γ0 ‖) ⊔ $! (S (‖ Γ0 ‖)) ⊔ {♦}) in H15. auto.
     erewrite subst1_fresh_id. f_equal. erewrite <- subst1_just_fv; eauto. f_equal.
     erewrite <- subst1_just_fv; eauto.
     auto. 1,2: apply H14. 1-2: simpl; lia. simpl.  erewrite length_app. simpl. lia.
  ...
  - (*  t_tapp *) simpl in *.  apply has_type_closed in HT as Hcl. intuition.
    ...
    eapply t_tapp with (Γ :=  ({ 0 |-> Tx ~ dx' }ᴳ Γ0))(φ := ({ 0 |-> dx' }ᵈ φ))(Σ := Σ)
                      (t := ({ 0 |-> tunit }ᵗ t0))(d2 := { 0 |-> dx' }ᵈ d2)
                      (T1 := ({ 0 |-> Tx ~ dx' }ᵀ  T1)) (T2 := ({ 0 |-> Tx ~ dx' }ᵀ  T2))
                      (d1 := { 0 |-> dx' }ᵈ d1) in H16; eauto.
    inversion H12. subst. erewrite length_app in *. intuition.
    ...
    1-2: rewrite subst1_tenv_length; rewrite length_app in *; simpl in *;
    replace  (‖ Γ0 ‖ + 1) with (S (‖Γ0‖)) in *; try lia.
    eapply closed_ty_subst1; eauto. eapply closed_qual_subst1; eauto.
  ...
  - (* tunit *) assert ({ 0 |-> dx' }ᵈ (∅) = (∅)). { erewrite subst1_qual_empty; eauto. } rewrite H4. constructor.
    erewrite subst1_tenv_length. eapply closed_qual_subst1; eauto. erewrite length_app in *. simpl in H.
  ...
  - (* $(x) *)  (bdestruct (x =? 0)).
    ...
    eapply closed_qual_subst1'; eauto. simpl. eapply closed_ty_subst1; eauto.
    simpl. eapply closed_qual_subst1; eauto.
    erewrite remove_singleton_inv; auto.

  - (* tabs *)
    assert( has_type (Γ0 ++ [bind_ty (false, Tx, dx)]) φ Σ (tabs t0) (TFun d1 d2 T1 T2) df). { econstructor; eauto. }
    apply has_type_closed in HT as Hcl. intuition. inversion H0. subst. rewrite app_comm_cons in *. rewrite length_app in  *.
    replace (‖ Γ0 ‖ + ‖ [bind_ty (false, Tx, dx)] ‖) with (S (‖ Γ0 ‖)) in *.
    ...
    rewrite subst1_tenv_length. (* simpl in *. *)
    (* replace (length Γ0 + 1) with (S (length Γ0)) in H17; try lia. *)
   ...
   simpl. lia.
   erewrite subst1_qlub_dist; eauto. f_equal.
   rewrite subst1_qlub_dist.  repeat rewrite subst1_qlub_dist. f_equal.
   repeat rewrite <- subst1_just_fv. auto. rewrite subst1_fresh_id. repeat rewrite <- subst1_just_fv. auto.
   simpl. auto.
   simpl; erewrite length_app; eauto; simpl; lia.
   erewrite length_app; eauto. simpl. lia.
   erewrite length_app; eauto. simpl. lia.
   simpl. lia.
  - (* t_app *) intuition. rename H9 into IHHT1. rename H8 into IHHT2. simpl.
    ...
    1-6 : apply has_type_closed in HT1; apply has_type_closed in HT2 ;intuition; inversion H1; subst; rewrite length_app in *; simpl in *; auto.
    1,4 : unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx');
        erewrite <- subst1_open_qual_comm; eauto; erewrite <- subst1_open_qual_comm; eauto.
    * apply subst_qual_subqual_monotone. unfold openq in H4. auto.
    * apply subst1_senv_saturated; auto.
  ...
  - (* t_app_fresh *) intuition. rename H15 into IHHT1. rename H14 into IHHT2. simpl.
    ...
    1-3,7-9 : apply has_type_closed in HT1; apply has_type_closed in HT2 ;intuition; inversion H1; subst; rewrite length_app in *; simpl in *; auto.
    simpl. rewrite subst1_qlub_dist. rewrite Hoverlap. rewrite subst1_fresh_id. auto.
    5 : unfold openq; rewrite <- @subst1_qual_empty with (dx:=dx');
        erewrite <- subst1_open_qual_comm; eauto; erewrite <- subst1_open_qual_comm; eauto.
    1,2,5,6,7 : apply subst_qual_subqual_monotone; auto.
  ...
  - (* t_loc *) erewrite length_app in *; eauto. simpl in H0, H1, H3, H, H6.
   ...
   eapply subst_qual_subqual_monotone; eauto.
   erewrite subst1_qlub_dist. repeat erewrite subst1_qual_id; eauto. eapply just_loc_closed; eauto.
   intuition.
   erewrite subst1_qual_id; eauto. eapply just_loc_closed; eauto. auto.
  - rewrite subst1_qlub_dist. rewrite subst1_qual_fresh. simpl. apply t_ref; auto.
    eapply IHHT; eauto. rewrite length_app in *; simpl in *; auto. inversion H1; auto.
    eapply closed_ty_subst1'; eauto.
    erewrite <- subst1_qual_fresh.
    eapply subst_qual_subqual_monotone; eauto. apply subst1_non_fresh; eauto.
  - (* t_deref  *)  intuition. inversion H1. subst. intuition. apply has_type_closed in HT as Hcl. intuition.
   specialize (H6 Γ0). intuition.
   replace ({0 |-> tunit }ᵗ ! (t0)) with (! ({0 |-> tunit }ᵗ (t0))).
 ...
 - (* t_sub *) intuition. apply has_type_closed in HT as Hcl. intuition.  simpl in *. erewrite length_app in *. simpl in *.
...
Lemma substitution_stp2_ty : forall{T1 T2},
    forall {Tx bf Tf df df' dx dx' Σ d1 d2},
      ...
      closed_qual 0 0 (‖Σ‖) df' -> closed_qual 0 0 (‖Σ‖) dx' -> ♦∉ df -> ♦∉ dx -> df ⊑ df' -> dx ⊑ dx' ->
      stp [] Σ ({ 0 |-> Tf ~ df; Tx ~ dx }ᵀ T1) ({ 0 |-> df ; dx }ᵈ d1) ({ 0 |-> Tf ~ df ; Tx ~ dx }ᵀ T2) ({ 0 |-> df ; dx }ᵈ d2).
Proof.  intros.  assert (Hcl : closed_qual 0 0 (‖ Σ ‖) (df' ⋒ dx')). { apply closed_qual_qlub; auto. apply closed_qual_qglb; auto. }
  ...
  assert (H' : stp [bind_ty(false, Tx, df ⋒ dx)] Σ ({0 |-> Tf ~ df }ᵀ T1) ({0 |-> df }ᵈ d1) ({0 |-> Tf ~ df }ᵀ T2) ({0 |-> df }ᵈ d2)). {
    eapply narrowing_stp_ty; eauto. apply stp_refl; auto.
  }
...
Proof. intros. induction H.
  + apply qstp_closed in H4 as Hcl_qstp.  intuition. apply qstp_empty in H4 as H4'.  eapply vtp_tabs; eauto.
    econstructor; eauto.  destruct df1. simpl in H6. subst. destruct df2. qdec.
  ...
  + apply qstp_closed in H5 as Hcl_qstp.  intuition. inversion H11.  subst. eapply vtp_abs; eauto.
    apply qstp_empty in H5. subst. econstructor; eauto.  destruct df1. simpl in H7. subst. qdec.
    apply senv_saturated_non_fresh. auto.
...
Lemma open_qual_not_free : forall {k q}, [[k ~> ∅ ]]ᵈ q = q -> forall {q'}, [[k ~> q' ]]ᵈ q = q.
  intros. destruct q. destruct q'. simpl in *. destruct (mem k t1) eqn:Hmem; auto.
...
Proof.
  ...
  + (* t_tapp *) right. subst. intuition.
     ...
     specialize (H13 (TAll d1 d2 T1 T2) t0). intuition.
     ...
     - (* contraction *)
       ...
       exists (t1 <~ᵗ (ttabs t1); tunit). exists σ. intuition.
       * constructor.
       ...
       * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
         ...
         assert (HT' : has_type [bind_ty (false, T1, d1) ; bind_tm (true, TAll d0 d3 T0 T3, df1)] (df1 ⊔ $!0 ⊔ $!1 ⊔ {♦}) Σ (open_tm' ([]:tenv) t1) (open_ty' ([]:tenv) T3) (openq' ([]:tenv) d3)). {
         ...
         fold (open_tm (ttabs t1) tunit t1) in HT''. fold (openq df1 d1 d3) in HT''. fold (open_ty (TAll d0 d3 T0 T3) df1 T1 d1 T3) in HT''.
         ...
         repeat apply qlub_bound; auto. apply has_type_filter in H. apply qstp_empty in H27.
         eapply subqual_trans; eauto.

     -  (* left congruence *)
        specialize (H17 σ H11). destruct H17 as [t0' [σ' HH7]]. exists (ttapp t0'). exists σ'. intuition. constructor. intuition.
        destruct H17. destruct (mem 0 (qbvs d2)) eqn:Hmem.
        * (* d2 is dependent on f, so the growth in df might be observable  *)
          ...
          -- destruct (♦∈? df) eqn:Hfresh.
             ** erewrite @openq_duplicate_eq_l with (f:=0) (l:=‖Σ'‖). 3,4 : eapply closed_qual_monotone; eauto. 2,3 : eauto.
                ...
                --- apply t_tapp; auto. 1,2 : apply has_type_closed in H22; intuition; inversion H24; auto.
                    1,2 : eapply subqual_trans; eauto. 1,2 : eapply weaken_store_senv_saturated; eauto.
             ...
             ** rewrite not_fresh_qqplus in H22; auto. apply t_sub with (T1:=(T2 <~ᵀ TUnit ~ ∅; T1 ~ d1)) (d1:=d2 <~ᵈ df; d1).
                ...
                --- apply qqplus_bound. apply has_type_filter in H. eapply openq_subqual; eauto.
  ...
  + (* t_app_fresh *) right. subst. intuition.
     apply has_type_closed in H as HH. intuition.
     specialize (H18 (TAll (df' ⋒ d1') d2 T1 T2) t0). intuition.
     ...
     - (* contraction *)
       ...
       * apply (Preserve Σ ∅); auto. rewrite qqplus_qbot_right_neutral.
         ...
         remember (qset false (qfvs d1) (qbvs d1) (qlocs d1)) as d1''.
         assert (Hd1'' : d1'' ⊑ d1'). { subst. eapply subqual_trans; eauto. }
         assert (Hdf1 : df1 ⊑ df'). { apply qstp_empty in H32. eapply subqual_trans; eauto. }
         assert (Hd1''fr : ♦∉ d1''). { subst. auto. }

         assert (HT' : has_type [bind_ty (false, T1, df' ⋒ d1') ; bind_tm (true, TAll d0 d3 T0 T3, df1)] (df1 ⊔ $!0 ⊔ $!1 ⊔ {♦}) Σ (open_tm' ([]:tenv) t1) (open_ty' ([]:tenv) T3) (openq' ([]:tenv) d3)). {
         ...
         erewrite <- open_subst2_ty in HT''; eauto.
         erewrite <- open_subst2_qual in HT''; eauto.
         fold (open_tm (ttabs t1) tunit t1) in HT''. fold (openq df1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) d3) in HT''.
         apply @weaken_flt with (φ':= (ldom Σ)) in HT''; auto; intuition.
         eapply t_sub; eauto.

         inversion H28. subst. rename H35 into Hsub.
         eapply @substitution_stp2_ty with (dx := (qset false (qfvs d1) (qbvs d1) (qlocs d1))) (df:=df1) in Hsub; eauto.
         ...
         fold (open_ty (TAll d0 d3 T0 T3) df1 T1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) T3) in Hsub. fold (open_ty (TAll d0 d3 T0 T3) df1 T1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) T2) in Hsub.
         fold (open_ty (TAll d0 d3 T0 T3) df1 T1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) T3).
         ...
         ++ (* d1 fresh, so the function can't be dependent on the argument *)
            ...
            replace (T2 <~ᵀ (TAll d0 d3 T0 T3) ~ df1; T1 ~ (qset false (qfvs d1) (qbvs d1) (qlocs d1))) with T2 in Hsub. (* since no dependence *)
            eapply s_trans; eauto. apply stp_refl; auto. apply not_free_prop3; auto. apply not_free_prop3; auto.
            constructor; auto. eapply openq_mono; eauto. apply qstp_empty in H32. auto.
         ...
         eapply openq_subqual; eauto. apply has_type_filter in H. auto. eapply subqual_trans; eauto.
         eapply senv_saturated_openq; eauto.
         repeat apply qlub_bound; auto. apply has_type_filter in H.
         eapply subqual_trans; eauto. eapply subqual_trans. eauto. auto.
     ...
     -  (* left congruence *)
        ...
        * (* df fresh *) destruct (mem 0 (qbvs d2)) eqn:Hmem.
          -- (* d2 dependent on f *) apply (Preserve Σ' d'); auto.
            ...
            ** apply t_tapp_fresh with (T1 := T1) (df':=df' ⋓ d') (d1' :=d1'); auto. erewrite <- @qqcap_fresh_l with (Σ':=Σ'); eauto.
               apply has_type_closed in H31; intuition; inversion H33; subst; auto. eapply closed_qual_monotone; eauto.
            ...
            ** apply has_type_filter in H. apply qqplus_bound. eapply openq_subqual; eauto.
               apply qqplus_bound. 1, 3, 4 : eapply subqual_trans; eauto. 1,2 : eapply subqual_trans; eauto. all : eauto.
          ...
          -- (* d2 not dependent on f *) apply (Preserve Σ' ∅); auto. rewrite qqplus_qbot_right_neutral.
             ...
             1,2,4 : eapply weaken_store_saturated; eauto. eapply weaken_store_senv_saturated; eauto.
   ...
   + (* tapp *) right. subst. intuition.
     ...
     - (* contraction *)
       ...
       * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
         ...
         assert (HT' : has_type [bind_tm(false, T1, d1) ; bind_tm(true, TFun d0 d3 T0 T3, df1)] (df1 ⊔ $!0 ⊔ $!1 ⊔ {♦}) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T3) (openq' ([]:tenv) d3)). {
         ...
         erewrite <- open_subst2_ty in HT''; eauto.
         erewrite <- open_subst2_qual in HT''; eauto.
         fold (open_tm (tabs t0) t2 t0) in HT''. fold (openq df1 d1 d3) in HT''. fold (open_ty (TFun d0 d3 T0 T3) df1 T1 d1 T3) in HT''.
         apply @weaken_flt with (φ':= (ldom Σ)) in HT''; auto; intuition.
         eapply t_sub; eauto.
    ...
    + (* t_app_fresh *) right. subst. intuition.
     ...
     - (* contraction *)
       ...
       * apply (Preserve Σ ∅); auto. rewrite qqplus_qbot_right_neutral.
         ...
         assert (HT' : has_type [bind_tm(false, T1, df' ⋒ d1') ; bind_tm(true, TFun d0 d3 T0 T3, df1)] (df1 ⊔ $!0 ⊔ $!1 ⊔ {♦}) Σ (open_tm' ([]:tenv) t0) (open_ty' ([]:tenv) T3) (openq' ([]:tenv) d3)). {
         ...
         erewrite <- open_subst2_ty in HT''; eauto.
         erewrite <- open_subst2_qual in HT''; eauto.
         fold (open_tm (tabs t0) t2 t0) in HT''. fold (openq df1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) d3) in HT''.
         apply @weaken_flt with (φ':= (ldom Σ)) in HT''; auto; intuition.
         eapply t_sub; eauto.
         ...
         erewrite <- open_subst2_qual in Hsub; eauto. erewrite <- open_subst2_qual in Hsub; eauto.
         fold (openq df1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) d3) in Hsub. fold (openq df1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) d2) in Hsub.
         fold (open_ty (TFun d0 d3 T0 T3) df1 T1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) T3) in Hsub. fold (open_ty (TFun d0 d3 T0 T3) df1 T1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) T2) in Hsub.
         fold (open_ty (TFun d0 d3 T0 T3) df1 T1 (qset false (qfvs d1) (qbvs d1) (qlocs d1)) T3).
         (* need to reason about growth of d1 *)
         { destruct (♦∈? d1) eqn:Hfresh.
         ++ (* d1 fresh, so the function can't be dependent on the argument *)
            intuition. replace (T2 <~ᵀ TUnit ~ ∅; T1 ~ d1) with T2. replace (T2 <~ᵀ (TFun d0 d3 T0 T3) ~ df1; T1 ~ (qset false (qfvs d1) (qbvs d1) (qlocs d1))) with T2 in Hsub. (* since no dependence *)
            eapply s_trans; eauto. apply stp_refl; auto. apply not_free_prop3; auto. apply not_free_prop3; auto.
    ...
    + (*tref*) subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H8 T t0). intuition.
      * (*contraction*) right. intros.
        exists (tloc (‖σ‖)). exists (t0 :: σ). intuition.
    ...
    + (*tderef*) subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H8 (TRef d1 T0) t0). intuition.
    ...
    + (*t_sub*) subst. intuition. specialize (stp_closed H0) as H00. intuition.
      specialize (H8 T1 t0). intuition. right.
