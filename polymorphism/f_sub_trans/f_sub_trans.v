(****************************************
*  This version
   Attempts to merge f_sub_trans with _fr2:
   - update Ctx_OK definition
   - store type: list (option value)
   - closedness condition for referent type
   - s_ref qualifier invariance
 ****************************************)
...
(*   - apply has_type_closed in H. intuition. eapply senv_saturated_openq; eauto. *)
(*   - apply has_type_closed in H, H1. intuition. eapply senv_saturated_openq; eauto. *)
(*   - apply has_type_closed in H, H1. intuition. eapply senv_saturated_openq; eauto. *)
...
(* split. intuition. eapply closed_qenv_monotone; eauto. *) 
(* split. intuition. eapply closed_qenv_qn_extend; eauto. apply has_type_closed in H0. intuition. Qcrush. *)
(* intros. *)
(* bdestruct (l =? (‖ Σ ‖)). *) 
(* - subst. destruct H as [H [Hqenv [Hqenvqn ?]]]. rewrite H in H3. rewrite indexr_head in H2. inversion H2. rewrite indexr_head in H3. inversion H3. subst. intuition. eapply weaken_store; eauto. *) 
(* - destruct H as [H [Hqenv [Hqenvqn Hctx]]]. rewrite indexr_skip in H2; auto. rewrite H in H4. rewrite indexr_skip in H3; auto. specialize (Hctx l v0 T0 q0 H2 H3). intuition. eapply weaken_store; eauto. *)
(* Qed. *)
...
(* Lemma CtxOK_empty : forall {Γ φ}, closed_qenv 0 (‖ Γ ‖) 0 Γ -> CtxOK Γ φ [] []. *)
(*   intros. constructor; intuition; eauto; simpl in H; try discriminate. *)
(* Qed. *)
...
(*   intros. inversion H. intuition. constructor; intuition. *)
(* Qcrush. *) 
(*   all : specialize (H6 _ _ _ _ H5 H7); intuition. *)
...
Proof.
  intros Σ φ T1 d1 T2 d2 t HVtp HStp. remember t as tt. remember [] as Γ.  induction HStp; subst.
  - (* TTop *) apply qstp_closed in H0 as Hcl_qstp. intuition.
    ...
    + (* TAll  *) apply qstp_closed in H9 as Hcl_qstp. intuition.
      ...
      assert (vtp Σ φ (ttabs t0)  (TAll d4 d5 T3 T4) df1). {  econstructor; eauto.  }
      assert (vtp Σ φ (ttabs t0)  (TAll d4 d5 T3 T4) d2). {  econstructor; eauto. }
      eapply vtp_top; eauto.
    + (* TUnit  *) assert  (vtp Σ φ tunit TUnit d2).  { econstructor; eauto. }
                   eapply vtp_top; auto.
    + (* Tloc *) inversion H. subst. econstructor; eauto; intuition.
    + assert  (vtp Σ φ (tabs t0) (TFun  d4 d5 T3 T4) d2). {  econstructor;  eauto. }
      eapply vtp_top; eauto.
    + assert (vtp Σ φ t T0 d2).  { eapply vtp_qual_ex; eauto. }
      eapply vtp_top; eauto.
  ...
  - inversion HVtp; subst. econstructor. 5: eapply H10. all : eauto.
    ...
    assert (narrow: stp [(bind_tm, false,T3, d3); (bind_tm, true,TFun d0 d7 T0 T5, {♦})] Σ (open_ty' ([]: tenv) T5)(openq' ([]:tenv) d7) (open_ty' ([]:tenv) T2) (openq' ([]: tenv) d2)). {
      eapply narrowing_stp; eauto. intros. inversion H15. apply weaken_stp. auto.
    }
    (* Set Printing All. *)
    (* apply trans *)
    simpl in *. eapply s_trans; eauto.
...
(* Goal forall (Γ : tenv) (Σ : senv) bd bx Tx dx d dx', ({0 |-> dx' }ᵈ (q_trans (Γ ++ [(bd, bx, Tx, dx)]) Σ d)) = *) 
(*   (q_trans ({0 |-> Tx ~ dx' }ᴳ Γ) Σ ({0 |-> dx' }. *)
(* unfold q_trans,q_trans_senv. intros. erewrite q_trans''_subst1_senv_comm. *)
...
Lemma substitution_gen :
  ...
  - (* t_tabs *) simpl. apply t_tabs; auto. eapply closed_tm_subst1'; eauto.
    ...
    eapply closed_qual_subst1'; eauto. apply subst_qual_subqual_monotone; auto. apply subst_qual_subqual_monotone; auto. eauto.
    ...
    eapply closed_qenv_qn_shrink in Hqn; eauto.
    Qcrush. 
    replace (S (S (‖ Γ0 ‖))) with (‖ (bind_tm, true, TAll d1 d2 T1 T2, df) :: Γ0 ++ [(bd, bx, Tx, dx)] ‖). rewrite q_trans_fv. rewrite qand_qor_dist_l. erewrite qglb_disjoint_freshv; eauto. rewrite qor_empty_right. rewrite q_trans_extend_tenv_closed_id'. eapply @Subq_trans with (d2:=(q_trans (Γ0 ++ [(bd, bx, Tx, dx)]) Σ dx' ⊓ q_trans (Γ0 ++ [(bd, bx, Tx, dx)]) Σ φ)); eauto. apply Subq_qand_split; auto.  
    rewrite q_trans_extend_tenv_closed_id'; auto. 
    eapply closed_qenv_qn_shrink in Hqn; eauto. 
    eapply closed_Nats_mono with (n:=0). Qcrush. lia.
    rewrite q_trans_extend_tenv_closed_id'. apply q_trans_subq. eapply Subq_trans; eauto.
    eapply closed_qenv_qn_shrink in Hqn; eauto.
...
inversion H0. subst. erewrite subst1_ty_id; eauto. inversion HSubst; subst.
  ...
  - (* t_abs *) simpl. apply t_abs; auto. eapply closed_tm_subst1'; eauto.
    ...
    replace (‖Γ0‖ + 1) with (S (‖Γ0‖)) in IHHT; try lia.
    erewrite <- open_subst1_tm_comm in IHHT; eauto. erewrite <- open_subst1_tm_comm in IHHT; eauto.
erewrite <- open_subst1_ty_comm in IHHT; eauto. erewrite <- open_subst1_ty_comm in IHHT; eauto. erewrite <- open_subst1_qual_comm in IHHT; eauto. erewrite <- open_subst1_qual_comm in IHHT; eauto. repeat erewrite subst1_qor_dist in IHHT. apply IHHT; auto. 
    apply has_type_filter in HT as Hft.
    subst.
    inversion H3. subst. rewrite app_length in *. simpl in *. rewrite Nat.add_1_r in *. 
    ...
    eapply closed_qenv_qn_shrink in Hqn; eauto.
    Qcrush. 
    replace (S (S (‖ Γ0 ‖))) with (‖ (bind_tm, true, TFun d1 d2 T1 T2, df) :: Γ0 ++ [(bd, bx, Tx, dx)] ‖). rewrite q_trans_fv. rewrite qand_qor_dist_l. erewrite qglb_disjoint_freshv; eauto. rewrite qor_empty_right. rewrite q_trans_extend_tenv_closed_id'. eapply @Subq_trans with (d2:=(q_trans (Γ0 ++ [(bd, bx, Tx, dx)]) Σ dx' ⊓ q_trans (Γ0 ++ [(bd, bx, Tx, dx)]) Σ φ)); eauto. apply Subq_qand_split; auto.  
    rewrite q_trans_extend_tenv_closed_id'; auto. 
    eapply closed_qenv_qn_shrink in Hqn; eauto. 
    eapply closed_Nats_mono with (n:=0). Qcrush. lia.
    rewrite q_trans_extend_tenv_closed_id'. apply q_trans_subq. eapply Subq_trans; eauto.
...
Lemma substitution_ty_gen :
  ...
  - (* t_tabs *) simpl. apply t_tabs; auto. eapply closed_tm_subst1'; eauto.
    ...
    repeat rewrite <- q_trans_or_dist. repeat rewrite qand_qor_dist_l. assert (Hqn: closed_qenv_qn (S (‖ Γ0 ‖)) ((bind_ty, false, T1, d1) :: (bind_tm, true, TAll d1 d2 T1 T2, df) :: Γ0 ++ [(bind_ty, bx, Tx, dx)])). {
      unfold closed_qenv_qn. intros. rename H8 into HHH.
    bdestruct (x =? (S (S (‖ Γ0 ‖)))). replace x with (‖ (bind_tm, true, TAll d1 d2 T1 T2, df) :: Γ0 ++ [(bind_ty, bx, Tx, dx)] ‖) in *. rewrite indexr_head in HHH. inversion HHH. subst. simpl. Qcrush. subst. simpl. rewrite app_length. simpl. lia. 
    bdestruct (x =? (S (‖ Γ0 ‖))). replace x with (‖ Γ0 ++ [(bind_ty, bx, Tx, dx)] ‖) in *. rewrite indexr_skip in HHH; auto. rewrite indexr_head in HHH. inversion HHH. subst. simpl. Qcrush. subst. simpl. rewrite app_length. simpl. lia.
    rewrite indexr_skip in HHH; auto. rewrite indexr_skip in HHH; auto. destruct a as [ [ [ b fr ] T' ] q']. eapply wf_tenv_prop in HwfΓ; eauto. simpl. apply indexr_var_some' in HHH. rewrite app_length in *. simpl in *. rewrite Nat.add_1_r in *. eapply closed_Nats_mono; eauto. Qcrush. 1,2: subst; simpl; rewrite app_length; simpl; lia.
    }
    ...
    eapply closed_qenv_qn_shrink in Hqn; eauto.
    Qcrush. 
    replace (S (S (‖ Γ0 ‖))) with (‖ (bind_tm, true, TAll d1 d2 T1 T2, df) :: Γ0 ++ [(bind_ty, bx, Tx, dx)] ‖). rewrite q_trans_fv. rewrite qand_qor_dist_l. erewrite qglb_disjoint_freshv; eauto. rewrite qor_empty_right. rewrite q_trans_extend_tenv_closed_id'. eapply @Subq_trans with (d2:=(q_trans (Γ0 ++ [(bind_ty, bx, Tx, dx)]) Σ dx' ⊓ q_trans (Γ0 ++ [(bind_ty, bx, Tx, dx)]) Σ φ)); eauto. apply Subq_qand_split; auto.  
    rewrite q_trans_extend_tenv_closed_id'; auto. 
    eapply closed_qenv_qn_shrink in Hqn; eauto. 
    eapply closed_Nats_mono with (n:=0). Qcrush. lia.
    rewrite q_trans_extend_tenv_closed_id'. apply q_trans_subq. eapply Subq_trans; eauto.
...
apply closed_Qual_q_trans''_monotone. eapply closed_qual_subst1; eauto. eapply closed_Qual_q_trans''_monotone; eauto. replace (S (‖ Γ0 ‖)) with (‖ Γ0 ++ [(bind_ty, bx, Tx, dx)] ‖). apply wf_tenv_closed_qenv; auto. rewrite app_length. simpl. lia. apply closed_qenv_subst1. eapply closed_qenv_monotone. apply wf_senv_closed_qenv; auto. 1-3: lia. eapply closed_Qual_monotone; eauto. lia.
  ...
  - (* t_abs *) simpl. apply t_abs; auto. eapply closed_tm_subst1'; eauto.
    ...
    repeat rewrite <- q_trans_or_dist. repeat rewrite qand_qor_dist_l. assert (Hqn: closed_qenv_qn (S (‖ Γ0 ‖)) ((bind_tm, false, T1, d1) :: (bind_tm, true, TFun d1 d2 T1 T2, df) :: Γ0 ++ [(bind_ty, bx, Tx, dx)])). {
      unfold closed_qenv_qn. intros. 
    ...
    eapply closed_qenv_qn_shrink in Hqn; eauto.
    Qcrush. 
    replace (S (S (‖ Γ0 ‖))) with (‖ (bind_tm, true, TFun d1 d2 T1 T2, df) :: Γ0 ++ [(bind_ty, bx, Tx, dx)] ‖). rewrite q_trans_fv. rewrite qand_qor_dist_l. erewrite qglb_disjoint_freshv; eauto. rewrite qor_empty_right. rewrite q_trans_extend_tenv_closed_id'. eapply @Subq_trans with (d2:=(q_trans (Γ0 ++ [(bind_ty, bx, Tx, dx)]) Σ dx' ⊓ q_trans (Γ0 ++ [(bind_ty, bx, Tx, dx)]) Σ φ)); eauto. apply Subq_qand_split; auto.  
    rewrite q_trans_extend_tenv_closed_id'; auto. 
    eapply closed_qenv_qn_shrink in Hqn; eauto. 
    eapply closed_Nats_mono with (n:=0). Qcrush. lia.
    rewrite q_trans_extend_tenv_closed_id'. apply q_trans_subq. eapply Subq_trans; eauto.
...
Proof.
  ...
  induction H; try (left; constructor); intros.
   ...
   + (* ttapp *) right. subst. intuition.
     ...
     -  (* left congruence *)
        specialize (H17 σ H11). destruct H17 as [t0' [σ' HH5]]. exists (ttapp t0'). exists σ'. intuition. constructor. intuition.
        destruct H17.
        ndestruct ((qbvs d2) 0).
        * (* d2 is dependent on f, so the growth in df might be observable  *)
          ...
          -- inversion H15. subst. destruct (♦∈? df) eqn:Hfresh.
             ** erewrite @openq_duplicate_eq_l with (f:=0) (l:=‖Σ'‖). 3,4 : eapply closed_Qual_monotone; eauto. 2,3 : eauto.
                ...
                --- apply extends_qdom in H17. apply Qqplus_bound.
                    eapply openq_subqual; eauto using closed_Qual_qor. unfold qqplus. rewrite Hfresh. apply Qor_bound. apply closed_Qual_subq_qdom in H18. eapply Subq_trans; eauto. eapply Subq_trans; eauto. apply disjointq_Qdom in H21. eapply @Subq_trans with (d2:=qdom Σ'); eauto. eapply Subq_trans; eauto. Qcrush. eapply Subq_trans; eauto. apply disjointq_Qdom in H21. eapply @Subq_trans with (d2:=qdom Σ'); eauto.
             ...
             ** rewrite not_fresh_qqplus in H22; auto. apply t_sub with (T1:=(T2 <~ᵀ TUnit ~ ∅; T1 ~ d1)) (d1:=d2 <~ᵈ df; d1).
                ...
                --- apply Qqplus_bound. apply has_type_filter in H. eapply openq_subqual. eapply closed_Qual_qor; eauto. 
                    eapply Subq_trans; eauto.
                    eapply Subq_trans; eauto. eapply @Subq_trans with (d2:=qdom Σ'); eauto. 
                    eapply Subq_trans; eauto.
                    eapply @Subq_trans with (d2:=qdom Σ'); eauto.
        ...
        * (* d2 is not dependent on f, so we don't observe the growth in df *)
          inversion H15. subst. apply (Preserve Σ' ∅); auto. rewrite qqplus_qbot_right_neutral.
  ...
  + (* t_tapp_fresh *) right. subst. intuition.
     ...
     - (* contraction *)
       ...
       * apply (Preserve Σ ∅); auto. rewrite qqplus_qbot_right_neutral.
         ...
         eapply @Subq_trans with (d2:=qdom Σ); eauto. eapply closed_Qual_subq_qdom; eauto.
         eapply senv_saturated_openq; eauto.
         apply Qor_bound; auto. eapply Subq_trans; eauto. eapply closed_Qual_subq_qdom; eauto. 
         clear - H1 H2. Qcrush; eauto.
         1-3: subst; apply senv_saturated_non_fresh; eauto.
         subst. Qcrush.
     -  (* left congruence *)
        apply has_type_closed in H as Hcl. intuition.
        specialize (H18 σ H12). destruct H18 as [t1' [σ' HH1]]. exists (ttapp t1'). exists σ'. intuition. apply step_c_tapp. intuition.
        inversion H16. subst. 
        destruct H22. destruct (♦∈? df) eqn:Hfresh.
        * (* df fresh *)
          ndestruct (qbvs d2 0).
          ...
          eapply weaken_store_senv_saturated; eauto.
          eauto using Subq_trans.
          apply Qor_bound; auto. apply has_type_closed in H27. intuition. eapply @Subq_trans with (d2:=q_trans [] Σ' d1). Qcrush. unfold q_trans,q_trans_tenv,q_trans_senv. simpl. erewrite q_trans''_extend_closed_id; eauto. rewrite q_trans''_fuel_max; auto. eapply @Subq_trans with (d2:=qdom Σ'); eauto. apply closed_Qual_subq_qdom. apply closed_Qual_q_trans''_monotone; auto. eapply closed_Qual_monotone; eauto. eapply closed_qenv_monotone. apply wf_senv_closed_qenv; auto. 1,2: auto. apply extends_length; auto. Qcrush.
          eapply weaken_store_senv_saturated. eauto. eapply closed_Qual_open2; eauto. eauto.
            ** apply has_type_closed in H27. intuition. inversion H20. subst.
               apply stp_refl. simpl. eapply closed_ty_monotone; eauto.
               constructor; auto. apply closed_Qual_qqplus; auto. apply openQ_closed. eauto using closed_Qual_monotone. apply closed_Qual_qqplus; eauto. 1,2: eapply closed_Qual_monotone; eauto.
            ** apply has_type_filter in H. apply Qqplus_bound. eapply openq_subqual. apply closed_Qual_qor; eauto.
               apply Qqplus_bound. 1,3,4: eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. all: eapply @Subq_trans with (d2:=qdom Σ'); eauto.
            ** apply senv_saturated_qqplus; eauto. eapply senv_saturated_openq. apply senv_saturated_qqplus.
               1,2,4,6 : eapply weaken_store_senv_saturated; eauto 2. eapply has_type_senv_saturated; eauto.
          ...
          eapply weaken_store_senv_saturated; eauto.
          eauto using Subq_trans.
          apply Qor_bound; auto. apply has_type_closed in H27. intuition. eapply @Subq_trans with (d2:=q_trans [] Σ' d1). Qcrush. unfold q_trans,q_trans_tenv,q_trans_senv. simpl. erewrite q_trans''_extend_closed_id; eauto. rewrite q_trans''_fuel_max; auto. apply @Subq_trans with (d2:=qdom Σ'); auto. apply closed_Qual_subq_qdom. apply closed_Qual_q_trans''_monotone; auto. eapply closed_Qual_monotone; eauto. eapply closed_qenv_monotone. apply wf_senv_closed_qenv; auto. 1,2: auto. apply extends_length; auto. Qcrush. 
          eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
          eapply openq_subqual_0_false; auto.
        * (* df not fresh *) rewrite not_fresh_qqplus in H27; auto. apply (Preserve Σ' ∅); auto.
   ...
   + (* tapp *) right. subst. intuition.
     ...
     - (* contraction *)
       ...
       * apply (Preserve Σ ∅); auto.  rewrite qqplus_qbot_right_neutral.
         ...
         eapply @Subq_trans with (d2:=qdom Σ); auto. apply closed_Qual_subq_qdom; auto. 
         eapply senv_saturated_openq; eauto. eapply has_type_senv_saturated; eauto.
         apply qstp_empty in H30. repeat apply Qor_bound; auto. eapply Subq_trans; eauto. all: apply closed_Qual_subq_qdom; auto. 
     ...
     -  (* right congruence *)
        ...
        destruct H23. apply (Preserve Σ' ∅); intuition.
        rewrite not_fresh_qqplus in H28; auto. rewrite qqplus_qbot_right_neutral.
        eapply t_app with (T1:=T1); eauto. eapply weaken_flt. eapply weaken_store; eauto. all: eauto. apply closed_qenv_empty. apply []. eapply Subq_trans; eauto.
     ...
     -  (* left congruence *)
        ...
        ndestruct (qbvs d2 0).
        * (* d2 is dependent on f, so the growth in df might be observable  *)
          inversion H13. subst. apply (Preserve Σ' d'); auto.
          -- eapply disjointq_scale; eauto. eapply openq_subqual_0; eauto. (* this is the sole reason why need to distinguish whether d2 is dependent on f or not *)
          -- destruct (♦∈? df) eqn:Hfresh.
             ** erewrite @openq_duplicate_eq_l with (f:=0) (l:=‖Σ'‖). 3,4 : eapply closed_qual_monotone; eauto. 2,3 : eauto.
                eapply t_sub with (T1 := (T2 <~ᵀ TUnit ~ ∅; T1 ~ d1))(d1 := (openq (df ⋓ d') d1 d2)).
                --- eapply t_app with (T1:=T1) (df:=(df ⋓ d')); eauto.
                    eapply weaken_flt. eapply weaken_store; eauto. all: eauto. apply closed_qenv_empty; apply []. eapply Subq_trans; eauto.
                    eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2. 
                --- apply stp_refl. simpl. eapply closed_ty_monotone; eauto.
                    constructor; auto. apply closed_qual_qqplus; auto.
                    apply openq_closed. 2 : apply closed_qual_qqplus.
                ...
                --- apply has_type_filter in H0. apply has_type_filter in H. apply Qqplus_bound.
                    eapply openq_subqual; eauto 2. apply closed_Qual_qor; auto.  
                    apply Qqplus_bound. eapply Subq_trans; eauto.
                    eapply @Subq_trans with (d2:=qdom Σ'); eauto. 
                    eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. 
                    eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. 
                    eapply @Subq_trans with (d2:=qdom Σ'); eauto. 
                --- apply senv_saturated_qqplus; eauto. eapply senv_saturated_openq.
                    apply senv_saturated_qqplus; eauto. 
                    1,3,5 : eapply weaken_store_senv_saturated; eauto 2.
                    1,2 : eapply has_type_senv_saturated; eauto. 
                    eapply closed_Qual_open2; eauto.
                    apply closed_Qual_qqplus; auto; eapply closed_Qual_monotone; eauto. 
                    eapply closed_Qual_monotone; eauto. 
             ** rewrite not_fresh_qqplus in H28; auto. apply t_sub with (T1:=(T2 <~ᵀ TUnit ~ ∅; T1 ~ d1)) (d1:=d2 <~ᵈ df; d1).
                --- eapply t_app with (T1:=T1); eauto. eapply weaken_flt. eapply weaken_store; eauto. auto. auto. apply closed_qenv_empty; apply []. apply extends_qdom; auto. apply closed_qual_qdom. eapply Subq_trans; eauto. eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
                --- apply stp_refl. simpl. eapply closed_ty_monotone; eauto.
                    constructor. auto. apply closed_qual_qqplus; auto.
                    apply openq_closed; try solve [eapply closed_qual_monotone; eauto]. eauto.
                --- apply Qqplus_bound. eapply openq_subqual. apply closed_Qual_qor; auto.
                    eapply @Subq_trans with (d2:=qdom Σ'); eauto. 
                    eapply @Subq_trans with (d2:=qdom Σ); eauto using closed_Qual_subq_qdom. 
                    eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. 
                    eapply @Subq_trans with (d2:=qdom Σ); eauto using closed_Qual_subq_qdom. 
                    eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. 
                    eapply @Subq_trans with (d2:=qdom Σ'); eauto. 
        ...
        * (* d2 is not dependent on f, so we don't observe the growth in df *)
          inversion H13. subst. apply (Preserve Σ' ∅); auto. rewrite qqplus_qbot_right_neutral.
          replace (d2 <~ᵈ df; d1) with (d2 <~ᵈ df ⋓ d'; d1). (* since f doesn't occur in d2 *)
          eapply t_app with (T1:=T1); eauto. eapply weaken_flt. eapply weaken_store; eauto. apply closed_qenv_empty; apply []. apply extends_qdom; auto. apply closed_qual_qdom. eauto using Subq_trans. eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2. apply openq_subqual_0_false; auto.

    + (* t_app_fresh *) right. subst. intuition.
     apply has_type_closed in H as HH. intuition. apply has_type_closed in H0 as HH0. intuition.
     (* t1 *) specialize (H12 (TFun (q_trans [] Σ df ⋒ q_trans [] Σ d1) d2 T1 T2) t1). intuition.
     ...
     - (* contraction *)
       ...
       * apply (Preserve Σ ∅); auto. rewrite qqplus_qbot_right_neutral.
         ...
         assert (HT' : has_type [(bind_tm, false, T1, q_trans [] Σ df'' ⋒ q_trans [] Σ d1''); (bind_tm, true, TFun d0 d3 T0 T3, df1)] (df1 ⊔ $!0 ⊔ $!1) Σ (open_tm' ([]:tenv) t) (open_ty' ([]:tenv) T3) (openq' ([]:tenv) d3)). {
           eapply narrowing. eapply H29. intuition. apply @s_trans with (T2:=T1) (d2:=q_trans [] Σ df ⋒ q_trans [] Σ d1); auto. apply stp_refl; auto. constructor; auto. apply closed_Qual_qor; auto. apply closed_Qual_qand; auto. 1,2: apply closed_Qual_q_trans''_monotone; auto; apply wf_senv_closed_qenv; auto.
         ...
         eapply openq_subqual. apply closed_Qual_qor; auto.
         eapply @Subq_trans with (d2:=qdom Σ); eauto using closed_Qual_subq_qdom. 
         eapply @Subq_trans with (d2:=qdom Σ); eauto using closed_Qual_subq_qdom. 
         eapply @Subq_trans with (d2:=qdom Σ). 
          apply closed_Qual_subq_qdom. apply closed_Qual_open2; auto. 
auto. 
         eapply senv_saturated_openq; eauto. eapply has_type_senv_saturated; eauto.
         eapply qstp_closed in H31. intuition. apply Qor_bound; eauto using closed_Qual_subq_qdom.
         subst. Qcrush.
         assert (stp [] Σ (TFun d0 d3 T0 T3) df1 (TFun d0 d3 T0 T3) df). { apply stp_refl; auto. } subst. apply vtp_non_fresh. eapply vtp_widening; eauto. 
         all: subst; eauto.
     -  (* right congruence *)
        inversion H14. subst.
        apply has_type_vtp in H as VH; intuition.
        ...
        destruct H9 as [t2' [σ' HH22]]. exists (tapp t1 t2'). exists σ'. intuition. constructor; intuition.

        destruct H12. destruct (♦∈? d1) eqn:Hfresh.
        * (* d1 fresh *)
          ...
          -- (* d2 dependent on x *) apply (Preserve Σ' d'); auto.
             ...
             ** eapply t_app_fresh with (T1 := T1) (df:=df); eauto. replace (q_trans [] Σ' df ⋒ q_trans [] Σ' (d1 ⋓ d')) with (q_trans [] Σ' df ⋒ q_trans [] Σ' d1).
                eapply weaken_flt. unfold q_trans,q_trans_senv,q_trans_tenv in *. simpl in *. erewrite q_trans''_extend_closed_id with (q:=df); eauto. erewrite q_trans''_extend_closed_id with (q:=d1); eauto. rewrite q_trans''_fuel_max with (q:=df); auto. rewrite q_trans''_fuel_max with (q:=d1); auto. eapply weaken_store; eauto. apply closed_qenv_empty. apply []. all : auto. 1,2: Qcrush. 
        { inversion H24; subst; simpl; destruct (♦∈? d1); auto. 
        ++ rewrite qor_empty_right; auto. 
        ++ repeat rewrite <- q_trans_or_dist. repeat rewrite qand_qor_dist_l. unfold q_trans, q_trans_tenv. simpl. rewrite q_trans_senv_locs. rewrite qand_qor_dist_l. 
           ...
           assert (q_trans_senv ((T, q) :: Σ) df ⊓ q_trans_senv Σ q ⊑↑ (q_trans_senv ((T, q) :: Σ) df ⊓ q_trans_senv ((T, q) :: Σ) d1)). { apply Subq_qand_split; auto. eapply @Subq_trans with (d2:=q_trans_senv ((T, q) :: Σ) q). unfold q_trans_senv. erewrite <- q_trans''_fuel_max with (E:=Σ). eapply q_trans''_extend_subq; eauto. simpl. lia. unfold q_trans_senv. apply q_trans''_subq; auto. }
           assert (q_trans_senv ((T, q) :: Σ) df ⊓ q_trans_senv ((T, q) :: Σ) q ⊑↑ (q_trans_senv ((T, q) :: Σ) df ⊓ q_trans_senv ((T, q) :: Σ) d1)). { apply Subq_qand_split; auto. eapply q_trans''_extend_subq; eauto. }
           apply Q_lift_eq. Qcrush. 
           assert (closed_Qual 0 0 (‖ Σ ‖) (q_trans_senv ((T, q) :: Σ) df)↑). { apply closed_Qual_q_trans''_monotone; auto. apply closed_qenv_extend; auto. } erewrite closed_qand_locs_empty; eauto.
        }
eauto using Subq_trans.
                apply Qor_bound; auto. apply has_type_closed in H25. intuition. eapply @Subq_trans with (d2:=q_trans [] Σ' df). Qcrush. unfold q_trans,q_trans_tenv,q_trans_senv. simpl. erewrite q_trans''_extend_closed_id; eauto. rewrite q_trans''_fuel_max; auto. apply @Subq_trans with (d2:=qdom Σ'); auto. apply closed_Qual_subq_qdom. apply closed_Qual_q_trans''_monotone; auto. eapply closed_Qual_monotone; eauto. eapply closed_qenv_monotone. apply wf_senv_closed_qenv; auto. 1,2: auto. apply extends_length; auto. Qcrush. 
                eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
            **  apply has_type_closed in H25. intuition.
            ...
            **  apply has_type_filter in H0. apply has_type_filter in H. apply Qqplus_bound. eapply openq_subqual; eauto 2. apply closed_Qual_qor; eauto. 
               eapply Subq_trans; eauto. apply has_type_closed in H25. intuition. apply Qqplus_bound; eauto. eapply Subq_trans; eauto. eapply @Subq_trans with (d2:=qdom Σ'); eauto. eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. eapply @Subq_trans with (d2:=qdom Σ'); eauto.
            **  apply senv_saturated_qqplus; eauto. eapply senv_saturated_openq.
          ...
          apply closed_qenv_empty. apply []. 
          eauto using Subq_trans.
                apply Qor_bound; auto. apply has_type_closed in H25. intuition. eapply @Subq_trans with (d2:=q_trans [] Σ' df). Qcrush. unfold q_trans,q_trans_tenv,q_trans_senv. simpl. erewrite q_trans''_extend_closed_id; eauto. rewrite q_trans''_fuel_max; auto. eapply @Subq_trans with (d2:=qdom Σ'); eauto. apply closed_Qual_subq_qdom. apply closed_Qual_q_trans''_monotone; auto. eapply closed_Qual_monotone; eauto. eapply closed_qenv_monotone. apply wf_senv_closed_qenv; auto. 1,2: auto. apply extends_length; auto. Qcrush. 
             eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
             unfold open_ty. repeat rewrite not_free_prop1; auto.
             eapply openq_subqual_1_false; eauto.
        * (* d1 not fresh *) rewrite not_fresh_qqplus in H25; auto. apply (Preserve Σ' ∅); auto.
          rewrite qqplus_qbot_right_neutral.
...
eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
     -  (* left congruence *)
        ...
        specialize (H20 σ H11). destruct H20 as [t1' [σ' HH1]]. exists (tapp t1' t2). exists σ'. intuition. apply step_c_app_l. intuition.
        ...
        * (* df fresh *)
          ...
          all: eauto.
          eauto using Subq_trans.
                apply Qor_bound; auto. apply has_type_closed in H33. intuition. eapply @Subq_trans with (d2:=q_trans [] Σ' d1). Qcrush. unfold q_trans,q_trans_tenv,q_trans_senv. simpl. erewrite q_trans''_extend_closed_id; eauto. rewrite q_trans''_fuel_max; auto. eapply @Subq_trans with (d2:=qdom Σ'); eauto. apply closed_Qual_subq_qdom. apply closed_Qual_q_trans''_monotone; auto. eapply closed_Qual_monotone; eauto. eapply closed_qenv_monotone. apply wf_senv_closed_qenv; auto. 1,2: auto. apply extends_length; auto. Qcrush. 
          eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
            ** apply has_type_closed in H33. intuition.
               apply stp_refl. simpl. eapply closed_ty_monotone; eauto.
               constructor; auto. apply closed_Qual_qqplus; auto. apply openQ_closed. eauto using closed_Qual_monotone. apply closed_Qual_qqplus; eauto. 1,2: eapply closed_Qual_monotone; eauto.
            ** apply has_type_filter in H0. apply has_type_filter in H. apply Qqplus_bound. eapply openq_subqual; eauto 2 using closed_Qual_qor.
               apply Qqplus_bound. eapply Subq_trans; eauto. 
               eapply @Subq_trans with (d2:=qdom Σ'); eauto. 
               eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. 
               eapply @Subq_trans with (d2:=qdom Σ ⊔ {♦}); eauto. 
          ...
          eauto using Subq_trans.
             ...
             eapply weaken_store_senv_saturated; eauto 2 using closed_Qual_open2.
             eapply openq_subqual_0_false; auto.
        * (* df not fresh *) rewrite not_fresh_qqplus in H33; auto. apply (Preserve Σ' ∅); auto.
          rewrite qqplus_qbot_right_neutral.
          eapply t_app_fresh with (T1:=T1); eauto. 
          unfold q_trans,q_trans_senv,q_trans_tenv in *. simpl in *. 
          rewrite q_trans''_extend_closed_id with (E:=Σ) (q:=d1); auto. rewrite q_trans''_fuel_max with (q:=d1). 
    ...
    * (*contraction*) right. intros.
      ...
      econstructor; eauto. inversion H7. destruct H13.
      eapply has_type_filter in H as Hfl.
      assert ({♦} ⊑↑ d1 -> False). intros. Qcrush.
      assert (d1 ⊑↑ qdom Σ). eapply Subqual_non_fresh; eauto.
      apply (Preserve ((T,d1) :: Σ) (d1 ⊔ &!‖σ‖)); auto.
      ...
      apply t_loc; auto. rewrite H13. Qcrush.
      apply indexr_head. simpl. eapply closed_ty_monotone; eauto.
      eapply closed_qual_monotone; eauto. rewrite H13. clear. Qcrush. simpl. lia.
      ...
      rewrite H13. rewrite <- qor_assoc. replace (qdom ((T, d1) :: Σ)) with (qdom Σ ⊔ &! (‖ Σ ‖)). repeat rewrite Q_lift_or. rewrite Q_lift_qdom. clear - H16. Qcrush; eauto. 
apply Q_lift_eq. rewrite Q_lift_or. repeat rewrite Q_lift_qdom. clear. Qcrush; simpl in *. lia. lia. bdestruct (x =? (‖ Σ ‖)); intuition. 
      ...
      apply closed_qenv_empty. apply []. 
      * (*congruence*) right. intros. specialize (H10 σ H7). destruct H10 as [t' [σ' HH10]].
        exists (tref t'). exists σ'. intuition. econstructor; eauto.
        destruct H12. apply (Preserve Σ' ∅); intuition. rewrite qqplus_qbot_right_neutral.
        rewrite not_fresh_qqplus in H16; auto. constructor; auto. apply has_type_closed in H16. intuition.
    + (*tderef*) subst. intuition. specialize (has_type_closed H) as HH. intuition. specialize (H8 (TRef d1 T0) t). intuition.
      * (* contraction *) right. intros. pose (HV := H). apply has_type_vtp in HV; intuition.
        ...
        pose (HHV := HV). inversion HHV. subst.  pose (HH3 := H8). inversion HH3. intuition.
      ...
      specialize (H28 l x T q1). apply t_sub with (T1 := T)(d1:= q1); auto.
      intuition.
    ...
    + (*tassign*) subst. intuition. rename H into Ht1. rename H0 into Ht2. intuition.
      ...
      * (* contraction *)
        ...
        inversion H5. subst. specialize (indexr_var_some' H22) as HH22. intuition.
        econstructor; eauto. rewrite H29; auto. auto. apply (Preserve Σ ∅); auto.
      ...
      * (* right congruence *)
        ...
        destruct H15. apply (Preserve Σ' ∅); eauto. rewrite not_fresh_qqplus in H31; auto. simpl.
        eapply t_assign; eauto. eapply weaken_flt. eapply weaken_store; eauto. 
        apply closed_qenv_empty. apply []. auto. auto.
      * (* left congruence *)
    ...
    + (*t_sub*) subst. intuition. specialize (stp_closed H0) as H00. intuition.
      ...
      apply Qqplus_bound. eapply Subq_trans; eauto. eapply @Subq_trans with (d2:=qdom Σ'); eauto. apply senv_saturated_qqplus; eauto. eapply weaken_store_senv_saturated; eauto.
      Unshelve. all: apply 0.
Qed.
...
Corollary preservation_of_separation : forall {Σ t1 T1 q1 t2 T2 q2},
  ...
  assert (HT2': has_type [] (qdom Σ') Σ' t2 T2 q2). {
    eapply weaken_flt. eapply weaken_store. all: eauto. apply closed_qenv_empty. apply [].
  }
  ...
  apply has_type_senv_saturated in HT1 as Hsat1; auto. apply has_type_closed in HT1 as Hcl1; auto. intuition. eapply weaken_store_senv_saturated; eauto.
  eapply weaken_store_senv_saturated; eauto.
  apply has_type_senv_saturated in HT2 as Hsat2; auto. apply has_type_closed in HT2 as Hcl2; auto. intuition. eapply weaken_store_senv_saturated; eauto.
Qed.
